...
/* v297b - Pilihan rencana terkait pada form jurnal + style badge */
(function(){
  'use strict';
  if(window.__ARIKA_V297_MANUAL_RENCANA_SELECT__) return;
  window.__ARIKA_V297_MANUAL_RENCANA_SELECT__ = true;
  function byId(id){ return document.getElementById(id); }
  function esc(v){ try { return typeof escapeHTML === 'function' ? escapeHTML(v) : String(v||''); } catch(_) { return String(v||''); } }
  function normDate(v){ try { return typeof normalizeDateKeyFromSheet === 'function' ? normalizeDateKeyFromSheet(v) : String(v||'').slice(0,10); } catch(_) { return String(v||'').slice(0,10); } }
  function isMine(r){ try { return typeof isCurrentUserRencana === 'function' ? isCurrentUserRencana(r) : true; } catch(_) { return true; } }
  function isDone(r){ try { return typeof isRencanaSelesai === 'function' ? isRencanaSelesai(r) : false; } catch(_) { return false; } }
  function rid(r){ return String((r&& (r.rencanaId || r.rencana_id || r['rencana id'] || r.id)) || '').trim(); }
  function injectStyle(){
    if(byId('arika-v297-realisasi-rencana-style')) return;
    var st=document.createElement('style');
    st.id='arika-v297-realisasi-rencana-style';
    st.textContent='\n.arika-archive-label.linked-plan{background:#ccfbf1;color:#0f766e;}\n#jurnal-rencana-select-card{border:1px solid rgba(16,185,129,.18);background:linear-gradient(135deg,#f0fdfa,#fff);border-radius:1.25rem;padding:.85rem;margin-bottom:1rem;}\n#jurnal-rencana-select{width:100%;border:1px solid #dbe3ee;background:#fff;border-radius:.9rem;padding:.72rem .85rem;font-size:.78rem;font-weight:800;color:#334155;outline:none;}\n#jurnal-rencana-select:focus{border-color:#10b981;box-shadow:0 0 0 3px rgba(16,185,129,.14);}\n';
    document.head.appendChild(st);
  }
  function getPlans(){
    var list = Array.isArray(window.rencanaData) ? window.rencanaData : [];
    return list.filter(isMine).filter(function(r){ return !isDone(r); }).slice().sort(function(a,b){ return String(a.tanggal||'').localeCompare(String(b.tanggal||'')); });
  }
  window.installJurnalRencanaSelect = function(){
    injectStyle();
    var form=byId('form-arika'); if(!form || !window.currentUser || window.isAdmin) return;
    var card=byId('jurnal-rencana-select-card');
    if(!card){
      card=document.createElement('div');
      card.id='jurnal-rencana-select-card';
      card.innerHTML='<label class="block text-[9px] font-black uppercase tracking-widest text-emerald-700 mb-2">Terkait Rencana</label><select id="jurnal-rencana-select"><option value="">Tidak terkait rencana</option></select><p class="mt-2 text-[10px] font-bold text-slate-400">Gunakan pilihan ini jika mengisi jurnal dari menu Isi Jurnal biasa.</p>';
      form.parentNode.insertBefore(card, form);
      var select=byId('jurnal-rencana-select');
      select.addEventListener('change', function(){
        var val=select.value;
        if(!val){ window.clearLinkedRencana && window.clearLinkedRencana(); return; }
        var plan=getPlans().find(function(r){ return String(r.id)===val || rid(r)===val; });
        if(plan){
          window.arikaLinkedRencana={ rencanaId: rid(plan), rencanaJudul: String(plan.judul || plan.catatan || 'Rencana kegiatan'), tanggal: normDate(plan.tanggal), localId: String(plan.id || '') };
          try { sessionStorage.setItem('arika_linked_rencana_v297', JSON.stringify(window.arikaLinkedRencana)); } catch(_) {}
          window.updateLinkedRencanaCard && window.updateLinkedRencanaCard();
        }
      });
    }
    var select=byId('jurnal-rencana-select'); if(!select) return;
    var current=window.arikaLinkedRencana && (window.arikaLinkedRencana.localId || window.arikaLinkedRencana.rencanaId) || '';
    var options=['<option value="">Tidak terkait rencana</option>'];
    getPlans().slice(0,80).forEach(function(r){
      var id=String(r.id || rid(r));
      var label=(r.tanggal ? normDate(r.tanggal)+' • ' : '') + String(r.judul || r.catatan || 'Rencana kegiatan');
      options.push('<option value="'+esc(id)+'">'+esc(label)+'</option>');
    });
    select.innerHTML=options.join('');
    if(current){ select.value=current; if(!select.value && window.arikaLinkedRencana.rencanaId) select.value=window.arikaLinkedRencana.rencanaId; }
  };
  var oldUpdate=window.updateLinkedRencanaCard;
  if(typeof oldUpdate==='function'){
    window.updateLinkedRencanaCard=function(){ var res=oldUpdate.apply(this,arguments); setTimeout(function(){ try{ window.installJurnalRencanaSelect && window.installJurnalRencanaSelect(); }catch(_){} },50); return res; };
  }
  var oldNav=window.nav;
  if(typeof oldNav==='function' && !oldNav.__arikaV297SelectWrapped){
    window.nav=function(id){ var res=oldNav.apply(this,arguments); if(id==='jurnal') setTimeout(function(){ try{ window.installJurnalRencanaSelect(); }catch(_){} },160); return res; };
    window.nav.__arikaV297SelectWrapped=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(window.installJurnalRencanaSelect,600); }); else setTimeout(window.installJurnalRencanaSelect,600);
  window.addEventListener('load', function(){ setTimeout(window.installJurnalRencanaSelect,900); });
})();

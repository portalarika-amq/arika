...
/* =========================================================
   v311 - Reload Full Cover Guard
   Basis: v310. Menutup seluruh viewport saat reload sampai view aktif stabil.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V311_RELOAD_FULL_COVER__) return;
  window.__ARIKA_V311_RELOAD_FULL_COVER__ = true;
  var bootTs = window.__ARIKA_V311_BOOT_TS__ || Date.now();
  var released = false;
  function byId(id){ return document.getElementById(id); }
  function isMobile(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  function hideTransient(){
    try{
      var panel=byId('arika-reminder-center-panel'); if(panel){ panel.classList.add('hidden'); panel.style.display=''; }
      var fab=byId('arika-reminder-center-fab'); if(fab && !window.currentUser) fab.classList.add('hidden');
      var help=byId('btn-arika-help-wa'); if(help && !window.currentUser) help.classList.add('hidden');
    }catch(e){}
  }
  function visibleViews(){
    try { return Array.from(document.querySelectorAll('.view-section')).filter(function(v){ return v && !v.classList.contains('hidden'); }); }
    catch(e){ return []; }
  }
  function viewStable(){
    var shown = visibleViews();
    if(!shown.length) return false;
    if(shown.length > 1) return false;
    var id = shown[0].id || '';
    if(!window.currentUser) return id === 'view-login-user' || id === 'view-login-admin';
    return id !== 'view-login-user' && id !== 'view-login-admin';
  }
  function keepCover(){
    if(released) return;
    try{
      document.documentElement.classList.add('arika-v311-boot');
      document.documentElement.classList.remove('arika-v311-ready');
      var loader=byId('global-loader');
      if(loader){ loader.style.display='flex'; loader.style.opacity='1'; loader.style.visibility='visible'; loader.style.pointerEvents='auto'; }
    }catch(e){}
  }
  function release(reason){
    ...
    try{
      ...
      document.documentElement.classList.add('arika-v311-ready','arika-v312-ready','arika-v313-ready');
      if(document.body) document.body.classList.add('arika-v311-ready');
      var loader=byId('global-loader');
      if(loader){ loader.style.opacity='0'; loader.style.pointerEvents='none'; setTimeout(function(){ try{ loader.style.display='none'; }catch(e){} }, isMobile()?80:140); }
      window.__ARIKA_V311_READY_REASON__ = reason || 'ready';
    }catch(e){}
  }
  function tryRelease(reason){
    ...
    hideTransient();
    keepCover();
    var min = isMobile()?650:1050;
    var wait = Math.max(100, min - (Date.now()-bootTs));
    clearTimeout(window.__arikaV311StableTimer);
    window.__arikaV311StableTimer = setTimeout(function(){
      if(viewStable()) release(reason || 'stable');
      else setTimeout(function(){ if(viewStable()) release((reason||'stable')+'-retry'); else keepCover(); }, 300);
    }, wait);
  }
  function patchNav(){
    if(!window.nav || window.nav.__arikaV311Guard) return;
    var old = window.nav;
    window.nav = function(id){
    ...
    window.nav.__arikaV311Guard = true;
  }
  function patchLoader(){
    if(!window.showLoader || window.showLoader.__arikaV311Guard) return;
    var old = window.showLoader;
    window.showLoader = function(show, text){
      var res = old.apply(this, arguments);
      if(!released) keepCover();
      return res;
    };
    window.showLoader.__arikaV311Guard = true;
  }
  function boot(){
    hideTransient(); keepCover(); patchNav(); patchLoader(); tryRelease('boot');
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('load', function(){ boot(); setTimeout(boot,250); setTimeout(boot,900); });
  setTimeout(function(){ if(!released && viewStable()) release('late-stable'); }, isMobile()?1500:2600);
  setTimeout(function(){ if(!released) release('failsafe'); }, isMobile()?2600:5200);
})();


/* =========================================================
   v312 - Mobile Login Light
   Basis: v311. Mengurangi lag saat klik username/kata sandi di HP.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V312_MOBILE_LOGIN_LIGHT__) return;
  window.__ARIKA_V312_MOBILE_LOGIN_LIGHT__ = true;
  function byId(id){ return document.getElementById(id); }
  function isMobile(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  function normalizeSafe(v){
    try { return typeof normalize === 'function' ? normalize(v || '') : String(v || '').trim().toLowerCase().replace(/[^a-z0-9]/g,''); }
    catch(e){ return String(v || '').trim().toLowerCase().replace(/[^a-z0-9]/g,''); }
  }
  function escapeSafe(v){
    try { return typeof escapeHTML === 'function' ? escapeHTML(v || '') : String(v || '').replace(/[&<>"]/g, function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];}); }
    catch(e){ return String(v || ''); }
  }
  window.hideLoginSuggestions = function(){
    var suggestions = byId('login-user-suggestions');
    if(suggestions) suggestions.classList.add('hidden');
  };
  function releaseLoginCoverFast(){
    try{
      if(!isMobile()) return;
      var login = byId('view-login-user');
      if(!login || login.classList.contains('hidden')) return;
      document.documentElement.classList.remove('arika-v311-boot','arika-v312-boot','arika-v313-boot');
      document.documentElement.classList.add('arika-v311-ready','arika-v312-ready','arika-v313-ready','arika-v312-login-ready','arika-v313-login-ready');
      var loader = byId('global-loader');
      if(loader){ loader.style.opacity='0'; loader.style.pointerEvents='none'; loader.style.display='none'; }
    }catch(e){}
  }
  window.prepareLoginNameInput = function(){
    releaseLoginCoverFast();
    var suggestions = byId('login-user-suggestions');
    var search = byId('login-user-search');
    if(!suggestions || !search) return;
    // Di HP, jangan render daftar saat fokus pertama karena membuat keyboard terasa macet.
    if(isMobile() && !String(search.value || '').trim()){
      suggestions.innerHTML = '<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Ketik minimal 2 huruf nama.</div>';
      suggestions.classList.remove('hidden');
      return;
    }
    if(typeof window.filterLoginNameOptions === 'function') window.filterLoginNameOptions();
  };
  var sortedCache = null;
  var sortedCacheLen = -1;
  function getSortedCached(){
    var list = Array.isArray(window.masterPegawai) ? window.masterPegawai : [];
    if(sortedCache && sortedCacheLen === list.length) return sortedCache;
    sortedCacheLen = list.length;
    sortedCache = list.filter(function(p){ return p && p.nama; }).slice().sort(function(a,b){ return String(a.nama||'').localeCompare(String(b.nama||''),'id',{sensitivity:'base'}); });
    return sortedCache;
  }
  function renderLoginMatches(){
    var search = byId('login-user-search');
    var hidden = byId('login-user-select');
    var suggestions = byId('login-user-suggestions');
    if(!search || !suggestions) return;
    releaseLoginCoverFast();
    var raw = String(search.value || '').trim();
    var query = normalizeSafe(raw);
    if(hidden) hidden.value = '';
    var minChars = isMobile() ? 2 : 1;
    if(!Array.isArray(window.masterPegawai) || !window.masterPegawai.length){
      suggestions.innerHTML = '<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Database belum terbaca. Klik Muat Ulang.</div>';
      suggestions.classList.remove('hidden');
      return;
    }
    if(query.length < minChars){
      suggestions.innerHTML = '<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Ketik minimal '+minChars+' huruf nama.</div>';
      suggestions.classList.remove('hidden');
      return;
    }
    var limit = isMobile() ? 5 : 8;
    var matches = [];
    var source = getSortedCached();
    for(var i=0; i<source.length && matches.length<limit; i++){
      var p = source[i];
      var nameNorm = normalizeSafe(p.nama);
      var wordStart = String(p.nama || '').split(/\s+/).some(function(part){ return normalizeSafe(part).startsWith(query); });
      if(nameNorm.startsWith(query) || wordStart) matches.push(p);
    }
    if(!matches.length){
      suggestions.innerHTML = '<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-rose-400">Nama tidak ditemukan.</div>';
      suggestions.classList.remove('hidden');
      return;
    }
    suggestions.innerHTML = matches.map(function(p){
      var nama = escapeSafe(p.nama || '');
      var meta = escapeSafe((p.lab || '-') + (p.status ? ' • ' + p.status : '') + (p.peran && normalizeSafe(p.peran) !== 'pegawai' ? ' • ' + p.peran : ''));
      return '<button type="button" class="login-suggestion-item" data-login-name="'+nama+'"><span class="text-xs font-black text-slate-800 uppercase">'+nama+'</span><span class="text-[9px] font-bold text-slate-400 uppercase">'+meta+'</span></button>';
    }).join('');
    suggestions.classList.remove('hidden');
  }
  ...
  window.filterLoginNameOptions = function(){
    releaseLoginCoverFast();
    clearTimeout(window.__arikaV312LoginFilterTimer);
    if(isMobile()) window.__arikaV312LoginFilterTimer = setTimeout(renderLoginMatches, 120);
    else renderLoginMatches();
  };
  document.addEventListener('click', function(ev){
    var btn = ev.target && ev.target.closest ? ev.target.closest('.login-suggestion-item[data-login-name]') : null;
    if(!btn) return;
    ev.preventDefault();
    var name = btn.getAttribute('data-login-name') || btn.textContent || '';
    // Decode HTML sederhana dari atribut.
    var tmp = document.createElement('textarea'); tmp.innerHTML = name; name = tmp.value;
    if(typeof window.setLoginSelectedName === 'function') window.setLoginSelectedName(name.trim());
    window.hideLoginSuggestions();
    var pwd = byId('login-user-nip'); if(pwd && isMobile()) setTimeout(function(){ try{ pwd.focus({preventScroll:true}); }catch(e){ pwd.focus(); } }, 80);
  }, true);
  document.addEventListener('focusin', function(ev){
    if(ev.target && ev.target.id === 'login-user-nip') window.hideLoginSuggestions();
    if(ev.target && (ev.target.id === 'login-user-search' || ev.target.id === 'login-user-nip')) releaseLoginCoverFast();
  }, true);
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(releaseLoginCoverFast, 450); });
  else setTimeout(releaseLoginCoverFast, 450);
  window.addEventListener('load', function(){ setTimeout(releaseLoginCoverFast, 350); setTimeout(releaseLoginCoverFast, 1200); });
})();


/* =========================================================
   v313 - Boot/Login Unstuck Safety
   Basis: v312. Memastikan splash ARIKA tidak menahan halaman login.
   ========================================================= */
(function(){
  if(window.__ARIKA_V313_BOOT_LOGIN_UNSTUCK__) return;
  window.__ARIKA_V313_BOOT_LOGIN_UNSTUCK__ = true;
  function byId(id){return document.getElementById(id);}
  function isMobile(){return window.matchMedia && window.matchMedia('(max-width:720px)').matches;}
  function visibleViews(){try{return Array.from(document.querySelectorAll('.view-section')).filter(function(v){return v && !v.classList.contains('hidden');});}catch(e){return [];}}
  function releaseCover(forceLogin){
    try{
      document.documentElement.classList.remove('arika-v311-boot','arika-v312-boot','arika-v313-boot');
      document.documentElement.classList.add('arika-v311-ready','arika-v312-ready','arika-v313-ready','arika-v313-released');
      var loader=byId('global-loader');
      if(loader){loader.style.opacity='0';loader.style.pointerEvents='none';loader.style.display='none';loader.style.visibility='hidden';}
      if(forceLogin && !window.currentUser){
        var views=visibleViews();
        var login=byId('view-login-user');
        if(login && (!views.length || views.every(function(v){return v.id!=='view-login-user' && v.id!=='view-login-admin';}))){
          document.querySelectorAll('.view-section').forEach(function(v){v.classList.add('hidden');});
          login.classList.remove('hidden');
          try{ if(typeof window.loadLoginOptions === 'function') window.loadLoginOptions(); }catch(e){}
        }
      }
    }catch(e){}
  }
  window.arikaV313ReleaseCover = releaseCover;
  function boot(){
    setTimeout(function(){releaseCover(false);}, isMobile()?650:950);
    setTimeout(function(){releaseCover(true);}, isMobile()?1800:2800);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('load', boot);
  document.addEventListener('focusin', function(ev){
    if(ev.target && (ev.target.id==='login-user-search' || ev.target.id==='login-user-nip')) releaseCover(false);
  }, true);
})();

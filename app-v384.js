/* v271: script utama ARIKA dipisah agar GitHub Pages lebih cepat dan browser bisa melakukan cache. */
// ARIKA v195 - main script externalized to avoid JavaScript text appearing in page
        ...
        const liburNasional = ["2026-01-01","2026-01-16","2026-02-17","2026-03-19","2026-03-21","2026-03-22","2026-04-03","2026-04-05","2026-05-01","2026-05-14","2026-05-27","2026-05-31","2026-06-01","2026-06-16","2026-08-17","2026-08-25","2026-12-25"];
        ...
        function normalizeDateKeyFromSheet(value) {
            ...
            const raw = rawOriginal.replace(/[.]/g, '/').replace(/\s+/g, '');

            const padDatePart = (n) => String(Number(n)).padStart(2, '0');
            const validDateParts = (y, m, d) => {
                y = Number(y); m = Number(m); d = Number(d);
                const dt = new Date(y, m - 1, d);
                return dt.getFullYear() === y && dt.getMonth() === m - 1 && dt.getDate() === d;
            };
            const makeKey = (y, m, d) => validDateParts(y, m, d) ? `${Number(y)}-${padDatePart(m)}-${padDatePart(d)}` : '';

            let match = raw.match(/^(\d{4})[-\/](\d{1,2})[-\/](\d{1,2})(?:T.*)?$/);
            if(match) return makeKey(match[1], match[2], match[3]) || rawOriginal.split('T')[0];

            match = raw.match(/^(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})$/);
            if(match) return makeKey(match[3], match[2], match[1]) || rawOriginal;

            // Angka murni: 03062026 -> 2026-06-03, 20260603 -> 2026-06-03
            match = raw.match(/^(\d{2})(\d{2})(\d{4})$/);
            if(match) return makeKey(match[3], match[2], match[1]) || rawOriginal;

            match = raw.match(/^(\d{4})(\d{2})(\d{2})$/);
            if(match && /^(19|20)\d{2}$/.test(match[1])) return makeKey(match[1], match[2], match[3]) || rawOriginal;

            const parsed = new Date(rawOriginal);
            if(!Number.isNaN(parsed.getTime())) return parsed.toLocaleDateString('en-CA');
            return rawOriginal.split('T')[0];
        }

        function normalizeMonthKeyFromInput(value) {
            if(!value) return '';
            if(value instanceof Date && !Number.isNaN(value.getTime())) return value.toLocaleDateString('en-CA').slice(0, 7);
            const rawOriginal = String(value || '').trim();
            if(!rawOriginal) return '';
            const raw = rawOriginal.replace(/[.]/g, '/').replace(/\s+/g, '');
            const dateKey = normalizeDateKeyFromSheet(raw);
            if(/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return dateKey.slice(0, 7);
            let y, m, match;
            const makeMonth = (yy, mm) => {
                yy = Number(yy); mm = Number(mm);
                if(!yy || mm < 1 || mm > 12) return '';
                return `${yy}-${String(mm).padStart(2, '0')}`;
            };
            match = raw.match(/^(\d{4})[-\/](\d{1,2})$/);
            if(match) return makeMonth(match[1], match[2]);
            match = raw.match(/^(\d{1,2})[-\/](\d{4})$/);
            if(match) return makeMonth(match[2], match[1]);
            match = raw.match(/^(\d{2})(\d{4})$/);
            if(match) return makeMonth(match[2], match[1]);
            match = raw.match(/^(\d{4})(\d{2})$/);
            if(match && /^(19|20)\d{2}$/.test(match[1])) return makeMonth(match[1], match[2]);
            return '';
        }

        function getManualDateValueById(id) {
            const el = document.getElementById(id);
            if(!el) return '';
            return normalizeDateKeyFromSheet(el.dataset?.arikaIsoValue || el.value || '') || '';
        }

        function getManualMonthValueById(id, fallback = '') {
            const el = document.getElementById(id);
            if(!el) return fallback || '';
            return normalizeMonthKeyFromInput(el.dataset?.arikaIsoValue || el.value || '') || fallback || '';
        }
        ...
        function mapJurnalRowsFromSheet(rawRows) {
            return dedupeJurnalData((rawRows || []).map((d, idx) => {
                ...
                return {
                    ...
                    rencanaId: (getVal(d, ['rencanaId', 'rencana id', 'rencana_id', 'Rencana ID']) || '').toString(),
                    rencanaJudul: (getVal(d, ['rencanaJudul', 'rencana judul', 'rencana_judul', 'Judul Rencana']) || '').toString(),
                    inputAt: inputAtVal,
                    _rawRow: d,
        ...
        window.nav = (id) => {
            ...
            // Admin hanya memerlukan Beranda, Panduan, WFH/WFA, dan Panel Admin.
            // Tab Isi Jurnal, Riwayat Jurnal, Laporan Lembur, dan Dashboard Saya tetap diblokir untuk akun admin.
            if (window.isAdmin && !['beranda', 'panduan', 'wfh', 'admin'].includes(id)) {
                window.showCustomAlert("Akun administrator hanya dapat mengakses Beranda, Panduan, WFH/WFA, dan Panel Admin.");
                window.nav('beranda');
                return;
            }
        ...
        function configureNavigationForRole(adminFlag) {
            ...
            const userMenuIds = ['jurnal', 'lembur', 'wfh', 'rekap', 'dashboard-pegawai'];

            userMenuIds.forEach(id => {
                const btn = document.getElementById('btn-' + id);
                if (btn) btn.classList.toggle('hidden', !!adminFlag && id !== 'wfh');

                const opt = document.querySelector(`#mobile-nav-select option[value="${id}"]`);
                if (opt) {
                    opt.hidden = !!adminFlag && id !== 'wfh';
                    opt.disabled = !!adminFlag && id !== 'wfh';
                }
            });
        ...
        function loginSetup(p, adminFlag) {
            ...
            // - Admin: Beranda, WFH/WFA, Panduan, dan Admin
            ...
            if (adminFlag) {
                ...
                window.nav(['beranda', 'wfh', 'panduan', 'admin'].includes(restoredView) ? restoredView : 'admin');
                ...
                if(welcomeSubtitle) {
                    welcomeSubtitle.innerHTML = 'Kerja kerasmu berharga. <span class="arika-home-brand-word">ARIKA</span> siap abadikan setiap progresmu hari ini. Semangat!';
                }
                ...
                const restoredView = window.__ARIKA_RESTORED_SESSION__?.lastView;
                const allowedViews = ['beranda', 'jurnal', 'lembur', 'wfh', 'rekap', 'dashboard-pegawai', 'panduan'];
        ...
        function renderCompactContributorRows(items) {
            if(!items.length) {
                return '<p class="text-center py-2 opacity-50 uppercase text-[9px] font-black tracking-widest text-slate-400">Belum ada data</p>';
            }
            return items.map((item, i) => {
                const safeName = escapeHTML(item.name || '-');
                const medal = i === 0 ? '🥇' : (i === 1 ? '🥈' : '🥉');
                return `<div class="arika-contributor-mini-row">
                    <span title="${safeName}">${medal} ${i + 1}. ${safeName}</span>
        ...
        function updateLeaderboard() {
            ...
            if(list) {
                const topPegawaiName = topPegawai[0]?.name ? escapeHTML(topPegawai[0].name) : 'Belum ada data';
                const topFungsiName = topFungsi[0]?.name ? escapeHTML(topFungsi[0].name) : 'Belum ada data';
                const pegawaiHtml = renderCompactContributorRows(topPegawai);
                const fungsiHtml = renderCompactContributorRows(topFungsi);

                list.innerHTML = `
                    <div class="arika-contributor-summary">
                        <span class="arika-contributor-pill">👤 ${topPegawaiName}</span>
                        <span class="arika-contributor-pill">🏅 ${topFungsiName}</span>
                    </div>
                    <details class="arika-contributor-details">
                        <summary>Detail kontributor</summary>
                        <div class="arika-contributor-detail-grid">
                            <div>
                                <div class="arika-contributor-mini-title">3 Pegawai Teraktif</div>
                                <div class="space-y-1">${pegawaiHtml}</div>
                            </div>
                            <div>
                                <div class="arika-contributor-mini-title">3 Fungsi Teraktif</div>
                                <div class="space-y-1">${fungsiHtml}</div>
                            </div>
                        </div>
                    </details>
                `;
        ...
        function compactForStorage(cache) {
            ...
            out.jurnal = (out.jurnal || []).slice(0, 700);
            out.rencana = applyRencanaDeleteTombstones(out.rencana || []).slice(0, 180);
            out.survei = (out.survei || []).slice(0, 180);
            out.agenda = (out.agenda || []).slice(0, 160);
            out.pengumuman = (out.pengumuman || []).slice(0, 120);
            out.kalenderLibur = (out.kalenderLibur || []).slice(0, 360);
            return out;
        }

        function saveLocalCoreCache() {
            const cache = compactForStorage({
                version: 'v52-loading-audit',
                ts: Date.now(),
            ...
            try {
                ...
                try {
                    cache.jurnal = (cache.jurnal || []).slice(0, 250);
                    cache.rencana = (cache.rencana || []).slice(0, 120);
                    cache.survei = (cache.survei || []).slice(0, 120);
                    cache.agenda = (cache.agenda || []).slice(0, 120);
                    cache.kalenderLibur = (cache.kalenderLibur || []).slice(0, 240);
        ...
        window.fetchCloudData = async function(options = {}) {
            ...
            try {
                ...
                if (data && data.rencana && Array.isArray(data.rencana)) {
                    const mappedRencana = data.rencana.map((r, idx) => normalizeRencanaItem({
                        id: (getVal(r, ['id', 'ID', 'id_rencana', 'ID Rencana', 'timestamp', 'Timestamp']) || `rencana-${idx}`).toString(),
                        ownerName: (getVal(r, ['ownerName', 'owner_name', 'nama', 'Nama', 'nama_pegawai', 'Nama Pegawai', 'pegawai', 'Pegawai', 'name', 'Name', 'pelaksana', 'Pelaksana', 'pemilik', 'Pemilik']) || '').toString(),
                        ownerNip: (getVal(r, ['ownerNip', 'owner_nip', 'nip', 'NIP', 'nip_pegawai', 'NIP Pegawai', 'Nomor Induk']) || '').toString(),
                        tanggal: normalizeDateKeyFromSheet(getVal(r, ['tanggal', 'Tanggal', 'date', 'Date', 'tgl', 'Tgl', 'tanggal rencana', 'Tanggal Rencana']) || ''),
                        jamReminder: extractTime(getVal(r, ['jamReminder', 'jam_reminder', 'jam reminder', 'Jam Reminder', 'waktuReminder', 'waktu_reminder', 'waktu reminder', 'Waktu Reminder', 'jam', 'Jam', 'waktu', 'Waktu']) || ''),
                        periode: (getVal(r, ['periode', 'Periode', 'jenis', 'Jenis', 'type', 'Type']) || 'Reminder').toString(),
                        judul: (getVal(r, ['judul', 'Judul', 'rencana', 'Rencana', 'kegiatan', 'Kegiatan', 'title', 'Title']) || '').toString(),
                        catatan: (getVal(r, ['catatan', 'Catatan', 'detail', 'Detail', 'deskripsi', 'Deskripsi', 'note', 'Note']) || '').toString(),
                        status: (getVal(r, ['status', 'Status']) || 'Aktif').toString(),
                        rencanaId: (getVal(r, ['rencanaId', 'rencana id', 'rencana_id', 'Rencana ID', 'ID Relasi Rencana']) || '').toString(),
                        createdAt: (getVal(r, ['createdAt', 'created_at', 'timestamp', 'Timestamp']) || '').toString(),
                        syncStatus: 'synced',
                        rawText: getRowSearchText(r)
            ...
            try {
                ...
                if(data && Array.isArray(data.jurnal)) {
                    ...
                    if(mappedJurnal.length > 0 || (window.arikaData || []).length === 0) {
                        ...
                        saveLocalCoreCache();
                        renderCurrentDataState({ fromCache: false });
                        if(!silent) updateSyncStatus('Jurnal sesuai database', 'ok');
                        return true;
                    }
        ...
        window.forceSyncFromSheet = async function(options = {}) {
            const silent = !!options.silent;
            if(!silent) window.showLoader(true, 'Menyinkronkan data terbaru dari database...');
            try {
                ...
                updateSyncStatus('Data sesuai database', 'ok');
            } catch(err) {
        ...
        function updateArikaReminderSoundPanel() {
            ...
            if(compactToggle) {
                ...
                if(hint) hint.textContent = needsSoundAttention ? 'Suara belum aktif.' : '';
            }
        }
        ...
        function getArikaReminderMp3Audio() {
            try {
                if(!arikaReminderMp3Audio) {
                    ...
                    arikaReminderMp3Audio.preload = 'none';
        ...
        function collectBerandaReminderEvents() {
            ...
            try {
                const surveyMonth = (typeof getCurrentSurveyMonth === 'function') ? getCurrentSurveyMonth() : getCurrentMonth();
                const surveyDone = (typeof hasUserFilledSurveyThisMonthForReminder === 'function')
                    ? hasUserFilledSurveyThisMonthForReminder()
                    : !!(typeof getUserSurveyThisMonth === 'function' && getUserSurveyThisMonth());
                if(!surveyDone) {
                    addReminderEvent(events, {
                        priority: 6,
                        kind: 'survei',
                        key: `survei-bulanan-${userKey}-${surveyMonth}`,
                        title: 'Survei Bulanan Belum Diisi',
                        body: `Survei ARIKA bulan ${formatBulanIndonesia(surveyMonth)} belum diisi.`,
                        speech: `Halo, survei ARIKA bulan ini belum diisi. Mohon isi survei singkat saat sempat.`,
                        repeatMinutes: 1440
                    });
                }
            } catch(e) {}
        ...
        window.renderLemburTable = function() {
            ...
            const filt = getManualMonthValueById('filt-lembur-bulan', '');
            if(!body) return;
            body.innerHTML = '';
        ...
        function getArchivePeriodLabel(monthFilter, exactDateFilter) {
            try {
                if(exactDateFilter) return `Tanggal ${formatDateIndo(exactDateFilter)}`;
                if(monthFilter) return formatBulanIndonesia(monthFilter);
                return 'Semua bulan';
            } catch(_) { return 'Mengikuti filter aktif'; }
        }

        function getArchiveCategoryClass(category) {
            const raw = String(category || '').toLowerCase();
            if(raw.includes('teknis') || raw.includes('laboratorium')) return 'technical';
            if(raw.includes('pengembangan') || raw.includes('kompetensi') || raw.includes('pelatihan') || raw.includes('workshop') || raw.includes('bimtek') || raw.includes('seminar') || raw.includes('webinar')) return 'competency';
            if(raw.includes('rapat') || raw.includes('meeting')) return 'meeting';
            if(raw.includes('administrasi') || raw.includes('admin')) return 'administration';
            return 'other';
        }

        function buildArchiveCategoryHtml(category) {
            const safeCategory = escapeHTML(category || '-');
            const categoryClass = getArchiveCategoryClass(category);
            return `<span class="arika-archive-category ${categoryClass}">${safeCategory}</span>`;
        }

        function buildArchiveLabelHtml(d) {
            const done = normalize(d.status) === normalize('Selesai');
            const chips = [];
            chips.push(`<span class="arika-archive-label ${done ? 'done' : 'pending'}">${done ? 'Selesai' : 'Belum Selesai'}</span>`);
            if(d.isLembur) chips.push(`<span class="arika-archive-label overtime">Lembur</span>`);
            if(d.rencanaId || d.rencanaJudul) chips.push(`<span class="arika-archive-label linked-plan" title="${escapeHTML(d.rencanaJudul || d.rencanaId || '')}">Terkait Rencana</span>`);
            return chips.join('');
        }

        function updateRekapArchiveSummary(rows) {
            const box = document.getElementById('rekap-month-summary');
            const period = document.getElementById('rekap-summary-period');
            const monthFilter = getRekapMonthValue();
            const exactDateFilter = getRekapDateValue();
            const data = Array.isArray(rows) ? rows : [];
            const done = data.filter(d => normalize(d.status) === normalize('Selesai')).length;
            const pending = data.filter(d => normalize(d.status) !== normalize('Selesai')).length;
            const overtime = data.filter(d => !!d.isLembur).length;
            if(period) period.innerText = getArchivePeriodLabel(monthFilter, exactDateFilter);
            if(!box) return;
            box.innerHTML = `
                <div class="arika-archive-kpi"><span>Total</span><strong>${data.length}</strong></div>
                <div class="arika-archive-kpi done"><span>Selesai</span><strong>${done}</strong></div>
                <div class="arika-archive-kpi pending"><span>Belum Selesai</span><strong>${pending}</strong></div>
                <div class="arika-archive-kpi overtime"><span>Lembur</span><strong>${overtime}</strong></div>
            `;
        }

        window.runFilterDebounced = function() {
            clearTimeout(window.__arikaRekapSearchTimer);
            window.__arikaRekapSearchTimer = setTimeout(() => {
                try { window.runFilter({ deferHeavy: true }); } catch(e) { console.warn(e); }
            }, 180);
        };
        ...
        function renderPersonalHistoryPage() {
            ...
            updateRekapArchiveSummary(filtered);
            ...
            if(total === 0) {
                ...
                const msg = totalRows === 0
                    ? 'Data jurnal belum terbaca dari database. Klik Sinkron / refresh ARIKA.'
                    : (samePersonRows === 0
                        ? 'Data jurnal sudah terbaca, tetapi belum ada baris yang cocok dengan akun ini.'
                        : 'Belum ada arsip jurnal yang cocok dengan filter aktif.');
                tbody.innerHTML = `<tr><td colspan="5" class="p-8 text-center text-slate-400 italic text-[9px] uppercase font-black">${msg}</td></tr>`;
                if(pagination) {
            ...
            tbody.innerHTML = rows.map(d => {
                ...
                const labels = buildArchiveLabelHtml(d);
                return `<tr class="arika-archive-row hover:bg-slate-50/80">
                    <td class="p-3 text-center font-mono text-slate-500 text-[9px] whitespace-nowrap">${escapeHTML(formatDateIndo(d.date))}</td>
                    <td class="p-3 text-center">${buildArchiveCategoryHtml(category)}</td>
                    <td class="p-3 text-left font-bold text-slate-700 text-[10px] max-w-[520px]"><div class="arika-history-desc-full text-slate-700 normal-case">${escapeHTMLPreserveLines(d.desc)}</div>${dataLink}${reviewBox}</td>
                    <td class="p-3 text-center"><div class="flex flex-wrap justify-center gap-1.5">${labels}</div></td>
                    <td class="p-3 text-center"><div class="flex items-center justify-center gap-1.5"><button onclick="window.editJurnal('${escapeHTML(d.id)}')" class="arika-archive-action edit" title="Edit jurnal">✏️</button><button onclick="window.hapusJurnal('${escapeHTML(d.date)}', \`${escapeHTML(d.desc).replaceAll('`', '')}\`, '${escapeHTML(d.id)}')" class="arika-archive-action delete" title="Hapus jurnal">🗑️</button></div></td>
                </tr>`;
            }).join('');
        ...
        window.runFilter = function(options = {}) {
            ...
            filtered = applyJurnalDeleteTombstones(window.arikaData || [])
                .filter(d => {
                    ...
                    const matchesCategory = jurnalCategoryMatches(d.cat, categorySelect);
            ...
            try { if(window.chartS) { window.chartS.destroy(); window.chartS = null; } } catch(_) {}
            try { if(window.chartTrendInstance) { window.chartTrendInstance.destroy(); window.chartTrendInstance = null; } } catch(_) {}
        };
                ...
                rows.forEach((d, i) => {
                    bodyRows += `
                        <tr>
                            ...
                            <td style="${cellStyle}text-align:center;font-weight:bold;">${d.isLembur ? 'Lembur' : '-'}</td>
                            <td style="${cellStyle}">${escapeHTML(d.review || d.catatan || d.note || '-')}</td>
                        </tr>
                    `;
                });
            } else {
                bodyRows = `<tr><td colspan="7" style="${cellStyle}text-align:center;">Belum ada arsip jurnal pada filter aktif.</td></tr>`;
            }

            const tableHtml = `
                <table style="border-collapse:collapse;width:100%;font-family:Arial,sans-serif;font-size:11pt;">
                    <tr><td colspan="7" style="font-size:16pt;font-weight:bold;text-align:center;padding:8px;">ARSIP JURNAL KEGIATAN HARIAN</td></tr>
                    <tr><td colspan="7" style="text-align:center;padding:6px;">ARIKA - BPOM AMBON</td></tr>
                    <tr><td colspan="7" style="padding:6px;"></td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">Nama</td><td colspan="6" style="${cellStyle}">${escapeHTML(window.currentUser.nama || '-')}</td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">NIP</td><td colspan="6" style="${cellStyle}">${escapeHTML(window.currentUser.nip || '-')}</td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">Laboratorium/Fungsi</td><td colspan="6" style="${cellStyle}">${escapeHTML(window.currentUser.lab || '-')}</td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">Periode</td><td colspan="6" style="${cellStyle}">${escapeHTML(periode)}</td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">Filter</td><td colspan="6" style="${cellStyle}">Kategori: ${escapeHTML(catFilter)} | Status: ${escapeHTML(statusFilter)}</td></tr>
                    <tr><td style="font-weight:bold;${cellStyle}">Tanggal Unduh</td><td colspan="6" style="${cellStyle}">${escapeHTML(tanggalUnduh)}</td></tr>
                    <tr><td colspan="7" style="padding:6px;"></td></tr>
                    <tr>
        ...
        function getAdminAnalyticsRows() {
            const monthVal = getManualMonthValueById('admin-analytics-month', getCurrentMonth());
        ...
        function renderChart(canvasId, instanceName, type, labels, data, options = {}) {
            ...
            const styledData = (data || []).map((ds, idx) => {
                ...
                if(type === 'doughnut') {
                    base.hoverOffset = 8;
                    // v229: warna chart Status Pekerjaan harus konsisten berdasarkan label,
                    // bukan berdasarkan urutan data. Selesai = hijau, Belum Selesai = merah.
                    const statusColorByLabel = (label) => {
                        const n = normalize(label || '');
                        if(n.includes('belum')) return '#ef4444';
                        if(n.includes('selesai')) return '#10b981';
                        return null;
                    };
                    const labelsList = Array.isArray(labels) ? labels : [];
                    const statusColors = labelsList.map(statusColorByLabel);
                    if(statusColors.some(Boolean)) {
                        base.backgroundColor = labelsList.map((label, i) => statusColors[i] || palette[(i + idx) % palette.length]);
                    }
                }
                return base;
            });
        ...
        function getClaimLemburMinutesForRow(d) {
            if(!d) return 0;

            // Dashboard Lembur admin/verifikator memakai DURASI KLAIM, bukan durasi absensi mentah.
            // Contoh: 09:45-12:20 = 2 jam 35 menit absensi, tetapi klaim yang ditampilkan/dijumlahkan = 2 jam.
            const byTime = hitungDurasiMenit(d.start, d.end);
            const rawMinutes = byTime > 0 ? byTime : parseLamaLemburMenit(d.lamaLembur);
            if(!rawMinutes || rawMinutes <= 0) return 0;
            const fullHours = Math.floor(rawMinutes / 60);
            return Math.max(0, Math.min(2, fullHours)) * 60;
        }

        function getLemburAbsensiMinutesForRow(d) {
            if(!d) return 0;
            const byTime = hitungDurasiMenit(d.start, d.end);
        ...
        function getLemburMinutesForRow(d) {
            return getClaimLemburMinutesForRow(d);
        }

        function formatAdminOvertimeClaimDuration(d) {
            const minutes = getClaimLemburMinutesForRow(d);
        ...
        function populateAdminOvertimeLabOptions() {
            ...
            const addLab = (lab) => {
                const key = canonicalLabKey(lab);
                if(!key || key === 'semua') return;
                labMap.set(key, getLabDisplayName(lab));
            };
            (window.masterPegawai || []).forEach(p => { if(p.lab) addLab(p.lab); });
            (window.arikaData || []).forEach(d => { if(d.lab) addLab(d.lab); });
            const sortedLabs = Array.from(labMap.values()).filter(Boolean).sort((a, b) => a.localeCompare(b, 'id', { sensitivity: 'base' }));
            ...
            const currentKey = canonicalLabKey(current);
            const matched = Array.from(select.options || []).find(opt => canonicalLabKey(opt.value) === currentKey);
            select.value = matched ? matched.value : 'Semua';
        ...
        function getAdminOvertimeFilters() {
            ...
            return {
                ...
                monthVal: normalizeMonthKeyFromInput(monthEl?.dataset?.arikaIsoValue || monthEl?.value || '') || '',
        ...
        function updateOvertimeDiagnostic(filters, filteredRows, allOvertimeRows) {
            ...
            if(diag.markedCount > 0) {
                ...
                el.innerHTML = `Terbaca <b>${diag.totalRows}</b> jurnal, tetapi belum ada nilai penanda lembur yang TRUE/YA. Pastikan kolom <b>Lembur?</b> pada database jurnal bernilai TRUE/Ya untuk kegiatan lembur. Penanda terbaca: ${escapeHTML(keyText)}.`;
            } else {
        ...
        window.forceReloadOvertimeData = async function() {
            if(!canAccessAdminPanel()) return;
            window.showLoader && window.showLoader(true, 'Mengambil ulang data lembur dari database...');
            try {
        ...
        window.renderAdminOvertimeDashboard = function(options = {}) {
            ...
            setText('admin-ot-top-person-note', stats.topPerson ? `${stats.topPerson.entries} entri • ${formatOvertimeHours(stats.topPerson.minutes)} jam lembur klaim • ${stats.topPerson.lab || '-'}` : 'Belum ada data.');
            setText('admin-ot-top-lab', stats.topLab ? stats.topLab[0] : '-');
            setText('admin-ot-top-lab-note', stats.topLab ? `${formatOvertimeHours(stats.topLab[1])} jam lembur klaim pada fungsi/lab ini.` : 'Belum ada data.');
            ...
            renderChart('admin-chart-overtime-lab', 'adminChartOvertimeLab', 'bar', labLabels, [
                { label: 'Jam Lembur Klaim', data: stats.labRows.slice(0, 8).map(item => Math.round((item[1] / 60) * 10) / 10) }
            ], {
                plugins: { legend: { display: false } },
                scales: {
                    y: { beginAtZero: true, ticks: { callback: value => `${value} jam` }, grid: { color: 'rgba(148,163,184,.16)' } },
                    ...
                    return `
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="text-center">
                                <div class="font-black text-amber-700">${formatAdminOvertimeClaimDuration(d)}</div>
                                <div class="text-[9px] text-slate-400 font-bold">Lembur Klaim • ${escapeHTML(d.start || '-')} - ${escapeHTML(d.end || '-')}</div>
                            </td>
        ...
        window.downloadAdminOvertimeExcel = function() {
            ...
            html += `<tr><th>Total Entri</th><th>Total Jam Lembur Klaim</th><th>Personil</th><th>Siap/Review</th><th>Perlu Lengkapi</th><th colspan="4">Filter</th></tr>`;
            ...
            html += `<tr><th>Tanggal</th><th>Nama</th><th>NIP</th><th>Fungsi/Lab</th><th>Uraian</th><th>Jam Mulai</th><th>Jam Selesai</th><th>Durasi Lembur Klaim</th><th>Status Verifikasi</th></tr>`;
            rows.forEach(d => {
                const statusInfo = getAdminOvertimeRowStatus(d);
                html += `<tr><td>${escapeHTML(d.date)}</td><td>${escapeHTML(d.name)}</td><td>${escapeHTML(d.nip)}</td><td>${escapeHTML(d.lab)}</td><td>${escapeHTML(d.desc)}</td><td>${escapeHTML(d.start)}</td><td>${escapeHTML(d.end)}</td><td>${escapeHTML(formatAdminOvertimeClaimDuration(d))}</td><td>${escapeHTML(statusInfo.label + (statusInfo.missing.length ? ' - Kurang: ' + statusInfo.missing.join(', ') : ''))}</td></tr>`;
            });
            html += `</table><br><table border="1"><tr><th colspan="5">REKAP PER PERSONIL</th></tr><tr><th>No</th><th>Nama</th><th>Fungsi/Lab</th><th>Entri</th><th>Jam Lembur Klaim</th></tr>`;
        ...
        window.downloadAdminOvertimeReportWord = function() {
            ...
            const html = `
                ...
                <p><b>Ringkasan:</b> Terdapat ${stats.totalEntries} entri lembur dari ${stats.personRows.length} personil dengan total jam lembur klaim ${stats.totalHours} jam lembur.</p>
                ...
                <table><tr><th>No</th><th>Nama</th><th>Fungsi/Lab</th><th>Entri</th><th>Jam Lembur Klaim</th><th>Perlu Lengkapi</th></tr>${personRows}</table>
        ...
        window.renderAdminAllTable = function(options = {}) {
            ...
            if(globalData.length === 0) {
                ...
                const msg = rawTotal === 0
                    ? 'Data jurnal belum terbaca. Klik Sinkron / refresh ARIKA.'
        ...
        function getDateRangeValues(startId, endId) {
            let start = getManualDateValueById(startId) || normalizeDateKeyFromSheet(document.getElementById(startId)?.value || '') || '';
            let end = getManualDateValueById(endId) || normalizeDateKeyFromSheet(document.getElementById(endId)?.value || '') || '';
        ...
        function getRekapMonthValue() {
            ...
            return getManualMonthValueById('filter-rekap-month', '');
        }

        function getRekapDateValue() {
            const dateEl = document.getElementById('filter-rekap-date');
            return getManualDateValueById('filter-rekap-date');
        }

        function dateMatchesRekapFilter(dateValue, monthValue, exactDateValue) {
            const dateKey = normalizeDateKeyFromSheet(dateValue) || String(dateValue || '').slice(0, 10);
            const exactKey = normalizeDateKeyFromSheet(exactDateValue) || '';
            const monthKey = normalizeMonthKeyFromInput(monthValue) || '';
            if(!dateKey) return false;
            if(exactKey) return dateKey === exactKey;
            if(monthKey) return dateKey.startsWith(monthKey);
            return true;
        }
        ...
        function normalizeRencanaItem(item = {}) {
            const normalized = { ...item };
            normalized.id = String(normalized.id || normalized.ID || normalized.timestamp || makeLocalId('rencana')).trim();
            normalized.ownerName = (normalized.ownerName || normalized.owner_name || normalized.nama || normalized.Nama || normalized.nama_pegawai || normalized['Nama Pegawai'] || normalized.pegawai || normalized.Pegawai || normalized.pelaksana || '').toString().trim();
            normalized.ownerNip = (normalized.ownerNip || normalized.owner_nip || normalized.nip || normalized.NIP || normalized.nip_pegawai || normalized['NIP Pegawai'] || '').toString().trim();
            normalized.tanggal = normalizeDateKeyFromSheet(normalized.tanggal || normalized.date || normalized.tgl || normalized.Tanggal || '') || getTodayKey();
            normalized.jamReminder = extractTime(normalized.jamReminder || normalized.jam_reminder || normalized['jam reminder'] || normalized.waktuReminder || normalized.waktu_reminder || normalized['waktu reminder'] || normalized.jam || normalized.waktu || '') || '';
            normalized.periode = (normalized.periode || normalized.jenis || normalized.type || 'Reminder').toString();
            normalized.judul = (normalized.judul || normalized.rencana || normalized.kegiatan || normalized.title || '').toString();
            normalized.catatan = (normalized.catatan || normalized.detail || normalized.deskripsi || normalized.note || '').toString();
            normalized.status = (normalized.status || 'Aktif').toString();
            normalized.rencanaId = (normalized.rencanaId || normalized.rencana_id || normalized['rencana id'] || normalized['Rencana ID'] || normalized.id || '').toString().trim();
            normalized.createdAt = (normalized.createdAt || normalized.timestamp || normalized.created_at || new Date().toISOString()).toString();
            return normalized;
        }

        function normalizeRencanaList(list = []) {
            return (list || []).map(normalizeRencanaItem).filter(r => r.judul || r.catatan);
        }

        function isRecentPendingLocalRencana(item) {
            item = normalizeRencanaItem(item || {});
            if(!item || normalize(item.syncStatus || '') !== 'pending') return false;
            const created = Date.parse(item.createdAt || '');
            if(!Number.isFinite(created)) return false;
            // v228: pending lokal dipertahankan lebih lama agar tidak hilang bila Apps Script/Sheet lambat terbaca.
            return Date.now() - created < 12 * 60 * 60 * 1000;
        }

        function applyCloudRencanaData(mappedList) {
            const cloudList = applyRencanaDeleteTombstones(dedupeRencanaList(normalizeRencanaList(mappedList || []).map(item => ({ ...item, syncStatus: 'synced' }))));
        ...
        function getRencanaKey(item) {
            ...
            return [
                ...
                normalizeDateKeyFromSheet(item.tanggal || '') || '',
        ...
        function dedupeRencanaList(list) {
            ...
            const result = [];
            normalizeRencanaList(list || []).forEach(item => {
                if(!item) return;
                const id = String(item.id || '').trim();
        ...
        function getStableRencanaData(extraList = []) {
            ...
            return applyRencanaDeleteTombstones(dedupeRencanaList(normalizeRencanaList([...(extraList || []), ...liveList, ...localList, ...coreCacheList])));
        }

        function saveStableRencanaData(list) {
            const stable = applyRencanaDeleteTombstones(dedupeRencanaList(normalizeRencanaList(list || [])));
            if(stable.length > 0) {
        ...
        function isCurrentUserRencana(item) {
            if(!window.currentUser || window.isAdmin) return false;
            item = normalizeRencanaItem(item || {});
            const userNip = normalize(window.currentUser.nip || '');
            const itemNip = normalize(item.ownerNip || '');
            if(userNip && itemNip && (userNip === itemNip || itemNip.includes(userNip) || userNip.includes(itemNip))) return true;

            const userName = normalize(window.currentUser.nama || '');
            const ownerName = normalize(item.ownerName || '');
            if(userName && ownerName && (userName === ownerName || userName.includes(ownerName) || ownerName.includes(userName))) return true;

            // v228: beberapa data rencana dari Sheet bisa punya header berbeda. Gunakan pencocokan fuzzy seperti jurnal.
            try {
                if(personMatchesRow({
                    ...item,
                    nip: item.ownerNip,
                    nama: item.ownerName,
                    pelaksana: item.ownerName,
                    raw: item.rawText || ''
                }, window.currentUser)) return true;
            } catch(_) {}
            return false;
        }
        ...
        function getRencanaJamReminder(item) {
            return extractTime(item?.jamReminder || item?.jam_reminder || item?.['jam reminder'] || item?.waktuReminder || item?.waktu_reminder || item?.['waktu reminder'] || item?.jam || item?.waktu || '');
        }
        ...
        function parseRencanaDueDate(item) {
            const tgl = normalizeDateKeyFromSheet(item?.tanggal || '');
            if(!tgl) return null;
        ...
        function isRencanaTanggalHariIni(item) {
            return !isRencanaSelesai(item) && normalizeDateKeyFromSheet(item.tanggal || '') === getTodayKey();
        }
        ...
        function isRencanaHariIni(item) {
            if(isRencanaSelesai(item) || normalizeDateKeyFromSheet(item.tanggal || '') !== getTodayKey()) return false;
            const diffMin = getRencanaDueMinutes(item);
        ...
        function isRencanaDalamRentangMinggu(item) {
            ...
            const tgl = normalizeDateKeyFromSheet(item.tanggal || today) || today;
        ...
        const ARIKA_JURNAL_CATEGORIES = ['Teknis Laboratorium', 'Rapat', 'Pengembangan Kompetensi', 'Administrasi', 'Lainnya'];

        function normalizeJurnalCategoryValue(value) {
            const raw = String(value || '').trim();
            const key = raw.toLowerCase().replace(/\s+/g, ' ');
            if(!raw) return '';
            if(['rapat/workshop', 'rapat / workshop', 'rapat workshop'].includes(key)) return 'Rapat';
            if(/^(workshop|pelatihan|bimtek|bimbingan teknis|sosialisasi|seminar|webinar|diklat|sertifikasi|coaching|mentoring|sharing|pengembangan kompetensi)$/.test(key)) return 'Pengembangan Kompetensi';
            if(key === 'teknis' || key === 'teknis laboratorium') return 'Teknis Laboratorium';
            if(key === 'administrasi') return 'Administrasi';
            if(key === 'lainnya' || key === 'lain-lain') return 'Lainnya';
            return raw;
        }

        function jurnalCategoryMatches(rowCat, selectedCat) {
            const selected = normalizeJurnalCategoryValue(selectedCat);
            if(!selected || selected === 'Semua') return true;
            return normalizeJurnalCategoryValue(rowCat) === selected;
        }

        function updateJurnalCategoryVisual(value) {
            const safeValue = normalizeJurnalCategoryValue(value);
        ...
        function buildJurnalPayload() {
            ...
            const cat = normalizeJurnalCategoryValue(document.getElementById('in-cat')?.value.trim() || '');
            ...
            return {
                ...
                lamaLembur,
                waktuLembur,
                rencanaId: (window.arikaLinkedRencana && window.arikaLinkedRencana.rencanaId) || '',
                rencanaJudul: (window.arikaLinkedRencana && window.arikaLinkedRencana.rencanaJudul) || '',
                createdAt: new Date().toISOString(),
        ...
        function resetJurnalForm() {
            ...
            window.arikaLinkedRencana = null;
            try { sessionStorage.removeItem('arika_linked_rencana_v297'); sessionStorage.removeItem('arika_linked_rencana_v301'); sessionStorage.removeItem('arika_linked_rencana_v326'); localStorage.removeItem('arika_linked_rencana_v301'); localStorage.removeItem('arika_linked_rencana_v326'); } catch(_) {}
            if(window.updateLinkedRencanaCard) window.updateLinkedRencanaCard();
        ...
        window.renderRencanaPribadi = function() {
            ...
            const list = dedupeRencanaList((window.rencanaData || [])
                .filter(isCurrentUserRencana)
                // v228: jangan batasi hanya minggu ini. Sebagian user membuat rencana di luar rentang 7 hari,
                // sehingga data sudah masuk Sheet tetapi Beranda tampak kosong. Tampilkan semua rencana aktif user,
                // lalu kartu hanya memperlihatkan 3 prioritas teratas agar tetap ringkas.
                .filter(item => !isRencanaSelesai(item)))
                .sort((a, b) => {
            ...
            if(list.length === 0) {
                listEl.innerHTML = '<div class="p-4 rounded-2xl bg-slate-50 border border-slate-100 text-center text-slate-400 text-[10px] uppercase tracking-widest font-black">Belum ada rencana pribadi aktif</div>';
                return;
            }
            ...
            const summaryHtml = `
                <div class="grid grid-cols-3 gap-2 mb-3">
                    ...
                    <div class="rounded-2xl border border-emerald-100 bg-emerald-50 p-2 text-center">
                        ...
                        <p class="text-[8px] font-black uppercase tracking-widest text-emerald-700">Aktif</p>
            ...
            const moreInfo = list.length > 6 ? `<div class="p-3 rounded-2xl bg-slate-50 border border-slate-100 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Menampilkan semua ${list.length} rencana aktif.</div>` : '';
            ...
            const listHtml = visibleList.map(item => {
                ...
                const label = statusDone ? 'Selesai' : (overdue ? 'Terlewat' : (todayActive ? 'Alarm Sekarang' : (todayScheduled ? 'Terjadwal Hari Ini' : 'Aktif')));
                ...
                return `
                    <div class="${baseClass} ${badge}">
                        ...
                        ${warningText}
                        ${!statusDone ? `<div class="mt-3 flex flex-wrap gap-2"><button onclick="window.tandaiRencanaSelesai('${escapeHTML(item.id)}')" class="px-3 py-1.5 rounded-lg bg-white/80 hover:bg-white text-[9px] font-black uppercase tracking-widest">Tandai Selesai</button><button onclick="window.mulaiRealisasiRencana('${escapeHTML(item.id)}')" class="arika-btn-isi-realisasi px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 text-[9px] font-black uppercase tracking-widest">Isi Realisasi</button></div>` : ''}
                    </div>
                `;
            }).join('');
        ...
        window.simpanRencanaPribadi = async function(e) {
            ...
            const payload = normalizeRencanaItem({
                id: makeLocalId('rencana'),
                rencanaId: window.generateArikaRencanaId ? window.generateArikaRencanaId(document.getElementById('rencana-tanggal')?.dataset?.arikaIsoValue || document.getElementById('rencana-tanggal')?.value || getTodayKey()) : '',
                ownerName: window.currentUser.nama || '',
                ownerNip: window.currentUser.nip || '',
                tanggal: normalizeDateKeyFromSheet(document.getElementById('rencana-tanggal')?.dataset?.arikaIsoValue || document.getElementById('rencana-tanggal')?.value || '') || getTodayKey(),
            ...
            try {
                // v320: tampilkan rencana secara instan dulu, lalu stabilkan cache di belakang layar.
                const optimisticList = [payload, ...(window.rencanaData || [])];
                window.rencanaData = optimisticList;
                try { saveLocalRencana(optimisticList); } catch(_) {}
                try { saveLocalCoreCache(); } catch(_) {}
                ...
                setDefaultDates();
                window.renderRencanaPribadi();
                try { window.applyRencanaFastDecorationsV320 && window.applyRencanaFastDecorationsV320(); } catch(_) {}
                window.showLoader(false);
                window.isSavingRencana = false;
                setTimeout(function(){
                    try {
                        window.rencanaData = saveStableRencanaData([payload, ...(window.rencanaData || []), ...loadLocalRencana()]);
                        window.renderRencanaPribadi && window.renderRencanaPribadi({ source: 'v320-stabilize-rencana' });
                        window.applyRencanaFastDecorationsV320 && window.applyRencanaFastDecorationsV320();
                    } catch(e) { console.warn('Stabilisasi cache rencana v320 gagal:', e); }
                }, 80);
                ...
                // v228: beberapa perangkat berhasil menulis ke Sheet tetapi cache cloud lambat. Tarik ulang data setelah simpan.
                [1200, 4500].forEach((delay) => setTimeout(() => {
                    try { window.fetchCloudData && window.fetchCloudData({ force: true }); } catch(e) {}
                }, delay));
                ...
                postToScript('save_rencana', payload)
                    .then(function(resp) {
                        try {
                            ...
                            if(syncedItem) {
                                ...
                                if(resp && resp.rencanaId) syncedItem.rencanaId = resp.rencanaId;
                                else if(payload.rencanaId && !syncedItem.rencanaId) syncedItem.rencanaId = payload.rencanaId;
                            }
                    ...
                    .catch(function(err) {
                        console.warn('Sinkronisasi reminder ke database gagal. Data tetap tersimpan lokal dan akan muncul di perangkat ini:', err);
                    });
            } catch(err) {
        ...
        window.renderAgendaSaya = function() {
            ...
            if(!board) return;
            board.classList.add('arika-board-list-modern');
            ...
            let list = [];
            if(window.isAdmin) {
                if(titleEl) titleEl.innerText = 'Agenda Kegiatan Tim';
                if(subtitleEl) subtitleEl.innerText = 'Jadwal terdekat yang perlu dipantau.';
                list = (window.agendaData || [])
                    .filter(a => isAgendaTampilDiBeranda(a, maxDate))
                    .sort(sortAgendaInteraktif);
            } else {
                if(titleEl) titleEl.innerText = 'Agenda Kegiatan Saya';
                if(subtitleEl) subtitleEl.innerText = 'Jadwal terdekat yang perlu diikuti.';

                if(!window.currentUser) {
                    board.innerHTML = '<div class="arika-board-empty">Agenda akan tampil setelah login</div>';
                    return;
                }
            ...
            if(!list.length) {
                board.innerHTML = `<div class="arika-board-empty">${window.isAdmin ? 'Belum ada agenda tim aktif dalam 14 hari ke depan' : 'Belum ada agenda aktif untuk kamu'}</div>`;
                return;
            }

            const visibleList = list;
            const moreInfo = list.length > 3 ? `<div class="arika-board-more">+${list.length - 3} agenda lainnya tersimpan di daftar agenda.</div>` : '';

            board.innerHTML = visibleList.map(item => {
                const info = getAgendaReminderInfo(item);
                const isUrgent = ['mulai', '24jam'].includes(info.status);
                const isSoft = ['lewat_hari_ini', 'mendatang', 'tanpa_jadwal'].includes(info.status);
                const toneClass = isUrgent ? 'is-urgent' : (isSoft ? 'is-soft' : '');
                const pillClass = info.status === 'mulai' ? 'danger' : (info.status === '24jam' ? 'warn' : '');
                const timeText = item.waktuMulai ? `${escapeHTML(item.waktuMulai)} WIT` : 'Waktu belum diisi';
                const dateText = formatDateIndo(item.tanggal) || 'Tanpa tanggal';
                const pesertaBerandaArr = uniqueAgendaTextList(item.pesertaNama || item.pesertaNip || '-');
                const pesertaBeranda = pesertaBerandaArr.length > 4 ? `${pesertaBerandaArr.slice(0, 4).join(', ')} +${pesertaBerandaArr.length - 4}` : (pesertaBerandaArr.join(', ') || '-');
                const pesertaText = window.isAdmin ? `<span class="arika-board-mini-tag">Peserta: ${escapeHTML(pesertaBeranda)}</span>` : '';
                const lokasiText = item.lokasi ? `<span class="arika-board-mini-tag">📍 ${linkifyText(item.lokasi)}</span>` : '';
                const keterangan = item.keterangan ? `<p class="arika-board-card-desc">${linkifyText(item.keterangan)}</p>` : '';
                return `
                    <div class="arika-board-card ${toneClass}">
                        <div class="arika-board-card-icon">${info.status === 'mulai' ? '🚨' : (info.status === '24jam' ? '🔔' : '🗓️')}</div>
                        <div class="arika-board-card-body">
                            <div class="arika-board-topline">
                                <span class="arika-board-pill ${pillClass}">${escapeHTML(info.label)}</span>
                                <span class="arika-board-meta">${escapeHTML(dateText)} • ${timeText}</span>
                            </div>
                            <h3 class="arika-board-title">${escapeHTML(item.judul || 'Agenda Kegiatan')}</h3>
                            <div class="arika-board-meta">${escapeHTML(item.jenis || 'Agenda')}</div>
                            ${keterangan}
                            <div class="arika-board-footline">${lokasiText}${pesertaText}</div>
                        </div>
                    </div>
                `;
            }).join('') + moreInfo;
        ...
        window.renderPengumumanBoard = function() {
            ...
            if(list.length === 0) {
                board.innerHTML = '<div class="arika-board-empty">Belum ada pengumuman aktif</div>';
                return;
            }

            const visibleList = list;
            const moreInfo = list.length > 3 ? `<div class="arika-board-more">+${list.length - 3} pengumuman lainnya tersimpan di papan informasi.</div>` : '';
            ...
            board.innerHTML = visibleList.map(item => {
                ...
                const todayActive = start <= today && end >= today;
                const tomorrowActive = !todayActive && start <= tomorrow && end >= tomorrow;
                const statusText = todayActive ? 'Aktif Hari Ini' : (tomorrowActive ? 'Untuk Besok' : 'Aktif');
                const pillClass = todayActive ? 'warn' : (tomorrowActive ? '' : '');
                const toneClass = todayActive ? 'is-urgent' : (tomorrowActive ? '' : 'is-soft');
                const icon = todayActive ? '🔔' : (tomorrowActive ? '⏰' : '📌');
                const rangeText = item.mulai && item.selesai
                    ? `${formatDateIndo(item.mulai)} – ${formatDateIndo(item.selesai)}`
                    : (item.mulai ? formatDateIndo(item.mulai) : 'Tanpa tanggal');
                const timeText = item.waktuMulai ? `<span class="arika-board-mini-tag">⏱️ ${escapeHTML(item.waktuMulai)} WIT</span>` : '';
                const targetText = item.targetFungsi && normalize(item.targetFungsi) !== normalize('Semua') ? `<span class="arika-board-mini-tag">🎯 ${escapeHTML(item.targetFungsi)}</span>` : '';
                const isi = item.isi ? `<p class="arika-board-card-desc">${linkifyText(item.isi)}</p>` : '';

                return `
                    <div class="arika-board-card ${toneClass}">
                        <div class="arika-board-card-icon">${icon}</div>
                        <div class="arika-board-card-body">
                            <div class="arika-board-topline">
                                <span class="arika-board-pill ${pillClass}">${escapeHTML(item.jenis || 'Informasi')}</span>
                                <span class="arika-board-meta">${escapeHTML(statusText)} • ${escapeHTML(rangeText)}</span>
                            </div>
                            <h3 class="arika-board-title">${escapeHTML(item.judul || 'Pengumuman')}</h3>
                            ${isi}
                            <div class="arika-board-footline">${timeText}${targetText}</div>
                        </div>
                    </div>
                `;
            }).join('') + moreInfo;
        ...
        window.simpanPengumuman = async function(e) {
            ...
            const payload = {
                ...
                mulai: getManualDateValueById('pengumuman-mulai') || getTodayKey(),
                selesai: getManualDateValueById('pengumuman-selesai') || getTodayKey(),
        ...
        window.simpanAgenda = async function(e) {
            ...
            const payload = {
                ...
                tanggal: getManualDateValueById('agenda-tanggal') || getTodayKey(),
        ...
        window.tambahPegawai = async function(e) {
            ...
            if(!payload.nama || !payload.nip) return window.showCustomAlert('Nama dan kata sandi wajib diisi.');
        ...
        function buildDashboardOvertimeTimeline(rows) {
            ...
            (rows || []).forEach(row => {
                const key = String(row.date || '').slice(0, 7);
                if(key in totals) totals[key] += getLemburAbsensiMinutesForRow(row);
            });
            return {
                labels: months.map(m => formatBulanIndonesia(m).replace(/^(\S+)\s+/, (match, p1) => p1.slice(0, 3) + ' ')),
                values: months.map(m => Math.round((totals[m] / 60) * 10) / 10)
        ...
        window.renderDashboardPegawai = function(options = {}) {
            ...
            const monthVal = monthEl ? ((monthEl.dataset && monthEl.dataset.periodMode === 'all') ? '' : getManualMonthValueById('pegawai-dashboard-month', getCurrentMonth())) : getCurrentMonth();
            ...
            const overtimeMinutes = myOvertimeRows.reduce((sum, d) => sum + getLemburAbsensiMinutesForRow(d), 0);
        ...
        window.downloadExcelLembur = function() {
            ...
            const month = getManualMonthValueById('filt-lembur-bulan', getCurrentMonth());
        ...
        window.downloadAdminSPKWord = function() {
            ...
            const spkDate = getManualDateValueById('admin-spk-date');
        ...
        function hasUserFilledSurveyThisMonthForReminder() {
            try {
                if(!window.currentUser || window.isAdmin) return true;
                if(getUserSurveyThisMonth()) return true;
                const month = getCurrentSurveyMonth();
                const nipKey = normalize(window.currentUser.nip || '');
                const nameKey = normalize(window.currentUser.nama || '');
                return (window.surveiData || []).some(s => {
                    const surveyMonth = String(s.bulan || '').slice(0, 7);
                    if(surveyMonth !== month) return false;
                    const surveyNip = normalize(s.nip || '');
                    const surveyName = normalize(s.nama || '');
                    return (nipKey && surveyNip && surveyNip === nipKey) || (nameKey && surveyName && surveyName === nameKey);
                });
            } catch(e) {
                return false;
            }
        ...
        window.simpanSurveiArika = async function(e) {
            ...
            try {
                ...
                try { window.updateArikaReminderCenter && window.updateArikaReminderCenter(); } catch(e) {}
                try { window.checkBerandaReminderSound && window.checkBerandaReminderSound({ source: 'survei-saved' }); } catch(e) {}
        ...
        function getAdminSurveyRows() {
            const monthVal = getManualMonthValueById('admin-survei-month', getCurrentMonth());
        ...
        function getArikaReminderCenterIcon(kind) {
            ...
            if(key.includes('survei') || key.includes('survey')) return '📝';
        ...
        function updateArikaReminderCenter() {
            ...
            if(!events.length) {
                list.innerHTML = `<div class="arika-reminder-center-empty">Tidak ada reminder aktif saat ini. Reminder akan muncul untuk jurnal, survei bulanan, catatan atasan, rencana pribadi, pengumuman, dan agenda yang perlu diperhatikan.</div>`;
                return;
            }
        ...
        window.updateArikaReminderCenter = updateArikaReminderCenter;
        ...
        // Mode fokus Panduan ARIKA: menyembunyikan info lain saat pengguna memilih topik tertentu.
        window.arikaPanduanFocus = function(sectionId, label, inlineId) {
            ...
            if (subtitle) subtitle.textContent = 'Info lainnya disembunyikan agar pengguna fokus membaca panduan ini.';
...
(function(){
  ...
  window.updateRekapFilterUI = function(){
    var cat = normalizeJurnalCategoryValue(getHiddenValue('filter-rekap-cat', 'Semua') || 'Semua') || 'Semua';
  ...
  window.setRekapCategory = function(value){
    setHiddenValue('filter-rekap-cat', normalizeJurnalCategoryValue(value || 'Semua') || 'Semua');
    window.updateRekapFilterUI();
    safeRunFilter();
  };
...
(function(){
  ...
  function injectManualDateStyle(){
    ...
    style.textContent = `
      ...
      .arika-date-helper { display:none !important; }
    `;
    document.head.appendChild(style);
  }

  function addInputHints(){
    document.querySelectorAll('.arika-date-helper').forEach(function(el){ el.remove(); });
  }
...
(function(){
  ...
  function restoreMobileNavDropdown(){
    try {
      ...
      if(!select) return;
      var isMobileNav = !(window.matchMedia && window.matchMedia('(min-width: 901px)').matches);
      var navWrapForMode = select.parentElement;
      var desktopMenuForMode = document.getElementById('nav-menu');
      if(!isMobileNav) {
        select.hidden = true;
        ...
        if(navWrapForMode) navWrapForMode.style.display = 'none';
        if(desktopMenuForMode) {
          desktopMenuForMode.style.display = 'flex';
          desktopMenuForMode.style.visibility = 'visible';
          desktopMenuForMode.style.pointerEvents = 'auto';
        }
        return;
      }
      select.hidden = false;
...
// ARIKA v219 - Pulihkan dropdown native dan hentikan sumber chip duplikat secara total.
// Catatan: chip manual yang memang desain asli tetap dipertahankan, seperti kategori Isi Jurnal,
// status akhir Isi Jurnal, status evaluasi, dan filter Riwayat Jurnal. Yang dibersihkan adalah
// chip otomatis hasil konversi <select> dari versi sebelumnya.
(function(){
  'use strict';

  const AUTO_CHIP_SELECTORS = [
  ...
  function injectStyle(){
    if(document.getElementById('arika-v219-native-select-style')) return;
    const style = document.createElement('style');
    style.id = 'arika-v219-native-select-style';
    style.textContent = `
      /* Matikan semua chip otomatis hasil konversi select dari versi lama */
      .arika-v206-chip-group,
      .arika-chip-select-group,
      #arika-mobile-nav-chips,
      .arika-mobile-nav-chips {
      ...
      /* Pulihkan dropdown asli yang sebelumnya sempat disembunyikan */
      ...
      select[id] { max-width: 100%; }
      @media (max-width: 768px) {
        #mobile-nav-select,
        #mobile-nav-select.mobile-nav-select-arika {
          display: block !important;
          visibility: visible !important;
          opacity: 1 !important;
          position: static !important;
        ...
        #main-nav .mobile-nav-wrap { display: flex !important; }
      }
    `;
    document.head.appendChild(style);
  }
  ...
  function removeAutoChipGroups(){
    document.querySelectorAll(AUTO_CHIP_SELECTORS).forEach(safeRemove);
  }
  ...
  function restoreNativeSelects(){
    document.querySelectorAll('select[id]').forEach(function(select){
      ...
      select.removeAttribute('aria-hidden');
      select.removeAttribute('data-arika-chip-converted');
      if(select.tabIndex === -1) select.removeAttribute('tabindex');
      const inline = select.style;
      ['clip','opacity','pointerEvents','overflow','whiteSpace','borderWidth','margin','position','width','height','minHeight','padding','visibility','maxWidth'].forEach(function(prop){
        try { inline[prop] = ''; } catch(_) {}
      });
      // Dropdown navigasi mobile memang harus tetap terlihat di HP.
      if(select.id === 'mobile-nav-select') {
        const isMobileNav = !(window.matchMedia && window.matchMedia('(min-width: 901px)').matches);
        const navWrapForMode = select.parentElement;
        const desktopMenuForMode = document.getElementById('nav-menu');
        if(isMobileNav) {
          select.hidden = false;
          select.style.display = 'block';
          select.style.visibility = 'visible';
          ...
          select.style.maxWidth = 'min(92vw, 360px)';
          select.style.pointerEvents = 'auto';
          if(navWrapForMode) navWrapForMode.style.display = 'flex';
          if(desktopMenuForMode) {
            desktopMenuForMode.style.display = 'none';
            desktopMenuForMode.style.visibility = 'hidden';
            desktopMenuForMode.style.pointerEvents = 'none';
          }
        } else {
          select.hidden = true;
          select.style.display = 'none';
  ...
  function cleanup(){
    injectStyle();
    removeAutoChipGroups();
    restoreNativeSelects();
  }

  // Jika fungsi dari versi lama masih dipanggil oleh handler lain, arahkan menjadi cleanup saja.
  window.refreshArikaChipSelects = cleanup;
  window.cleanArikaDuplicateChips = cleanup;
  window.stabilkanDropdownArika = cleanup;

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', cleanup);
  else cleanup();
  [50, 150, 400, 900, 1600, 2600, 4200].forEach(function(ms){ setTimeout(cleanup, ms); });
...
// ARIKA v220 - Normalisasi fungsi/lab dan navigasi dropdown hanya untuk mobile.
(function(){
  'use strict';
  if (window.__ARIKA_V220_PATCH__) return;
  window.__ARIKA_V220_PATCH__ = true;
  ...
  function labKey(value){
    ...
    if(!key || key === 'semua' || key === 'semualab' || key === 'semuafungsilab' || key === 'semuafungsi') return 'semua';
    if(key.includes('mikrobiologi')) return 'mikrobiologi';
    if(key.includes('pangan')) return 'kimiapangan';
    if(key.includes('kosmetik')) return 'kimiakosmetik';
    if(key.includes('obnaz') || raw.includes('obat dan narkotika') || raw.includes('narkotika')) return 'kimiaobnaz';
    if(key.includes('kimiaoba') || key.includes('pengujiankimiaoba') || key.includes('staffungsipengujiankimiaoba') || key.includes('obatbahanalam') || (key.includes('oba') && !key.includes('obnaz'))) return 'kimiaoba';
    return key;
  }
  function labDisplay(value){
    const map = {
      kimiapangan: 'Kimia Pangan',
      kimiaoba: 'Kimia OBA',
      kimiaobnaz: 'Kimia Obnaz',
    ...
    return String(value || '').replace(/^\s*(staf\s+fungsi\s+)?pengujian\s+/i, '').replace(/^\s*staf\s+fungsi\s+pengujian\s+/i, '').trim() || '';
  }
  window.arikaLabKey = labKey;
  window.arikaLabDisplay = labDisplay;

  function normalizeDataLabs(){
    try {
      (window.masterPegawai || []).forEach(function(p){ if(p && p.lab) p.lab = labDisplay(p.lab); });
      (window.arikaData || []).forEach(function(d){ if(d && d.lab) d.lab = labDisplay(d.lab); });
      (window.rencanaData || []).forEach(function(d){ if(d && d.lab) d.lab = labDisplay(d.lab); });
    } catch(e) {}
  }

  function dedupeSelectOptions(select){
    if(!select) return;
    const keepAllText = /status|type|peran|jenis|fitur|peserta/i.test(select.id || '') ? null : 'Semua Fungsi/Lab';
    const isLabSelect = /lab|fungsi/i.test(select.id || '') || select.id === 'add-lab';
    if(!isLabSelect) return;
    const currentKey = labKey(select.value || 'Semua');
    const seen = new Set();
    const options = Array.from(select.options || []);
    options.forEach(function(opt){
      const key = labKey(opt.value || opt.textContent || '');
      if(!key) return;
      if(key === 'semua') {
        opt.value = 'Semua';
        opt.textContent = keepAllText || opt.textContent || 'Semua';
      } else {
        opt.value = labDisplay(opt.value || opt.textContent);
        opt.textContent = labDisplay(opt.textContent || opt.value);
      }
      if(seen.has(key)) opt.remove();
      else seen.add(key);
    });
    const match = Array.from(select.options || []).find(function(opt){ return labKey(opt.value) === currentKey; });
    if(match) select.value = match.value;
    else if(Array.from(select.options || []).some(function(opt){ return labKey(opt.value) === 'semua'; })) select.value = 'Semua';
  }

  function dedupeLabControls(){
    ['add-lab','admin-analytics-lab','admin-overtime-lab','admin-filter-lab','pengumuman-target-fungsi'].forEach(function(id){
      dedupeSelectOptions(document.getElementById(id));
    });
    // Agenda group buttons: pastikan label dan data group memakai nama kanonik.
    document.querySelectorAll('[data-agenda-group]').forEach(function(btn){
      const v = btn.getAttribute('data-agenda-group');
      const key = labKey(v);
      if(key && key !== 'semua' && ['kimiapangan','kimiaoba','kimiaobnaz','kimiakosmetik','mikrobiologi'].includes(key)) {
        const label = labDisplay(v);
        btn.setAttribute('data-agenda-group', label);
        if(/^\+/.test(btn.textContent || '')) btn.textContent = '+ ' + label;
      }
    });
  }

  function hideAutoChipDuplicates(){
    // Hapus chip otomatis dari patch lama. Chip manual yang punya id/konteks asli tidak disentuh.
  ...
  function syncNavForViewport(){
    ...
    const wrap = document.querySelector('#main-nav .mobile-nav-wrap');
  ...
  function injectStyle(){
    if(document.getElementById('arika-v220-final-style')) return;
    const style = document.createElement('style');
    style.id = 'arika-v220-final-style';
    style.textContent = `
      .arika-v206-chip-group, .arika-chip-select-group, #arika-mobile-nav-chips, .arika-mobile-nav-chips { display:none!important; visibility:hidden!important; pointer-events:none!important; height:0!important; overflow:hidden!important; }
      @media (min-width: 901px) {
        #main-nav #nav-menu { display:flex!important; visibility:visible!important; pointer-events:auto!important; }
        #main-nav .mobile-nav-wrap, #mobile-nav-select { display:none!important; visibility:hidden!important; pointer-events:none!important; opacity:0!important; }
      }
      @media (max-width: 900px) {
        #main-nav #nav-menu { display:none!important; visibility:hidden!important; pointer-events:none!important; }
        #main-nav .mobile-nav-wrap { display:flex!important; visibility:visible!important; pointer-events:auto!important; }
        #mobile-nav-select { display:block!important; visibility:visible!important; opacity:1!important; pointer-events:auto!important; }
      }
    `;
    document.head.appendChild(style);
  }
  ...
  function run(){
    ...
    normalizeDataLabs();
    dedupeLabControls();
    hideAutoChipDuplicates();
    syncNavForViewport();
  }

  ['renderAdminAnalytics','renderAdminOvertimeDashboard','renderAdminAllTable','renderAgendaPersonilPicker','renderPengumumanList','renderAgendaList'].forEach(function(name){
    const fn = window[name];
    if(typeof fn === 'function' && !fn.__arikaV220Wrapped){
      const wrapped = function(){
        normalizeDataLabs();
        dedupeLabControls();
        const res = fn.apply(this, arguments);
        setTimeout(function(){ normalizeDataLabs(); dedupeLabControls(); hideAutoChipDuplicates(); syncNavForViewport(); }, 30);
        return res;
      };
      wrapped.__arikaV220Wrapped = true;
      window[name] = wrapped;
    }
  });
  ...
  [80, 250, 700, 1500, 3000, 5200].forEach(function(ms){ setTimeout(run, ms); });
  setInterval(run, 1200);
})();

/* ===== ARIKA v227: Isi Jurnal Ringkas (integrated) ===== */
/* ARIKA v227 Patch - Isi Jurnal ringkas
   Pasang setelah script utama ARIKA versi aktif.
   Fungsi: menghapus keterangan berlebih pada tab Isi Jurnal dan merapikan hero section.
*/
(function () {
  'use strict';

  const PATCH_ID = 'arika-v227-isi-jurnal-ringkas';
  let cleanTimer = null;
  ...
  function addStyle() {
    ...
    style.textContent = `
      /* v227: ringkas area pembuka Isi Jurnal */
      #view-jurnal .arika-ij-hero-compact {
        margin-bottom: 1.25rem !important;
        gap: .75rem !important;
      }
      #view-jurnal .arika-ij-title-compact {
        font-size: clamp(1.35rem, 2vw, 2rem) !important;
        line-height: 1.15 !important;
        margin-top: .25rem !important;
      }
      #view-jurnal .arika-ij-remove,
      #view-jurnal [data-arika-v227-hidden="true"] {
        display: none !important;
      }
      @media (min-width: 1024px) {
        #view-jurnal .relative.p-5.md\\:p-10 {
  ...
  function removeIfTextMatches(selector, matchers) {
    document.querySelectorAll(selector).forEach((el) => {
      const t = norm(el.textContent);
      if (!t) return;
      if (matchers.some((rx) => rx.test(t))) {
        el.setAttribute('data-arika-v227-hidden', 'true');
      }
    });
  }

  function findIsiJurnalHero() {
    const title = Array.from(document.querySelectorAll('#view-jurnal h1, #view-jurnal h2, #view-jurnal h3'))
      .find((el) => /Yuk catat kegiatan hari ini/i.test(norm(el.textContent)) || /^Isi Jurnal$/i.test(norm(el.textContent)));
    if (!title) return null;
    return title.closest('.text-white') || title.closest('div');
  }

  function compactIsiJurnalHero() {
    const hero = findIsiJurnalHero();
    if (!hero) return;
    hero.classList.add('arika-ij-hero-compact');

    const title = Array.from(hero.querySelectorAll('h1, h2, h3'))
      .find((el) => /Yuk catat kegiatan hari ini/i.test(norm(el.textContent)) || /^Isi Jurnal$/i.test(norm(el.textContent)));
    if (title) {
      title.textContent = 'Isi Jurnal';
      title.classList.add('arika-ij-title-compact');
    }

    // Hapus paragraf penjelasan panjang di area pembuka
    hero.querySelectorAll('p').forEach((p) => {
      const t = norm(p.textContent);
      if (/Isi jurnal singkat, rapi, dan mudah dilacak/i.test(t) || /Untuk kegiatan lembur, lengkapi jam/i.test(t)) {
        p.setAttribute('data-arika-v227-hidden', 'true');
      }
    });

    // Hapus badge: Cepat, Mudah dipantau, Siap direkap
    hero.querySelectorAll('span, div').forEach((el) => {
      const t = norm(el.textContent);
      if (/^(⚡\s*)?Cepat$/i.test(t) || /^(✅\s*)?Mudah dipantau$/i.test(t) || /^(📄\s*)?Siap direkap$/i.test(t)) {
        el.setAttribute('data-arika-v227-hidden', 'true');
      }
    });

    // Jika parent badge sudah kosong, sembunyikan parentnya
    hero.querySelectorAll('div').forEach((div) => {
      const t = norm(div.textContent);
      if (/^(⚡\s*)?Cepat\s*(✅\s*)?Mudah dipantau\s*(📄\s*)?Siap direkap$/i.test(t)) {
        div.setAttribute('data-arika-v227-hidden', 'true');
      }
    });

    // Hapus kartu ikon Tanggal/Status pada hero kanan
    hero.querySelectorAll('div').forEach((div) => {
      const t = norm(div.textContent);
      if ((/📅/.test(t) && /Tanggal/i.test(t)) || (/✅/.test(t) && /^✅\s*Status$/i.test(t))) {
        const grid = div.closest('.grid') || div;
        grid.setAttribute('data-arika-v227-hidden', 'true');
      }
    });
  }

  function cleanIsiJurnalNotes() {
    removeIfTextMatches('#view-jurnal p, #view-jurnal small, #view-jurnal .text-slate-400, #view-jurnal .text-slate-500', [
      /Klik salah satu kategori/i,
      /Model ini menggantikan dropdown/i,
      /Klik salah satu status/i,
      /Tidak memakai dropdown/i,
      /lebih ringan dan tidak memunculkan layar hitam/i
    ]);
  }

  function clean() {
    addStyle();
    compactIsiJurnalHero();
    cleanIsiJurnalNotes();
  }

  function scheduleClean() {
    clearTimeout(cleanTimer);
    cleanTimer = setTimeout(clean, 40);
  }
  ...
  // ARIKA banyak melakukan render ulang; observer menjaga agar teks tidak muncul lagi.
...
/* v233: Warna teks "ARIKA" disesuaikan dengan warna logo tanpa mengubah fungsi lain. */
(function(){
  if (window.__ARIKA_V233_BRAND_TEXT_COLOR__) return;
  window.__ARIKA_V233_BRAND_TEXT_COLOR__ = true;

  var STYLE_ID = 'arika-v233-brand-word-style';
  var WORD_CLASS = 'arika-brand-word';
  var SKIP_TAGS = {
    SCRIPT: true, STYLE: true, NOSCRIPT: true, TEXTAREA: true, INPUT: true,
    SELECT: true, OPTION: true, CANVAS: true, SVG: true, CODE: true, PRE: true
  };
  ...
  function injectStyle(){
    ...
    style.textContent = [
      '.arika-brand-word{',
      '  color:#0891b2;',
      '  background:linear-gradient(90deg,#0284c7 0%,#06b6d4 48%,#10b981 100%);',
      '  -webkit-background-clip:text;',
      '  background-clip:text;',
      '  -webkit-text-fill-color:transparent;',
      '  font-weight:inherit;',
      '}',
      '.arika-brand-word::selection{',
  ...
  function shouldSkipNode(node){
    ...
    if (p.closest && p.closest('.' + WORD_CLASS)) return true;
    if (p.closest && p.closest('[data-arika-no-brand-color="true"]')) return true;
    return false;
  }
  ...
  function scan(root){
    try {
      ...
      if (!root) return;
      if (root.nodeType === 3) return wrapTextNode(root);
      if (root.nodeType !== 1 && root.nodeType !== 9 && root.nodeType !== 11) return;
      if (root.tagName && SKIP_TAGS[root.tagName]) return;
      var walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
        acceptNode: function(node){
          if (!node.nodeValue || !/arika/i.test(node.nodeValue)) return NodeFilter.FILTER_REJECT;
      ...
      console.warn('ARIKA v233 brand text color gagal:', err);
    }
  }
  ...
  function scheduleScan(){
    if (scheduled) return;
    scheduled = true;
    var run = function(){ scheduled = false; scan(document.body); };
    ...
    try {
      var observer = new MutationObserver(function(mutations){
        for (var i = 0; i < mutations.length; i++){
          var m = mutations[i];
          if (m.type === 'characterData' && /arika/i.test(m.target.nodeValue || '')) { scheduleScan(); return; }
          if (m.addedNodes && m.addedNodes.length) { scheduleScan(); return; }
        }
      });
      observer.observe(document.body, { childList: true, subtree: true, characterData: true });
      window.__ARIKA_V233_BRAND_TEXT_OBSERVER__ = observer;
    } catch(e) {}
...
/* v234: Perbaikan pilihan "Ditujukan Untuk" pada Papan Informasi/Pengumuman di tampilan mobile.
   Native multiple select pada beberapa browser HP sulit dipilih, jadi dibuat picker chip khusus mobile
   yang tetap menyinkronkan nilai ke select asli agar fungsi simpan/edit tetap sama. */
(function(){
  if (window.__ARIKA_V234_PENGUMUMAN_TARGET_MOBILE_PICKER__) return;
  window.__ARIKA_V234_PENGUMUMAN_TARGET_MOBILE_PICKER__ = true;

  var STYLE_ID = 'arika-v234-pengumuman-target-mobile-style';
  var PICKER_ID = 'arika-pengumuman-target-mobile-picker';
  ...
  function injectStyle(){
    ...
    style.textContent = [
      ...
      '  }',
      '  #'+PICKER_ID+' .arika-target-chip.is-active{',
      '    background:linear-gradient(135deg,#059669,#0891b2);',
      '    border-color:transparent;',
      '    color:#ffffff;',
      '    box-shadow:0 10px 20px rgba(8,145,178,.18);',
      '  }',
      '  #'+PICKER_ID+' .arika-target-help{',
      '    flex:0 0 100%;',
      '    margin-top:2px;',
      '    color:#94a3b8;',
      '    font-size:9px;',
      ...
      '  #'+PICKER_ID+'{display:none !important;}',
      '}'
    ].join('\n');
    document.head.appendChild(style);
  }

  function getSelect(){ return document.getElementById('pengumuman-target-fungsi'); }

  function getSelectedValues(select){
    if (!select) return [];
    return Array.from(select.options || []).filter(function(opt){ return opt.selected; }).map(function(opt){ return opt.value; }).filter(Boolean);
  }
  ...
  function syncPicker(){
    ...
    var selected = getSelectedValues(select).map(norm);
    var useSemua = !selected.length || selected.indexOf(norm('Semua')) !== -1;
    picker.querySelectorAll('.arika-target-chip').forEach(function(btn){
      var value = btn.getAttribute('data-value') || '';
      var active = useSemua ? norm(value) === norm('Semua') : selected.indexOf(norm(value)) !== -1;
  ...
  function applySelection(value){
    var select = getSelect();
    if (!select) return;
    var valNorm = norm(value);
    var options = Array.from(select.options || []);
    if (valNorm === norm('Semua')){
      options.forEach(function(opt){ opt.selected = norm(opt.value) === norm('Semua'); });
    } else {
      options.forEach(function(opt){
        if (norm(opt.value) === norm('Semua')) opt.selected = false;
        if (norm(opt.value) === valNorm) opt.selected = !opt.selected;
      });
      var hasSpecific = options.some(function(opt){ return opt.selected && norm(opt.value) !== norm('Semua'); });
      if (!hasSpecific){
        options.forEach(function(opt){ opt.selected = norm(opt.value) === norm('Semua'); });
      }
    }
  ...
  function buildPicker(){
    ...
    if (!picker){
      ...
      picker.setAttribute('aria-label','Ditujukan Untuk');
      select.insertAdjacentElement('afterend', picker);
    }
    var signature = Array.from(select.options || []).map(function(opt){ return opt.value + ':' + opt.textContent; }).join('|');
    if (picker.getAttribute('data-signature') !== signature){
      picker.setAttribute('data-signature', signature);
      picker.innerHTML = '';
      Array.from(select.options || []).forEach(function(opt){
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'arika-target-chip';
        btn.setAttribute('data-value', opt.value || opt.textContent || '');
        btn.setAttribute('aria-pressed', 'false');
        btn.textContent = opt.textContent || opt.value;
        btn.addEventListener('click', function(ev){
          ev.preventDefault();
          ev.stopPropagation();
          applySelection(btn.getAttribute('data-value') || '');
        });
        picker.appendChild(btn);
      });
    }
    select.removeEventListener('change', syncPicker);
    select.addEventListener('change', syncPicker);
    syncPicker();
  }

  function patchButtons(){
    if (!window.__ARIKA_V234_TARGET_PATCHED_SET__ && typeof window.setPengumumanTargetSemua === 'function'){
      window.__ARIKA_V234_TARGET_PATCHED_SET__ = true;
      var oldSet = window.setPengumumanTargetSemua;
      window.setPengumumanTargetSemua = function(){
        var result = oldSet.apply(this, arguments);
        setTimeout(function(){ buildPicker(); syncPicker(); }, 0);
        return result;
      };
    }
    if (!window.__ARIKA_V234_TARGET_PATCHED_CLEAR__ && typeof window.clearPengumumanTarget === 'function'){
      window.__ARIKA_V234_TARGET_PATCHED_CLEAR__ = true;
      var oldClear = window.clearPengumumanTarget;
      window.clearPengumumanTarget = function(){
        var result = oldClear.apply(this, arguments);
        setTimeout(function(){ buildPicker(); syncPicker(); }, 0);
        return result;
      };
    }
  ...
  function install(){
    try { buildPicker(); patchButtons(); } catch(e){ console.warn('ARIKA v234 target picker gagal:', e); }
  }
  ...
  document.addEventListener('click', function(ev){
    if (!ev.target) return;
    if (ev.target.closest && ev.target.closest('#admin-tab-pengumuman, [onclick*="pengumuman"], [onclick*="Pengumuman"]')){
      setTimeout(install, 60);
      setTimeout(syncPicker, 260);
    }
  }, true);

  setTimeout(install, 400);
  setTimeout(install, 1200);
  setTimeout(syncPicker, 2200);
})();


/* v240: Guard tambahan agar teks Penyemangat Hari Ini tidak pernah dipotong menjadi elipsis. */
(function(){
  if (window.__ARIKA_V239_MOTIVATION_FULLTEXT__) return;
  window.__ARIKA_V239_MOTIVATION_FULLTEXT__ = true;

  var FULL_QUOTES = [
    'Langkah kecil yang dilakukan konsisten akan membantu pekerjaan besar terasa lebih ringan.',
    'Keberanian memulai dan kemauan memperbaiki adalah bagian penting dari kemajuan.',
    'Kualitas kerja tumbuh dari kebiasaan baik yang dilakukan berulang setiap hari.',
    'Perubahan besar sering dimulai dari tindakan sederhana yang dikerjakan dengan sungguh-sungguh.',
    'Belajar, menyesuaikan diri, dan terus mencoba adalah cara menjaga karya tetap bermakna.',
    'Pekerjaan yang tercatat rapi hari ini akan membantu keputusan yang lebih baik esok hari.'
  ];

  function injectStyle(){
    if (document.getElementById('arika-v240-motivation-fulltext-runtime-style')) return;
    var s = document.createElement('style');
    s.id = 'arika-v240-motivation-fulltext-runtime-style';
    s.textContent = [
      '#view-beranda .home-motivation-card #motivation-text{',
      'display:block!important;',
      ...
      'max-height:none!important;',
      '}',
      '#view-beranda .home-motivation-card{height:auto!important;max-height:none!important;}'
    ].join('');
    document.head.appendChild(s);
  }

  function restoreFullQuote(text){
    var raw = String(text || '').trim();
    if (!raw) return raw;
    var cleaned = raw.replace(/[.\u2026]+$/g, '').toLowerCase();
    for (var i = 0; i < FULL_QUOTES.length; i++) {
      var full = FULL_QUOTES[i];
      if (full.toLowerCase().indexOf(cleaned) === 0 || cleaned.indexOf(full.toLowerCase().slice(0, Math.min(24, full.length))) === 0) {
        return full;
      }
    }
    return raw;
  ...
  function fixMotivationText(){
    injectStyle();
    var el = document.getElementById('motivation-text');
    if (!el) return;
    ['truncate','line-clamp-1','line-clamp-2','line-clamp-3','overflow-hidden','text-ellipsis'].forEach(function(c){ el.classList.remove(c); });
    ...
    var full = restoreFullQuote(el.textContent);
    if (full && full !== el.textContent) el.textContent = full;
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', fixMotivationText);
  } else {
    fixMotivationText();
  }
  setTimeout(fixMotivationText, 500);
  setTimeout(fixMotivationText, 1500);
  document.addEventListener('visibilitychange', fixMotivationText);
})();


/* =========================================================
   ARIKA v241 - Optimasi Loading & Audit Final
   Fokus: lazy-render chart, cache lebih rapi, overlay ringan,
   jeda fungsi berat, dan guard agar UI tetap responsif.
   ========================================================= */
(function(){
  'use strict';
  const VERSION = 'v241-loading-audit-final';
  window.__ARIKA_OPTIMIZATION_VERSION__ = VERSION;

  function idle(fn, timeout){
    if (typeof window.requestIdleCallback === 'function') {
      return window.requestIdleCallback(fn, { timeout: timeout || 700 });
    }
    return window.setTimeout(fn, Math.min(timeout || 120, 220));
  }
  function raf(fn){
  ...
  function isHidden(el){
    if(!el) return true;
    return el.classList.contains('hidden') || el.hidden || el.getAttribute('aria-hidden') === 'true' || window.getComputedStyle(el).display === 'none';
  }
  function isViewVisible(id){
    const el = document.getElementById(id);
    return !!el && !isHidden(el);
  }
  function isAdminContentVisible(id){
    const el = document.getElementById(id);
    return !!el && !isHidden(el) && isViewVisible('view-admin');
  }
  function canvasVisible(canvasId){
    const el = document.getElementById(canvasId);
    if(!el) return false;
    const rect = el.getBoundingClientRect ? el.getBoundingClientRect() : null;
    return !isHidden(el) && rect && rect.width > 0 && rect.height > 0;
  }
  function currentVisibleTab(){
    const map = [
      ['beranda','view-beranda'],
      ['jurnal','view-jurnal'],
      ['rekap','view-rekap'],
      ['lembur','view-lembur'],
      ['dashboard-pegawai','view-dashboard-pegawai'],
      ['panduan','view-panduan'],
      ['admin','view-admin']
    ];
    for (const pair of map) if (isViewVisible(pair[1])) return pair[0];
    return '';
  }

  function installLoaderGuard(){
    if(!window.showLoader || window.__ARIKA_LOADER_GUARD_V241__) return;
    window.__ARIKA_LOADER_GUARD_V241__ = true;
    const original = window.showLoader;
    let timer = 0;
    let shown = false;
    window.showLoader = function(show, text){
      clearTimeout(timer);
      const msg = String(text || 'Memproses...');
      if(show){
        const critical = /masuk|keluar|menyimpan|menghapus|pegawai|sinkron|google sheet|mengambil ulang/i.test(msg);
        if(critical){
          shown = true;
          return original.call(window, true, msg);
        }
        timer = setTimeout(function(){
          shown = true;
          original.call(window, true, msg);
        }, 260);
        return;
      }
      shown = false;
      return original.call(window, false, msg);
    };
  }

  function installChartLazyGuard(){
    if(window.__ARIKA_CHART_LAZY_V241__) return;
    window.__ARIKA_CHART_LAZY_V241__ = true;

    const pending = Object.create(null);
    const originalRenderChart = window.renderChart;
    if(typeof originalRenderChart === 'function'){
      window.renderChart = function(canvasId, instanceName, type, labels, data, options){
        const args = arguments;
        // Jangan render chart pada panel yang sedang hidden. Simpan permintaan terakhir, lalu render saat tab dibuka.
        if(!canvasVisible(canvasId)){
          pending[canvasId] = Array.prototype.slice.call(args);
          return null;
        }
        return idle(function(){
          try { return originalRenderChart.apply(window, args); } catch(e) { console.warn('ARIKA chart skipped:', canvasId, e && e.message ? e.message : e); }
        }, 420);
      };
    }

    function flushPendingCharts(){
      Object.keys(pending).forEach(function(canvasId){
        if(!pending[canvasId] || !canvasVisible(canvasId)) return;
        const args = pending[canvasId];
        delete pending[canvasId];
        if(typeof originalRenderChart === 'function') idle(function(){
          try { originalRenderChart.apply(window, args); } catch(e) { console.warn('ARIKA chart retry skipped:', canvasId); }
        }, 450);
      });
    }
    window.__ARIKA_FLUSH_PENDING_CHARTS_V241__ = flushPendingCharts;

    // Grafik pribadi pada tab Riwayat hanya dirender saat tabnya terlihat.
    ['renderCharts','renderTrendChart'].forEach(function(name){
      const original = window[name];
      if(typeof original !== 'function') return;
      window[name] = debounce(function(){
        if(!isViewVisible('view-rekap')) return;
        const args = arguments;
        idle(function(){
          try { original.apply(window, args); } catch(e) { console.warn('ARIKA chart render skipped:', name); }
        }, 500);
      }, 70);
    });
  }

  function installHeavyRenderScheduler(){
    if(window.__ARIKA_HEAVY_RENDER_SCHEDULER_V241__) return;
    window.__ARIKA_HEAVY_RENDER_SCHEDULER_V241__ = true;

    const configs = {
      renderDashboardPegawai: function(){ return isViewVisible('view-dashboard-pegawai'); },
      renderAdminAnalytics: function(){ return isAdminContentVisible('admin-content-analitik'); },
      renderAdminOvertimeDashboard: function(){ return isAdminContentVisible('admin-content-lembur'); },
      renderAdminSurvei: function(){ return isAdminContentVisible('admin-content-survei'); },
      renderAdminAllTable: function(){ return isAdminContentVisible('admin-content-rekap'); },
      renderAdminPengumuman: function(){ return isAdminContentVisible('admin-content-pengumuman'); },
      renderAdminAgenda: function(){ return isAdminContentVisible('admin-content-agenda'); }
    };
    ...
    Object.keys(configs).forEach(function(name){
      ...
      window[name] = function(){
        ...
        idle(function(){
          scheduled = false;
          try { original.apply(window, lastArgs || []); } catch(e) { console.warn('ARIKA render skipped:', name, e && e.message ? e.message : e); }
          try { window.__ARIKA_FLUSH_PENDING_CHARTS_V241__ && window.__ARIKA_FLUSH_PENDING_CHARTS_V241__(); } catch(_) {}
        }, 520);
        return null;
      };
    });
  ...
  function installNavFlush(){
    if(window.__ARIKA_NAV_FLUSH_V241__ || typeof window.nav !== 'function') return;
    window.__ARIKA_NAV_FLUSH_V241__ = true;
    const originalNav = window.nav;
    window.nav = function(id){
      const result = originalNav.apply(window, arguments);
      raf(function(){
        idle(function(){
          try { window.__ARIKA_FLUSH_PENDING_CHARTS_V241__ && window.__ARIKA_FLUSH_PENDING_CHARTS_V241__(); } catch(_) {}
          try {
            if(id === 'dashboard-pegawai' && typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({ fromNav: true });
          } catch(_) {}
        }, 380);
      });
      return result;
  ...
  function installBerandaRenderLimit(){
    if(window.__ARIKA_BERANDA_LIMIT_V241__) return;
    window.__ARIKA_BERANDA_LIMIT_V241__ = true;
    const limitMap = {
      renderPengumumanBoard: ['pengumumanData', 40],
      renderAgendaSaya: ['agendaData', 40],
      renderRencanaPribadi: ['rencanaData', 40]
    };
    Object.keys(limitMap).forEach(function(name){
      const original = window[name];
      if(typeof original !== 'function') return;
      const key = limitMap[name][0];
      const max = limitMap[name][1];
      window[name] = debounce(function(){
        const args = arguments;
        const old = Array.isArray(window[key]) ? window[key] : null;
        let restore = false;
        try {
          if(old && old.length > max){
            // Untuk render awal Beranda, proses hanya item terbaru/terdekat secukupnya.
            window[key] = old.slice(0, max);
            restore = true;
          }
          return original.apply(window, args);
        } finally {
          if(restore) window[key] = old;
        }
      }, 40);
    });
  }

  function installCacheCleanup(){
    if(window.__ARIKA_CACHE_CLEANUP_V241__) return;
    window.__ARIKA_CACHE_CLEANUP_V241__ = true;
    function trimJsonArrayKey(key, max){
      try {
        const raw = localStorage.getItem(key);
        if(!raw) return;
        const arr = JSON.parse(raw);
        if(Array.isArray(arr) && arr.length > max) localStorage.setItem(key, JSON.stringify(arr.slice(0, max)));
      } catch(_) {}
    }
    function clean(){
      trimJsonArrayKey('arika_pengumuman', 80);
      trimJsonArrayKey('arika_rencana_pribadi_v2', 120);
      trimJsonArrayKey('arika_survei', 120);
      // Hapus cache lama yang tidak lagi dipakai agar storage tidak penuh.
      ['arika_core_cache_v36','arika_session_v43','arika_session_v49_persist','arika_session_v51_persist','arika_session_v52_cloud_persist'].forEach(function(k){
  ...
  function installConsoleGuard(){
    if(window.__ARIKA_CONSOLE_GUARD_V241__) return;
    window.__ARIKA_CONSOLE_GUARD_V241__ = true;
    window.addEventListener('unhandledrejection', function(ev){
      const msg = ev && ev.reason && (ev.reason.message || String(ev.reason));
      if(msg && /ResizeObserver|Chart|canvas|AbortError/i.test(msg)) {
        try { ev.preventDefault(); } catch(_) {}
        console.warn('ARIKA warning ringan:', msg);
      }
    });
  }
  ...
  function install(){
    installLoaderGuard();
    /* v255: disabled unstable chart lazy guard */
    /* v255: disabled unstable heavy render scheduler */
    installNavFlush();
    installBerandaRenderLimit();
    installCacheCleanup();
    installConsoleGuard();
    document.documentElement.classList.add('arika-v241-optimized');
  }
...
/* v242: Rencana Saya dibuat compact/collapsible agar Beranda lebih fokus pada daftar rencana aktif. */
(function(){
  function getForm(){ return document.getElementById('form-rencana-pribadi'); }
  function getBtn(){ return document.getElementById('toggle-rencana-form-btn'); }
  window.setRencanaFormExpanded = function(expanded){
    var form = getForm();
    var btn = getBtn();
    var card = document.getElementById('personal-reminder-card');
    if(!form) return;
    var open = !!expanded;
    form.classList.toggle('arika-rencana-form-collapsed', !open);
    form.classList.toggle('arika-rencana-form-open', open);
    if(card) card.classList.toggle('arika-rencana-form-is-open', open);
    if(btn) btn.textContent = open ? 'Tutup Form' : '+ Tambah Rencana';
    if(open) {
      try {
        if(typeof window.setDefaultDates === 'function') window.setDefaultDates();
        var title = document.getElementById('rencana-judul');
        setTimeout(function(){ title && title.focus && title.focus(); }, 80);
      } catch(_) {}
    }
  };
  window.toggleRencanaForm = function(force){
    var form = getForm();
    if(!form) return;
    var willOpen = typeof force === 'boolean' ? force : form.classList.contains('arika-rencana-form-collapsed');
    window.setRencanaFormExpanded(willOpen);
  };
  function init(){
    var form = getForm();
    if(!form || form.dataset.rencanaCompactReady === 'true') return;
    form.dataset.rencanaCompactReady = 'true';
    form.classList.add('arika-rencana-compact-form');
    window.setRencanaFormExpanded(false);
    var original = window.simpanRencanaPribadi;
    if(typeof original === 'function' && !original.__arikaRencanaCompactWrapped) {
      var wrapped = async function(){
        ...
        setTimeout(function(){
          try {
            var f = getForm();
            var title = document.getElementById('rencana-judul');
            var jam = document.getElementById('rencana-jam');
            // Setelah submit berhasil, form lama direset. Saat itu form ditutup agar Beranda kembali fokus ke daftar rencana.
            if(f && !window.isSavingRencana && (!title || !title.value) && (!jam || jam.value)) {
              window.setRencanaFormExpanded(false);
            }
          } catch(_) {}
        }, 350);
      ...
      wrapped.__arikaRencanaCompactWrapped = true;
      window.simpanRencanaPribadi = wrapped;
      try {
  ...
  window.addEventListener('arika:data-rendered', function(){ setTimeout(init, 60); });
})();

/* =========================================================
   ARIKA v245 - Optimasi Loading & Audit Final Lanjutan
   Fokus: render hanya saat dibutuhkan, overlay ringan, cache rapi,
   dan UI tetap responsif tanpa mengubah fitur utama.
   ========================================================= */
(function(){
  'use strict';
  const VERSION = 'v245-loading-audit-final-plus';
  if(window.__ARIKA_OPTIMIZATION_VERSION__ === VERSION) return;
  window.__ARIKA_OPTIMIZATION_VERSION__ = VERSION;

  const state = {
    installed: false,
    pendingRender: Object.create(null),
    pendingReason: Object.create(null),
    lastFlush: 0
  };

  function idle(fn, timeout){
    if(typeof window.requestIdleCallback === 'function'){
      return window.requestIdleCallback(function(){ try{ fn(); }catch(e){ safeWarn('idle task', e); } }, { timeout: timeout || 900 });
    }
    return window.setTimeout(function(){ try{ fn(); }catch(e){ safeWarn('timeout task', e); } }, Math.min(timeout || 160, 260));
  }
  function raf(fn){
    if(typeof window.requestAnimationFrame === 'function') return window.requestAnimationFrame(fn);
    return window.setTimeout(fn, 16);
  ...
  function safeWarn(label, err){
    try{
      ...
      if(/ResizeObserver|canvas has already been used|AbortError|The operation was aborted|Chart/i.test(msg)) return;
      console.warn('ARIKA v245:', label, msg);
    }catch(_){}
  }
  ...
  function isVisibleById(id){ return !isHidden(getEl(id)); }
  function isAdminVisible(id){ return isVisibleById('view-admin') && isVisibleById(id); }
  ...
  function debounce(fn, wait){
    ...
    return function(){
      ...
      clearTimeout(t);
      t = setTimeout(function(){ fn.apply(ctx,args); }, wait || 90);
    };
  }

  function injectPerformanceCss(){
    if(getEl('arika-v245-performance-css')) return;
    const css = document.createElement('style');
    css.id = 'arika-v245-performance-css';
    css.textContent = `
      .arika-v245-optimized .card,
      .arika-v245-optimized .feature-card,
      .arika-v245-optimized .table-card,
      .arika-v245-optimized .dashboard-card,
      .arika-v245-optimized .glass-card { content-visibility: auto; contain-intrinsic-size: 1px 260px; }
      .arika-v245-optimized canvas { max-width: 100%; }
      .arika-v245-optimized .hidden { content-visibility: hidden; }
      @media (prefers-reduced-motion: reduce){
        .arika-v245-optimized *, .arika-v245-optimized *::before, .arika-v245-optimized *::after { animation-duration: .01ms !important; transition-duration: .01ms !important; }
      }
    `;
  ...
  function installLoaderGuard(){
    if(!window.showLoader || window.__ARIKA_LOADER_GUARD_V245__) return;
    window.__ARIKA_LOADER_GUARD_V245__ = true;
    const original = window.showLoader;
    let timer = 0;
    const criticalRe = /login|masuk|keluar|logout|menyimpan|simpan|menghapus|hapus|sinkron|mengambil ulang|memuat data|pegawai|google sheet|restore/i;
    window.showLoader = function(show, text){
      const msg = String(text || 'Memproses...');
      clearTimeout(timer);
      if(show){
        if(criticalRe.test(msg)) return original.call(window, true, msg);
        timer = setTimeout(function(){ original.call(window, true, msg); }, 360);
        return;
      }
      return original.call(window, false, msg);
    };
  ...
  function installChartGuard(){
    if(window.__ARIKA_CHART_GUARD_V245__) return;
    window.__ARIKA_CHART_GUARD_V245__ = true;
    const pendingCharts = Object.create(null);
    ...
    if(typeof originalRenderChart === 'function'){
      window.renderChart = function(canvasId, instanceName, type, labels, data, options){
        ...
        if(!canvasVisible(canvasId)){
          pendingCharts[canvasId] = args;
          return null;
        }
        idle(function(){
          try{ originalRenderChart.apply(window, args); }
          catch(e){ safeWarn('chart render ' + canvasId, e); }
        }, 520);
        return null;
      };
    }
    ...
    function flushCharts(){
      Object.keys(pendingCharts).forEach(function(canvasId){
        if(!canvasVisible(canvasId)) return;
        const args = pendingCharts[canvasId];
        delete pendingCharts[canvasId];
        if(typeof originalRenderChart === 'function') idle(function(){
          try{ originalRenderChart.apply(window, args); }
          catch(e){ safeWarn('chart flush ' + canvasId, e); }
        }, 520);
      });
    }
    window.__ARIKA_FLUSH_CHARTS_V245__ = flushCharts;

    ['renderCharts','renderTrendChart'].forEach(function(name){
      const original = window[name];
      if(typeof original !== 'function' || original.__v245Wrapped) return;
      const wrapped = debounce(function(){
        const args = arguments;
        if(!isVisibleById('view-rekap')) return;
        idle(function(){ try{ original.apply(window,args); }catch(e){ safeWarn(name,e); } }, 560);
      }, 110);
      wrapped.__v245Wrapped = true;
      window[name] = wrapped;
    });
  }

  function installRenderScheduler(){
    if(window.__ARIKA_RENDER_SCHEDULER_V245__) return;
    window.__ARIKA_RENDER_SCHEDULER_V245__ = true;
    const visibility = {
      renderDashboardPegawai: function(){ return isVisibleById('view-dashboard-pegawai'); },
      renderAdminAnalytics: function(){ return isAdminVisible('admin-content-analitik'); },
      renderAdminOvertimeDashboard: function(){ return isAdminVisible('admin-content-lembur'); },
      renderAdminSurvei: function(){ return isAdminVisible('admin-content-survei'); },
      renderAdminAllTable: function(){ return isAdminVisible('admin-content-rekap'); },
      renderAdminPengumuman: function(){ return isAdminVisible('admin-content-pengumuman'); },
      renderAdminAgenda: function(){ return isAdminVisible('admin-content-agenda'); }
    };
    Object.keys(visibility).forEach(function(name){
      ...
      const wrapped = function(){
        const args = arguments;
        state.pendingRender[name] = args;
        if(!visibility[name]()){
          state.pendingReason[name] = 'hidden';
          return null;
        }
        if(wrapped.__scheduled) return null;
        wrapped.__scheduled = true;
        idle(function(){
          wrapped.__scheduled = false;
          const lastArgs = state.pendingRender[name] || args;
          delete state.pendingRender[name];
          try{ original.apply(window, lastArgs); }catch(e){ safeWarn(name,e); }
          try{ window.__ARIKA_FLUSH_CHARTS_V245__ && window.__ARIKA_FLUSH_CHARTS_V245__(); }catch(_){}
        }, 650);
        return null;
      };
      wrapped.__v245Wrapped = true;
  ...
  function flushVisibleRenders(){
    const now = Date.now();
    if(now - state.lastFlush < 180) return;
    state.lastFlush = now;
    raf(function(){ idle(function(){
      try{ window.__ARIKA_FLUSH_CHARTS_V245__ && window.__ARIKA_FLUSH_CHARTS_V245__(); }catch(_){}
      // Panggil render hanya untuk tab yang terlihat; wrapper scheduler akan menolak panel hidden.
      [
        ['view-dashboard-pegawai','renderDashboardPegawai'],
        ['admin-content-analitik','renderAdminAnalytics'],
        ['admin-content-lembur','renderAdminOvertimeDashboard'],
        ['admin-content-survei','renderAdminSurvei'],
        ['admin-content-rekap','renderAdminAllTable'],
        ['admin-content-pengumuman','renderAdminPengumuman'],
        ['admin-content-agenda','renderAdminAgenda']
      ].forEach(function(pair){
        const visible = pair[0].startsWith('admin-') ? isAdminVisible(pair[0]) : isVisibleById(pair[0]);
        if(visible && typeof window[pair[1]] === 'function'){
          try{ window[pair[1]]({ fromVisibleFlush: true }); }catch(e){ safeWarn('flush ' + pair[1], e); }
        }
      });
    }, 480); });
  }
  ...
  function installNavObserver(){
    if(window.__ARIKA_NAV_OBSERVER_V245__) return;
    window.__ARIKA_NAV_OBSERVER_V245__ = true;
    const originalNav = window.nav;
    ...
    const adminTabs = document.querySelectorAll('[data-admin-tab], .admin-tab, [onclick*="setAdminTab"], [onclick*="admin"]');
    adminTabs.forEach(function(el){ el.addEventListener('click', function(){ setTimeout(flushVisibleRenders, 180); }, { passive:true }); });
    document.addEventListener('visibilitychange', function(){ if(!document.hidden) flushVisibleRenders(); });
  }

  function installBerandaLimits(){
    if(window.__ARIKA_BERANDA_LIMIT_V245__) return;
    window.__ARIKA_BERANDA_LIMIT_V245__ = true;
    const cfg = {
      renderPengumumanBoard: ['pengumumanData', 24],
      renderAgendaSaya: ['agendaData', 24],
      renderRencanaPribadi: ['rencanaData', 32]
    };
    Object.keys(cfg).forEach(function(name){
      const original = window[name];
      if(typeof original !== 'function' || original.__v245Wrapped) return;
      const key = cfg[name][0], max = cfg[name][1];
      const wrapped = debounce(function(){
        const args = arguments;
        const old = Array.isArray(window[key]) ? window[key] : null;
        let restore = false;
        try{
          if(old && old.length > max){
            // Gunakan item terbaru/awal secukupnya untuk render kartu Beranda, data penuh tetap dikembalikan setelah render.
            window[key] = old.slice(0, max);
            restore = true;
          }
          return original.apply(window, args);
        }catch(e){ safeWarn(name,e); }
        finally{ if(restore) window[key] = old; }
      }, 70);
      wrapped.__v245Wrapped = true;
      window[name] = wrapped;
    });
  ...
  function installStorageCleanup(){
    if(window.__ARIKA_STORAGE_CLEANUP_V245__) return;
    window.__ARIKA_STORAGE_CLEANUP_V245__ = true;
    function trimObjectCache(key){
      try{
        const raw = localStorage.getItem(key);
        if(!raw || raw.length < 1200000) return;
        const obj = JSON.parse(raw);
        if(obj && typeof obj === 'object'){
          if(Array.isArray(obj.jurnal)) obj.jurnal = obj.jurnal.slice(0, 320);
          if(Array.isArray(obj.rencana)) obj.rencana = obj.rencana.slice(0, 140);
          if(Array.isArray(obj.pengumuman)) obj.pengumuman = obj.pengumuman.slice(0, 90);
          if(Array.isArray(obj.agenda)) obj.agenda = obj.agenda.slice(0, 90);
          if(Array.isArray(obj.survei)) obj.survei = obj.survei.slice(0, 100);
    ...
    function clean(){
      ['arika_data_cache_v98','arika_core_cache_v36'].forEach(trimObjectCache);
      ['app.js','app-v221.js','app-v222.js','app-v223.js','app-v224.js'].forEach(function(name){
        // Tidak menghapus file GitHub; hanya membersihkan penanda cache lama bila pernah tersimpan di storage.
        try{ localStorage.removeItem('arika_loaded_' + name); sessionStorage.removeItem('arika_loaded_' + name); }catch(_){}
      });
    }
  ...
  function installConsoleGuard(){
    if(window.__ARIKA_CONSOLE_GUARD_V245__) return;
    window.__ARIKA_CONSOLE_GUARD_V245__ = true;
    window.addEventListener('error', function(ev){
      const msg = ev && ev.message ? ev.message : '';
      if(/ResizeObserver loop|Script error|AbortError/i.test(msg)){
        try{ ev.preventDefault(); }catch(_){}
      }
    });
    window.addEventListener('unhandledrejection', function(ev){
      ...
      if(msg && /ResizeObserver|AbortError|canvas|Chart/i.test(msg)){
        try{ ev.preventDefault(); }catch(_){}
        safeWarn('warning ringan', msg);
      }
    });
  }

  function audit(){
    window.ARIKA_AUDIT_FINAL = {
      version: VERSION,
      script: 'app-v245.js?v=245',
      date: new Date().toISOString(),
      checks: {
        lazyChart: !!window.__ARIKA_CHART_GUARD_V245__,
        renderScheduler: !!window.__ARIKA_RENDER_SCHEDULER_V245__,
        loaderGuard: !!window.__ARIKA_LOADER_GUARD_V245__,
        storageCleanup: !!window.__ARIKA_STORAGE_CLEANUP_V245__,
        noDatePicker: !document.querySelector('input[type="date"],input[type="month"]'),
        oldScriptActive: !!document.querySelector('script[src*="app.js"],script[src*="app-v244.js"],script[src*="app-v241.js"]')
      }
    };
  }
  ...
  function install(){
    ...
    document.documentElement.classList.add('arika-v245-optimized');
    injectPerformanceCss();
    installLoaderGuard();
    /* v255: disabled unstable chart guard */
    /* v255: disabled unstable render scheduler */
    installNavObserver();
    installBerandaLimits();
    installStorageCleanup();
    installConsoleGuard();
    setTimeout(flushVisibleRenders, 600);
...
/* =========================================================
   ARIKA v248 - Dashboard Admin/Verifikator Ringan & Fokus
   - Grafik render saat tab dashboard dibuka/terlihat
   - KPI/kartu ringkasan lebih compact
   - Filter lab/status lebih konsisten
   - Tombol export lebih jelas
   - Tidak mengubah sumber data atau fungsi simpan/export lama
   ========================================================= */
(function(){
  'use strict';
  const VERSION = 'v252-kalender-libur-nasional-cuti-bersama';
  if(window.__ARIKA_DASHBOARD_FOCUS_V248__) return;
  window.__ARIKA_DASHBOARD_FOCUS_V248__ = true;
  ...
  function isAdminDashboardVisible(){ return isVisible(byId('view-admin')) && isVisible(byId('admin-content-analitik')); }
  function isAdminOvertimeVisible(){ return isVisible(byId('view-admin')) && isVisible(byId('admin-content-lembur')); }
  function isPegawaiDashboardVisible(){ return isVisible(byId('view-dashboard-pegawai')); }
  function isCanvasDashboard(canvasId){
    return /^admin-chart-|^pegawai-chart-|^survey-chart-/.test(String(canvasId || ''));
  }
  function idle(fn, timeout){
    if(typeof requestIdleCallback === 'function') return requestIdleCallback(function(){ try{ fn(); }catch(e){ softWarn('idle', e); } }, { timeout: timeout || 900 });
    return setTimeout(function(){ try{ fn(); }catch(e){ softWarn('timeout', e); } }, Math.min(timeout || 120, 280));
  }
  function softWarn(label, err){
    try{
      const msg = err && err.message ? err.message : String(err || '');
      if(/ResizeObserver|AbortError|canvas|Chart/i.test(msg)) return;
      console.warn('ARIKA v248:', label, msg);
    }catch(_){}
  }

  function injectCss(){
    if(byId('arika-v248-dashboard-focus-css')) return;
    const css = document.createElement('style');
    css.id = 'arika-v248-dashboard-focus-css';
    css.textContent = `
      /* Dashboard admin/verifikator dibuat padat, ringan, dan fokus */
      .arika-v248-dashboard-focus #admin-content-analitik .admin-panel,
      .arika-v248-dashboard-focus #admin-content-lembur .admin-panel,
      .arika-v248-dashboard-focus #admin-content-rekap .bg-white.rounded-3xl,
      .arika-v248-dashboard-focus #view-dashboard-pegawai .rounded-3xl {
        content-visibility: auto;
        contain-intrinsic-size: 1px 420px;
      }
      .arika-v248-dashboard-focus .admin-panel-inner{ padding: 1.15rem !important; }
      .arika-v248-dashboard-focus .admin-panel-header{
      ...
      .arika-v248-dashboard-focus .admin-section-kicker{
        font-size:.58rem !important;
        letter-spacing:.16em !important;
      }
      .arika-v248-dashboard-focus .admin-section-title{
        font-size:1.15rem !important;
        line-height:1.15 !important;
        margin-top:.2rem !important;
      }
      .arika-v248-dashboard-focus .admin-section-desc,
      .arika-v248-dashboard-focus .admin-kpi-note,
      .arika-v248-dashboard-focus .admin-overtime-note,
      .arika-v248-dashboard-focus .admin-chart-badge{
        display:none !important;
      }
      .arika-v248-dashboard-focus .admin-filterbar{
        display:flex !important;
      ...
      .arika-v248-dashboard-focus .admin-control,
      .arika-v248-dashboard-focus #admin-content-rekap select,
      .arika-v248-dashboard-focus #admin-content-rekap input{
      ...
      .arika-v248-dashboard-focus .admin-action-btn,
      .arika-v248-dashboard-focus #admin-rekap-download-all-btn{
        min-height:34px !important;
      ...
      .arika-v248-dashboard-focus .admin-action-btn::first-letter{ text-transform:none; }
      .arika-v248-dashboard-focus .admin-kpi-grid{
        display:grid !important;
        grid-template-columns:repeat(5,minmax(0,1fr)) !important;
        gap:.65rem !important;
        margin-bottom:.9rem !important;
      }
      .arika-v248-dashboard-focus .admin-kpi-card{
        padding:.75rem .8rem !important;
        border-radius:1rem !important;
        min-height:auto !important;
      }
      .arika-v248-dashboard-focus .admin-kpi-top{ margin-bottom:.15rem !important; }
      .arika-v248-dashboard-focus .admin-kpi-label{
        font-size:.55rem !important;
        letter-spacing:.12em !important;
        line-height:1.1 !important;
      }
      .arika-v248-dashboard-focus .admin-kpi-icon{
        width:24px !important;
        height:24px !important;
        font-size:.78rem !important;
      }
      .arika-v248-dashboard-focus .admin-kpi-value{
        font-size:1.5rem !important;
        line-height:1.05 !important;
        margin-top:.15rem !important;
      }
      .arika-v248-dashboard-focus .admin-monev-chart-grid,
      .arika-v248-dashboard-focus #admin-content-lembur .grid{
        gap:.8rem !important;
      }
      .arika-v248-dashboard-focus .admin-chart-card,
      .arika-v248-dashboard-focus .admin-monev-chart-card{
        padding:.85rem !important;
        border-radius:1.1rem !important;
      }
      .arika-v248-dashboard-focus .admin-chart-head{ margin-bottom:.55rem !important; }
      .arika-v248-dashboard-focus .admin-chart-title{
        font-size:.72rem !important;
        letter-spacing:.1em !important;
        text-transform:uppercase !important;
      }
      .arika-v248-dashboard-focus .admin-chart-canvas-wrap,
      .arika-v248-dashboard-focus .admin-monev-chart-wrap{
        height:215px !important;
        min-height:215px !important;
      }
      .arika-v248-dashboard-focus #admin-content-rekap .grid.grid-cols-1.md\\:grid-cols-6{
        grid-template-columns:repeat(6,minmax(0,1fr)) !important;
        gap:.6rem !important;
        margin-bottom:.8rem !important;
      }
      .arika-v248-dashboard-focus #admin-content-rekap label{
        font-size:.55rem !important;
        letter-spacing:.1em !important;
      }
      .arika-v248-dashboard-focus #admin-content-rekap .mb-5.flex.flex-wrap.gap-2{
        margin-bottom:.75rem !important;
        gap:.35rem !important;
      }
      .arika-v248-dashboard-focus #admin-content-rekap .mb-5.flex.flex-wrap.gap-2 button{
        padding:.38rem .65rem !important;
        font-size:.55rem !important;
      }
      .arika-dashboard-chart-skeleton{
      ...
      @media (max-width:1100px){
        .arika-v248-dashboard-focus .admin-panel-header{ grid-template-columns:1fr !important; }
        .arika-v248-dashboard-focus .admin-filterbar{ justify-content:flex-start !important; }
        .arika-v248-dashboard-focus .admin-kpi-grid{ grid-template-columns:repeat(2,minmax(0,1fr)) !important; }
        .arika-v248-dashboard-focus #admin-content-rekap .grid.grid-cols-1.md\\:grid-cols-6{ grid-template-columns:repeat(2,minmax(0,1fr)) !important; }
      }
      @media (max-width:640px){
        .arika-v248-dashboard-focus .admin-kpi-grid{ grid-template-columns:1fr 1fr !important; }
        .arika-v248-dashboard-focus .admin-filterbar > *{ width:100% !important; }
        .arika-v248-dashboard-focus #admin-content-rekap .grid.grid-cols-1.md\\:grid-cols-6{ grid-template-columns:1fr !important; }
        .arika-v248-dashboard-focus .admin-chart-canvas-wrap,
        .arika-v248-dashboard-focus .admin-monev-chart-wrap{ height:200px !important; min-height:200px !important; }
      }
    `;
    document.head.appendChild(css);
  }
  ...
  function relabelExportButtons(){
    const map = [
      ['button[onclick*="downloadAdminMonthlyReportWord"]', '📄 Export Laporan Word'],
      ['button[onclick*="downloadAdminMonevExcel"]', '📊 Export Monev XLS'],
      ['#admin-rekap-download-all-btn', '📥 Export Rekap XLS'],
      ['button[onclick*="downloadAdminOvertimeExcel"]', '📥 Export Lembur XLS'],
      ['button[onclick*="downloadAdminOvertimeWord"]', '📄 Export SPK Word'],
      ['button[onclick*="downloadAdminOvertimeSpk"]', '📄 Export SPK Word']
    ];
    map.forEach(function(pair){
      document.querySelectorAll(pair[0]).forEach(function(btn){
        if(btn.dataset.v248Relabel === 'true') return;
        btn.dataset.v248Relabel = 'true';
        btn.textContent = pair[1];
        btn.title = pair[1].replace(/[📄📊📥]/g,'').trim();
      });
    });
  }
  ...
  function normalizeFilterLabels(){
    const labels = {
      'admin-analytics-month':'Bulan',
      'admin-analytics-lab':'Fungsi/Lab',
      'admin-overtime-lab':'Fungsi/Lab',
      'admin-overtime-status':'Status',
      'admin-filter-start':'Awal',
      'admin-filter-end':'Akhir',
      'admin-filter-lab':'Fungsi/Lab',
      'admin-filter-status':'Jenis Pegawai',
      'admin-filter-type':'Jam Kerja'
    };
    Object.keys(labels).forEach(function(id){
      const el = byId(id);
      if(!el || el.dataset.v248FilterReady === 'true') return;
      el.dataset.v248FilterReady = 'true';
      el.setAttribute('aria-label', labels[id]);
      el.title = labels[id];
      if(el.tagName === 'SELECT'){
        el.classList.add('arika-admin-filter-select');
      }
    });
  }

  function installStrictChartLazyGuard(){
    if(window.__ARIKA_STRICT_DASHBOARD_LAZY_V248__) return;
    window.__ARIKA_STRICT_DASHBOARD_LAZY_V248__ = true;
    const pending = Object.create(null);
    const originalRenderChart = window.renderChart;
    ...
    window.renderChart = function(canvasId, instanceName, type, labels, data, options){
      ...
      if(isCanvasDashboard(id) && !isVisible(canvas)){
        pending[id] = args;
        const wrap = canvas && canvas.parentElement;
        if(wrap && !wrap.querySelector('.arika-dashboard-chart-skeleton')){
          const sk = document.createElement('div');
          sk.className = 'arika-dashboard-chart-skeleton';
          sk.textContent = 'Grafik siap ditampilkan';
          sk.dataset.for = id;
          // Jangan sembunyikan canvas; skeleton hanya muncul ketika chart belum bisa dirender.
          try{ wrap.appendChild(sk); }catch(_){}
        }
        return null;
      }
      return idle(function(){
        try{
          const wrap = canvas && canvas.parentElement;
          if(wrap) wrap.querySelectorAll('.arika-dashboard-chart-skeleton').forEach(function(x){ x.remove(); });
          return originalRenderChart.apply(window, args);
        }catch(e){ softWarn('chart ' + id, e); }
      }, 650);
    };
    window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ = function(){
      Object.keys(pending).forEach(function(id){
        const canvas = byId(id);
        if(!isVisible(canvas)) return;
        const args = pending[id];
        delete pending[id];
        idle(function(){
          try{
            const wrap = canvas && canvas.parentElement;
            if(wrap) wrap.querySelectorAll('.arika-dashboard-chart-skeleton').forEach(function(x){ x.remove(); });
            originalRenderChart.apply(window, args);
          }catch(e){ softWarn('flush ' + id, e); }
        }, 600);
      });
    };
  }

  function installDashboardRenderGate(){
    if(window.__ARIKA_DASHBOARD_RENDER_GATE_V248__) return;
    window.__ARIKA_DASHBOARD_RENDER_GATE_V248__ = true;
    const gates = {
      renderAdminAnalytics: isAdminDashboardVisible,
      renderAdminOvertimeDashboard: isAdminOvertimeVisible,
      renderDashboardPegawai: isPegawaiDashboardVisible
    };
    Object.keys(gates).forEach(function(name){
      const original = window[name];
      if(typeof original !== 'function' || original.__v248DashboardGate) return;
      let scheduled = false;
      let lastArgs = null;
      const wrapped = function(){
        lastArgs = arguments;
        if(!gates[name]()) return null;
        if(scheduled) return null;
        scheduled = true;
        idle(function(){
          scheduled = false;
          try{ original.apply(window, lastArgs || []); }catch(e){ softWarn(name, e); }
          try{ window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ && window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); }catch(_){}
        }, 500);
        return null;
      };
      wrapped.__v248DashboardGate = true;
      window[name] = wrapped;
    });
  }

  function flushDashboards(){
    if(isAdminDashboardVisible() && typeof window.renderAdminAnalytics === 'function') window.renderAdminAnalytics({ fromDashboardFocus: true });
    if(isAdminOvertimeVisible() && typeof window.renderAdminOvertimeDashboard === 'function') window.renderAdminOvertimeDashboard({ fromDashboardFocus: true });
    if(isPegawaiDashboardVisible() && typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({ fromDashboardFocus: true });
    try{ window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ && window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); }catch(_){}
  }

  function installTabHooks(){
    if(window.__ARIKA_DASHBOARD_TAB_HOOKS_V248__) return;
    window.__ARIKA_DASHBOARD_TAB_HOOKS_V248__ = true;
    const originalSwitch = window.switchAdminTab;
    if(typeof originalSwitch === 'function'){
      window.switchAdminTab = function(){
        const result = originalSwitch.apply(window, arguments);
        setTimeout(function(){ relabelExportButtons(); normalizeFilterLabels(); flushDashboards(); }, 140);
        return result;
      };
    }
    const originalNav = window.nav;
    ...
    document.addEventListener('visibilitychange', function(){ if(!document.hidden) setTimeout(flushDashboards, 180); });
  }

  function compactAdminCards(){
    // Tambahkan kelas penanda tanpa mengubah isi data.
    ['admin-content-analitik','admin-content-lembur','admin-content-rekap'].forEach(function(id){
      const el = byId(id);
      if(el) el.classList.add('arika-admin-focused-panel');
    });
  }

  function audit(){
    window.ARIKA_DASHBOARD_AUDIT_V248 = {
      version: VERSION,
      date: new Date().toISOString(),
      lazyChart: !!window.__ARIKA_STRICT_DASHBOARD_LAZY_V248__,
      renderGate: !!window.__ARIKA_DASHBOARD_RENDER_GATE_V248__,
      mobileSafe: !document.querySelector('input[type="date"],input[type="month"]'),
      activeScript: 'app-v255.js?v=255'
    };
  }

  function install(){
    document.documentElement.classList.add('arika-v248-dashboard-focus');
    injectCss();
    compactAdminCards();
    relabelExportButtons();
    normalizeFilterLabels();
    /* v255: disabled strict chart lazy guard */
    /* v255: disabled dashboard render gate */
    installTabHooks();
    setTimeout(function(){ relabelExportButtons(); normalizeFilterLabels(); flushDashboards(); audit(); }, 900);
  }
...
(function(){
  ...
  function injectArchiveCss(){
    if(document.getElementById('arika-v248-archive-css')) return;
    const style = document.createElement('style');
    style.id = 'arika-v248-archive-css';
    style.textContent = `
      #view-rekap .bg-white.p-6.rounded-3xl.shadow-xl.mb-6{ padding:1rem !important; }
      #view-rekap .rekap-filter-chip{ padding:.55rem .8rem !important; font-size:.58rem !important; border-radius:999px !important; }
      .arika-archive-summary-card{ margin-top:.25rem; }
      .arika-archive-kpi{ border:1px solid rgba(226,232,240,.95); background:#fff; border-radius:1rem; padding:.85rem 1rem; display:flex; align-items:center; justify-content:space-between; gap:.75rem; min-height:56px; }
      .arika-archive-kpi span{ font-size:.6rem; line-height:1; font-weight:900; letter-spacing:.16em; text-transform:uppercase; color:#94a3b8; }
      .arika-archive-kpi strong{ font-size:1.35rem; line-height:1; font-weight:950; color:#0f172a; }
      .arika-archive-kpi.done{ background:linear-gradient(135deg,#ecfdf5,#fff); border-color:rgba(16,185,129,.18); }
      .arika-archive-kpi.done strong{ color:#059669; }
      .arika-archive-kpi.pending{ background:linear-gradient(135deg,#fff1f2,#fff); border-color:rgba(244,63,94,.16); }
      .arika-archive-kpi.pending strong{ color:#e11d48; }
      .arika-archive-kpi.overtime{ background:linear-gradient(135deg,#eff6ff,#fff); border-color:rgba(59,130,246,.16); }
      .arika-archive-kpi.overtime strong{ color:#2563eb; }
      .arika-archive-table-card table th{ padding:.75rem .6rem !important; }
      .arika-archive-table-card table td{ padding:.7rem .6rem !important; vertical-align:top; }
      .arika-archive-category{ display:inline-flex; align-items:center; justify-content:center; padding:.3rem .58rem; border-radius:999px; border:1px solid transparent; font-weight:950; font-size:.56rem; text-transform:uppercase; letter-spacing:.075em; white-space:nowrap; box-shadow:inset 0 0 0 1px rgba(255,255,255,.42); }
      .arika-archive-category.technical{ background:#dcfce7; border-color:#bbf7d0; color:#047857; }
      .arika-archive-category.meeting{ background:#dbeafe; border-color:#bfdbfe; color:#1d4ed8; }
      .arika-archive-category.competency{ background:#f3e8ff; border-color:#e9d5ff; color:#7e22ce; }
      .arika-archive-category.administration{ background:#fef3c7; border-color:#fde68a; color:#b45309; }
      .arika-archive-category.other{ background:#f1f5f9; border-color:#e2e8f0; color:#475569; }
      .arika-archive-label{ display:inline-flex; align-items:center; justify-content:center; padding:.25rem .48rem; border-radius:999px; font-size:.52rem; line-height:1; font-weight:950; text-transform:uppercase; letter-spacing:.07em; white-space:nowrap; }
      .arika-archive-label.done{ background:#dcfce7; color:#047857; }
      .arika-archive-label.pending{ background:#ffe4e6; color:#be123c; }
      .arika-archive-label.overtime{ background:#dbeafe; color:#1d4ed8; }
      .arika-archive-action{ width:1.85rem; height:1.85rem; display:inline-flex; align-items:center; justify-content:center; border-radius:.65rem; font-weight:900; transition:.15s ease; }
      .arika-archive-action.edit{ background:#ecfeff; color:#0e7490; }
      .arika-archive-action.edit:hover{ background:#0891b2; color:#fff; }
      .arika-archive-action.delete{ background:#fff1f2; color:#e11d48; }
      .arika-archive-action.delete:hover{ background:#e11d48; color:#fff; }
      @media(max-width:720px){
        .arika-archive-kpi{ min-height:50px; padding:.7rem .8rem; }
        .arika-archive-kpi strong{ font-size:1.1rem; }
        .arika-archive-table{ min-width:720px; }
      }
    `;
    document.head.appendChild(style);
  }
  function removeOldHistoryVisuals(){
    ['chart-status','chart-trend'].forEach(id => {
      const el = document.getElementById(id);
      if(el){ const card = el.closest('.bg-white, .chart-container'); if(card) card.style.display='none'; }
    });
  }
  function install(){
    document.documentElement.classList.add('arika-v252-kalender-libur-nasional-cuti-bersama');
    injectArchiveCss();
    removeOldHistoryVisuals();
    window.ARIKA_RIWAYAT_AUDIT_V248 = {
      version: VERSION,
      activeScript: 'app-v255.js?v=255',
      historyChartsRemoved: !document.getElementById('chart-status') && !document.getElementById('chart-trend'),
      archiveSummary: !!document.getElementById('rekap-month-summary')
    };
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install);
  else setTimeout(install,0);
...
/* v249: Riwayat Jurnal tetap arsip kerja, tetapi Kalender Kerja & Riwayat Aktivitas dipertahankan. */
(function(){
  if(window.__ARIKA_V249_KEEP_CALENDAR_ACTIVITY__) return;
  window.__ARIKA_V249_KEEP_CALENDAR_ACTIVITY__ = true;
  function refreshCalendar(){
    try { if(typeof window.renderVisualCalendar === 'function') window.renderVisualCalendar(); } catch(e) { console.warn('Kalender riwayat gagal dirender:', e); }
  }
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(refreshCalendar, 700); });
  const originalRunFilter = window.runFilter;
  if(typeof originalRunFilter === 'function' && !originalRunFilter.__v249CalendarWrapped){
    const wrapped = function(){
      const result = originalRunFilter.apply(this, arguments);
      setTimeout(refreshCalendar, 140);
      return result;
    };
    wrapped.__v249CalendarWrapped = true;
    window.runFilter = wrapped;
  }
  const originalShowTab = window.showTab;
  if(typeof originalShowTab === 'function' && !originalShowTab.__v249CalendarWrapped){
    const wrappedTab = function(id){
      const result = originalShowTab.apply(this, arguments);
      if(id === 'rekap') setTimeout(refreshCalendar, 180);
      return result;
    };
    wrappedTab.__v249CalendarWrapped = true;
    window.showTab = wrappedTab;
  }
})();


/* v250: Fix Kalender Kerja & Riwayat Aktivitas agar grid tanggal selalu tampil. */
/* v251: Tambah default Libur Nasional & Cuti Bersama 2026 pada kalender. */
(function(){
  if (window.__ARIKA_V250_CALENDAR_GRID_FIX__) return;
  window.__ARIKA_V250_CALENDAR_GRID_FIX__ = true;
  try {
    var style = document.createElement('style');
    style.id = 'arika-v250-calendar-grid-fix-style';
    style.textContent = '#view-rekap .arika-archive-calendar-card{display:block!important;overflow:visible!important}#view-rekap #calendar-grid{display:grid!important;grid-template-columns:repeat(7,minmax(0,1fr))!important;gap:.35rem!important;min-height:220px!important}#view-rekap .calendar-empty-cell{min-height:54px;border:1px solid rgba(241,245,249,.95);background:rgba(248,250,252,.55);border-radius:1rem}#view-rekap button.calendar-day-card{width:100%;text-align:left;cursor:pointer;display:block;border-width:1px}@media(max-width:720px){#view-rekap #calendar-grid{gap:.2rem!important;padding:.35rem!important}#view-rekap .calendar-empty-cell{min-height:42px;border-radius:.65rem}}';
  ...
  function parseDateKey(value){
    ...
    function out(y, mo, d){ return valid(y,mo,d) ? (Number(y)+'-'+pad2(mo)+'-'+pad2(d)) : ''; }
    m = raw.match(/^(\d{4})[-\/](\d{1,2})[-\/](\d{1,2})(?:T.*)?$/); if(m) return out(m[1],m[2],m[3]);
    m = raw.match(/^(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})$/); if(m) return out(m[3],m[2],m[1]);
    m = raw.match(/^(\d{2})(\d{2})(\d{4})$/); if(m) return out(m[3],m[2],m[1]);
    m = raw.match(/^(\d{4})(\d{2})(\d{2})$/); if(m) return out(m[1],m[2],m[3]);
    return '';
  }
  function parseMonthKey(value){
    if(!value) return '';
    ...
    raw = raw.replace(/[.]/g,'/').replace(/\s+/g,'');
    var m;
    function out(y, mo){ y=Number(y); mo=Number(mo); return (y && mo>=1 && mo<=12) ? (y+'-'+pad2(mo)) : ''; }
    m = raw.match(/^(\d{4})[-\/](\d{1,2})$/); if(m) return out(m[1],m[2]);
    m = raw.match(/^(\d{1,2})[-\/](\d{4})$/); if(m) return out(m[2],m[1]);
    m = raw.match(/^(\d{2})(\d{4})$/); if(m) return out(m[2],m[1]);
    m = raw.match(/^(\d{4})(\d{2})$/); if(m) return out(m[1],m[2]);
    return '';
  }
  function getCurrentMonthKey(){
    if(typeof window.getCurrentMonth === 'function') return window.getCurrentMonth();
    var now = new Date(); return now.getFullYear() + '-' + pad2(now.getMonth()+1);
  }
  function getTodayKey(){
    if(typeof window.getTodayKey === 'function') return window.getTodayKey();
    var now = new Date(); return now.getFullYear() + '-' + pad2(now.getMonth()+1) + '-' + pad2(now.getDate());
  }
  function getActiveCalendarPeriod(){
    var hiddenDate = parseDateKey(document.getElementById('filter-rekap-date')?.value || '');
    var hiddenMonth = parseMonthKey(document.getElementById('filter-rekap-month')?.value || '');
    var manual = document.getElementById('filter-rekap-manual')?.value || '';
    var manualDate = parseDateKey(manual);
    var manualMonth = parseMonthKey(manual);
    var date = hiddenDate || manualDate || '';
    var month = date ? date.slice(0,7) : (hiddenMonth || manualMonth || getCurrentMonthKey());
    return { date: date, month: month };
  }
  function monthLabel(monthKey){
    if(typeof window.formatBulanIndonesia === 'function') return window.formatBulanIndonesia(monthKey);
    var names = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
    var p = String(monthKey || '').split('-').map(Number);
    return (p[0] && p[1]) ? (names[p[1]-1] + ' ' + p[0]) : String(monthKey || '');
  }
  function dateLabel(dateKey){
    if(typeof window.formatDateIndo === 'function') return window.formatDateIndo(dateKey);
    return dateKey;
  }
  function normalizeRowDate(row){ return parseDateKey(row && (row.date || row.tanggal || row.tgl || row.timestamp)); }
  function isOvertime(row){
    var txt = normText((row && (row.tipe || row.jenis || row.type || row.cat || row.kategori || row.desc || row.keterangan)) || '');
    return !!(row && (row.isLembur || row.lembur || row.jamLembur || row.jamMulaiLembur || row.startLembur || txt.includes('lembur')));
  }
  /* v251: Default kalender libur nasional & cuti bersama Indonesia 2026
     Berbasis SKB 3 Menteri 2026. Data sheet KalenderLibur tetap dapat menimpa/menambah tanggal. */
  var ARIKA_DEFAULT_HOLIDAY_MAP_2026_V251 = {
    '2026-01-01': { type:'holiday', label:'Tahun Baru Masehi' },
    '2026-01-16': { type:'holiday', label:'Isra Mikraj' },
    '2026-02-16': { type:'leave', label:'Cuti Bersama Imlek' },
    '2026-02-17': { type:'holiday', label:'Tahun Baru Imlek' },
  ...
  function getHolidayInfo(dateKey){
    var dtParts = dateKey.split('-').map(Number);
    var dt = new Date(dtParts[0], dtParts[1]-1, dtParts[2]);
    var weekend = dt.getDay() === 0 || dt.getDay() === 6;
    var special = null;
    (window.kalenderLiburData || []).some(function(item){
      var key = parseDateKey(item && (item.tanggal || item.date));
      if(key === dateKey){ special = item; return true; }
      return false;
    });
    if(special){
      var typ = normText(special.jenis || special.type || '');
      var isLeave = typ.includes('cuti') || typ.includes('bersama') || typ.includes('leave');
      return { type: isLeave ? 'leave' : 'holiday', label: special.keterangan || special.label || special.nama || special.jenis || 'Tanggal Merah', isHoliday: true };
    }
    var fallbackSpecial = ARIKA_DEFAULT_HOLIDAY_MAP_2026_V251[dateKey];
    if(fallbackSpecial){
      return { type: fallbackSpecial.type || 'holiday', label: fallbackSpecial.label || 'Tanggal Merah', isHoliday: true };
    }
    if(weekend) return { type:'weekend', label:'Akhir Pekan', isHoliday:true };
    return { type:'workday', label:'Hari Kerja', isHoliday:false };
  }
  function ensureCalendarVisible(){
    var card = document.querySelector('#view-rekap .arika-archive-calendar-card');
    if(card){ card.style.display = ''; card.hidden = false; card.classList.remove('hidden'); }
    var grid = document.getElementById('calendar-grid');
    if(grid){ grid.style.display = 'grid'; grid.hidden = false; grid.classList.remove('hidden'); }
  }
  ...
  window.renderVisualCalendar = function(){
    ...
    var period = getActiveCalendarPeriod();
    var p = String(period.month || getCurrentMonthKey()).split('-').map(Number);
    var year = p[0], month = p[1];
    if(!year || !month || month < 1 || month > 12){
      period.month = getCurrentMonthKey();
      p = period.month.split('-').map(Number); year = p[0]; month = p[1];
    }
    var title = document.getElementById('calendar-month-title');
    if(title){
      title.textContent = period.date ? ('Kalender ' + monthLabel(period.month) + ' • Filter tanggal ' + dateLabel(period.date)) : ('Kalender ' + monthLabel(period.month) + ' • Klik tanggal untuk melihat detail aktivitas');
    }

    var currentName = normText(window.currentUser && window.currentUser.nama);
    var activityByDate = new Map();
    (window.arikaData || []).forEach(function(row){
      var date = normalizeRowDate(row);
      if(!date || date.slice(0,7) !== period.month) return;
      if(currentName && normText(row && row.name) !== currentName) return;
      if(!activityByDate.has(date)) activityByDate.set(date, []);
    ...
    var html = '';
    for(var i=0; i<firstDayIndex; i++) html += '<div class="calendar-empty-cell"></div>';
    for(var day=1; day<=totalDays; day++){
      var dateKey = year + '-' + pad2(month) + '-' + pad2(day);
      var info = getHolidayInfo(dateKey);
      var activities = activityByDate.get(dateKey) || [];
      var hasActivity = activities.length > 0;
      var allDone = hasActivity && activities.every(function(a){ return normText(a && a.status) === normText('Selesai'); });
      var cls = 'calendar-day-card ' + (info.type === 'weekend' ? 'calendar-weekend' : info.type === 'holiday' ? 'calendar-holiday' : info.type === 'leave' ? 'calendar-leave' : 'calendar-workday') + (dateKey === getTodayKey() ? ' calendar-today' : '');
      var color = info.type === 'holiday' || info.type === 'weekend' ? 'text-rose-700' : (info.type === 'leave' ? 'text-amber-700' : 'text-slate-700');
      var emptyDotClass = info.type === 'leave' ? 'bg-amber-300' : (info.isHoliday ? 'bg-rose-300' : 'bg-slate-200');
      var dot = hasActivity ? ('<span class="inline-flex items-center gap-1 text-[8px] font-black uppercase ' + (allDone ? 'text-emerald-700' : 'text-rose-700') + '"><i class="w-2 h-2 rounded-full ' + (allDone ? 'bg-emerald-500' : 'bg-rose-500') + ' inline-block"></i>' + activities.length + '</span>') : ('<span class="inline-block w-2 h-2 rounded-full ' + emptyDotClass + '"></span>');
      html += '<button type="button" onclick="window.showCalendarDayDetail(\'' + dateKey + '\')" class="' + cls + '">' +
        '<div class="flex items-start justify-between gap-1"><span class="calendar-day-number ' + color + '">' + day + '</span>' + dot + '</div>' +
        '<div class="calendar-day-label ' + (info.isHoliday ? (info.type === 'leave' ? 'text-amber-700' : 'text-rose-700') : 'text-slate-400') + '">' + esc(info.label) + '</div>' +
        '</button>';
    }
    grid.innerHTML = html;
  };
  ...
  window.showCalendarDayDetail = function(dateKey){
    ...
    var title = document.getElementById('modal-date-title');
    var list = document.getElementById('modal-activities-list');
    if(!modal || !title || !list) return;
    var info = getHolidayInfo(dateKey);
    title.textContent = (typeof window.formatHariTanggal === 'function' ? window.formatHariTanggal(dateKey) : dateKey) + ' • ' + info.label;
    var rows = (window.arikaData || []).filter(function(row){
      var sameDate = normalizeRowDate(row) === dateKey;
      var sameUser = !window.currentUser || normText(row && row.name) === normText(window.currentUser && window.currentUser.nama);
      return sameDate && sameUser;
    });
    var tone = info.type === 'leave' ? 'bg-amber-50 text-amber-800 border border-amber-100' : (info.isHoliday ? 'bg-rose-50 text-rose-800 border border-rose-100' : 'bg-emerald-50 text-emerald-800 border border-emerald-100');
    var html = '<div class="mb-3 p-3 rounded-xl ' + tone + '"><p class="text-[10px] font-black uppercase tracking-widest">' + (info.type === 'leave' ? 'Cuti Bersama' : (info.isHoliday ? 'Tanggal Merah / Libur' : 'Hari Kerja')) + '</p><p class="text-xs font-bold mt-1">' + esc(info.label) + '</p></div>';
    if(!rows.length){
      html += '<p class="text-center text-slate-400 italic text-xs py-4">Tidak ada riwayat aktivitas tercatat.</p>';
    } else {
      rows.forEach(function(row){
        var done = normText(row.status) === normText('Selesai');
        html += '<div class="p-3 bg-slate-50 rounded-xl border border-slate-200 mb-2"><div class="flex justify-between items-start gap-2 mb-2"><span class="text-[9px] uppercase font-black text-slate-400">' + esc(row.cat || row.kategori || '-') + '</span><div class="flex flex-wrap justify-end gap-1"><span class="px-2 py-0.5 rounded-full text-[8px] font-black uppercase ' + (done ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700') + '">' + esc(row.status || '-') + '</span>' + (isOvertime(row) ? '<span class="px-2 py-0.5 rounded-full text-[8px] font-black uppercase bg-blue-50 text-blue-700">Lembur</span>' : '') + '</div></div><p class="arika-calendar-activity-desc text-xs font-bold text-slate-700">' + esc(row.desc || row.kegiatan || row.uraian || '-') + '</p></div>';
      });
    }
    list.innerHTML = html;
    modal.classList.remove('hidden');
  ...
  function schedule(){
    clearTimeout(window.__arikaV250CalendarTimer);
    window.__arikaV250CalendarTimer = setTimeout(function(){
      try { window.renderVisualCalendar(); } catch(e){ console.warn('Kalender kerja gagal dirender:', e); }
    }, 120);
  }

  var oldRunFilter = window.runFilter;
  if(typeof oldRunFilter === 'function' && !oldRunFilter.__arikaV250CalendarSafe){
    window.runFilter = function(){
      var res = oldRunFilter.apply(this, arguments);
      schedule();
      return res;
    };
    window.runFilter.__arikaV250CalendarSafe = true;
  }
  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV250CalendarSafe){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      if(id === 'rekap') schedule();
      return res;
    };
    window.nav.__arikaV250CalendarSafe = true;
  }
  document.addEventListener('input', function(ev){ if(ev.target && ev.target.id === 'filter-rekap-manual') schedule(); }, true);
  document.addEventListener('change', function(ev){ if(ev.target && String(ev.target.id || '').startsWith('filter-rekap')) schedule(); }, true);
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(schedule, 350); setTimeout(schedule, 1200); });
  setTimeout(schedule, 500);
  setTimeout(schedule, 1500);
})();


/* v252 visual guard: libur nasional merah, cuti bersama amber. */
(function(){
  if(document.getElementById('arika-v251-holiday-style')) return;
  var style=document.createElement('style');
  style.id='arika-v251-holiday-style';
  style.textContent='.calendar-holiday{background:linear-gradient(135deg,#fff1f2,#fff)!important;border-color:#fecdd3!important}.calendar-weekend{background:linear-gradient(135deg,#fff7ed,#fff)!important;border-color:#fed7aa!important}.calendar-leave{background:linear-gradient(135deg,#fffbeb,#fff)!important;border-color:#fde68a!important}.calendar-day-label{white-space:normal!important;overflow:visible!important;text-overflow:clip!important;line-height:1.15!important}';
  document.head.appendChild(style);
})();


/* v252: Kalender Kerja menampilkan Libur Nasional + Cuti Bersama secara eksplisit. */
(function(){
  const NATIONAL_2026 = {
  ...
  window.ARIKA_NATIONAL_HOLIDAYS_2026 = NATIONAL_2026;
  window.ARIKA_JOINT_LEAVE_2026 = JOINT_LEAVE_2026;
  ...
  function normKey(v){
    ...
    if(d.length === 8){
      if(d.startsWith('20')) return d.slice(0,4)+'-'+d.slice(4,6)+'-'+d.slice(6,8);
      return d.slice(4,8)+'-'+d.slice(2,4)+'-'+d.slice(0,2);
    }
    return s;
  }

  function mergeHolidayRows(){
    const rows = Array.isArray(window.kalenderLiburData) ? window.kalenderLiburData.slice() : [];
    const exists = new Set(rows.map(r => normKey(r && (r.tanggal || r.date))).filter(Boolean));
    Object.keys(NATIONAL_2026).forEach(tgl => {
      if(!exists.has(tgl)) rows.push({ tanggal:tgl, jenis:'Libur Nasional', keterangan:NATIONAL_2026[tgl], aktif:true, sumber:'default-2026' });
    });
    Object.keys(JOINT_LEAVE_2026).forEach(tgl => {
      if(!exists.has(tgl)) rows.push({ tanggal:tgl, jenis:'Cuti Bersama', keterangan:JOINT_LEAVE_2026[tgl], aktif:true, sumber:'default-2026' });
    });
    window.kalenderLiburData = rows;
  }

  function relabelCalendarLegend(){
    document.querySelectorAll('.calendar-legend-dot').forEach(dot => {
      const pill = dot.closest('span');
      if(!pill) return;
      const txt = (pill.textContent || '').trim().toLowerCase();
      if(txt === 'merah' || txt === 'tanggal merah') pill.childNodes[pill.childNodes.length-1].textContent = ' Libur Nasional';
      if(txt === 'cuti') pill.childNodes[pill.childNodes.length-1].textContent = ' Cuti Bersama';
    });
  }

  const oldRender = window.renderVisualCalendar;
  if(typeof oldRender === 'function' && !oldRender.__arikaV252HolidayComplete){
    window.renderVisualCalendar = function(){
      mergeHolidayRows();
      const result = oldRender.apply(this, arguments);
      relabelCalendarLegend();
      return result;
    };
    window.renderVisualCalendar.__arikaV252HolidayComplete = true;
  }
  mergeHolidayRows();
  document.addEventListener('DOMContentLoaded', function(){
    mergeHolidayRows();
    relabelCalendarLegend();
    setTimeout(function(){ try{ if(typeof window.renderVisualCalendar === 'function') window.renderVisualCalendar(); }catch(_){} }, 300);
    setTimeout(relabelCalendarLegend, 900);
  });
  setTimeout(function(){ try{ if(typeof window.renderVisualCalendar === 'function') window.renderVisualCalendar(); }catch(_){} relabelCalendarLegend(); }, 800);
})();


/* =========================================================
   ARIKA v255 - Stabilitas Login & Dashboard Final
   Perbaikan setelah audit v254: layout login dirapikan,
   grafik dashboard dikembalikan render normal saat tab dibuka,
   filter Dashboard Saya diberi status aktif jelas.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V255_STABILITY_FIX__) return;
  window.__ARIKA_V255_STABILITY_FIX__ = true;

  function byId(id){ return document.getElementById(id); }
  function safe(fn){ try{ fn && fn(); }catch(e){ try{ console.warn('[ARIKA v255]', e && e.message ? e.message : e); }catch(_){} } }
  function isVisible(el){
    if(!el) return false;
  ...
  function injectCss(){
    if(byId('arika-v255-stability-css')) return;
    const css = document.createElement('style');
    css.id = 'arika-v255-stability-css';
    css.textContent = `
      /* Login awal: lebih rapi dan tidak terlalu sempit di PC */
      #view-login-user, #view-login-admin{
        max-width: 30rem !important;
        width: min(92vw, 30rem) !important;
        margin-left: auto !important;
        margin-right: auto !important;
      }
      #view-login-user > .bg-white, #view-login-admin > .bg-white{
        padding: 2rem 2.15rem !important;
        border-radius: 2rem !important;
      }
      #view-login-user .arika-logo-login, #view-login-admin .arika-logo-login{
        width: min(82%, 270px) !important;
        max-width: 270px !important;
        margin-bottom: 1rem !important;
      }
      #view-login-user h2, #view-login-admin h2{
        font-size: clamp(1.35rem, 2.15vw, 1.9rem) !important;
        line-height: 1.15 !important;
        white-space: nowrap !important;
        letter-spacing: -0.02em !important;
      }
      #view-login-user p, #view-login-admin p{ word-break: normal !important; }
      #view-login-user form, #view-login-admin form{ margin-top: 1.35rem !important; }
      #view-login-user input, #view-login-admin input{ min-height: 3.1rem !important; }
      @media(max-width:520px){
        #view-login-user, #view-login-admin{ width: min(94vw, 27rem) !important; margin-top: 1.25rem !important; }
        #view-login-user > .bg-white, #view-login-admin > .bg-white{ padding: 1.45rem 1.25rem !important; }
        #view-login-user h2, #view-login-admin h2{ font-size: 1.28rem !important; white-space: normal !important; }
        #view-login-user .arika-logo-login, #view-login-admin .arika-logo-login{ width: min(78%, 230px) !important; }
      }

      /* Dashboard: canvas/chart diberi ruang pasti supaya tidak tampak kosong */
      #view-dashboard-pegawai canvas,
      #view-admin canvas{
        display:block !important;
        max-width:100% !important;
      }
      #view-dashboard-pegawai .admin-chart-canvas-wrap,
      #view-dashboard-pegawai .admin-monev-chart-wrap,
      #view-dashboard-pegawai .chart-wrap,
      #view-dashboard-pegawai .relative:has(canvas),
      #view-admin .admin-chart-canvas-wrap,
      #view-admin .admin-monev-chart-wrap,
      #view-admin .chart-wrap,
      #view-admin .relative:has(canvas){
        min-height: 220px !important;
      }
      .arika-dashboard-period-btn{
        border: 1px solid rgba(255,255,255,.28) !important;
        transition: transform .15s ease, background .15s ease, color .15s ease, box-shadow .15s ease !important;
      }
      .arika-dashboard-period-btn.is-active,
      .arika-dashboard-period-btn[aria-pressed="true"]{
      ...
      .arika-dashboard-period-btn:not(.is-active):not([aria-pressed="true"]){
        background: rgba(6,78,59,.26) !important;
        color: #ffffff !important;
      }
      .arika-dashboard-period-btn:not(.is-active):not([aria-pressed="true"]):hover{ background: rgba(6,78,59,.36) !important; }
    `;
    document.head.appendChild(css);
  }

  function normalizeManualMonthDisplay(){
    const el = byId('pegawai-dashboard-month');
    if(!el) return;
    const raw = String(el.value || '').trim();
    const digits = raw.replace(/\D/g,'');
    if(/^\d{4}-\d{1,2}$/.test(raw)){
      const parts = raw.split('-');
      el.value = String(parts[1]).padStart(2,'0') + '/' + parts[0];
    } else if(digits.length === 6 && !/[\/\-]/.test(raw)){
      el.value = digits.slice(0,2) + '/' + digits.slice(2);
    }
  }

  window.ARIKA_updatePegawaiDashboardFilterState = function(){
    normalizeManualMonthDisplay();
    const el = byId('pegawai-dashboard-month');
    const val = el ? String(el.value || '').trim() : '';
    const current = byId('pegawai-dashboard-month-current');
    const all = byId('pegawai-dashboard-month-all');
    const isAll = !val;
    if(current){ current.classList.toggle('is-active', !isAll); current.setAttribute('aria-pressed', String(!isAll)); }
    if(all){ all.classList.toggle('is-active', isAll); all.setAttribute('aria-pressed', String(isAll)); }
  };

  function forceDashboardRender(){
    safe(function(){ window.ARIKA_updatePegawaiDashboardFilterState && window.ARIKA_updatePegawaiDashboardFilterState(); });
    setTimeout(function(){
      safe(function(){ if(isVisible(byId('view-dashboard-pegawai')) && typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({fromV255:true}); });
      safe(function(){ if(typeof window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ === 'function') window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); });
      safe(function(){ if(typeof window.__ARIKA_FLUSH_CHARTS_V245__ === 'function') window.__ARIKA_FLUSH_CHARTS_V245__(); });
      safe(function(){ if(typeof window.__ARIKA_FLUSH_PENDING_CHARTS_V241__ === 'function') window.__ARIKA_FLUSH_PENDING_CHARTS_V241__(); });
    }, 120);
    setTimeout(function(){
      safe(function(){ if(isVisible(byId('view-dashboard-pegawai')) && typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({fromV255Retry:true}); });
      safe(function(){ if(typeof window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ === 'function') window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); });
    }, 650);
  }

  function hookNavigation(){
    if(!window.__ARIKA_V255_NAV_HOOKED__ && typeof window.nav === 'function'){
      window.__ARIKA_V255_NAV_HOOKED__ = true;
      const originalNav = window.nav;
      window.nav = function(id){
        const result = originalNav.apply(window, arguments);
        if(id === 'dashboard-pegawai') forceDashboardRender();
        if(id === 'admin') setTimeout(function(){ safe(function(){ if(typeof window.renderAdminAnalytics === 'function') window.renderAdminAnalytics({fromV255:true}); }); }, 250);
        return result;
      };
    }
    if(!window.__ARIKA_V255_ADMIN_TAB_HOOKED__ && typeof window.switchAdminTab === 'function'){
      window.__ARIKA_V255_ADMIN_TAB_HOOKED__ = true;
      const originalSwitch = window.switchAdminTab;
      window.switchAdminTab = function(){
        const result = originalSwitch.apply(window, arguments);
        setTimeout(function(){
          safe(function(){ if(typeof window.renderAdminAnalytics === 'function') window.renderAdminAnalytics({fromV255Tab:true}); });
          safe(function(){ if(typeof window.renderAdminOvertimeDashboard === 'function') window.renderAdminOvertimeDashboard({fromV255Tab:true}); });
          safe(function(){ if(typeof window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ === 'function') window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); });
        }, 250);
        return result;
      };
  ...
  function hookDashboardMonth(){
    const el = byId('pegawai-dashboard-month');
    if(el && !el.__arikaV255MonthHooked){
      el.__arikaV255MonthHooked = true;
      ['input','change','blur'].forEach(function(evt){
        el.addEventListener(evt, function(){
          normalizeManualMonthDisplay();
          window.ARIKA_updatePegawaiDashboardFilterState();
          if(evt !== 'input') forceDashboardRender();
        });
      });
    }
    document.querySelectorAll('#view-dashboard-pegawai button').forEach(function(btn){
      const text = (btn.textContent || '').toLowerCase();
      if(/bulan ini|semua periode|sinkron/.test(text) && !btn.classList.contains('arika-dashboard-period-btn')){
        btn.classList.add('arika-dashboard-period-btn');
      }
    });
    window.ARIKA_updatePegawaiDashboardFilterState();
  }
  ...
  function audit(){
    window.ARIKA_AUDIT_FINAL = Object.assign({}, window.ARIKA_AUDIT_FINAL || {}, {
      version: 'v255-stabilitas-login-dashboard',
      script: 'app-v255.js?v=255',
      loginFix: true,
      dashboardRenderNormal: true,
      dashboardFilterState: true,
  ...
  function install(){
    ...
    hookDashboardMonth();
    audit();
    setTimeout(function(){ hookNavigation(); hookDashboardMonth(); if(isVisible(byId('view-dashboard-pegawai'))) forceDashboardRender(); }, 600);
    setTimeout(function(){ hookNavigation(); hookDashboardMonth(); if(isVisible(byId('view-dashboard-pegawai'))) forceDashboardRender(); audit(); }, 1800);
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install);
  else install();
})();
...
/* =========================================================
   v257 - Dashboard Saya filter fix + Monev Aktivitas Pegawai
   - Semua Periode benar-benar membaca semua data, bukan bulan berjalan.
   - Tambah export monev aktivitas pribadi: keseimbangan rutinitas, lembur, dan pengembangan kompetensi.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V257_DASHBOARD_MONEV__) return;
  window.__ARIKA_V257_DASHBOARD_MONEV__ = true;
  ...
  function monthDisplayToKey(v){
    const raw = String(v || '').trim();
    if(!raw) return '';
    let m = raw.match(/^(\d{4})[-\/](\d{1,2})$/); if(m) return m[1] + '-' + pad(m[2]);
    m = raw.match(/^(\d{1,2})[-\/](\d{4})$/); if(m) return m[2] + '-' + pad(m[1]);
    m = raw.replace(/\D/g,'').match(/^(\d{2})(\d{4})$/); if(m) return m[2] + '-' + pad(m[1]);
    m = raw.replace(/\D/g,'').match(/^(\d{4})(\d{2})$/); if(m) return m[1] + '-' + pad(m[2]);
    return '';
  }
  ...
  function activeDashboardMonth(){
    const el = byId('pegawai-dashboard-month');
    if(!el) return monthNow();
    if(el.dataset.periodMode === 'all') return '';
    const key = monthDisplayToKey(el.value || el.dataset.arikaIsoValue || '');
    return key || monthNow();
  }
  function setFilterState(mode){
    const el = byId('pegawai-dashboard-month');
    if(el){
      el.dataset.periodMode = mode === 'all' ? 'all' : 'month';
      if(mode === 'all'){
        el.value = '';
        el.dataset.arikaIsoValue = '';
      } else {
        const key = monthDisplayToKey(el.value || '') || monthNow();
        el.value = monthKeyToDisplay(key);
        el.dataset.arikaIsoValue = key;
      }
    }
    const current = byId('pegawai-dashboard-month-current');
  ...
  window.ARIKA_updatePegawaiDashboardFilterState = function(){
    const el = byId('pegawai-dashboard-month');
    const mode = el && el.dataset.periodMode === 'all' ? 'all' : 'month';
    setFilterState(mode);
  };
  function rerenderDashboard(){
    try { if(typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({fromV257:true}); } catch(e){ console.warn('Render Dashboard Saya v257 gagal:', e); }
    try { if(typeof window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ === 'function') window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); } catch(_){}
  }
  function hookDashboardFilter(){
    const el = byId('pegawai-dashboard-month');
    const current = byId('pegawai-dashboard-month-current');
    const all = byId('pegawai-dashboard-month-all');
    if(el && !el.__arikaV257Hooked){
      el.__arikaV257Hooked = true;
      el.addEventListener('input', function(){
        if(el.value.trim()) el.dataset.periodMode = 'month';
      });
      ...
      el.addEventListener('blur', function(){
        if(el.value.trim()) setFilterState('month');
      });
    }
    if(current && !current.__arikaV257Hooked){
      current.__arikaV257Hooked = true;
      current.addEventListener('click', function(ev){
        ev.preventDefault(); ev.stopPropagation();
        setFilterState('month');
        setTimeout(rerenderDashboard, 20);
      }, true);
    }
    if(all && !all.__arikaV257Hooked){
      all.__arikaV257Hooked = true;
  ...
  function dateKey(row){ return String(row && row.date || '').slice(0,10); }
  function rowMatchesCurrentUser(row){
    const u = window.currentUser || {};
    const userNip = norm(u.nip || u.NIP || '');
    const userName = norm(u.nama || u.name || '');
    const rowNip = norm(row && (row.nip || row.NIP || row.nomorInduk || ''));
    const rowName = norm(row && (row.name || row.nama || row.pegawai || ''));
    return (userNip && rowNip && userNip === rowNip) || (userName && rowName && userName === rowName);
  }
  function rowsForMonev(monthKey){
    return (window.arikaData || [])
      .filter(rowMatchesCurrentUser)
      .filter(function(r){ return !monthKey || dateKey(r).startsWith(monthKey); })
      .sort(function(a,b){ return String(dateKey(a)).localeCompare(String(dateKey(b))); });
  }
  function isSelesai(row){
    const s = norm(row && row.status || '');
    return s.includes('selesai') && !s.includes('belum');
  }
  function isLembur(row){
    if(!row) return false;
    const keys = ['isLembur','lembur','kegiatanLembur','statusLembur','tandaLembur','Lembur','LEMBUR'];
    for(const k of keys){
      if(k in row){
        const v = norm(row[k]);
        if(row[k] === true || v === 'true' || v === 'ya' || v === 'y' || v === 'lembur' || v === '1') return true;
      }
    }
    return !!(row.start && row.end && String(row.start).trim() && String(row.end).trim());
  }
  function minutesFromTimes(row){
    const start = String(row && (row.start || row.jamMulai || row.mulai) || '').trim();
    const end = String(row && (row.end || row.jamSelesai || row.selesai) || '').trim();
    const toMin = function(t){ const m = t.match(/(\d{1,2})[:.](\d{2})/); return m ? Number(m[1])*60 + Number(m[2]) : NaN; };
    let a = toMin(start), b = toMin(end);
    if(!Number.isFinite(a) || !Number.isFinite(b)) return 0;
    if(b < a) b += 24*60;
    return Math.max(0, b-a);
  }
  function containsKompetensi(row){
    if (normalizeJurnalCategoryValue(row && row.cat) === 'Pengembangan Kompetensi') return true;
    const text = norm([row && row.cat, row && row.desc, row && row.aktivitas, row && row.kegiatan].join(' '));
    return /(pelatihan|bimtek|bimbingan teknis|workshop|seminar|webinar|kompetensi|diklat|sertifikasi|coaching|mentoring|belajar|sosialisasi|sharing|diseminasi)/.test(text);
  }
  function containsRutin(row){
    const text = norm([row && row.cat, row && row.desc, row && row.aktivitas, row && row.kegiatan].join(' '));
    return /(rutin|administrasi|laporan|pengujian|sampling|rapat|verifikasi|input|rekap|dokumen|pemeriksaan)/.test(text);
  }
  function periodLabel(monthKey){
    if(!monthKey) return 'Semua Periode';
    const nama = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
    const y = monthKey.slice(0,4), m = Number(monthKey.slice(5,7));
    return (nama[m-1] || monthKey) + ' ' + y;
  }
  function evaluationRows(rows, monthKey){
    const total = rows.length;
    const done = rows.filter(isSelesai).length;
    const notDone = total - done;
    const days = new Set(rows.map(dateKey).filter(Boolean));
    const overtimeRows = rows.filter(isLembur);
    const overtimeMinutes = overtimeRows.reduce(function(sum, r){ return sum + minutesFromTimes(r); }, 0);
    const kompetensi = rows.filter(containsKompetensi).length;
    const rutin = rows.filter(containsRutin).length;
    const categorySet = new Set(rows.map(function(r){ return normalizeJurnalCategoryValue(r.cat || r.kategori || '') || 'Lainnya'; }));
    const reviewNotes = rows.filter(function(r){ return String(r.catatanAtasan || r.review || r.catatan || '').trim(); }).length;
    const lemburJam = Math.round((overtimeMinutes/60)*10)/10;
    const lemburRatio = days.size ? overtimeRows.length / days.size : 0;
    const keseimbangan = lemburRatio > 0.45 ? 'Perlu perhatian' : (lemburRatio > 0.25 ? 'Cukup padat' : 'Terkendali');
    return [
      ['Periode', periodLabel(monthKey), 'Informasi', monthKey ? 'Monev mengikuti filter bulan Dashboard Saya.' : 'Monev memakai seluruh data pribadi yang tersedia.'],
      ['Total aktivitas', total + ' jurnal', total ? 'Terdata' : 'Belum ada data', total ? 'Pertahankan pencatatan harian yang konsisten.' : 'Mulai isi jurnal agar aktivitas kerja dapat dipantau.'],
      ['Ketuntasan pekerjaan', (total ? Math.round(done/total*100) : 0) + '% (' + done + ' selesai, ' + notDone + ' belum selesai)', done === total && total ? 'Baik' : 'Perlu dipantau', notDone ? 'Selesaikan/rapikan jurnal yang belum selesai.' : 'Ketuntasan sudah baik.'],
      ['Hari aktif tercatat', days.size + ' hari', days.size >= 15 || !monthKey ? 'Cukup' : 'Perlu konsistensi', 'Gunakan jurnal sebagai arsip singkat aktivitas harian.'],
      ['Beban lembur', overtimeRows.length + ' jurnal lembur, estimasi ' + lemburJam + ' jam', keseimbangan, lemburRatio > 0.25 ? 'Pantau ritme kerja dan distribusi beban agar tidak menumpuk di luar jam kerja.' : 'Beban lembur masih relatif terkendali berdasarkan data jurnal.'],
      ['Rutinitas pekerjaan', rutin + ' aktivitas rutin teridentifikasi', rutin > total*0.7 && total ? 'Dominan rutin' : 'Bervariasi', 'Seimbangkan pekerjaan rutin dengan kegiatan pengembangan dan perbaikan proses.'],
      ['Pengembangan kompetensi diri', kompetensi + ' aktivitas', kompetensi ? 'Ada peningkatan' : 'Belum tampak', kompetensi ? 'Dokumentasikan hasil pelatihan/learning agar dampaknya terlihat.' : 'Tambahkan aktivitas belajar, pelatihan, sharing, atau peningkatan kompetensi jika ada.'],
      ['Variasi aktivitas', categorySet.size + ' kategori', categorySet.size >= 3 ? 'Bervariasi' : 'Terbatas', 'Variasi kategori membantu membaca komposisi rutinitas kerja.'],
      ['Catatan/review atasan', reviewNotes + ' catatan', reviewNotes ? 'Ada tindak lanjut' : 'Tidak ada catatan', reviewNotes ? 'Tindak lanjuti catatan agar perbaikan terdokumentasi.' : 'Tetap pertahankan kualitas jurnal.']
    ];
  }
  function tableHtml(rows){
    let html = '<table border="1"><tr><th>Indikator</th><th>Kondisi</th><th>Evaluasi</th><th>Rekomendasi</th></tr>';
    rows.forEach(function(r){ html += '<tr><td>'+esc(r[0])+'</td><td>'+esc(r[1])+'</td><td>'+esc(r[2])+'</td><td>'+esc(r[3])+'</td></tr>'; });
    html += '</table>';
    return html;
  }
  function downloadXlsLocal(filename, html){
    const blob = new Blob(['\ufeff', html], {type:'application/vnd.ms-excel;charset=utf-8;'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
  ...
  window.downloadPegawaiMonevAktivitas = function(){
    const monthKey = activeDashboardMonth();
    const rows = rowsForMonev(monthKey);
    const u = window.currentUser || {};
    const evalRows = evaluationRows(rows, monthKey);
    const title = 'MONEV AKTIVITAS PEGAWAI ARIKA';
    let html = '<table border="1">'
      + '<tr><th colspan="4">' + esc(title) + '</th></tr>'
      + '<tr><td>Nama Pegawai</td><td colspan="3">' + esc(u.nama || u.name || '-') + '</td></tr>'
      + '<tr><td>NIP</td><td colspan="3">' + esc(u.nip || '-') + '</td></tr>'
      + '<tr><td>Fungsi/Lab</td><td colspan="3">' + esc(u.lab || '-') + '</td></tr>'
      + '<tr><td>Periode</td><td colspan="3">' + esc(periodLabel(monthKey)) + '</td></tr>'
      + '<tr><td>Tanggal Unduh</td><td colspan="3">' + esc(new Date().toLocaleString('id-ID')) + '</td></tr>'
      + '<tr><td colspan="4">Catatan: monev ini bersifat indikator internal berbasis jurnal ARIKA, bukan penilaian medis atau diagnosis.</td></tr>'
      + '</table><br>' + tableHtml(evalRows);
    // Tambah data mentah ringkas agar dapat diverifikasi
    html += '<br><table border="1"><tr><th>No</th><th>Tanggal</th><th>Kategori</th><th>Status</th><th>Lembur</th><th>Aktivitas</th></tr>';
    rows.forEach(function(r, i){
      html += '<tr><td>'+(i+1)+'</td><td>'+esc(dateKey(r))+'</td><td>'+esc(r.cat || r.kategori || '-')+'</td><td>'+esc(r.status || '-')+'</td><td>'+esc(isLembur(r)?'Ya':'Tidak')+'</td><td>'+esc(r.desc || r.aktivitas || r.kegiatan || '-')+'</td></tr>';
    });
    html += '</table>';
    const suffix = monthKey || 'semua-periode';
    downloadXlsLocal('monev-aktivitas-pegawai-' + norm(u.nama || 'pegawai') + '-' + suffix + '.xls', html);
  };
  function addMonevButton(){
    const syncBtn = Array.from(document.querySelectorAll('#view-dashboard-pegawai button')).find(function(b){ return /sinkron/i.test(b.textContent || ''); });
    if(syncBtn && !byId('pegawai-dashboard-export-monev')){
      const btn = document.createElement('button');
      btn.id = 'pegawai-dashboard-export-monev';
      btn.type = 'button';
      btn.className = 'px-4 py-3 rounded-2xl bg-white/95 text-emerald-700 text-[10px] font-black uppercase tracking-widest shadow-lg hover:scale-[1.02] transition-all';
      btn.textContent = '📥 Download Monev Aktivitas';
      btn.onclick = function(){ window.downloadPegawaiMonevAktivitas && window.downloadPegawaiMonevAktivitas(); };
  ...
  function install(){
    hookDashboardFilter();
    addMonevButton();
    window.ARIKA_AUDIT_FINAL = Object.assign({}, window.ARIKA_AUDIT_FINAL || {}, { version_v257: 'filter-dashboard-dan-monev-aktivitas', dashboardSemuaPeriodeFix: true, monevAktivitasPegawai: true, timestamp_v257: new Date().toISOString() });
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install); else install();
...
/* ARIKA v259 - Papan Pengumuman: Ditujukan Untuk dibuat picker chip global (PC & mobile)
   Tujuan: pengguna dapat memilih lebih dari satu fungsi/lab tanpa harus menahan Ctrl/Command pada PC.
   Select asli tetap dipertahankan sebagai sumber data agar fungsi simpan/edit lama tidak rusak. */
(function(){
  if (window.__ARIKA_V259_PENGUMUMAN_TARGET_GLOBAL_PICKER__) return;
  window.__ARIKA_V259_PENGUMUMAN_TARGET_GLOBAL_PICKER__ = true;

  var SELECT_ID = 'pengumuman-target-fungsi';
  var PICKER_ID = 'arika-pengumuman-target-mobile-picker';
  var STYLE_ID = 'arika-v259-pengumuman-target-global-style';
  ...
  function isSemua(value){
    var n = norm(value);
    return n === 'semua' || n === 'semua fungsi/lab' || n === 'semua fungsi';
  }
  ...
  function selectedValues(select){
    ...
    return Array.from(select.options || [])
      ...
      .map(function(opt){ return opt.value || opt.textContent || ''; })
      .filter(Boolean);
  }
  ...
  function ensureStyle(){
    ...
    style.textContent = [
      ...
      '}',
      '#'+PICKER_ID+' .arika-target-chip:hover{',
      ...
      '  color:#ffffff !important;',
      '  box-shadow:0 10px 20px rgba(8,145,178,.16) !important;',
      '}',
      '#'+PICKER_ID+' .arika-target-chip.is-active::before{',
      '  content:"✓ ";',
      '}',
      '@media (max-width:768px){',
      '  #'+PICKER_ID+'{gap:6px !important;padding:8px !important;}',
      '  #'+PICKER_ID+' .arika-target-chip{font-size:9px !important;min-height:31px !important;padding:7px 10px !important;}',
      '}'
    ].join('\n');
    document.head.appendChild(style);
  }
  ...
  function setSelection(value){
    ...
    if (!select) return;
    var options = Array.from(select.options || []);
    var targetNorm = norm(value);

    if (isSemua(value)) {
      options.forEach(function(opt){ opt.selected = isSemua(opt.value || opt.textContent); });
    } else {
      options.forEach(function(opt){
        if (isSemua(opt.value || opt.textContent)) opt.selected = false;
        if (norm(opt.value || opt.textContent) === targetNorm) opt.selected = !opt.selected;
      });
      var hasSpecific = options.some(function(opt){ return opt.selected && !isSemua(opt.value || opt.textContent); });
      if (!hasSpecific) {
        options.forEach(function(opt){ opt.selected = isSemua(opt.value || opt.textContent); });
      }
    }
  ...
  function getOrCreatePicker(select){
    var picker = document.getElementById(PICKER_ID);
    if (!picker) {
      picker = document.createElement('div');
      picker.id = PICKER_ID;
  ...
  function rebuildPicker(force){
    var select = getSelect();
    if (!select) return;
    ensureStyle();
    var picker = getOrCreatePicker(select);
    var signature = Array.from(select.options || [])
      .map(function(opt){ return (opt.value || '') + '::' + (opt.textContent || ''); })
      .join('|');

    if (!force && picker.getAttribute('data-v259-signature') === signature) {
      syncPickerState();
      return;
    }

    picker.setAttribute('data-v259-signature', signature);
    picker.innerHTML = '';
    Array.from(select.options || []).forEach(function(opt){
      var value = opt.value || opt.textContent || '';
      var label = (opt.textContent || value || '').replace(/^🌐\s*/, '').trim();
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'arika-target-chip';
    ...
    select.removeEventListener('change', syncPickerState);
  ...
  function syncPickerState(){
    ...
    var picker = document.getElementById(PICKER_ID);
    if (!select || !picker) return;
    var selected = selectedValues(select).map(norm);
    var useSemua = !selected.length || selected.some(function(v){ return isSemua(v); });
    picker.querySelectorAll('.arika-target-chip').forEach(function(btn){
      var value = btn.getAttribute('data-value') || '';
      var active = useSemua ? isSemua(value) : selected.indexOf(norm(value)) !== -1;
      btn.classList.toggle('is-active', active);
      btn.setAttribute('aria-pressed', active ? 'true' : 'false');
    });
  }

  function patchTargetHelpers(){
    if (!window.__ARIKA_V259_PATCH_SET_TARGET__ && typeof window.setPengumumanTargetSemua === 'function') {
      window.__ARIKA_V259_PATCH_SET_TARGET__ = true;
      var oldSetSemua = window.setPengumumanTargetSemua;
      window.setPengumumanTargetSemua = function(){
        var result = oldSetSemua.apply(this, arguments);
        setTimeout(function(){ rebuildPicker(true); }, 0);
        return result;
      };
    }
    if (!window.__ARIKA_V259_PATCH_CLEAR_TARGET__ && typeof window.clearPengumumanTarget === 'function') {
      window.__ARIKA_V259_PATCH_CLEAR_TARGET__ = true;
      var oldClear = window.clearPengumumanTarget;
      window.clearPengumumanTarget = function(){
        var result = oldClear.apply(this, arguments);
        setTimeout(function(){ rebuildPicker(true); }, 0);
    ...
    if (!window.__ARIKA_V259_PATCH_EDIT_PENGUMUMAN__ && typeof window.editPengumuman === 'function') {
      window.__ARIKA_V259_PATCH_EDIT_PENGUMUMAN__ = true;
      var oldEdit = window.editPengumuman;
      window.editPengumuman = function(){
        var result = oldEdit.apply(this, arguments);
        setTimeout(function(){ rebuildPicker(true); syncPickerState(); }, 120);
        return result;
      };
    }
    if (!window.__ARIKA_V259_PATCH_RESET_PENGUMUMAN__ && typeof window.resetFormPengumuman === 'function') {
      window.__ARIKA_V259_PATCH_RESET_PENGUMUMAN__ = true;
      var oldReset = window.resetFormPengumuman;
      window.resetFormPengumuman = function(){
        var result = oldReset.apply(this, arguments);
        setTimeout(function(){ rebuildPicker(true); syncPickerState(); }, 80);
        return result;
      };
    }
  }
  ...
  function install(){
    try {
      ...
      console.warn('ARIKA v259 target fungsi picker gagal:', err);
    }
  }
  ...
  document.addEventListener('click', function(ev){
    ...
    if (ev.target.closest('#admin-tab-pengumuman, [onclick*="pengumuman"], [onclick*="Pengumuman"]')) {
      setTimeout(install, 80);
      setTimeout(function(){ rebuildPicker(true); }, 260);
    }
  }, true);
...
/* ARIKA v260 - fix chip status aktif pada filter Rekap Jurnal Pegawai */
(function(){
  'use strict';
  var STYLE_ID = 'arika-v260-admin-rekap-chip-style';
  function ensureStyle(){
    if(document.getElementById(STYLE_ID)) return;
    var style = document.createElement('style');
    style.id = STYLE_ID;
    style.textContent = `
      #admin-content-rekap .arika-rekap-range-chip,
      #admin-content-rekap .admin-range-chip {
      ...
      #admin-content-rekap .arika-rekap-range-chip:hover,
      #admin-content-rekap .admin-range-chip:hover {
        transform: translateY(-1px);
        border-color: rgba(20,184,166,.35) !important;
        color: #0f766e !important;
        background: rgba(240,253,250,.96) !important;
      }
      #admin-content-rekap .arika-rekap-range-chip.is-active,
      #admin-content-rekap .admin-range-chip.is-active {
        color: #fff !important;
        border-color: transparent !important;
        background: linear-gradient(135deg,#059669,#06b6d4) !important;
        box-shadow: 0 10px 22px rgba(6,182,212,.22) !important;
      }
      #admin-content-rekap .arika-rekap-range-chip.is-active:hover,
      #admin-content-rekap .admin-range-chip.is-active:hover {
        color: #fff !important;
        transform: translateY(-1px);
      }
    `;
  ...
  function currentWeekRange(){
    ...
    monday.setDate(now.getDate() - ((day + 6) % 7));
    var start = monday.getFullYear() + '-' + pad(monday.getMonth()+1) + '-' + pad(monday.getDate());
  ...
  function parseDateKey(v){
    ...
    if(digits.length === 8){
      var dd = digits.slice(0,2), mm = digits.slice(2,4), yy = digits.slice(4);
      return yy + '-' + mm + '-' + dd;
    }
    return '';
  }
  function getRange(){
    var startEl = document.getElementById('admin-filter-start');
    var endEl = document.getElementById('admin-filter-end');
    return {
      start: parseDateKey(startEl && startEl.value),
      end: parseDateKey(endEl && endEl.value)
    };
  }
  function chipTypeFromText(txt){
    var t = String(txt || '').toLowerCase();
    if(t.indexOf('hari ini') !== -1) return 'today';
    if(t.indexOf('minggu ini') !== -1) return 'week';
    if(t.indexOf('bulan ini') !== -1) return 'month';
    if(t.indexOf('semua waktu') !== -1) return 'all';
    return '';
  }
  function installChipMarkers(){
    var container = document.getElementById('admin-content-rekap');
    if(!container) return;
    var buttons = Array.from(container.querySelectorAll('button')).filter(function(btn){
      return /Hari Ini|Minggu Ini|Bulan Ini|Semua Waktu/i.test(btn.textContent || '');
    });
    buttons.forEach(function(btn){
      var type = chipTypeFromText(btn.textContent);
      if(!type) return;
      btn.classList.add('arika-rekap-range-chip');
      btn.setAttribute('data-arika-rekap-range', type);
      if(!btn.__arikaV260ClickBound){
        btn.__arikaV260ClickBound = true;
        btn.addEventListener('click', function(){
          setTimeout(updateRangeState, 0);
          setTimeout(updateRangeState, 120);
          setTimeout(updateRangeState, 350);
        });
      }
    });
  }
  function setActive(type){
    var container = document.getElementById('admin-content-rekap');
    if(!container) return;
    container.querySelectorAll('.arika-rekap-range-chip,[data-arika-rekap-range]').forEach(function(btn){
      var active = btn.getAttribute('data-arika-rekap-range') === type;
      btn.classList.toggle('is-active', active);
      btn.setAttribute('aria-pressed', active ? 'true' : 'false');
    });
  }
  function updateRangeState(){
    ...
    var r = getRange();
    var t = todayKey();
    if(!r.start && !r.end) return setActive('all');
    if(r.start === t && r.end === t) return setActive('today');
    var w = currentWeekRange();
    if(r.start === w.start && r.end === w.end) return setActive('week');
    var m = currentMonthRange();
    if(r.start === m.start && r.end === m.end) return setActive('month');
    setActive('');
  }

  function wrapRender(){
    if(window.__ARIKA_V260_REKAP_RENDER_WRAPPED__) return;
    if(typeof window.renderAdminAllTable !== 'function') return;
    window.__ARIKA_V260_REKAP_RENDER_WRAPPED__ = true;
    var oldRender = window.renderAdminAllTable;
    window.renderAdminAllTable = function(){
  ...
  function install(){
    ...
    wrapRender();
    updateRangeState();
    ['admin-filter-start','admin-filter-end','admin-filter-lab','admin-filter-status','admin-filter-type'].forEach(function(id){
      var el = document.getElementById(id);
      if(el && !el.__arikaV260RangeBound){
        el.__arikaV260RangeBound = true;
        el.addEventListener('input', function(){ setTimeout(updateRangeState, 0); });
        el.addEventListener('change', function(){ setTimeout(updateRangeState, 0); });
      }
    });
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install);
  ...
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#admin-tab-rekap, [onclick*="rekap"], [onclick*="renderAdminAllTable"]')) {
      setTimeout(install, 80);
      setTimeout(updateRangeState, 260);
    }
  }, true);
  var tries = 0;
  var timer = setInterval(function(){
...
/* v263: Mode Fokus Hari Ini pada Beranda */
(function(){
  if(window.__ARIKA_FOKUS_HARI_INI_V263__) return;
  window.__ARIKA_FOKUS_HARI_INI_V263__ = true;

  function safeNormalize(v){
    try { return typeof normalize === 'function' ? normalize(v) : String(v || '').trim().toLowerCase().replace(/[^a-z0-9]/g,''); }
    catch(_) { return String(v || '').trim().toLowerCase().replace(/[^a-z0-9]/g,''); }
  }
  function todayKey(){
    try { return typeof getTodayKey === 'function' ? getTodayKey() : new Date().toLocaleDateString('en-CA'); }
    catch(_) { return new Date().toLocaleDateString('en-CA'); }
  }
  function dateKey(v){
    try { return typeof normalizeDateKeyFromSheet === 'function' ? normalizeDateKeyFromSheet(v) : String(v || '').slice(0,10); }
    catch(_) { return String(v || '').slice(0,10); }
  }
  function fmtDate(v){
    try { return typeof formatDateIndo === 'function' ? formatDateIndo(v) : String(v || ''); }
    catch(_) { return String(v || ''); }
  }
  function fmtMonth(v){
    try { return typeof formatBulanIndonesia === 'function' ? formatBulanIndonesia(v) : String(v || ''); }
    catch(_) { return String(v || ''); }
  }
  function userMatches(row){
    if(!window.currentUser || window.isAdmin) return false;
    const userNip = safeNormalize(window.currentUser.nip || '');
    const userName = safeNormalize(window.currentUser.nama || '');
    const rowNip = safeNormalize(row.nip || row.ownerNip || row.NIP || row['NIP Pegawai'] || '');
    const rowName = safeNormalize(row.name || row.nama || row.ownerName || row.pelaksana || row['Nama Pegawai'] || '');
    if(userNip && rowNip && (userNip === rowNip || userNip.includes(rowNip) || rowNip.includes(userNip))) return true;
    if(userName && rowName && (userName === rowName || userName.includes(rowName) || rowName.includes(userName))) return true;
    return false;
  }
  function getMyTodayJournalRows(){
    const today = todayKey();
    return (Array.isArray(window.arikaData) ? window.arikaData : []).filter(row => userMatches(row) && dateKey(row.date || row.tanggal) === today);
  }
  function getFocusRencana(){
    if(window.isAdmin || !window.currentUser) return [];
    let source = [];
    try { source = (typeof getStableRencanaData === 'function') ? getStableRencanaData() : (window.rencanaData || []); }
    catch(_) { source = window.rencanaData || []; }
    return (source || []).filter(item => {
      try { if(typeof isCurrentUserRencana === 'function' && !isCurrentUserRencana(item)) return false; } catch(_) {}
      try { if(typeof isRencanaSelesai === 'function' && isRencanaSelesai(item)) return false; } catch(_) {}
      return true;
    });
  }
  function rencanaNeedsAttention(item){
    try { if(typeof isRencanaTerlewat === 'function' && isRencanaTerlewat(item)) return true; } catch(_) {}
    try { if(typeof isRencanaWaktuSudahTiba === 'function' && isRencanaWaktuSudahTiba(item)) return true; } catch(_) {}
    try { if(typeof isRencanaTerjadwalHariIni === 'function' && isRencanaTerjadwalHariIni(item)) return true; } catch(_) {}
    return dateKey(item && item.tanggal) === todayKey();
  }
  function isRencanaHariIniUntukFokus(item){
    if(!item) return false;
    try { if(typeof isRencanaSelesai === 'function' && isRencanaSelesai(item)) return false; } catch(_) {}
    return dateKey(item.tanggal || item.date || item.tgl || '') === todayKey();
  }
  function getAgendaFocus(){
    if(!window.currentUser && !window.isAdmin) return null;
    const today = todayKey();
    let maxDate = today;
    try { maxDate = typeof addDaysToKey === 'function' ? addDaysToKey(today, 7) : today; } catch(_) {}
    let list = (window.agendaData || []).filter(item => {
      try { if(typeof isAgendaTampilDiBeranda === 'function' && !isAgendaTampilDiBeranda(item, maxDate)) return false; } catch(_) {}
      try { if(!window.isAdmin && typeof isAgendaUntukUser === 'function' && !isAgendaUntukUser(item)) return false; } catch(_) {}
      return true;
    });
    try { if(typeof sortAgendaInteraktif === 'function') list = list.sort(sortAgendaInteraktif); } catch(_) {
      list = list.sort((a,b) => String(a.tanggal || '').localeCompare(String(b.tanggal || '')) || String(a.waktuMulai || '').localeCompare(String(b.waktuMulai || '')));
    }
    return list[0] || null;
  }
  function getReminderCount(){
    try { return (typeof collectBerandaReminderEvents === 'function' ? (collectBerandaReminderEvents() || []) : []).length; }
    catch(_) { return 0; }
  }
  function hasSurveyPending(){
    try {
      if(window.isAdmin || !window.currentUser) return false;
      if(typeof hasUserFilledSurveyThisMonthForReminder === 'function') return !hasUserFilledSurveyThisMonthForReminder();
      const month = (typeof getCurrentSurveyMonth === 'function') ? getCurrentSurveyMonth() : new Date().toLocaleDateString('en-CA').slice(0,7);
      const key = safeNormalize(window.currentUser.nip || window.currentUser.nama || '');
      return !(window.surveiData || []).some(s => String(s.bulan || '').slice(0,7) === month && safeNormalize(s.nip || s.nama) === key);
    } catch(_) { return false; }
  }
  function itemHtml({tone='ok', icon='✅', title='', desc='', action='', onClick='', priority=false}){
    const actionHtml = action ? `<button class="arika-focus-action" type="button" onclick="${onClick}">${action}</button>` : '';
    const priorityClass = priority ? ' is-priority-blink' : '';
    return `<div class="arika-focus-item is-${tone}${priorityClass}"><div class="arika-focus-icon">${icon}</div><div class="arika-focus-body"><p class="arika-focus-title">${escapeHTML(title)}</p><p class="arika-focus-desc">${desc}</p>${actionHtml}</div></div>`;
  }
  window.renderFokusHariIni = function(){
    const listEl = document.getElementById('focus-hari-list');
    const summaryEl = document.getElementById('focus-hari-summary');
    if(!listEl || !summaryEl) return;
    if(!window.currentUser || window.isAdmin) {
      summaryEl.innerHTML = '<strong>Ringkasan prioritas harian.</strong> Mode ini aktif untuk akun pegawai.';
      listEl.innerHTML = '<div class="arika-focus-empty">Fokus Hari Ini tersedia setelah login sebagai pegawai.</div>';
      return;
    }
    const items = [];
    const todayJournals = getMyTodayJournalRows();
    if(todayJournals.length) {
      items.push({tone:'ok', icon:'📝', title:'Jurnal hari ini', desc:`Sudah ada <strong>${todayJournals.length}</strong> jurnal hari ini.`, action:'Lihat riwayat', onClick:"window.nav('rekap')"});
    } else {
      items.push({tone:'urgent', icon:'📝', title:'Jurnal belum diisi', desc:'Belum ada catatan aktivitas hari ini.', action:'Isi jurnal', onClick:"window.nav('jurnal')"});
    }
    const rencana = getFocusRencana();
    const rencanaHariIni = rencana.filter(isRencanaHariIniUntukFokus);
    if(rencanaHariIni.length) {
      items.push({tone:'warn', icon:'✅', title:'Rencana hari ini', desc:`Ada <strong>${rencanaHariIni.length}</strong> rencana hari ini yang perlu dicek.`, action:'Buka rencana', onClick:"window.arikaHomeJump ? window.arikaHomeJump('personal-reminder-card') : window.nav('beranda')"});
    } else if(rencana.length) {
      items.push({tone:'ok', icon:'✅', title:'Rencana aktif', desc:`Ada <strong>${rencana.length}</strong> rencana aktif.`, action:'Lihat', onClick:"window.arikaHomeJump ? window.arikaHomeJump('personal-reminder-card') : window.nav('beranda')"});
    } else {
      items.push({tone:'ok', icon:'✅', title:'Rencana saya', desc:'Belum ada rencana aktif yang perlu ditindaklanjuti.', action:'Tambah', onClick:"window.arikaHomeJump ? window.arikaHomeJump('personal-reminder-card') : window.nav('beranda')"});
    }
    const agenda = getAgendaFocus();
    if(agenda) {
      const when = [fmtDate(agenda.tanggal), agenda.waktuMulai ? agenda.waktuMulai + ' WIT' : ''].filter(Boolean).join(' • ');
      items.push({tone:'warn', icon:'🗓️', title:'Agenda terdekat', desc:`${escapeHTML(agenda.judul || 'Agenda kegiatan')}${when ? '<br><strong>' + escapeHTML(when) + '</strong>' : ''}`, action:'Cek agenda', onClick:"window.arikaHomeJump ? window.arikaHomeJump('home-agenda-section') : window.nav('beranda')"});
    } else {
      items.push({tone:'ok', icon:'🗓️', title:'Agenda', desc:'Belum ada agenda aktif yang perlu disiapkan dalam waktu dekat.'});
    }
    const reminderCount = getReminderCount();
    const surveyPending = hasSurveyPending();
    if(reminderCount > 0 || surveyPending) {
      items.push({tone: surveyPending ? 'warn' : 'ok', icon:'🔔', title:'Reminder aktif', desc: surveyPending ? 'Survei bulan berjalan belum diisi.' : `Ada <strong>${reminderCount}</strong> reminder aktif di Pusat Reminder.`, action: surveyPending ? 'Isi survei' : 'Buka reminder', onClick: surveyPending ? "window.arikaHomeJump ? window.arikaHomeJump('survey-arika-card') : window.nav('beranda')" : "(window.openArikaReminderCenterFromCard || window.openArikaReminderCenter || window.toggleArikaReminderCenter || function(){})()"});
    }
    const urgentCount = items.filter(x => x.tone === 'urgent' || x.tone === 'warn').length;
    if(!urgentCount) {
      summaryEl.innerHTML = '<strong>Hari ini terkendali.</strong> Tidak ada prioritas mendesak yang terdeteksi.';
      listEl.innerHTML = '<div class="arika-focus-empty">Hari ini terkendali. Tidak ada prioritas mendesak yang terdeteksi.</div>';
      return;
    }
    summaryEl.innerHTML = `<strong>${urgentCount} hal perlu dicek hari ini.</strong> Gunakan panel ini sebagai daftar prioritas sebelum mulai bekerja.`;
    listEl.innerHTML = items.filter(x => x.tone === 'urgent' || x.tone === 'warn').slice(0,4).map(itemHtml).join('') || '<div class="arika-focus-empty">Hari ini terkendali. Tidak ada prioritas mendesak yang terdeteksi.</div>';
  };
  function scheduleFocus(){ setTimeout(() => { try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(e) {} }, 80); }
  function wrapRender(name){
    const old = window[name];
    if(typeof old !== 'function' || old.__arikaFocusWrapped) return;
    const wrapped = function(){ const res = old.apply(this, arguments); scheduleFocus(); return res; };
    wrapped.__arikaFocusWrapped = true;
    window[name] = wrapped;
  }
  wrapRender('renderRencanaPribadi');
  wrapRender('renderAgendaSaya');
  wrapRender('renderPengumumanBoard');
  const oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaFocusWrapped){
    const navWrapped = function(id){ const res = oldNav.apply(this, arguments); if(id === 'beranda') scheduleFocus(); return res; };
    navWrapped.__arikaFocusWrapped = true;
    window.nav = navWrapped;
  }
  document.addEventListener('DOMContentLoaded', scheduleFocus);
  window.addEventListener('focus', scheduleFocus);
  setTimeout(scheduleFocus, 1000);
  setInterval(scheduleFocus, 5 * 60 * 1000);
})();


/* v264: Gabungkan Pemberitahuan Penting ke Mode Fokus Hari Ini */
(function(){
  if(window.__ARIKA_V264_FOKUS_ALERT_MERGED__) return;
  window.__ARIKA_V264_FOKUS_ALERT_MERGED__ = true;

  function injectStyle(){
    if(document.getElementById('arika-v264-fokus-alert-style')) return;
    var style = document.createElement('style');
    style.id = 'arika-v264-fokus-alert-style';
    style.textContent = `
      #view-beranda #reminder-banner,
      #view-beranda #journal-gap-alert,
      #view-beranda #jurnal-review-alert{display:none!important;}
      #view-beranda .arika-focus-card.arika-focus-alert-mode{
        border-color:rgba(244,63,94,.22)!important;
        background:linear-gradient(135deg, rgba(255,241,242,.98), rgba(255,255,255,.98) 58%, rgba(255,247,237,.92))!important;
        box-shadow:0 14px 36px rgba(190,18,60,.09)!important;
      }
      #view-beranda .arika-focus-card.arika-focus-alert-mode .home-section-icon{
        background:linear-gradient(135deg,#fff1f2,#ffe4e6)!important;
        border-color:rgba(244,63,94,.18)!important;
      }
      #view-beranda .arika-focus-card.arika-focus-alert-mode .home-section-title{color:#9f1239!important;}
      #view-beranda .arika-focus-card.arika-focus-alert-mode .arika-focus-summary strong{color:#be123c!important;}
      #view-beranda .arika-focus-card.arika-focus-alert-mode .arika-focus-refresh{
        color:#be123c!important;
        border-color:rgba(244,63,94,.18)!important;
        background:rgba(255,255,255,.82)!important;
      }
      #view-beranda .arika-focus-item.is-info{border-color:rgba(14,165,233,.22);background:linear-gradient(135deg,#f0f9ff,#fff);} 
      #view-beranda .arika-focus-item.is-review{border-color:rgba(99,102,241,.22);background:linear-gradient(135deg,#eef2ff,#fff);} 
      #view-beranda .arika-focus-item.is-survey{border-color:rgba(168,85,247,.22);background:linear-gradient(135deg,#faf5ff,#fff);} 
      #view-beranda .arika-focus-note{font-size:9px;font-weight:900;color:#9f1239;text-transform:uppercase;letter-spacing:.08em;}
    `;
    document.head.appendChild(style);
  }
  function safeNormalize(v){
  ...
  function userMatches(row){
    if(!window.currentUser || window.isAdmin || !row) return false;
    try { if(typeof personMatchesRow === 'function') return personMatchesRow(row, window.currentUser); } catch(_) {}
    const userNip = safeNormalize(window.currentUser.nip || '');
    const userName = safeNormalize(window.currentUser.nama || '');
    const rowNip = safeNormalize(row.nip || row.ownerNip || row.NIP || row['NIP Pegawai'] || '');
    const rowName = safeNormalize(row.name || row.nama || row.ownerName || row.pelaksana || row['Nama Pegawai'] || '');
  ...
  function hasSurveyPending(){
    try {
      ...
      if(typeof hasUserFilledSurveyThisMonthForReminder === 'function') return !hasUserFilledSurveyThisMonthForReminder();
      const month = (typeof getCurrentSurveyMonth === 'function') ? getCurrentSurveyMonth() : new Date().toLocaleDateString('en-CA').slice(0,7);
      const nip = safeNormalize(window.currentUser.nip || '');
      const name = safeNormalize(window.currentUser.nama || '');
      return !(window.surveiData || []).some(s => {
        const smonth = String(s.bulan || '').slice(0,7);
        const snip = safeNormalize(s.nip || '');
        const sname = safeNormalize(s.nama || '');
        return smonth === month && ((nip && snip === nip) || (name && sname === name));
      });
    } catch(_) { return false; }
  }
  function getReviewPendingRows(){
    try {
      if(typeof getMyReviewedJurnalRowsForAlert === 'function' && typeof isJurnalReviewNeedsFollowUp === 'function') {
        return getMyReviewedJurnalRowsForAlert().filter(isJurnalReviewNeedsFollowUp);
      }
    } catch(_) {}
    try {
      return (window.arikaData || []).filter(row => {
        if(!userMatches(row)) return false;
        const catatan = String(row.catatanAtasan || row.review || row.catatan || '').trim();
        if(!catatan) return false;
        const statusEval = safeNormalize(row.statusEvaluasiAtasan || row.statusEvaluasi || row.statusFollowUp || '');
        if(statusEval.includes('sudahsesuai') || statusEval.includes('sudahditindaklanjuti') || statusEval.includes('selesai')) return false;
        return true;
      });
    } catch(_) { return []; }
  }
  function getGapInfoSafe(){
    try { return (typeof getJournalGapInfo === 'function') ? getJournalGapInfo() : null; }
    catch(_) { return null; }
  }
  function itemHtml({tone='ok', icon='✅', title='', desc='', action='', onClick='', priority=false}){
    const actionHtml = action ? `<button class="arika-focus-action" type="button" onclick="${onClick}">${action}</button>` : '';
  ...
  function hideOldBanners(){
    ['reminder-banner','journal-gap-alert','jurnal-review-alert'].forEach(id => {
      const el = document.getElementById(id);
      if(el) el.classList.add('hidden');
    });
  }
  const oldTrigger = window.triggerDailyReminder;
  if(typeof oldTrigger === 'function' && !oldTrigger.__arikaV264Merged){
    window.triggerDailyReminder = function(){
      const res = oldTrigger.apply(this, arguments);
      hideOldBanners();
      setTimeout(() => { try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(_) {} }, 80);
      return res;
    };
    window.triggerDailyReminder.__arikaV264Merged = true;
  }

  window.renderFokusHariIni = function(){
    injectStyle();
    hideOldBanners();
    const cardEl = document.getElementById('home-focus-section');
    const listEl = document.getElementById('focus-hari-list');
    const summaryEl = document.getElementById('focus-hari-summary');
    if(!listEl || !summaryEl) return;
    if(!window.currentUser || window.isAdmin) {
      if(cardEl) cardEl.classList.remove('arika-focus-alert-mode');
      summaryEl.innerHTML = '<strong>Ringkasan prioritas harian.</strong> Mode ini aktif untuk akun pegawai.';
      listEl.innerHTML = '<div class="arika-focus-empty">Fokus Hari Ini tersedia setelah login sebagai pegawai.</div>';
      return;
    }
    const items = [];
    const gapInfo = getGapInfoSafe();
    const todayJournals = getMyTodayJournalRows();
    const isTodayJournalMissing = gapInfo ? (gapInfo.todayIsWorkingDay && !gapInfo.hasFilledToday) : !todayJournals.length;
    const isLongGap = gapInfo && gapInfo.todayIsWorkingDay && !gapInfo.hasFilledToday && Number(gapInfo.missedWorkingDays) > 2;
    if(isLongGap) {
      const lastText = gapInfo.lastDate ? fmtDate(gapInfo.lastDate) : 'belum ada riwayat jurnal';
      items.push({tone:'urgent', icon:'🔔', title:'Jurnal terlewat', desc:`Terakhir diisi <strong>${escapeHTML(lastText)}</strong>. Terlewat <strong>${Number(gapInfo.missedWorkingDays)||0}</strong> hari kerja.`, action:'Isi jurnal', onClick:"window.nav('jurnal')"});
    } else if(isTodayJournalMissing) {
      items.push({tone:'urgent', icon:'🔔', title:'Jurnal belum diisi', desc:'Belum ada jurnal untuk hari ini.', action:'Isi jurnal', onClick:"window.nav('jurnal')"});
    } else {
      items.push({tone:'ok', icon:'📝', title:'Jurnal hari ini', desc:`Sudah ada <strong>${todayJournals.length}</strong> jurnal hari ini.`, action:'Lihat riwayat', onClick:"window.nav('rekap')"});
    }

    const reviewRows = getReviewPendingRows();
    if(reviewRows.length) {
      const latest = reviewRows[0] || {};
      const tanggal = latest.date ? fmtDate(latest.date) : (latest.tanggal ? fmtDate(latest.tanggal) : 'jurnal terbaru');
      items.push({tone:'review', icon:'💬', title:'Catatan atasan', desc:`Ada <strong>${reviewRows.length}</strong> catatan atasan. Terbaru <strong>${escapeHTML(tanggal)}</strong>.`, action:'Lihat catatan', onClick:"window.nav('dashboard-pegawai')"});
    }

    const surveyPending = hasSurveyPending();
    if(surveyPending) {
      items.push({tone:'survey', icon:'📝', title:'Survei bulan ini', desc:'Survei bulan ini belum diisi.', action:'Isi survei', onClick:"window.arikaHomeJump ? window.arikaHomeJump('survey-arika-card') : window.nav('beranda')"});
    }
    ...
    if(agenda) {
      const when = [fmtDate(agenda.tanggal), agenda.waktuMulai ? agenda.waktuMulai + ' WIT' : ''].filter(Boolean).join(' • ');
      items.push({tone:'info', icon:'🗓️', title:'Agenda terdekat', desc:`${escapeHTML(agenda.judul || 'Ada agenda')}${when ? '<br><strong>' + escapeHTML(when) + '</strong>' : ''}`, action:'Cek agenda', onClick:"window.arikaHomeJump ? window.arikaHomeJump('home-agenda-section') : window.nav('beranda')"});
    }

    const reminderCount = getReminderCount();
    if(reminderCount > 0) {
      items.push({tone:'warn', icon:'🔔', title:'Reminder aktif', desc:`Ada <strong>${reminderCount}</strong> reminder aktif.`, action:'Buka reminder', onClick:"(window.openArikaReminderCenterFromCard || window.openArikaReminderCenter || window.toggleArikaReminderCenter || function(){})()"});
    }

    const urgentCount = items.filter(x => ['urgent','warn','review','survey'].includes(x.tone)).length;
    if(cardEl) cardEl.classList.toggle('arika-focus-alert-mode', urgentCount > 0);
    if(!urgentCount) {
      summaryEl.innerHTML = '<strong>Hari ini terkendali.</strong> Tidak ada prioritas mendesak yang terdeteksi.';
      listEl.innerHTML = '<div class="arika-focus-empty">Hari ini terkendali. Tidak ada prioritas mendesak yang terdeteksi.</div>';
      return;
    }
    summaryEl.innerHTML = `<strong>${urgentCount} hal perlu dicek hari ini.</strong> Prioritas utama ditampilkan ringkas di bawah.`;
    const visibleItems = items.filter(x => ['urgent','review','survey','warn'].includes(x.tone)).slice(0,6);
    const priorityIndex = visibleItems.findIndex(x => ['urgent','review','survey','warn'].includes(x.tone));
    listEl.innerHTML = visibleItems.map((item, idx) => itemHtml(Object.assign({}, item, { priority: idx === priorityIndex && priorityIndex > -1 }))).join('') || '<div class="arika-focus-empty">Hari ini terkendali. Tidak ada prioritas mendesak yang terdeteksi.</div>';
  };

  function scheduleFocus(){ setTimeout(() => { try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(e) {} }, 80); }
  ['renderRencanaPribadi','renderAgendaSaya','renderPengumumanBoard','renderJurnalReviewAlert','updateSurveyHomeCard','renderSurveyHomeCard'].forEach(function(name){
    const old = window[name];
    if(typeof old === 'function' && !old.__arikaV264FocusWrapped){
      const wrapped = function(){ const res = old.apply(this, arguments); scheduleFocus(); return res; };
      wrapped.__arikaV264FocusWrapped = true;
      window[name] = wrapped;
    }
  });
  document.addEventListener('DOMContentLoaded', function(){ injectStyle(); hideOldBanners(); scheduleFocus(); });
  window.addEventListener('focus', scheduleFocus);
  window.addEventListener('storage', scheduleFocus);
  setTimeout(scheduleFocus, 300);
  setInterval(scheduleFocus, 2 * 60 * 1000);
})();


// v265: Bersihkan duplikasi tombol di Beranda - Pusat Reminder sudah ada di Fokus Hari Ini/FAB,
// dan tombol navigasi cepat di sapaan disembunyikan agar Beranda tidak penuh.
(function(){
  if (window.__ARIKA_V265_HOME_COMPACT__) return;
  window.__ARIKA_V265_HOME_COMPACT__ = true;
  function apply(){
    try {
      document.querySelectorAll('#view-beranda .reminder-sound-card').forEach(function(card){
        var txt = (card.textContent || '').toLowerCase();
        if (txt.indexOf('pusat reminder') >= 0) card.remove();
      });
      document.querySelectorAll('#view-beranda .arika-home-jump-actions').forEach(function(el){ el.remove(); });
      var grid = document.querySelector('#view-beranda #reminder-sound-panel .reminder-sound-grid');
      if (grid) grid.style.gridTemplateColumns = '1fr';
      var hero = document.querySelector('#view-beranda .arika-home-hero-line');
      if (hero) hero.style.justifyContent = 'flex-start';
      var left = document.querySelector('#view-beranda .arika-home-hero-line .arika-home-line-left');
      if (left) { left.style.width = '100%'; left.style.flex = '1 1 100%'; }
    } catch(e) {}
  }
  ...
  document.addEventListener('click', function(ev){
    if (ev.target && (ev.target.closest('.nav-btn') || ev.target.closest('.mobile-nav-select'))) setTimeout(apply, 80);
  }, true);
})();



// v273: Menu khusus WFH/WFA - semua pegawai bisa melihat rencana dan realisasi harian.
(function(){
  if(window.__ARIKA_V273_WFH_WFA_MENU__) return;
  window.__ARIKA_V273_WFH_WFA_MENU__ = true;

  function esc(v){
    try { return typeof escapeHTML === 'function' ? escapeHTML(v) : String(v ?? '').replace(/[&<>"]/g, function(c){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]; }); }
    catch(_) { return String(v ?? ''); }
  }
  function norm(v){
    try { return typeof normalize === 'function' ? normalize(v) : String(v || '').toLowerCase().replace(/[^a-z0-9]+/g,''); }
    catch(_) { return String(v || '').toLowerCase().replace(/[^a-z0-9]+/g,''); }
  }
  function textNorm(v){ return String(v || '').toLowerCase().replace(/[\s_\-]+/g,' ').trim(); }
  function dateKey(v){
    try { return typeof normalizeDateKeyFromSheet === 'function' ? normalizeDateKeyFromSheet(v) : String(v || '').slice(0,10); }
    catch(_) { return String(v || '').slice(0,10); }
  }
  ...
  function lineText(v){
    const raw = String(v || '').trim();
    if(!raw) return '-';
    try { return typeof linkifyText === 'function' ? linkifyText(raw) : esc(raw); }
    catch(_) { return esc(raw); }
  }
  function isWfhText(parts){
    const t = textNorm(Array.isArray(parts) ? parts.filter(Boolean).join(' ') : parts);
    if(!t) return false;
    return /(\bwfh\b|\bwfa\b|work\s*from\s*home|work\s*from\s*anywhere|work\s*form\s*home|work\s*form\s*anywhere|kerja\s*dari\s*rumah|bekerja\s*dari\s*rumah|kerja\s*dari\s*mana\s*saja|bekerja\s*dari\s*mana\s*saja|\bkdr\b)/i.test(t);
  }
  function personNameFromPlan(r){ return String(r.ownerName || r.nama || r.name || r.pelaksana || r.pegawai || 'Pegawai').trim(); }
  function personNipFromPlan(r){ return String(r.ownerNip || r.nip || r.NIP || r.nipPegawai || '').trim(); }
  function personNameFromJournal(r){ return String(r.name || r.nama || r.pelaksana || r.pegawai || r.ownerName || 'Pegawai').trim(); }
  function personNipFromJournal(r){ return String(r.nip || r.NIP || r.ownerNip || r.nipPegawai || '').trim(); }
  function personKey(name,nip){ return nip ? ('nip:' + norm(nip)) : ('nama:' + norm(name)); }
  function getPlanDate(r){ return dateKey(r.tanggal || r.date || r.tgl || r.targetDate || r.createdAt || ''); }
  function getJournalDate(r){ return dateKey(r.date || r.tanggal || r.tgl || r.createdAt || ''); }
  function planText(r){ return [r.judul, r.catatan, r.periode, r.status, r.rawText].filter(Boolean).join(' '); }
  function journalText(r){ return [r.cat, r.kategori, r.desc, r.kegiatan, r.uraian, r.status, r.linkDataDukung].filter(Boolean).join(' '); }
  function getRencanaSource(){
    try { return typeof getStableRencanaData === 'function' ? getStableRencanaData() : (window.rencanaData || []); }
    catch(_) { return window.rencanaData || []; }
  }
  function collectWfhWfaRows(targetDate){
    const date = dateKey(targetDate) || todayKey();
    const groups = new Map();
    const ensure = (name, nip) => {
      const safeName = String(name || 'Pegawai').trim() || 'Pegawai';
      const safeNip = String(nip || '').trim();
      const key = personKey(safeName, safeNip);
      if(!groups.has(key)) groups.set(key, { name:safeName, nip:safeNip, plans:[], journals:[] });
      return groups.get(key);
    };

    (getRencanaSource() || []).forEach(function(r){
      if(!r) return;
      if(getPlanDate(r) !== date) return;
      if(!isWfhText(planText(r))) return;
      const name = personNameFromPlan(r), nip = personNipFromPlan(r);
      ensure(name, nip).plans.push(r);
    });
    (Array.isArray(window.arikaData) ? window.arikaData : []).forEach(function(r){
      if(!r) return;
      if(getJournalDate(r) !== date) return;
      if(!isWfhText(journalText(r))) return;
      const name = personNameFromJournal(r), nip = personNipFromJournal(r);
      ensure(name, nip).journals.push(r);
    });
    return Array.from(groups.values()).sort(function(a,b){ return String(a.name).localeCompare(String(b.name)); });
  }
  function setText(id, value){ const el = document.getElementById(id); if(el) el.textContent = String(value); }
  function statusInfo(group){
    if(group.plans.length && group.journals.length) return { cls:'wfh-status-done', text:'✅ Rencana & Realisasi' };
    if(group.plans.length && !group.journals.length) return { cls:'wfh-status-plan', text:'⏳ Belum Realisasi' };
    return { cls:'wfh-status-realisasi', text:'📝 Realisasi Tanpa Rencana' };
  }
  function renderPlanRows(rows){
    if(!rows.length) return '<p class="wfh-mini-content text-slate-400">Belum ada rencana WFH/WFA pada tanggal ini.</p>';
    return rows.map(function(r){
      const jam = r.jamReminder ? ` <span class="text-slate-400">• ${esc(r.jamReminder)}</span>` : '';
      const status = r.status ? ` <span class="text-slate-400">• ${esc(r.status)}</span>` : '';
      return `<div class="mb-2 last:mb-0"><div class="font-black text-slate-800">${esc(r.judul || 'Rencana kegiatan')}${jam}${status}</div>${r.catatan ? `<div class="mt-1">${lineText(r.catatan)}</div>` : ''}</div>`;
    }).join('');
  }
  function renderJournalRows(rows){
    if(!rows.length) return '<p class="wfh-mini-content text-slate-400">Belum ada realisasi jurnal WFH/WFA pada tanggal ini.</p>';
    return rows.map(function(r){
      const title = [r.cat || r.kategori || 'Jurnal', r.status || ''].filter(Boolean).join(' • ');
      const link = r.linkDataDukung ? `<div class="mt-1 text-cyan-700">Data dukung: ${lineText(r.linkDataDukung)}</div>` : '';
      return `<div class="mb-2 last:mb-0"><div class="font-black text-slate-800">${esc(title)}</div><div class="mt-1">${lineText(r.desc || r.kegiatan || r.uraian || '-')}</div>${link}</div>`;
    }).join('');
  }

  window.renderWfhWfaDay = function(){
    const listEl = document.getElementById('wfh-list');
    const dateEl = document.getElementById('wfh-date');
    if(!listEl) return;
    if(dateEl && !dateEl.value) dateEl.value = todayKey();
    const targetDate = dateKey(dateEl && dateEl.value ? dateEl.value : todayKey()) || todayKey();
    if(dateEl && dateEl.value !== targetDate) dateEl.value = targetDate;

    const groups = collectWfhWfaRows(targetDate);
    const planCount = groups.reduce((n,g) => n + g.plans.length, 0);
    const journalCount = groups.reduce((n,g) => n + g.journals.length, 0);
    const waiting = groups.filter(g => g.plans.length && !g.journals.length).length;
    setText('wfh-kpi-pegawai', groups.length);
    setText('wfh-kpi-rencana', planCount);
    setText('wfh-kpi-realisasi', journalCount);
    setText('wfh-kpi-belum', waiting);
    setText('wfh-period-label', 'Tanggal dipilih: ' + prettyDate(targetDate));

    if(!groups.length){
      listEl.innerHTML = `
        <div class="p-6 rounded-2xl bg-slate-50 border border-slate-100 text-center">
          <div class="text-3xl mb-2">🏡</div>
          <p class="text-xs font-black text-slate-500 uppercase tracking-widest">Belum ada data WFH/WFA</p>
          <p class="text-xs text-slate-400 mt-2 font-semibold leading-relaxed">Pastikan rencana atau jurnal pegawai memuat kata WFH, WFA, Work From Home, atau Work From Anywhere agar otomatis muncul di daftar semua pegawai.</p>
        </div>`;
      return;
    }
    listEl.innerHTML = '<div class="wfh-all-note">Menampilkan seluruh pegawai yang memiliki rencana atau realisasi WFH/WFA pada tanggal dipilih.</div>' + groups.map(function(g){
      const st = statusInfo(g);
      return `
        <article class="wfh-entry-card">
          <div class="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-3">
            <div class="min-w-0">
              <h3 class="text-sm md:text-base font-black text-slate-900 uppercase tracking-wide leading-snug">${esc(g.name)}</h3>
              ${((window.isAdmin === true) || String(window.userRole || '').toLowerCase().includes('admin')) ? `<p class="text-[10px] font-bold text-slate-400 mt-1">${g.nip ? 'NIP: ' + esc(g.nip) : 'NIP belum tersedia'}</p>` : ''}
            </div>
            <span class="wfh-status-badge ${st.cls}">${st.text}</span>
          </div>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div class="wfh-mini-box">
              <div class="wfh-mini-title">📌 Perencanaan</div>
              <div class="wfh-mini-content">${renderPlanRows(g.plans)}</div>
            </div>
            <div class="wfh-mini-box">
              <div class="wfh-mini-title">✅ Realisasi Jurnal</div>
              <div class="wfh-mini-content">${renderJournalRows(g.journals)}</div>
            </div>
          </div>
        </article>`;
    }).join('');
  ...
  window.setWfhDateToday = function(){
    const el = document.getElementById('wfh-date');
    if(el) el.value = todayKey();
    window.renderWfhWfaDay();
  };

  window.refreshWfhWfaData = async function(){
    try {
      const btnText = document.getElementById('wfh-list');
      if(btnText && !btnText.children.length) btnText.innerHTML = '<div class="p-5 rounded-2xl bg-slate-50 text-center text-slate-400 text-[10px] uppercase tracking-widest font-black">Menyinkronkan data...</div>';
      if(typeof window.fetchCloudData === 'function') await window.fetchCloudData({ force:true, full: !(window.matchMedia && window.matchMedia('(max-width: 720px)').matches) });
    } catch(e) { console.warn('Refresh WFH/WFA gagal:', e); }
    window.renderWfhWfaDay();
  };

  window.prefillWfhJournal = function(){
    try {
      const dateEl = document.getElementById('wfh-date');
      const targetDate = dateKey(dateEl && dateEl.value ? dateEl.value : todayKey()) || todayKey();
      window.nav('jurnal');
      setTimeout(function(){
        ...
        if(d) d.value = targetDate;
        if(window.setJurnalCategory) window.setJurnalCategory('Administrasi');
        if(desc && !desc.value) desc.value = 'WFH/WFA - Realisasi kegiatan: ';
        try { desc && desc.focus(); } catch(_) {}
      }, 120);
    } catch(e) { window.nav('jurnal'); }
  };

  const oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV273Wfh){
    const wrapped = function(id){
      const res = oldNav.apply(this, arguments);
      if(id === 'wfh') setTimeout(function(){ if(window.renderWfhWfaDay) window.renderWfhWfaDay(); if(!(window.matchMedia && window.matchMedia('(max-width: 720px)').matches) && window.refreshWfhWfaData) window.refreshWfhWfaData(); }, 80);
      return res;
    };
    wrapped.__arikaV273Wfh = true;
    window.nav = wrapped;
  }
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(function(){ if(document.getElementById('view-wfh') && !document.getElementById('view-wfh').classList.contains('hidden')) window.renderWfhWfaDay(); }, 200); });
  window.addEventListener('focus', function(){ if(document.getElementById('view-wfh') && !document.getElementById('view-wfh').classList.contains('hidden')) window.renderWfhWfaDay(); });
})();

// v275: Sinkron label WFH/WFA tanpa mengubah alur inti ARIKA.
// Data tetap masuk ke Jurnal Harian dan Rencana Saya umum, hanya diberi label teks agar bisa difilter/ditandai.
(function(){
  if(window.__ARIKA_V275_WFH_WFA_LABEL_SYNC__) return;
  window.__ARIKA_V275_WFH_WFA_LABEL_SYNC__ = true;

  var LABEL = '[WFH/WFA]';
  var SESSION_JURNAL = 'arika_wfh_wfa_journal_mode';
  var SESSION_RENCANA = 'arika_wfh_wfa_rencana_mode';

  function norm(v){ return String(v || '').toLowerCase().replace(/\s+/g,' ').trim(); }
  function hasWfhLabel(text){
    var t = norm(text);
    return /(^|\W)(wfh|wfa)(\W|$)|work\s*from\s*home|work\s*from\s*anywhere|work\s*form\s*home|work\s*form\s*anywhere|kerja\s*dari\s*rumah|bekerja\s*dari\s*rumah|kerja\s*dari\s*mana\s*saja|bekerja\s*dari\s*mana\s*saja|\[wfh\/wfa\]/i.test(t);
  }
  ...
  function ensurePrefix(value){
    var raw = String(value || '').trim();
    if(!raw) return LABEL + ' Realisasi kegiatan: ';
    if(hasWfhLabel(raw)) return raw;
    return LABEL + ' ' + raw;
  }
  function todayKey(){
    try { return typeof getTodayKey === 'function' ? getTodayKey() : new Date().toLocaleDateString('en-CA'); }
  ...
  function injectStyle(){
    if(document.getElementById('arika-v275-wfh-label-style')) return;
    var style = document.createElement('style');
    style.id = 'arika-v275-wfh-label-style';
    style.textContent = `
      .arika-archive-label.wfh-wfa,
      .arika-wfh-wfa-badge{
        display:inline-flex;align-items:center;gap:.25rem;
        border-radius:999px;padding:.22rem .48rem;
        background:#ecfeff;color:#0e7490;border:1px solid #a5f3fc;
        font-size:8px;font-weight:950;text-transform:uppercase;letter-spacing:.09em;
      }
      .arika-wfh-mode-card{
        border:1px solid rgba(14,165,233,.18);background:linear-gradient(135deg,#ecfeff,#ffffff);
        color:#0f766e;border-radius:1.1rem;padding:.75rem .9rem;margin-bottom:.75rem;
        font-size:10px;font-weight:900;line-height:1.5;
      }
      .arika-wfh-mode-card b{color:#0e7490;}
    `;
    document.head.appendChild(style);
  }

  function showJurnalModeNotice(){
    var form = document.getElementById('form-arika');
    if(!form) return;
    var old = document.getElementById('arika-wfh-jurnal-mode-card');
    if(old) old.remove();
    if(!safeSessionGet(SESSION_JURNAL)) return;
    var card = document.createElement('div');
    card.id = 'arika-wfh-jurnal-mode-card';
    card.className = 'arika-wfh-mode-card';
    card.innerHTML = '🏡 <b>Mode Jurnal WFH/WFA aktif.</b><br>Jurnal yang disimpan tetap masuk ke <b>Riwayat Jurnal umum</b>, sekaligus otomatis terbaca pada menu <b>WFH/WFA</b>.';
    form.parentNode.insertBefore(card, form);
  }

  function applyWfhJournalPrefill(targetDate){
    safeSessionSet(SESSION_JURNAL, '1');
    setTimeout(function(){
      var d = document.getElementById('in-date');
      var desc = document.getElementById('in-desc');
      if(d && targetDate) d.value = targetDate;
      try { if(window.setJurnalCategory) window.setJurnalCategory('Administrasi'); } catch(_) {}
      if(desc && !hasWfhLabel(desc.value)) {
        desc.value = desc.value && desc.value.trim() ? ensurePrefix(desc.value) : LABEL + ' Realisasi kegiatan: ';
      }
      showJurnalModeNotice();
      try { desc && desc.focus(); } catch(_) {}
    }, 150);
  }

  var oldPrefill = window.prefillWfhJournal;
  if(typeof oldPrefill === 'function' && !oldPrefill.__arikaV275LabelSync){
    window.prefillWfhJournal = function(){
      var targetDate = todayKey();
      try {
        var dateEl = document.getElementById('wfh-date');
        targetDate = dateKey(dateEl && dateEl.value ? dateEl.value : todayKey()) || todayKey();
      } catch(_) {}
      safeSessionSet(SESSION_JURNAL, '1');
      var res = oldPrefill.apply(this, arguments);
      applyWfhJournalPrefill(targetDate);
      return res;
    };
    window.prefillWfhJournal.__arikaV275LabelSync = true;
  }

  window.openWfhRencana = function(){
    var targetDate = todayKey();
    try {
      var dateEl = document.getElementById('wfh-date');
      targetDate = dateKey(dateEl && dateEl.value ? dateEl.value : todayKey()) || todayKey();
    } catch(_) {}
    safeSessionSet(SESSION_RENCANA, '1');
    if(typeof window.nav === 'function') window.nav('beranda');
    setTimeout(function(){
      try { if(typeof window.arikaHomeJump === 'function') window.arikaHomeJump('personal-reminder-card'); }
      catch(_) {}
      try { if(typeof window.toggleRencanaForm === 'function') window.toggleRencanaForm(true); }
      catch(_) {}
      var tgl = document.getElementById('rencana-tanggal');
      var judul = document.getElementById('rencana-judul');
      var catatan = document.getElementById('rencana-catatan');
      if(tgl && targetDate) tgl.value = targetDate;
      if(judul && !judul.value) {
        judul.value = LABEL + ' ';
        try { judul.focus(); judul.setSelectionRange(judul.value.length, judul.value.length); } catch(_) {}
      }
      if(catatan && !catatan.placeholder.includes('WFH/WFA')) catatan.placeholder = 'Catatan rencana WFH/WFA bila perlu...';
    }, 180);
  };

  // Pengaman sebelum submit: jika form dibuka dari menu WFH/WFA, label otomatis ditempel.
  document.addEventListener('submit', function(ev){
    var form = ev.target;
    if(!form || !form.id) return;
    if(form.id === 'form-arika' && safeSessionGet(SESSION_JURNAL)){
      var desc = document.getElementById('in-desc');
      if(desc) desc.value = ensurePrefix(desc.value);
      setTimeout(function(){ safeSessionRemove(SESSION_JURNAL); showJurnalModeNotice(); }, 800);
    }
    if(form.id === 'form-rencana-pribadi' && safeSessionGet(SESSION_RENCANA)){
      var judul = document.getElementById('rencana-judul');
      var catatan = document.getElementById('rencana-catatan');
      var combined = String((judul && judul.value) || '') + ' ' + String((catatan && catatan.value) || '');
      if(!hasWfhLabel(combined)){
        if(judul) judul.value = ensurePrefix(judul.value).replace('Realisasi kegiatan: ', '');
      }
      setTimeout(function(){ safeSessionRemove(SESSION_RENCANA); }, 800);
    }
  }, true);

  function addBadgeToHistoryRows(){
    injectStyle();
    var rows = document.querySelectorAll('#tbl-body tr.arika-archive-row');
    rows.forEach(function(row){
      if(row.dataset.wfhBadgeChecked === 'true') return;
      row.dataset.wfhBadgeChecked = 'true';
      var desc = row.querySelector('.arika-history-desc-full');
      if(!desc || !hasWfhLabel(desc.textContent || '')) return;
      var labelBox = row.querySelector('td:nth-child(4) > div') || row.querySelector('td:nth-child(4)');
      if(labelBox && !labelBox.querySelector('.arika-archive-label.wfh-wfa')){
        labelBox.insertAdjacentHTML('beforeend', '<span class="arika-archive-label wfh-wfa">🏡 WFH/WFA</span>');
      }
    });

    document.querySelectorAll('#admin-history-body tr').forEach(function(row){
      if(row.dataset.wfhBadgeChecked === 'true') return;
      row.dataset.wfhBadgeChecked = 'true';
      var desc = row.querySelector('.arika-admin-history-desc-full');
      if(!desc || !hasWfhLabel(desc.textContent || '')) return;
      var statusBox = row.querySelector('td:last-child .flex');
      if(statusBox && !statusBox.querySelector('.arika-wfh-wfa-badge')){
        statusBox.insertAdjacentHTML('afterbegin', '<span class="arika-wfh-wfa-badge">🏡 WFH/WFA</span>');
      }
    });
  }

  function scheduleBadges(){
    clearTimeout(window.__arikaV275WfhBadgeTimer);
    window.__arikaV275WfhBadgeTimer = setTimeout(addBadgeToHistoryRows, 80);
  }

  var oldRunFilter = window.runFilter;
  if(typeof oldRunFilter === 'function' && !oldRunFilter.__arikaV275WfhBadges){
    window.runFilter = function(){
      var res = oldRunFilter.apply(this, arguments);
      scheduleBadges();
      return res;
    };
    window.runFilter.__arikaV275WfhBadges = true;
  }
  var oldRenderAdmin = window.renderAdminAnalytics;
  if(typeof oldRenderAdmin === 'function' && !oldRenderAdmin.__arikaV275WfhBadges){
    window.renderAdminAnalytics = function(){
      var res = oldRenderAdmin.apply(this, arguments);
      scheduleBadges();
      return res;
    };
    window.renderAdminAnalytics.__arikaV275WfhBadges = true;
  }

  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV275WfhLabelSync){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      if(id === 'jurnal') setTimeout(showJurnalModeNotice, 120);
      if(id === 'rekap' || id === 'admin') scheduleBadges();
      if(id === 'wfh') setTimeout(function(){
        var btn = Array.from(document.querySelectorAll('#view-wfh button')).find(function(b){ return /buka rencana/i.test(b.textContent || ''); });
        if(btn) btn.onclick = function(){ window.openWfhRencana(); };
      }, 120);
      return res;
    };
    window.nav.__arikaV275WfhLabelSync = true;
  }

  document.addEventListener('DOMContentLoaded', function(){ injectStyle(); showJurnalModeNotice(); scheduleBadges(); });
  setTimeout(function(){ injectStyle(); showJurnalModeNotice(); scheduleBadges(); }, 600);
})();


/* =========================================================
   v290 - Fix Dashboard Saya filter + Logout smooth + WFH/WFA long-list comfort
   Basis: v289 upload, tetapi patch bermasalah v285-v289 untuk filter dihapus.
   Perubahan dibatasi pada UI filter Dashboard Saya, logout, dan kenyamanan daftar WFH/WFA.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V290_DASHBOARD_FILTER_WFH_SCALING__) return;
  window.__ARIKA_V290_DASHBOARD_FILTER_WFH_SCALING__ = true;

  function byId(id){ return document.getElementById(id); }
  function pad(n){ return String(n).padStart(2,'0'); }
  function nowKey(){ var d = new Date(); return d.getFullYear() + '-' + pad(d.getMonth()+1); }
  function keyToDisplay(key){ var m = String(key || '').match(/^(\d{4})-(\d{2})$/); return m ? (m[2] + '/' + m[1]) : ''; }
  function displayToKey(v){
    var raw = String(v || '').trim();
    if(!raw) return '';
    var m = raw.match(/^(\d{4})[-\/](\d{1,2})$/); if(m) return m[1] + '-' + pad(m[2]);
    m = raw.match(/^(\d{1,2})[-\/](\d{4})$/); if(m) return m[2] + '-' + pad(m[1]);
    var digits = raw.replace(/\D/g,'');
    m = digits.match(/^(\d{2})(\d{4})$/); if(m) return m[2] + '-' + pad(m[1]);
    m = digits.match(/^(\d{4})(\d{2})$/); if(m) return m[1] + '-' + pad(m[2]);
    return '';
  }
  function addMonth(key, delta){
    var m = String(key || nowKey()).match(/^(\d{4})-(\d{2})$/);
    var d = m ? new Date(Number(m[1]), Number(m[2])-1 + delta, 1) : new Date();
    return d.getFullYear() + '-' + pad(d.getMonth()+1);
  }
  function activeDashKey(){
    var el = byId('pegawai-dashboard-month');
    if(!el) return nowKey();
    if(el.dataset && el.dataset.periodMode === 'all') return '';
    return displayToKey(el.value || el.dataset.arikaIsoValue || '') || nowKey();
  }
  function rerenderDashboard(){
    try { if(typeof window.ARIKA_updatePegawaiDashboardFilterState === 'function') window.ARIKA_updatePegawaiDashboardFilterState(); } catch(_) {}
    setTimeout(function(){
      try { if(typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({ fromV290Filter:true }); } catch(e){ console.warn('ARIKA v290 dashboard render:', e); }
      try { if(typeof window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__ === 'function') window.__ARIKA_FLUSH_DASHBOARD_CHARTS_V248__(); } catch(_) {}
      setTimeout(installDashboardFilter, 80);
    }, 30);
  }
  function setDashMonth(key){
    var el = byId('pegawai-dashboard-month');
    if(el){
      el.dataset.periodMode = 'month';
      el.dataset.arikaIsoValue = key;
  ...
  window.shiftPegawaiDashboardMonth = function(delta){
    var base = activeDashKey() || nowKey();
    setDashMonth(addMonth(base, Number(delta)||0));
  };
  window.setPegawaiDashboardMonthToCurrent = function(){ setDashMonth(nowKey()); };
  window.clearPegawaiDashboardMonth = function(){ setDashAll(); };

  function injectStyle(){
    if(byId('arika-v290-style')) return;
    var st = document.createElement('style');
    st.id = 'arika-v290-style';
    st.textContent = `
      #view-dashboard-pegawai .arika-dashboard-filter-v290{
        width:100%!important;max-width:500px!important;margin-left:auto!important;
        padding:.72rem!important;border-radius:1.2rem!important;border:1px solid rgba(255,255,255,.38)!important;
        background:rgba(255,255,255,.94)!important;box-shadow:0 12px 30px rgba(15,23,42,.13)!important;backdrop-filter:blur(10px)!important;
      }
      #view-dashboard-pegawai .arika-dashboard-filter-v290-title{
        display:flex!important;align-items:center!important;justify-content:space-between!important;gap:.5rem!important;
        margin:0 0 .42rem 0!important;color:#64748b!important;font-size:9px!important;font-weight:950!important;letter-spacing:.14em!important;text-transform:uppercase!important;
      }
      #view-dashboard-pegawai .arika-dashboard-filter-v290-current{color:#0f766e!important;letter-spacing:.06em!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290-row{display:grid!important;grid-template-columns:34px minmax(104px,1fr) 34px!important;gap:.42rem!important;align-items:center!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290-actions{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:.42rem!important;margin-top:.42rem!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 #pegawai-dashboard-month{
        position:static!important;display:block!important;opacity:1!important;pointer-events:auto!important;overflow:visible!important;
        width:100%!important;height:34px!important;min-height:34px!important;margin:0!important;padding:.45rem .55rem!important;
        border:1px solid #e2e8f0!important;border-radius:.82rem!important;background:#fff!important;color:#0f172a!important;
        text-align:center!important;font-size:11px!important;font-weight:950!important;box-shadow:none!important;outline:none!important;
      }
      #view-dashboard-pegawai .arika-dashboard-filter-v290 #pegawai-dashboard-month:focus{border-color:#10b981!important;box-shadow:0 0 0 3px rgba(16,185,129,.14)!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .rekap-filter-chip{
        min-height:34px!important;height:34px!important;padding:.42rem .62rem!important;border-radius:.82rem!important;border:1px solid #e2e8f0!important;background:#f8fafc!important;color:#475569!important;
        font-size:8px!important;font-weight:950!important;letter-spacing:.06em!important;text-transform:uppercase!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:.25rem!important;white-space:nowrap!important;box-shadow:none!important;transform:none!important;
      }
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .rekap-filter-chip:hover{background:#ecfdf5!important;color:#047857!important;border-color:#99f6e4!important;transform:translateY(-1px)!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .rekap-filter-chip.active,
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .rekap-filter-chip.is-active,
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .rekap-filter-chip[aria-pressed="true"]{background:linear-gradient(135deg,#059669,#0891b2)!important;color:#fff!important;border-color:transparent!important;box-shadow:0 8px 18px rgba(8,145,178,.16)!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-arrow{font-size:14px!important;padding:0!important;letter-spacing:0!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-sync{background:#eff6ff!important;color:#1d4ed8!important;border-color:#bfdbfe!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-sync:hover{background:#dbeafe!important;color:#1e40af!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-monev{background:#ecfdf5!important;color:#047857!important;border-color:#a7f3d0!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-monev:hover{background:#d1fae5!important;color:#065f46!important;}
      #view-dashboard-pegawai .arika-dashboard-filter-old-hidden{display:none!important;}

      #view-wfh .wfh-long-toolbar{display:flex!important;flex-wrap:wrap!important;gap:.55rem!important;align-items:center!important;justify-content:space-between!important;margin:.75rem 0!important;padding:.75rem!important;border-radius:1rem!important;border:1px solid rgba(226,232,240,.9)!important;background:#f8fafc!important;}
      #view-wfh .wfh-long-toolbar input,#view-wfh .wfh-long-toolbar select{height:34px!important;border:1px solid #e2e8f0!important;border-radius:.82rem!important;background:#fff!important;color:#334155!important;font-size:11px!important;font-weight:800!important;padding:.45rem .7rem!important;outline:none!important;}
      #view-wfh .wfh-long-toolbar input{min-width:220px!important;flex:1 1 240px!important;}
      #view-wfh .wfh-long-toolbar select{flex:0 0 auto!important;}
      #view-wfh #wfh-list{max-height:620px!important;overflow:auto!important;padding-right:.2rem!important;scrollbar-width:thin;}
      #view-wfh #wfh-list.wfh-compact-list .wfh-entry-card{padding:.72rem!important;border-radius:1rem!important;box-shadow:0 8px 18px rgba(15,23,42,.045)!important;}
      #view-wfh #wfh-list.wfh-compact-list .wfh-entry-card h3{font-size:.8rem!important;letter-spacing:.04em!important;}
      #view-wfh #wfh-list.wfh-compact-list .wfh-mini-box{padding:.62rem!important;border-radius:.82rem!important;}
      #view-wfh #wfh-list.wfh-compact-list .wfh-mini-content{max-height:78px!important;overflow:auto!important;font-size:.66rem!important;line-height:1.45!important;}
      #view-wfh .wfh-entry-card.is-filter-hidden{display:none!important;}
      @media(max-width:720px){
        #view-dashboard-pegawai .arika-dashboard-filter-v290{max-width:100%!important;margin-left:0!important;}
        #view-dashboard-pegawai .arika-dashboard-filter-v290-actions{grid-template-columns:repeat(2,minmax(0,1fr))!important;}
        #view-dashboard-pegawai .arika-dashboard-filter-v290 .arika-filter-monev{grid-column:span 2!important;}
        #view-wfh #wfh-list{max-height:520px!important;}
      }
    `;
    document.head.appendChild(st);
  }
  function makeBtn(id, text, cls, handler, title){
    Array.prototype.slice.call(document.querySelectorAll('#'+id)).forEach(function(old){ try { old.remove(); } catch(_){} });
    var b = document.createElement('button');
    b.id = id; b.type = 'button'; b.className = 'rekap-filter-chip ' + (cls||''); b.textContent = text;
    if(title) b.title = title;
    b.addEventListener('click', function(ev){ ev.preventDefault(); ev.stopPropagation(); handler(); }, true);
    return b;
  }
  function findDashboardTarget(){
    var view = byId('view-dashboard-pegawai');
    if(!view) return null;
    return view.querySelector('.relative.flex.flex-col.lg\\:flex-row') || view.querySelector('.relative.flex') || view.querySelector('.bg-gradient-to-br .relative') || view.firstElementChild;
  }
  function ensureDashboardMonth(){
    var el = byId('pegawai-dashboard-month');
    if(el) return el;
    el = document.createElement('input');
    el.id = 'pegawai-dashboard-month'; el.type = 'text'; el.inputMode = 'numeric'; el.placeholder = 'mm/yyyy'; el.autocomplete = 'off';
    return el;
  }
  function hideOldDashboardControls(panel){
    var view = byId('view-dashboard-pegawai'); if(!view) return;
    Array.prototype.slice.call(view.querySelectorAll('.arika-dashboard-filter-compact,.arika-dashboard-filter-final,.arika-dashboard-filter-panel,.arika-dashboard-filter-lembur-style,#arika-dashboard-filter-compact-panel,#arika-dashboard-filter-final-panel')).forEach(function(el){
      if(el === panel || (panel && panel.contains(el)) || (el && el.contains(panel))) return;
      try { el.remove(); } catch(_) { try { el.classList.add('arika-dashboard-filter-old-hidden'); } catch(__){} }
    });
    Array.prototype.slice.call(view.querySelectorAll('.arika-dashboard-period-btn,.arika-dashboard-chip-filter')).forEach(function(el){
      if(panel && panel.contains(el)) return;
      try { el.remove(); } catch(_) { try { el.classList.add('arika-dashboard-filter-old-hidden'); } catch(__){} }
    });
  }
  function updateDashboardState(){
    var key = activeDashKey();
    var label = byId('pegawai-dashboard-filter-v290-label');
    if(label) label.textContent = key ? keyToDisplay(key) : 'Semua';
    var current = byId('pegawai-dashboard-month-current');
    var all = byId('pegawai-dashboard-month-all');
    var now = nowKey();
    if(current){ current.classList.toggle('active', key === now); current.classList.toggle('is-active', key === now); current.setAttribute('aria-pressed', String(key === now)); }
    if(all){ all.classList.toggle('active', !key); all.classList.toggle('is-active', !key); all.setAttribute('aria-pressed', String(!key)); }
  }
  function installDashboardFilter(){
    injectStyle();
    var view = byId('view-dashboard-pegawai'); if(!view) return;
    var target = findDashboardTarget(); if(!target) return;
    var month = ensureDashboardMonth();
    var panel = byId('arika-dashboard-filter-v290');
    if(!panel){ panel = document.createElement('div'); panel.id = 'arika-dashboard-filter-v290'; panel.className = 'arika-dashboard-filter-v290'; target.appendChild(panel); }
    hideOldDashboardControls(panel);
    panel.innerHTML = '<div class="arika-dashboard-filter-v290-title"><span>📅 Filter Bulan Dashboard Saya</span><span class="arika-dashboard-filter-v290-current" id="pegawai-dashboard-filter-v290-label"></span></div>';
    var row = document.createElement('div'); row.className = 'arika-dashboard-filter-v290-row'; panel.appendChild(row);
    row.appendChild(makeBtn('pegawai-dashboard-month-prev','‹','arika-filter-arrow', function(){ window.shiftPegawaiDashboardMonth(-1); }, 'Bulan sebelumnya'));
    row.appendChild(month);
    row.appendChild(makeBtn('pegawai-dashboard-month-next','›','arika-filter-arrow', function(){ window.shiftPegawaiDashboardMonth(1); }, 'Bulan berikutnya'));
    var actions = document.createElement('div'); actions.className = 'arika-dashboard-filter-v290-actions'; panel.appendChild(actions);
    actions.appendChild(makeBtn('pegawai-dashboard-month-current','Bulan Ini','', function(){ window.setPegawaiDashboardMonthToCurrent(); }));
    actions.appendChild(makeBtn('pegawai-dashboard-month-all','Semua','', function(){ window.clearPegawaiDashboardMonth(); }));
    actions.appendChild(makeBtn('pegawai-dashboard-sync','↻ Sinkron','arika-filter-sync', function(){
      try { if(window.fetchLiveJurnalFromSheet) window.fetchLiveJurnalFromSheet({ silent:false, timeoutMs:12000 }).then(function(){ rerenderDashboard(); }); else rerenderDashboard(); } catch(_) { rerenderDashboard(); }
    }));
    actions.appendChild(makeBtn('pegawai-dashboard-export-monev','📥 Download Monev Aktivitas','arika-filter-monev', function(){ try { window.downloadPegawaiMonevAktivitas && window.downloadPegawaiMonevAktivitas(); } catch(e){ console.warn('Monev Aktivitas gagal:', e); } }));
    month.className = 'arika-manual-date-input';
    month.placeholder = 'mm/yyyy';
    month.autocomplete = 'off';
    month.inputMode = 'numeric';
    if(!month.__arikaV290Hooked){
      month.__arikaV290Hooked = true;
      month.addEventListener('input', function(){ if(month.value.trim()) month.dataset.periodMode = 'month'; setTimeout(updateDashboardState, 20); });
      month.addEventListener('change', function(){ var key=displayToKey(month.value); if(key) setDashMonth(key); else if(!month.value.trim()) setDashAll(); });
    }
    updateDashboardState();
  }
  window.v290DashboardFilterInstall = installDashboardFilter;

  // Smooth logout: login tampil dulu, session tetap dibersihkan lewat logout asli tanpa loader yang terasa lama.
  (function(){
    var oldLogout = window.logout;
    if(typeof oldLogout === 'function' && !oldLogout.__arikaV290Smooth){
      window.logout = function(){
        try { window.__ARIKA_LOGGING_OUT__ = true; } catch(_) {}
        try { window.currentUser = null; window.isAdmin = false; window.userRole = 'Pegawai'; window.isReviewer = false; } catch(_) {}
        try { document.querySelectorAll('.view-section').forEach(function(v){ v.classList.add('hidden'); }); } catch(_) {}
        var login = byId('view-login-user'); if(login) login.classList.remove('hidden');
        var nav = byId('main-nav'); if(nav) nav.classList.add('hidden');
        var mobile = byId('mobile-nav-select'); if(mobile) mobile.value = 'beranda';
        try { window.scrollTo({top:0, behavior:'auto'}); } catch(_) {}
        var oldShow = window.showLoader;
        try { window.showLoader = function(){}; } catch(_) {}
        var res;
        try { res = oldLogout.apply(this, arguments); } catch(e){ console.warn('Logout lama gagal:', e); }
        Promise.resolve(res).finally(function(){ try { window.showLoader = oldShow; } catch(_) {} try { oldShow && oldShow(false); } catch(_) {} try { window.__ARIKA_LOGGING_OUT__ = false; } catch(_) {} });
        return res;
      };
      window.logout.__arikaV290Smooth = true;
    }
  })();

  // WFH/WFA long list comfort for 30+ pegawai: compact scroll, search, status filter.
  function installWfhToolbar(){
    injectStyle();
    var list = byId('wfh-list'); if(!list) return;
    list.classList.add('wfh-compact-list');
    var parent = list.parentNode; if(!parent) return;
    if(!byId('wfh-long-toolbar')){
      var toolbar = document.createElement('div');
      toolbar.id = 'wfh-long-toolbar';
      toolbar.className = 'wfh-long-toolbar';
      toolbar.innerHTML = '<input id="wfh-search" type="search" placeholder="Cari nama, NIP, rencana, atau realisasi..." autocomplete="off"/><select id="wfh-status-filter"><option value="">Semua status</option><option value="done">Sudah realisasi</option><option value="plan">Belum realisasi</option><option value="realisasi">Realisasi tanpa rencana</option></select><span id="wfh-visible-count" class="text-[10px] font-black text-slate-400 uppercase tracking-widest"></span>';
      parent.insertBefore(toolbar, list);
      var apply = function(){
        var q = String((byId('wfh-search')||{}).value || '').toLowerCase().trim();
        var st = String((byId('wfh-status-filter')||{}).value || '');
        var cards = Array.prototype.slice.call(list.querySelectorAll('.wfh-entry-card'));
        var visible = 0;
        cards.forEach(function(card){
          var text = (card.textContent || '').toLowerCase();
          var statusText = text;
          var okQ = !q || text.indexOf(q) > -1;
          var okS = !st || (st === 'done' && /rencana\s*&\s*realisasi|rencana dan realisasi/i.test(statusText)) || (st === 'plan' && /belum realisasi/i.test(statusText)) || (st === 'realisasi' && /realisasi tanpa rencana/i.test(statusText));
          var show = okQ && okS;
          card.classList.toggle('is-filter-hidden', !show);
          if(show) visible++;
        });
        var count = byId('wfh-visible-count'); if(count) count.textContent = cards.length ? (visible + ' / ' + cards.length + ' tampil') : '';
      };
      byId('wfh-search').addEventListener('input', apply);
      byId('wfh-status-filter').addEventListener('change', apply);
      toolbar.__applyWfhFilter = apply;
    }
    var tb = byId('wfh-long-toolbar'); if(tb && tb.__applyWfhFilter) setTimeout(tb.__applyWfhFilter, 50);
  }
  var oldWfhRender = window.renderWfhWfaDay;
  if(typeof oldWfhRender === 'function' && !oldWfhRender.__arikaV290Wrapped){
    window.renderWfhWfaDay = function(){
      var res = oldWfhRender.apply(this, arguments);
      setTimeout(installWfhToolbar, 80);
      return res;
    };
    window.renderWfhWfaDay.__arikaV290Wrapped = true;
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ installDashboardFilter(); installWfhToolbar(); }); else { installDashboardFilter(); installWfhToolbar(); }
  window.addEventListener('load', function(){ setTimeout(installDashboardFilter, 120); setTimeout(installDashboardFilter, 800); setTimeout(installWfhToolbar, 250); });
  window.addEventListener('resize', function(){ clearTimeout(window.__arikaV290Resize); window.__arikaV290Resize = setTimeout(installDashboardFilter, 120); });
  document.addEventListener('click', function(ev){ if(ev.target && (ev.target.closest('#btn-dashboard-pegawai') || ev.target.closest('#mobile-nav-select'))) setTimeout(installDashboardFilter, 160); if(ev.target && ev.target.closest('#btn-wfh')) setTimeout(installWfhToolbar, 200); }, true);
  setTimeout(installDashboardFilter, 600); setTimeout(installDashboardFilter, 1800); setTimeout(installWfhToolbar, 900); setTimeout(installWfhToolbar, 2200);
})();


/* v291: tombol Monev lebih jelas dan Fokus Hari Ini menampilkan status terkendali saat tidak ada prioritas */
(function(){
  if(window.__ARIKA_V291_MONEV_FOCUS_STYLE__) return;
  window.__ARIKA_V291_MONEV_FOCUS_STYLE__ = true;
  function inject(){
    if(document.getElementById('arika-v291-monev-focus-style')) return;
    var st=document.createElement('style');
    st.id='arika-v291-monev-focus-style';
    st.textContent=`
      #view-dashboard-pegawai #pegawai-dashboard-export-monev,
      #view-dashboard-pegawai .arika-filter-monev{
        min-width: 168px !important;
        white-space: nowrap !important;
      }
      #view-beranda .arika-focus-empty{
        background: linear-gradient(135deg, rgba(236,253,245,.96), rgba(240,253,250,.88)) !important;
        border-color: rgba(16,185,129,.25) !important;
        color: #047857 !important;
      }
      @media(max-width:720px){
        #view-dashboard-pegawai #pegawai-dashboard-export-monev,
        #view-dashboard-pegawai .arika-filter-monev{min-width:0!important; white-space:normal!important;}
      }
    `;
    document.head.appendChild(st);
  }
...
/* =========================================================
   v294 - WFH/WFA: nomor urut pegawai + pembatas jelas + hilangkan inner scrollbar
   Basis: v293. Perubahan dibatasi pada tampilan Daftar Perencanaan & Realisasi WFH/WFA.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V294_WFH_NUMBERED_NO_INNER_SCROLL__) return;
  window.__ARIKA_V294_WFH_NUMBERED_NO_INNER_SCROLL__ = true;

  function byId(id){ return document.getElementById(id); }
  function injectStyle(){
    if(byId('arika-v294-wfh-numbered-style')) return;
    var st = document.createElement('style');
    st.id = 'arika-v294-wfh-numbered-style';
    st.textContent = `
      /* WFH/WFA: gunakan scroll halaman, bukan scrollbar kecil di dalam card/list. */
      #view-wfh #wfh-list{
        max-height:none!important;
        overflow:visible!important;
      ...
      #view-wfh #wfh-list.wfh-compact-list .wfh-mini-content,
      #view-wfh .wfh-mini-content{
        max-height:none!important;
        overflow:visible!important;
        scrollbar-width:auto!important;
      }
      #view-wfh .wfh-entry-card{
        position:relative!important;
        border-left:5px solid rgba(13,148,136,.85)!important;
        margin-bottom:.95rem!important;
        background:linear-gradient(135deg,#ffffff,#f8fafc)!important;
      }
      #view-wfh .wfh-entry-card:nth-of-type(even){
        background:linear-gradient(135deg,#ffffff,#f0fdfa)!important;
      }
      #view-wfh .wfh-entry-card::before{
        content:'';
        position:absolute;
        top:-.5rem;
        left:1rem;
        right:1rem;
        height:1px;
        background:linear-gradient(90deg,rgba(20,184,166,.0),rgba(20,184,166,.35),rgba(20,184,166,.0));
      }
      #view-wfh .wfh-person-head{
        display:flex!important;
        align-items:flex-start!important;
        gap:.72rem!important;
        min-width:0!important;
      }
      #view-wfh .wfh-order-pill{
        ...
        box-shadow:0 8px 18px rgba(8,145,178,.18)!important;
      }
      #view-wfh .wfh-person-meta{
        min-width:0!important;
      }
      #view-wfh .wfh-person-name{
        display:block!important;
        color:#0f172a!important;
      }
      #view-wfh .wfh-person-separator{
        margin-top:.55rem!important;
        padding-top:.55rem!important;
        border-top:1px dashed rgba(148,163,184,.32)!important;
      }
      #view-wfh .wfh-long-toolbar{
        position:sticky!important;
        top:.75rem!important;
        z-index:10!important;
        box-shadow:0 10px 24px rgba(15,23,42,.08)!important;
      }
      #view-wfh .wfh-long-toolbar input::placeholder{
        color:#94a3b8!important;
      }
      @media(max-width:720px){
        #view-wfh .wfh-person-head{gap:.55rem!important;}
        #view-wfh .wfh-order-pill{min-width:1.9rem!important;height:1.9rem!important;font-size:.65rem!important;border-radius:.7rem!important;}
        #view-wfh .wfh-entry-card{border-left-width:4px!important;margin-bottom:.82rem!important;}
        #view-wfh .wfh-long-toolbar{position:static!important;}
      }
    `;
    document.head.appendChild(st);
  }

  function normalizeSearchPlaceholder(){
    var search = byId('wfh-search');
    if(!search) return;
    var isAdmin = (window.isAdmin === true) || String(window.userRole || '').toLowerCase().includes('admin');
    search.placeholder = isAdmin ? 'Cari nama, NIP, rencana, atau realisasi...' : 'Cari nama, rencana, atau realisasi...';
  }

  function applyWfhNumbering(){
    injectStyle();
    var list = byId('wfh-list');
    if(!list) return;
    var cards = Array.prototype.slice.call(list.querySelectorAll('.wfh-entry-card'));
    var visibleNumber = 0;
    cards.forEach(function(card){
      var isHidden = card.classList.contains('is-filter-hidden') || card.offsetParent === null;
      if(!isHidden) visibleNumber += 1;
      var num = String(isHidden ? (card.dataset.wfhOriginalNumber || '') : visibleNumber).padStart(2,'0');
      if(!card.dataset.wfhOriginalNumber) card.dataset.wfhOriginalNumber = String(cards.indexOf(card)+1);
      var h3 = card.querySelector('h3');
      if(!h3) return;
      h3.classList.add('wfh-person-name');
      var oldWrapper = h3.closest('.wfh-person-head');
      if(oldWrapper){
        var pillExisting = oldWrapper.querySelector('.wfh-order-pill');
        if(pillExisting) pillExisting.textContent = isHidden ? String(card.dataset.wfhOriginalNumber).padStart(2,'0') : num;
        return;
      }
      var meta = h3.parentElement;
      if(!meta) return;
      meta.classList.add('wfh-person-meta');
      var wrapper = document.createElement('div');
      wrapper.className = 'wfh-person-head';
      var pill = document.createElement('span');
      pill.className = 'wfh-order-pill';
      pill.textContent = num;
      meta.parentNode.insertBefore(wrapper, meta);
      wrapper.appendChild(pill);
      wrapper.appendChild(meta);
      var sep = card.querySelector('.grid.grid-cols-1');
      if(sep) sep.classList.add('wfh-person-separator');
    });
    normalizeSearchPlaceholder();
    var count = byId('wfh-visible-count');
    if(count && cards.length) count.textContent = visibleNumber + ' / ' + cards.length + ' pegawai tampil';
  }

  var oldRender = window.renderWfhWfaDay;
  if(typeof oldRender === 'function' && !oldRender.__arikaV294Numbered){
    window.renderWfhWfaDay = function(){
      var res = oldRender.apply(this, arguments);
      setTimeout(applyWfhNumbering, 90);
      setTimeout(applyWfhNumbering, 240);
      return res;
    };
    window.renderWfhWfaDay.__arikaV294Numbered = true;
  }

  document.addEventListener('input', function(ev){
    if(ev.target && ev.target.id === 'wfh-search') setTimeout(applyWfhNumbering, 60);
  }, true);
  document.addEventListener('change', function(ev){
    if(ev.target && ev.target.id === 'wfh-status-filter') setTimeout(applyWfhNumbering, 60);
  }, true);
  document.addEventListener('click', function(ev){
    if(ev.target && (ev.target.closest('#btn-wfh') || ev.target.closest('[onclick*="wfh"]'))) setTimeout(applyWfhNumbering, 180);
  }, true);

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(applyWfhNumbering, 400); });
  else setTimeout(applyWfhNumbering, 300);
  window.addEventListener('load', function(){ setTimeout(applyWfhNumbering, 800); });
})();


/* =========================================================
   v295 - WFH/WFA menu visible for Administrator
   Patch keamanan: pastikan tab/menu WFH/WFA tidak disembunyikan untuk admin.
   ========================================================= */
(function(){
  if(window.__ARIKA_V295_ADMIN_WFH_MENU__) return;
  window.__ARIKA_V295_ADMIN_WFH_MENU__ = true;
  function ensureAdminWfhVisible(){
    try {
      if(!window.isAdmin) return;
      var btn = document.getElementById('btn-wfh');
      if(btn) btn.classList.remove('hidden');
      var opt = document.querySelector('#mobile-nav-select option[value="wfh"]');
      if(opt){ opt.hidden = false; opt.disabled = false; opt.classList.remove('hidden'); }
    } catch(e) {}
  }
  var oldConfigure = window.configureNavigationForRole;
  if(typeof oldConfigure === 'function' && !oldConfigure.__arikaV295AdminWfh){
    window.configureNavigationForRole = function(adminFlag){
      var res = oldConfigure.apply(this, arguments);
      setTimeout(ensureAdminWfhVisible, 0);
      setTimeout(ensureAdminWfhVisible, 120);
      return res;
    };
    window.configureNavigationForRole.__arikaV295AdminWfh = true;
  }
  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV295AdminWfh){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      setTimeout(ensureAdminWfhVisible, 80);
      return res;
    };
    window.nav.__arikaV295AdminWfh = true;
  }
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(ensureAdminWfhVisible, 200); });
  window.addEventListener('load', function(){ setTimeout(ensureAdminWfhVisible, 300); setTimeout(ensureAdminWfhVisible, 1200); });
  setInterval(ensureAdminWfhVisible, 2500);
})();

/* =========================================================
   v296 - Mobile/Safari stability lite + Riwayat title clean
   Patch terbatas: tidak mengubah data atau alur kerja, hanya mempercepat first paint/mobile.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V296_MOBILE_SAFARI_STABILITY__) return;
  window.__ARIKA_V296_MOBILE_SAFARI_STABILITY__ = true;
  ...
  function applyLite(){
    try{
      document.documentElement.classList.toggle('arika-mobile-lite', !!isMobile());
      if(document.body) document.body.classList.toggle('arika-mobile-lite', !!isMobile());
    }catch(e){}
  }
  function ensureMobileMenu(){
    try{
      var sel=document.getElementById('mobile-nav-select');
      if(!sel) return;
      if(!Array.from(sel.options).some(function(o){return o.value==='wfh';})){
        var opt=document.createElement('option'); opt.value='wfh'; opt.textContent='🏡 WFH/WFA';
        var panduan=Array.from(sel.options).find(function(o){return o.value==='panduan';});
        sel.insertBefore(opt, panduan || null);
      }
    }catch(e){}
  }
  // Jangan biarkan overlay loader menutup Safari HP terlalu lama; sinkron data tetap berjalan di belakang.
  var oldShowLoader=window.showLoader;
  if(typeof oldShowLoader==='function' && !oldShowLoader.__arikaV296MobileLite){
    window.showLoader=function(show,text){
      if(isMobile() && show){
        try{
        ...
        clearTimeout(window.__arikaV296ShowLoaderTimer);
        window.__arikaV296ShowLoaderTimer=setTimeout(hideLoaderFast,700);
        return;
      }
      return oldShowLoader.apply(this,arguments);
    };
    window.showLoader.__arikaV296MobileLite=true;
  }
  function boot(){ applyLite(); ensureMobileMenu(); setTimeout(hideLoaderFast,650); setTimeout(hideLoaderFast,1400); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  window.addEventListener('load',function(){ boot(); setTimeout(hideLoaderFast,250); });
  window.addEventListener('resize',function(){ clearTimeout(window.__arikaV296Resize); window.__arikaV296Resize=setTimeout(applyLite,120); });
})();

/* =========================================================
   v297 - Relasi Rencana -> Realisasi Jurnal
   Basis: v296. Perubahan dibatasi pada fitur Isi Realisasi dari Rencana.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V297_REALISASI_RENCANA__) return;
  window.__ARIKA_V297_REALISASI_RENCANA__ = true;

  function byId(id){ return document.getElementById(id); }
  function safe(v){ try { return String(v || ''); } catch(_) { return ''; } }
  function norm(v){ return safe(v).trim().toLowerCase().replace(/[^a-z0-9]+/g,''); }
  function today(){ try { return typeof getTodayKey === 'function' ? getTodayKey() : new Date().toISOString().slice(0,10); } catch(_) { return new Date().toISOString().slice(0,10); } }
  function normalizeDate(v){ try { return typeof normalizeDateKeyFromSheet === 'function' ? normalizeDateKeyFromSheet(v) : safe(v).slice(0,10); } catch(_) { return safe(v).slice(0,10); } }
  function escape(v){ try { return typeof escapeHTML === 'function' ? escapeHTML(v) : safe(v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];}); } catch(_) { return safe(v); } }
  function fmtDate(v){ try { return typeof formatHariTanggal === 'function' ? formatHariTanggal(v) : safe(v); } catch(_) { return safe(v); } }

  window.generateArikaRencanaId = function(tanggal){
    var date = normalizeDate(tanggal) || today();
    var code = '';
    try { code = (crypto && crypto.getRandomValues) ? Array.from(crypto.getRandomValues(new Uint8Array(4))).map(function(x){return x.toString(16).padStart(2,'0');}).join('').toUpperCase() : Math.random().toString(36).slice(2,10).toUpperCase(); }
    catch(_) { code = Math.random().toString(36).slice(2,10).toUpperCase(); }
    return 'RNC-' + date.replace(/[^0-9]/g,'') + '-' + code;
  };

  function getRencanaId(item){
    item = item || {};
    return safe(item.rencanaId || item.rencana_id || item['rencana id'] || item.id || '').trim();
  }
  function findRencana(id){
    var target = safe(id).trim();
    var list = [];
    try { list = Array.isArray(window.rencanaData) ? window.rencanaData : []; } catch(_) {}
    return list.find(function(r){ return safe(r.id).trim() === target || getRencanaId(r) === target; }) || null;
  }

  function ensureCard(){
    var form = byId('form-arika');
    if(!form) return null;
    var card = byId('jurnal-linked-rencana-card');
    if(!card){
      card = document.createElement('div');
      card.id = 'jurnal-linked-rencana-card';
      card.className = 'hidden mb-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-xs font-bold text-slate-700';
      form.parentNode.insertBefore(card, form);
    }
    return card;
  }

  window.updateLinkedRencanaCard = function(){
    var card = ensureCard();
    if(!card) return;
    var linked = window.arikaLinkedRencana || null;
    if(!linked || !linked.rencanaId){
      card.classList.add('hidden');
      card.innerHTML = '';
      return;
    }
    card.classList.remove('hidden');
    card.innerHTML = '<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">'
      + '<div><p class="text-[9px] font-black uppercase tracking-widest text-emerald-700 mb-1">✅ Realisasi untuk rencana terpilih</p>'
      + '<h3 class="text-sm font-black text-slate-900 leading-snug">' + escape(linked.rencanaJudul || 'Rencana kegiatan') + '</h3>'
      + '<p class="mt-1 text-[10px] font-bold text-slate-500">' + escape(fmtDate(linked.tanggal || today())) + ' • ID: ' + escape(linked.rencanaId) + '</p></div>'
      + '<button type="button" onclick="window.clearLinkedRencana()" class="px-3 py-2 rounded-xl bg-white text-rose-600 border border-rose-100 text-[9px] font-black uppercase tracking-widest">Batal kaitkan</button>'
      + '</div>';
  };

  window.clearLinkedRencana = function(){
    window.arikaLinkedRencana = null;
    try { sessionStorage.removeItem('arika_linked_rencana_v297'); } catch(_) {}
    window.updateLinkedRencanaCard && window.updateLinkedRencanaCard();
  };

  window.mulaiRealisasiRencana = function(id){
    var item = findRencana(id);
    if(!item) return (window.showCustomAlert ? window.showCustomAlert('Rencana tidak ditemukan. Silakan sinkron data lalu coba lagi.') : alert('Rencana tidak ditemukan.'));
    var rencanaId = getRencanaId(item) || window.generateArikaRencanaId(item.tanggal || today());
    item.rencanaId = rencanaId;
    window.arikaLinkedRencana = {
      rencanaId: rencanaId,
      rencanaJudul: safe(item.judul || item.catatan || 'Rencana kegiatan'),
      tanggal: normalizeDate(item.tanggal || today()),
      localId: safe(item.id || '')
    };
    try { sessionStorage.setItem('arika_linked_rencana_v297', JSON.stringify(window.arikaLinkedRencana)); } catch(_) {}
    try { if(typeof window.nav === 'function') window.nav('jurnal'); } catch(_) {}
    setTimeout(function(){
      try {
        var d = byId('in-date'); if(d) d.value = window.arikaLinkedRencana.tanggal || today();
        if(window.setJurnalCategory) window.setJurnalCategory('Administrasi');
        var desc = byId('in-desc');
        if(desc && !desc.value.trim()) {
          desc.value = '[REALISASI RENCANA: ' + (window.arikaLinkedRencana.rencanaJudul || 'Rencana kegiatan') + ']\n';
          desc.focus();
          try { desc.setSelectionRange(desc.value.length, desc.value.length); } catch(_) {}
        }
        window.updateLinkedRencanaCard && window.updateLinkedRencanaCard();
        var card = byId('jurnal-linked-rencana-card'); if(card) card.scrollIntoView({ behavior:'smooth', block:'center' });
      } catch(e){ console.warn('Gagal prefill realisasi rencana:', e); }
    }, 180);
  };

  function restoreLinked(){
    try {
      if(!window.arikaLinkedRencana) {
        var raw = sessionStorage.getItem('arika_linked_rencana_v297');
        if(raw) window.arikaLinkedRencana = JSON.parse(raw);
      }
    } catch(_) {}
    setTimeout(function(){ try { window.updateLinkedRencanaCard && window.updateLinkedRencanaCard(); } catch(_) {} }, 80);
  }

  // Tambahkan tombol Isi Realisasi bila render lama belum memiliki tombol ini.
  function enrichRencanaButtons(){
    var list = byId('rencana-pribadi-list');
    if(!list) return;
    var cards = Array.from(list.querySelectorAll('.rounded-2xl.border.relative'));
    var data = Array.isArray(window.rencanaData) ? window.rencanaData.filter(function(r){
      try { return typeof isCurrentUserRencana === 'function' ? isCurrentUserRencana(r) : true; } catch(_) { return true; }
    }).filter(function(r){ try { return typeof isRencanaSelesai === 'function' ? !isRencanaSelesai(r) : true; } catch(_) { return true; } }) : [];
    cards.forEach(function(card, idx){
      if(card.querySelector('.arika-btn-isi-realisasi')) return;
      var h = card.querySelector('h4');
      var title = norm(h ? h.textContent : '');
      var item = data.find(function(r){ return norm(r.judul || 'Rencana kegiatan') === title; }) || data[idx];
      if(!item) return;
      var row = card.querySelector('.mt-3.flex.flex-wrap.gap-2');
      if(!row) return;
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'arika-btn-isi-realisasi px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 text-[9px] font-black uppercase tracking-widest';
      btn.textContent = 'Isi Realisasi';
      btn.addEventListener('click', function(ev){ ev.preventDefault(); ev.stopPropagation(); window.mulaiRealisasiRencana(item.id || item.rencanaId); });
      row.appendChild(btn);
    });
  }

  var oldRender = window.renderRencanaPribadi;
  if(typeof oldRender === 'function' && !oldRender.__arikaV297Wrapped){
    window.renderRencanaPribadi = function(){
      var res = oldRender.apply(this, arguments);
      setTimeout(enrichRencanaButtons, 80);
      return res;
    };
    window.renderRencanaPribadi.__arikaV297Wrapped = true;
  }

  // Setelah simpan jurnal sukses, reset form akan membersihkan kaitan. Namun payload sudah membawa rencanaId/rencanaJudul.
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', restoreLinked); else restoreLinked();
  window.addEventListener('load', function(){ restoreLinked(); setTimeout(enrichRencanaButtons, 60); });
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(enrichRencanaButtons, 200); }, true);
  setTimeout(enrichRencanaButtons, 180);
})();


/* =========================================================
   v303 - Kembali ke layout WFH/WFA v297 + hapus dropdown "Tidak terkait rencana"
   Basis: v297. Patch dibatasi pada alur relasi rencana-jurnal.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V303_FOLLOW_V297_NO_MANUAL_RENCANA_DROPDOWN__) return;
  window.__ARIKA_V303_FOLLOW_V297_NO_MANUAL_RENCANA_DROPDOWN__ = true;
  function byId(id){ return document.getElementById(id); }
  function removeManualDropdown(){
    var card = byId('jurnal-rencana-select-card');
    if(card) card.remove();
    var select = byId('jurnal-rencana-select');
    if(select) select.remove();
  }
  function injectStyle(){
    if(byId('arika-v303-realisasi-rencana-style')) return;
    var st=document.createElement('style');
    st.id='arika-v303-realisasi-rencana-style';
    st.textContent='\n.arika-archive-label.linked-plan{background:#ccfbf1;color:#0f766e;}\n#jurnal-rencana-select-card,#jurnal-rencana-select{display:none!important;}\n';
    document.head.appendChild(st);
  }
  // Mulai v303, tidak ada pilihan manual "Tidak terkait rencana" di form Isi Jurnal.
  // Relasi rencana hanya dibawa dari tombol Isi Realisasi pada Rencana Saya atau tab WFH/WFA.
  window.installJurnalRencanaSelect = function(){ removeManualDropdown(); };
  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV303NoDropdown){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      if(id === 'jurnal') setTimeout(removeManualDropdown, 80);
      return res;
    };
    window.nav.__arikaV303NoDropdown = true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ injectStyle(); removeManualDropdown(); });
  else { injectStyle(); removeManualDropdown(); }
  window.addEventListener('load', function(){ injectStyle(); setTimeout(removeManualDropdown, 120); setTimeout(removeManualDropdown, 800); });
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#btn-jurnal')) setTimeout(removeManualDropdown, 100);
  }, true);
})();


/* v304: Hilangkan tombol jurnal umum di tab WFH/WFA agar realisasi wajib lewat rencana yang dituju. */
(function(){
  function hideGeneralWfhJournalButton(){
    try {
      document.querySelectorAll('#view-wfh button').forEach(function(btn){
        if(/isi\s+jurnal\s+wfh\/wfa/i.test(btn.textContent || '')) btn.remove();
      });
    } catch(_) {}
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', hideGeneralWfhJournalButton); else hideGeneralWfhJournalButton();
  window.addEventListener('load', hideGeneralWfhJournalButton);
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest('#btn-wfh')) setTimeout(hideGeneralWfhJournalButton, 120); }, true);
  setTimeout(hideGeneralWfhJournalButton, 600);
})();


/* =========================================================
   v314 - Mobile Safe Login Mode
   Basis: v310. Menghindari boot cover yang mengunci login HP dan meringankan input username/sandi.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V314_MOBILE_SAFE__) return;
  window.__ARIKA_V314_MOBILE_SAFE__ = true;
  function byId(id){ return document.getElementById(id); }
  function isMobile(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  function hideLoader(){
    try{
      document.documentElement.classList.remove('arika-v311-boot','arika-v312-boot','arika-v313-boot');
      document.documentElement.classList.add('arika-v314-mobile-login-ready');
      var loader=byId('global-loader');
      if(loader){ loader.classList.add('arika-v314-hide'); loader.style.display='none'; loader.style.opacity='0'; loader.style.pointerEvents='none'; }
      document.body && document.body.classList.remove('arika-boot-open','arika-session-restoring');
    }catch(e){}
  }
  function showLoginIfNeeded(){
    try{
      if(!isMobile() || window.currentUser) return;
      var login=byId('view-login-user');
      var admin=byId('view-login-admin');
      var nav=byId('main-nav');
      var app=byId('arika-app-main');
      if(app) app.style.visibility='visible';
      if(nav) nav.classList.add('hidden');
      if(login){
        document.querySelectorAll('.view-section').forEach(function(v){ if(v!==login && v!==admin) v.classList.add('hidden'); });
        login.classList.remove('hidden');
        login.style.visibility='visible'; login.style.opacity='1'; login.style.pointerEvents='auto';
        var card=login.querySelector('.bg-white'); if(card){card.style.height='auto';card.style.minHeight='0';card.style.maxHeight='none';card.style.overflow='visible';}
        var form=login.querySelector('form'); if(form){form.style.display='block';form.style.visibility='visible';form.style.opacity='1';}
      }
      hideLoader();
    }catch(e){}
  }
  function makeLoginLight(){
    if(!isMobile()) return;
    try{
      hideLoader();
      var search=byId('login-user-search');
      var pwd=byId('login-user-nip');
      var suggestions=byId('login-user-suggestions');
      if(search){
        search.setAttribute('autocomplete','off');
        search.setAttribute('inputmode','text');
        search.addEventListener('focus', function(){ hideLoader(); }, {passive:true});
        search.addEventListener('click', function(){ hideLoader(); }, {passive:true});
      }
      if(pwd){
        pwd.setAttribute('autocomplete','current-password');
        pwd.addEventListener('focus', function(){ hideLoader(); if(suggestions) suggestions.classList.add('hidden'); }, {passive:true});
        pwd.addEventListener('click', function(){ hideLoader(); if(suggestions) suggestions.classList.add('hidden'); }, {passive:true});
      }
      // Batasi saran nama di HP agar tidak lag ketika data pegawai banyak.
      var oldFilter=window.filterLoginNameOptions;
      if(typeof oldFilter==='function' && !oldFilter.__arikaV314MobileSafe){
        window.filterLoginNameOptions=function(){
          if(!isMobile()) return oldFilter.apply(this,arguments);
          clearTimeout(window.__arikaV314FilterTimer);
          window.__arikaV314FilterTimer=setTimeout(function(){
            try{
              var input=byId('login-user-search');
              var q=(input&&input.value||'').trim();
              if(q.length<2){ var s=byId('login-user-suggestions'); if(s) s.classList.add('hidden'); return; }
              oldFilter.apply(window,arguments);
              var box=byId('login-user-suggestions');
              if(box){
                Array.from(box.querySelectorAll('.login-suggestion-item')).slice(8).forEach(function(el){el.remove();});
              }
            }catch(e){ try{oldFilter.apply(window,arguments);}catch(_){} }
          },160);
        };
        window.filterLoginNameOptions.__arikaV314MobileSafe=true;
      }
    }catch(e){}
  }
  function boot(){ makeLoginLight(); setTimeout(showLoginIfNeeded,250); setTimeout(showLoginIfNeeded,900); setTimeout(showLoginIfNeeded,1800); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('load', boot);
  document.addEventListener('touchstart', function(ev){ if(ev.target && (ev.target.id==='login-user-search' || ev.target.id==='login-user-nip')) hideLoader(); }, {passive:true});
  document.addEventListener('click', function(ev){ if(ev.target && (ev.target.id==='login-user-search' || ev.target.id==='login-user-nip')) hideLoader(); }, true);
})();


/* v315: desktop reload veil hook. Mobile login safe tetap dari v314. */
(function(){
  if(window.__ARIKA_V315_DESKTOP_RELOAD_VEIL_HOOK__) return;
  window.__ARIKA_V315_DESKTOP_RELOAD_VEIL_HOOK__ = true;
  function isMobile(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  function release(reason){
    if(isMobile()) return;
    try{
      document.documentElement.classList.remove('arika-v315-boot');
      document.documentElement.classList.add('arika-v315-ready');
      var veil=document.getElementById('arika-v315-reload-veil');
      if(veil){ veil.style.opacity='0'; veil.style.pointerEvents='none'; setTimeout(function(){ try{ veil.style.display='none'; }catch(e){} },220); }
      window.__ARIKA_V315_READY_REASON__=reason||'app-hook';
    }catch(e){}
  }
  function hook(){
    if(window.nav && !window.nav.__arikaV315Veil){
      var old=window.nav;
      window.nav=function(id){ var res=old.apply(this,arguments); setTimeout(function(){release('nav-'+(id||'view'));},360); return res; };
      window.nav.__arikaV315Veil=true;
    }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',function(){ hook(); setTimeout(hook,600); }); else { hook(); setTimeout(hook,600); }
  window.addEventListener('load',function(){ hook(); setTimeout(function(){release('app-load');},1300); });
})();

/* =========================================================
   v316 - Mobile Login Ultra Lite
   Fokus: input nama/sandi harus responsif di HP. Data utama ditunda sampai setelah login.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V316_MOBILE_LOGIN_ULTRALITE__) return;
  window.__ARIKA_V316_MOBILE_LOGIN_ULTRALITE__ = true;
  function byId(id){ return document.getElementById(id); }
  function isMobile(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  ...
  function hideLoader(){
    try{
      var loader=byId('global-loader');
      if(loader){ loader.style.display='none'; loader.style.opacity='0'; loader.style.pointerEvents='none'; }
      document.documentElement.classList.add('arika-v316-mobile-login-ready');
      document.body && document.body.classList.remove('arika-session-restoring','arika-boot-open');
    }catch(e){}
  }
  function showLoginNow(){
    ...
    try{
      hideLoader();
      var main=byId('arika-app-main'); if(main){ main.style.visibility='visible'; main.style.opacity='1'; main.style.pointerEvents='auto'; }
      var nav=byId('main-nav'); if(nav) nav.classList.add('hidden');
      var login=byId('view-login-user'); var admin=byId('view-login-admin');
      if(login){
        document.querySelectorAll('.view-section').forEach(function(v){ if(v!==login && v!==admin) v.classList.add('hidden'); });
        login.classList.remove('hidden');
        login.style.visibility='visible'; login.style.opacity='1'; login.style.pointerEvents='auto';
  ...
  function mapPegawaiLite(arr){
    return (arr||[]).map(function(p){
      function gv(keys){
        for(var i=0;i<keys.length;i++){
          var k=keys[i];
          if(p && Object.prototype.hasOwnProperty.call(p,k) && p[k] !== undefined && p[k] !== null && p[k] !== '') return p[k];
        }
        // fallback case-insensitive
        var target=keys.map(function(x){return String(x).toLowerCase().replace(/[^a-z0-9]/g,'');});
        var objKeys=Object.keys(p||{});
        for(var j=0;j<objKeys.length;j++){
          var nk=String(objKeys[j]).toLowerCase().replace(/[^a-z0-9]/g,'');
          if(target.indexOf(nk)>=0) return p[objKeys[j]];
        }
        return '';
      }
      return {
        id: safe(gv(['id','ID','ID Unik','id_unik'])),
        nama: safe(gv(['nama','Nama','name','pelaksana','Pelaksana'])),
        nip: safe(gv(['nip','NIP','nip_pegawai','NIP Pegawai'])),
        status: safe(gv(['status','Status','statusPegawai','Status Pegawai'])) || 'PNS',
        lab: safe(gv(['lab','Lab','fungsi','Fungsi Kerja','fungsi kerja'])) || '',
        peran: safe(gv(['peran','Peran','role','Role'])) || 'Pegawai',
        cakupanUnit: safe(gv(['cakupanUnit','Cakupan Unit','unitBinaan','Unit Binaan'])) || ''
      };
    }).filter(function(p){return p.nama;});
  }
  async function fetchLoginOnlyMobile(){
    if(!isMobile() || window.currentUser) return false;
    if(window.__ARIKA_V316_FETCH_LOGIN_ONLY__) return true;
    window.__ARIKA_V316_FETCH_LOGIN_ONLY__ = true;
    try{
      var msg=byId('login-status-msg');
      if(msg){ msg.textContent='Memuat daftar pegawai ringan...'; msg.classList.remove('hidden','text-rose-500'); msg.classList.add('text-slate-500'); }
      var url = SCRIPT_URL + (SCRIPT_URL.indexOf('?')>=0 ? '&' : '?') + 'mode=login&maxRows=0&mobile=1&_=' + Date.now();
      var controller = new AbortController();
      var timer = setTimeout(function(){ try{controller.abort();}catch(e){} }, 8500);
      var res = await fetch(url,{method:'GET',cache:'no-store',signal:controller.signal});
      clearTimeout(timer);
      if(!res.ok) throw new Error('HTTP '+res.status);
      var data = await res.json();
      if(data && Array.isArray(data.pegawai) && data.pegawai.length){
        window.masterPegawai = mapPegawaiLite(data.pegawai);
        if(typeof window.populateLoginDropdown === 'function') window.populateLoginDropdown();
        rebuildCache();
        if(msg){ msg.textContent='Daftar pegawai siap. Silakan login.'; msg.classList.remove('hidden','text-rose-500','text-slate-500'); msg.classList.add('text-emerald-600'); setTimeout(function(){msg.classList.add('hidden');},1800); }
        return true;
      }
    }catch(e){
      console.warn('v316 login-only gagal:', e);
      var m=byId('login-status-msg');
      if(m){ m.textContent='Daftar pegawai belum termuat. Coba tekan Muat Ulang.'; m.classList.remove('hidden','text-slate-500','text-emerald-600'); m.classList.add('text-rose-500'); }
    }finally{
      window.__ARIKA_V316_FETCH_LOGIN_ONLY__ = false;
    }
    return false;
  }
  var cacheLen=-1, cache=[];
  function rebuildCache(){
    var arr = Array.isArray(window.masterPegawai) ? window.masterPegawai : [];
    if(arr.length === cacheLen && cache.length) return cache;
    cacheLen = arr.length;
    cache = arr.filter(function(p){return p && p.nama;}).map(function(p){
      var nama=safe(p.nama); var parts=nama.split(/\s+/).map(norm).filter(Boolean);
      return {raw:p,nama:nama,norm:norm(nama),parts:parts,lab:safe(p.lab||''),status:safe(p.status||''),peran:safe(p.peran||'')};
    }).sort(function(a,b){return a.nama.localeCompare(b.nama,'id',{sensitivity:'base'});});
    return cache;
  }
  window.mobileFilterLoginOptions = function(){
    hideLoader();
    var input=byId('login-user-search'); var hidden=byId('login-user-select'); var box=byId('login-user-suggestions');
    if(!input || !box) return;
    if(hidden) hidden.value='';
    var q=safe(input.value); var nq=norm(q);
    if(!Array.isArray(window.masterPegawai) || !window.masterPegawai.length){
      box.innerHTML='<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Daftar pegawai sedang dimuat...</div>';
      box.classList.remove('hidden');
      fetchLoginOnlyMobile();
      return;
    }
    if(nq.length < 1){ box.classList.add('hidden'); return; }
    var list=rebuildCache();
    var matches=[];
    for(var i=0;i<list.length && matches.length<7;i++){
      var item=list[i];
      var ok=item.norm.indexOf(nq)===0 || item.parts.some(function(p){return p.indexOf(nq)===0;}) || item.norm.indexOf(nq)>=0;
      if(ok) matches.push(item);
    }
    if(!matches.length){
      box.innerHTML='<div class="p-3 text-center text-[10px] font-black uppercase tracking-widest text-rose-400">Nama tidak ditemukan.</div>';
      box.classList.remove('hidden'); return;
    }
    box.innerHTML=matches.map(function(it){
      var p=it.raw;
      var meta=[p.lab||'-', p.status||'', (p.peran && norm(p.peran)!=='pegawai') ? p.peran : ''].filter(Boolean).join(' • ');
      return '<button type="button" class="login-suggestion-item" data-name="'+esc(p.nama)+'"><span class="text-xs font-black text-slate-800 uppercase">'+esc(p.nama)+'</span><span class="text-[9px] font-bold text-slate-400 uppercase">'+esc(meta)+'</span></button>';
    }).join('');
    box.classList.remove('hidden');
  };
  function installInputUltraLite(){
    if(!isMobile()) return;
    showLoginNow();
    var input=byId('login-user-search'); var pwd=byId('login-user-nip'); var box=byId('login-user-suggestions');
    if(input && !input.__arikaV316Ultra){
      input.__arikaV316Ultra=true;
      input.setAttribute('oninput','window.mobileFilterLoginOptions && window.mobileFilterLoginOptions()');
      input.setAttribute('onfocus','window.mobileFilterLoginOptions && window.mobileFilterLoginOptions()');
      input.addEventListener('input', window.mobileFilterLoginOptions, {passive:true});
      input.addEventListener('focus', function(){ hideLoader(); setTimeout(window.mobileFilterLoginOptions,0); }, {passive:true});
      input.addEventListener('touchstart', hideLoader, {passive:true});
    }
    if(pwd && !pwd.__arikaV316Ultra){
      pwd.__arikaV316Ultra=true;
      pwd.addEventListener('focus', function(){ hideLoader(); if(box) box.classList.add('hidden'); }, {passive:true});
      pwd.addEventListener('touchstart', function(){ hideLoader(); if(box) box.classList.add('hidden'); }, {passive:true});
    }
    if(box && !box.__arikaV316Delegate){
      box.__arikaV316Delegate=true;
      box.addEventListener('click',function(ev){
        var btn=ev.target && ev.target.closest ? ev.target.closest('.login-suggestion-item') : null;
        if(!btn) return;
        var name=btn.getAttribute('data-name')||'';
        var input=byId('login-user-search'); var hidden=byId('login-user-select');
        if(input) input.value=name;
        if(hidden) hidden.value=name;
        box.classList.add('hidden');
        var pwd=byId('login-user-nip'); if(pwd) setTimeout(function(){pwd.focus();},30);
      }, true);
    }
  }
  // Mobile sebelum login: fetchCloudData cukup mengambil daftar pegawai, bukan core/jurnal besar.
  if(window.fetchCloudData && !window.fetchCloudData.__arikaV316MobileLoginOnly){
    var oldFetch=window.fetchCloudData;
    window.fetchCloudData = function(options){
      options=options||{};
      if(isMobile() && !window.currentUser && !options.full && !options.afterLogin){
        return fetchLoginOnlyMobile();
      }
      return oldFetch.apply(this, arguments);
    };
    window.fetchCloudData.__arikaV316MobileLoginOnly=true;
  }
  // Login button: pastikan daftar pegawai sudah ada; kalau belum, ambil ringan dulu tanpa render core.
  if(window.handleUserLogin && !window.handleUserLogin.__arikaV316LoginGuard){
    var oldLogin=window.handleUserLogin;
    window.handleUserLogin = async function(e){
      hideLoader();
      if(isMobile() && (!window.masterPegawai || !window.masterPegawai.length)){
        if(e && e.preventDefault) e.preventDefault();
        var ok = await fetchLoginOnlyMobile();
        if(!ok) return;
      }
      return oldLogin.apply(this, arguments);
    };
    window.handleUserLogin.__arikaV316LoginGuard=true;
  }
  function boot(){
    if(!isMobile()) return;
    hideLoader();
    showLoginNow();
    installInputUltraLite();
    setTimeout(installInputUltraLite,250);
    setTimeout(fetchLoginOnlyMobile,450);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('load', boot);
  document.addEventListener('visibilitychange', function(){ if(!document.hidden) setTimeout(installInputUltraLite,80); });
})();

/* =========================================================
   v317 - Penanda Sudah Realisasi pada Rencana Saya di Beranda
   Basis: v316. Patch hanya untuk Rencana Saya; tab WFH/WFA tidak diubah.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V317_RENCANA_SUDAH_REALISASI__) return;
  window.__ARIKA_V317_RENCANA_SUDAH_REALISASI__ = true;
  ...
  function norm(v){ return safe(v).toLowerCase().replace(/[^a-z0-9]+/g,''); }
  function dateKey(v){
    try { if(typeof normalizeDateKeyFromSheet === 'function') return normalizeDateKeyFromSheet(v || '') || ''; } catch(_) {}
    if(v instanceof Date && !isNaN(v)) return v.getFullYear() + '-' + String(v.getMonth()+1).padStart(2,'0') + '-' + String(v.getDate()).padStart(2,'0');
    var s = safe(v); if(!s) return '';
    var m = s.match(/^(\d{4})[-/](\d{1,2})[-/](\d{1,2})/);
    if(m) return m[1] + '-' + String(m[2]).padStart(2,'0') + '-' + String(m[3]).padStart(2,'0');
    m = s.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{4})/);
  ...
  function injectStyle(){
    if(byId('arika-v317-rencana-realized-style')) return;
    var st = document.createElement('style');
    st.id = 'arika-v317-rencana-realized-style';
    st.textContent = `
      #rencana-pribadi-list .arika-rencana-real-status-v317{
        display:inline-flex!important;align-items:center!important;gap:.28rem!important;
        padding:.34rem .55rem!important;border-radius:999px!important;
        font-size:8px!important;font-weight:950!important;letter-spacing:.10em!important;text-transform:uppercase!important;
        border:1px solid rgba(226,232,240,.9)!important;background:rgba(255,255,255,.78)!important;
        color:#64748b!important;margin:.1rem 0 .45rem 0!important;
      }
      #rencana-pribadi-list .arika-rencana-real-status-v317.done{
        background:#ecfdf5!important;border-color:#a7f3d0!important;color:#047857!important;
      }
      #rencana-pribadi-list .arika-rencana-real-status-v317.waiting{
        background:#fffbeb!important;border-color:#fde68a!important;color:#92400e!important;
      }
      #rencana-pribadi-list .arika-btn-lihat-realisasi-v317{
        background:linear-gradient(135deg,#059669,#0891b2)!important;color:#fff!important;
      }
      .arika-v317-modal-backdrop{position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,.48);display:flex;align-items:center;justify-content:center;padding:1rem;backdrop-filter:blur(4px);}
      .arika-v317-modal{width:min(620px,100%);max-height:86vh;overflow:auto;background:#fff;border-radius:1.35rem;border:1px solid rgba(226,232,240,.9);box-shadow:0 24px 70px rgba(15,23,42,.24);padding:1rem;}
      .arika-v317-modal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;border-bottom:1px solid #e2e8f0;padding-bottom:.75rem;margin-bottom:.75rem;}
      .arika-v317-modal-title{font-size:.95rem;font-weight:950;color:#0f172a;line-height:1.35;margin:0;}
      .arika-v317-modal-sub{font-size:.68rem;font-weight:800;color:#64748b;margin-top:.22rem;}
      .arika-v317-modal-close{border:0;border-radius:.8rem;background:#f1f5f9;color:#475569;font-size:.75rem;font-weight:950;padding:.52rem .7rem;}
      .arika-v317-real-item{border:1px solid #d1fae5;background:#f0fdf4;border-radius:1rem;padding:.75rem;margin:.6rem 0;color:#334155;font-size:.75rem;line-height:1.5;font-weight:750;}
      .arika-v317-real-item b{display:block;color:#0f172a;font-size:.72rem;margin-bottom:.25rem;}
      .arika-v317-real-item .meta{font-size:.62rem;color:#0f766e;font-weight:950;letter-spacing:.08em;text-transform:uppercase;margin-bottom:.3rem;}
      @media(max-width:720px){.arika-v317-modal-backdrop{align-items:flex-end;padding:.7rem}.arika-v317-modal{border-radius:1.1rem;max-height:82vh}.arika-v317-real-item{font-size:.7rem}}
    `;
    document.head.appendChild(st);
  }
  function planId(item){ return safe(item && (item.rencanaId || item.rencana_id || item['rencana id'] || item.id)); }
  function planTitle(item){ return safe(item && (item.judul || item.rencanaJudul || item.title || item.catatan || 'Rencana kegiatan')); }
  function planDate(item){ return dateKey(item && (item.tanggal || item.date || item.tgl)); }
  function journalId(j){ return safe(j && (j.rencanaId || j.rencana_id || j['rencana id'] || j['Rencana ID'])); }
  function journalTitle(j){ return safe(j && (j.rencanaJudul || j.rencana_judul || j['rencana judul'] || j['Judul Rencana'])); }
  function journalDate(j){ return dateKey(j && (j.date || j.tanggal || j.Tanggal || j.tgl)); }
  function journalDesc(j){ return safe(j && (j.desc || j.deskripsi || j.Deskripsi || j.uraian || j.kegiatan)); }
  function journalCat(j){ return safe(j && (j.cat || j.kategori || j.Kategori || 'Jurnal')); }
  function journalStatus(j){ return safe(j && (j.status || j.Status || '')); }
  function journalName(j){ return safe(j && (j.name || j.nama || j.Pelaksana || j.pelaksana)); }
  function journalNip(j){ return safe(j && (j.nip || j.NIP)); }
  function currentUserKey(){
    var u = window.currentUser || {};
    return norm(u.nip || u.NIP || u.nama || u.name || '');
  }
  function sameUserJournal(j){
    var key = currentUserKey();
    if(!key) return true;
    var jk = norm(journalNip(j) || journalName(j));
    return !jk || jk === key || norm(journalName(j)) === norm((window.currentUser||{}).nama || (window.currentUser||{}).name || '');
  }
  function markerTitle(desc){
    var s = safe(desc);
    var m = s.match(/\[\s*REALISASI\s+RENCANA\s*:\s*([^\]]+)\]/i);
    return m ? safe(m[1]) : '';
  }
  function journalMatchesPlan(plan, journal){
    if(!sameUserJournal(journal)) return false;
    var pid = planId(plan);
    var jid = journalId(journal);
    if(pid && jid && pid === jid) return true;
    var pt = norm(planTitle(plan));
    var jt = norm(journalTitle(journal) || markerTitle(journalDesc(journal)));
    if(pt && jt && pt === jt){
      var pd = planDate(plan), jd = journalDate(journal);
      return !pd || !jd || pd === jd;
    }
    var descCompact = norm(journalDesc(journal));
    if(pt && descCompact && descCompact.indexOf(pt) >= 0){
      var pd2 = planDate(plan), jd2 = journalDate(journal);
      return !pd2 || !jd2 || pd2 === jd2;
    }
    return false;
  }
  function getRealizations(plan){
    var rows = Array.isArray(window.arikaData) ? window.arikaData : [];
    return rows.filter(function(j){ return journalMatchesPlan(plan, j); });
  }
  function getHomePlans(){
    var data = [];
    try { data = typeof getStableRencanaData === 'function' ? getStableRencanaData() : (window.rencanaData || []); } catch(_) { data = window.rencanaData || []; }
    try { data = (data || []).filter(function(item){ return typeof isCurrentUserRencana === 'function' ? isCurrentUserRencana(item) : true; }); } catch(_) {}
    try { data = (data || []).filter(function(item){ return typeof isRencanaSelesai === 'function' ? !isRencanaSelesai(item) : true; }); } catch(_) {}
    try { data = typeof dedupeRencanaList === 'function' ? dedupeRencanaList(data) : data; } catch(_) {}
    try {
      data = data.slice().sort(function(a,b){
        var rankA = typeof getRencanaSortRank === 'function' ? getRencanaSortRank(a) : 0;
        var rankB = typeof getRencanaSortRank === 'function' ? getRencanaSortRank(b) : 0;
        if(rankA !== rankB) return rankA - rankB;
        var da = safe(a.tanggal), db = safe(b.tanggal);
        if(da !== db) return da.localeCompare(db);
        var ja = typeof getRencanaJamReminder === 'function' ? getRencanaJamReminder(a) : '';
        var jb = typeof getRencanaJamReminder === 'function' ? getRencanaJamReminder(b) : '';
        return safe(ja || '99:99').localeCompare(safe(jb || '99:99'));
      });
    } catch(_) {}
    return data.slice(0,3);
  }
  function showRealizationModal(plan, rows){
    closeRealizationModal();
    injectStyle();
    var backdrop = document.createElement('div');
    backdrop.id = 'arika-v317-real-modal';
    backdrop.className = 'arika-v317-modal-backdrop';
    var items = rows.map(function(r){
      var meta = [journalDate(r), journalCat(r), journalStatus(r)].filter(Boolean).join(' • ');
      return '<div class="arika-v317-real-item"><div class="meta">'+esc(meta || 'Realisasi')+'</div><b>'+esc(journalTitle(r) || planTitle(plan))+'</b>'+esc(journalDesc(r) || '-').replace(/\n/g,'<br>')+'</div>';
    }).join('');
    backdrop.innerHTML = '<div class="arika-v317-modal"><div class="arika-v317-modal-head"><div><h3 class="arika-v317-modal-title">Realisasi Rencana</h3><div class="arika-v317-modal-sub">'+esc(planTitle(plan))+'</div></div><button type="button" class="arika-v317-modal-close" onclick="window.closeRencanaRealizationModalV317()">Tutup</button></div>'+items+'</div>';
    backdrop.addEventListener('click', function(ev){ if(ev.target === backdrop) closeRealizationModal(); });
    document.body.appendChild(backdrop);
  }
  function closeRealizationModal(){ var old = byId('arika-v317-real-modal'); if(old) old.remove(); }
  window.closeRencanaRealizationModalV317 = closeRealizationModal;

  function applyPlanStatus(){
    injectStyle();
    var listEl = byId('rencana-pribadi-list');
    if(!listEl || !window.currentUser || window.isAdmin) return;
    var cards = Array.prototype.slice.call(listEl.querySelectorAll(':scope > .p-4.rounded-2xl.border.relative, :scope > div.p-4.rounded-2xl.border.relative'));
    var plans = getHomePlans();
    if(!cards.length || !plans.length) return;
    cards.forEach(function(card, idx){
      var h = card.querySelector('h4');
      var title = norm(h ? h.textContent : '');
      var plan = plans.find(function(p){ return norm(planTitle(p)) === title; }) || plans[idx];
      if(!plan) return;
      var rows = getRealizations(plan);
      var done = rows.length > 0;
      var status = card.querySelector('.arika-rencana-real-status-v317');
      if(!status){
        status = document.createElement('div');
        status.className = 'arika-rencana-real-status-v317';
        var anchor = card.querySelector('p.text-\[10px\], p');
        if(anchor && anchor.parentNode === card) card.insertBefore(status, anchor.nextSibling);
        else card.appendChild(status);
      }
      status.classList.toggle('done', done);
      status.classList.toggle('waiting', !done);
      status.innerHTML = done ? 'Sudah Realisasi' : 'Belum Realisasi';

      var row = card.querySelector('.mt-3.flex.flex-wrap.gap-2');
      if(!row) return;
      var btns = Array.prototype.slice.call(row.querySelectorAll('button'));
      var realBtn = card.querySelector('.arika-btn-isi-realisasi') || btns.find(function(b){ return /isi\s+realisasi|lihat\s+realisasi/i.test(b.textContent || ''); });
      if(!realBtn) return;
      if(done){
        realBtn.textContent = 'Lihat Realisasi';
        realBtn.className = 'arika-btn-isi-realisasi arika-btn-lihat-realisasi-v317 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest';
        realBtn.onclick = function(ev){ ev.preventDefault(); ev.stopPropagation(); showRealizationModal(plan, rows); return false; };
      } else {
        realBtn.textContent = 'Isi Realisasi';
        realBtn.className = 'arika-btn-isi-realisasi px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 text-[9px] font-black uppercase tracking-widest';
        realBtn.onclick = function(ev){ ev.preventDefault(); ev.stopPropagation(); if(window.mulaiRealisasiRencana) window.mulaiRealisasiRencana(plan.id || planId(plan)); return false; };
      }
    });
  }

  window.applyRencanaRealizationStatusV317 = applyPlanStatus;
  var oldRender = window.renderRencanaPribadi;
  if(typeof oldRender === 'function' && !oldRender.__arikaV317Wrapped){
    window.renderRencanaPribadi = function(){
      var res = oldRender.apply(this, arguments);
      setTimeout(applyPlanStatus, 0);
      setTimeout(applyPlanStatus, 80);
      return res;
    };
    window.renderRencanaPribadi.__arikaV317Wrapped = true;
  }
  var oldFetch = window.fetchLiveJurnalFromSheet;
  if(typeof oldFetch === 'function' && !oldFetch.__arikaV317Wrapped){
    window.fetchLiveJurnalFromSheet = async function(){
      var res = await oldFetch.apply(this, arguments);
      setTimeout(applyPlanStatus, 200);
      return res;
    };
    window.fetchLiveJurnalFromSheet.__arikaV317Wrapped = true;
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(applyPlanStatus, 800); });
  else setTimeout(applyPlanStatus, 80);
  window.addEventListener('load', function(){ setTimeout(applyPlanStatus, 160); setTimeout(applyPlanStatus, 900); });
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(applyPlanStatus, 80);
  }, true);
})();


/* =========================================================
   v318 - Rencana Saya icon lebih netral dan status realisasi tidak mencolok
   Basis: v317. Patch hanya tampilan Rencana Saya di Beranda.
   ========================================================= */
(function(){
  if(window.__ARIKA_V318_RENCANA_ICON_SUBTLE__) return;
  window.__ARIKA_V318_RENCANA_ICON_SUBTLE__ = true;
  function injectStyle(){
    if(document.getElementById('arika-v318-rencana-subtle-style')) return;
    var st=document.createElement('style');
    st.id='arika-v318-rencana-subtle-style';
    st.textContent='\n#personal-reminder-card .home-section-icon{background:linear-gradient(135deg,#ecfeff,#f8fafc)!important;color:#0f766e!important;border:1px solid rgba(20,184,166,.18)!important;}\n#rencana-pribadi-list .arika-rencana-real-status-v317{font-size:7.5px!important;letter-spacing:.08em!important;padding:.28rem .5rem!important;box-shadow:none!important;}\n#rencana-pribadi-list .arika-rencana-real-status-v317.done{background:#f8fafc!important;border-color:#dbe3ee!important;color:#64748b!important;}\n#rencana-pribadi-list .arika-rencana-real-status-v317.done::before{content:"";display:inline-block;width:.38rem;height:.38rem;border-radius:999px;background:#10b981;margin-right:.12rem;}\n#rencana-pribadi-list .arika-rencana-real-status-v317.waiting{background:#fffbeb!important;border-color:#fde68a!important;color:#92400e!important;}\n#rencana-pribadi-list .arika-rencana-real-status-v317.waiting::before{content:"";display:inline-block;width:.38rem;height:.38rem;border-radius:999px;background:#f59e0b;margin-right:.12rem;}\n';
    document.head.appendChild(st);
  }
...
/* =========================================================
   v320 - Rencana Saya fast render
   Fokus: rencana baru, label WFH/WFA, status realisasi, dan tombol Isi Realisasi muncul lebih cepat.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V320_RENCANA_FAST_RENDER__) return;
  window.__ARIKA_V320_RENCANA_FAST_RENDER__ = true;

  function byId(id){ return document.getElementById(id); }
  function safe(v){ return String(v == null ? '' : v).trim(); }
  function norm(v){ return safe(v).toLowerCase().replace(/[^a-z0-9]+/g,''); }
  function isWfhText(v){ return /\bWFH\b|\bWFA\b|work\s*from\s*home|work\s*from\s*anywhere|\[wfh\/wfa\]/i.test(safe(v)); }
  function injectStyle(){
    if(byId('arika-v320-rencana-fast-style')) return;
    var st=document.createElement('style');
    st.id='arika-v320-rencana-fast-style';
    st.textContent=`
      #rencana-pribadi-list .arika-rencana-wfh-badge-v320{
        display:inline-flex!important;align-items:center!important;gap:.25rem!important;
        width:max-content!important;margin:.05rem 0 .42rem!important;padding:.26rem .52rem!important;
        border-radius:999px!important;border:1px solid #a5f3fc!important;background:#ecfeff!important;color:#0e7490!important;
        font-size:7.5px!important;font-weight:950!important;letter-spacing:.08em!important;text-transform:uppercase!important;
      }
      #rencana-pribadi-list .arika-rencana-fast-pulse-v320{animation:arikaV320FadeIn .22s ease-out both;}
      @keyframes arikaV320FadeIn{from{opacity:.35;transform:translateY(3px)}to{opacity:1;transform:none}}
    `;
    document.head.appendChild(st);
  }
  function getPlans(){
    var data=[];
    try { data = typeof getStableRencanaData === 'function' ? getStableRencanaData() : (window.rencanaData || []); } catch(_) { data = window.rencanaData || []; }
    try { data = (data||[]).filter(function(r){ return typeof isCurrentUserRencana === 'function' ? isCurrentUserRencana(r) : true; }); } catch(_) {}
    try { data = (data||[]).filter(function(r){ return typeof isRencanaSelesai === 'function' ? !isRencanaSelesai(r) : true; }); } catch(_) {}
    try { data = typeof dedupeRencanaList === 'function' ? dedupeRencanaList(data) : data; } catch(_) {}
    try {
      data = data.slice().sort(function(a,b){
        var ra = typeof getRencanaSortRank === 'function' ? getRencanaSortRank(a) : 0;
        var rb = typeof getRencanaSortRank === 'function' ? getRencanaSortRank(b) : 0;
        if(ra !== rb) return ra-rb;
        var da=safe(a.tanggal), db=safe(b.tanggal); if(da!==db) return da.localeCompare(db);
        var ja = typeof getRencanaJamReminder === 'function' ? getRencanaJamReminder(a) : '';
        var jb = typeof getRencanaJamReminder === 'function' ? getRencanaJamReminder(b) : '';
        return safe(ja||'99:99').localeCompare(safe(jb||'99:99'));
      });
  ...
  function addWfhBadges(){
    injectStyle();
    var list=byId('rencana-pribadi-list'); if(!list) return;
    var cards=Array.prototype.slice.call(list.querySelectorAll(':scope > .p-4.rounded-2xl.border.relative, :scope > div.p-4.rounded-2xl.border.relative'));
    var plans=getPlans();
    cards.forEach(function(card,idx){
      var titleEl=card.querySelector('h4');
      var titleNorm=norm(titleEl ? titleEl.textContent : '');
      var plan=plans.find(function(p){ return norm(p.judul || p.rencanaJudul || p.title || 'Rencana kegiatan') === titleNorm; }) || plans[idx];
      if(!plan) return;
      var text=[plan.judul, plan.catatan, plan.periode].filter(Boolean).join(' ');
      if(!isWfhText(text)) return;
      if(card.querySelector('.arika-rencana-wfh-badge-v320')) return;
      var badge=document.createElement('div');
      badge.className='arika-rencana-wfh-badge-v320';
      badge.textContent='🏡 WFH/WFA';
      var anchor=card.querySelector('.arika-rencana-real-status-v317') || card.querySelector('p.text-\\[10px\\]') || card.querySelector('p');
      if(anchor && anchor.parentNode===card) card.insertBefore(badge, anchor.nextSibling);
      else card.appendChild(badge);
    });
  }
  function ensureRealisasiButtons(){
    var list=byId('rencana-pribadi-list'); if(!list) return;
    var cards=Array.prototype.slice.call(list.querySelectorAll(':scope > .p-4.rounded-2xl.border.relative, :scope > div.p-4.rounded-2xl.border.relative'));
    var plans=getPlans();
    cards.forEach(function(card,idx){
      var row=card.querySelector('.mt-3.flex.flex-wrap.gap-2'); if(!row) return;
      if(row.querySelector('.arika-btn-isi-realisasi')) return;
      var plan=plans[idx]; if(!plan) return;
      var btn=document.createElement('button');
      btn.type='button';
      btn.className='arika-btn-isi-realisasi px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 text-[9px] font-black uppercase tracking-widest';
      btn.textContent='Isi Realisasi';
      btn.onclick=function(ev){ ev.preventDefault(); ev.stopPropagation(); if(window.mulaiRealisasiRencana) window.mulaiRealisasiRencana(plan.id || plan.rencanaId); return false; };
      row.appendChild(btn);
    });
  }
  function decorate(){
    injectStyle();
    addWfhBadges();
    ensureRealisasiButtons();
    try { window.applyRencanaRealizationStatusV317 && window.applyRencanaRealizationStatusV317(); } catch(_) {}
  }
  window.applyRencanaFastDecorationsV320 = decorate;
  function schedule(){
    clearTimeout(window.__arikaV320DecorTimer);
    decorate();
    window.__arikaV320DecorTimer=setTimeout(decorate,60);
  }
  var oldRender=window.renderRencanaPribadi;
  if(typeof oldRender === 'function' && !oldRender.__arikaV320FastDecor){
    window.renderRencanaPribadi=function(){
      var res=oldRender.apply(this,arguments);
      schedule();
      return res;
    };
    window.renderRencanaPribadi.__arikaV320FastDecor=true;
  }
  var oldSave=window.simpanRencanaPribadi;
  if(typeof oldSave === 'function' && !oldSave.__arikaV320Fast){
    window.simpanRencanaPribadi=async function(){
    ...
    window.simpanRencanaPribadi.__arikaV320Fast=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(schedule,120); });
  else setTimeout(schedule,80);
  window.addEventListener('load', function(){ setTimeout(schedule,180); });
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(schedule,80); }, true);
})();

/* =========================================================
   v321 - Rencana Saya instant create/delete feedback
   Basis: v320. Patch hanya untuk mempercepat respons tampilan Beranda saat rencana dibuat/dihapus.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V321_RENCANA_INSTANT_FEEDBACK__) return;
  window.__ARIKA_V321_RENCANA_INSTANT_FEEDBACK__ = true;

  function byId(id){ return document.getElementById(id); }
  function safe(v){ return String(v == null ? '' : v).trim(); }
  function esc(v){ return safe(v).replace(/[&<>"']/g, function(c){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]; }); }
  function today(){ try { return typeof getTodayKey === 'function' ? getTodayKey() : new Date().toLocaleDateString('en-CA'); } catch(_) { return new Date().toLocaleDateString('en-CA'); } }
  function readDate(){ var el=byId('rencana-tanggal'); return safe(el && (el.dataset && el.dataset.arikaIsoValue || el.value)) || today(); }
  function readJam(){ var el=byId('rencana-jam'); return safe(el && el.value); }
  function readJudul(){ var el=byId('rencana-judul'); return safe(el && el.value); }
  function readCatatan(){ var el=byId('rencana-catatan'); return safe(el && el.value); }
  function formatDate(v){ try { return typeof formatHariTanggal === 'function' ? formatHariTanggal(v) : safe(v); } catch(_) { return safe(v); } }

  function injectStyle(){
    if(byId('arika-v321-rencana-instant-style')) return;
    var st=document.createElement('style');
    st.id='arika-v321-rencana-instant-style';
    st.textContent=`
      #rencana-pribadi-list .arika-v321-pending-card{animation:arikaV321In .16s ease-out both;border:1px solid #a7f3d0!important;background:#ecfdf5!important;color:#065f46!important;}
      #rencana-pribadi-list .arika-v321-deleting-card{opacity:.28!important;transform:scale(.985)!important;pointer-events:none!important;transition:.12s ease!important;}
      #rencana-pribadi-list .arika-v321-pending-label{display:inline-flex;align-items:center;gap:.25rem;width:max-content;margin:.05rem 0 .42rem;padding:.26rem .52rem;border-radius:999px;border:1px solid #a7f3d0;background:#f0fdf4;color:#047857;font-size:7.5px;font-weight:950;letter-spacing:.08em;text-transform:uppercase;}
      @keyframes arikaV321In{from{opacity:.3;transform:translateY(4px)}to{opacity:1;transform:none}}
    `;
    document.head.appendChild(st);
  }
  ...
  function findCards(){
    var list=byId('rencana-pribadi-list');
    if(!list) return [];
    return Array.prototype.slice.call(list.querySelectorAll(':scope > .p-4.rounded-2xl.border.relative, :scope > div.p-4.rounded-2xl.border.relative, :scope > .arika-v321-pending-card'));
  }
  function cardMatchesId(card, id){
    if(!card || !id) return false;
    var btn = card.querySelector('button[onclick*="'+String(id).replace(/"/g,'')+'"]');
    return !!btn || safe(card.dataset.rencanaId) === safe(id);
  }
  function forceDecorate(){
    try { if(window.applyRencanaFastDecorationsV320) window.applyRencanaFastDecorationsV320(); } catch(_) {}
    try { if(window.applyRencanaRealizationStatusV317) window.applyRencanaRealizationStatusV317(); } catch(_) {}
  }
  function quickRefresh(){
    clearTimeout(window.__arikaV321RefreshTimer);
    try { if(window.renderRencanaPribadi) window.renderRencanaPribadi({ fromV321Instant:true }); } catch(_) {}
    forceDecorate();
    window.__arikaV321RefreshTimer = setTimeout(function(){ try { window.renderRencanaPribadi && window.renderRencanaPribadi({ fromV321InstantLater:true }); } catch(_) {} forceDecorate(); }, 90);
  }

  function ensurePendingCreateCard(){
    injectStyle();
    var list=byId('rencana-pribadi-list');
    if(!list || !window.currentUser || window.isAdmin) return;
    var judul=readJudul();
    var jam=readJam();
    if(!judul || !jam) return;
    var tanggal=readDate();
    var catatan=readCatatan();
    var id='v321-pending-'+Date.now();
    Array.prototype.slice.call(list.querySelectorAll('.arika-v321-pending-card')).forEach(function(old){ old.remove(); });
    var empty=list.querySelector('.text-center.text-slate-400');
    if(empty && list.children.length===1) list.innerHTML='';
    var card=document.createElement('div');
    card.className='p-4 rounded-2xl border relative overflow-hidden arika-v321-pending-card';
    card.dataset.rencanaId=id;
    card.innerHTML='<div class="flex justify-between items-start gap-3 mb-2"><div class="min-w-0"><span class="inline-flex items-center gap-1 mb-2 text-[8px] font-black uppercase tracking-widest">⏰ '+esc(jam)+' • Baru dibuat</span><h4 class="font-black text-slate-900 text-xs uppercase leading-snug">'+esc(judul)+'</h4></div><span class="text-[9px] font-black uppercase tracking-widest text-emerald-700">Menyimpan...</span></div><p class="text-[10px] font-bold opacity-80 mb-2">'+esc(formatDate(tanggal))+' • Pukul '+esc(jam)+'</p>'+(catatan?'<p class="text-xs text-slate-600 leading-relaxed whitespace-pre-line">'+esc(catatan)+'</p>':'')+'<div class="arika-v321-pending-label">Tampil langsung • sinkron berjalan</div>';
    // Sisipkan setelah summary 3 KPI bila ada; kalau tidak ada, di paling atas.
    var firstRealCard=findCards()[0];
    if(firstRealCard) list.insertBefore(card, firstRealCard);
    else list.insertBefore(card, list.firstChild);
  }

  // Saat form rencana disubmit, tampilkan kartu lokal seketika sebelum proses sinkron lama berjalan.
  document.addEventListener('submit', function(ev){
    var form=ev.target;
    if(form && form.id === 'form-rencana-pribadi'){
      setTimeout(ensurePendingCreateCard, 0);
      setTimeout(forceDecorate, 30);
    }
  }, true);

  // Wrap save agar sesudah fungsi lama berjalan, tampilan dikuatkan lagi cepat.
  function wrapSave(){
    if(typeof window.simpanRencanaPribadi !== 'function' || window.simpanRencanaPribadi.__arikaV321Wrapped) return;
    var old=window.simpanRencanaPribadi;
    window.simpanRencanaPribadi=function(){
      ensurePendingCreateCard();
      var res=old.apply(this, arguments);
      setTimeout(function(){ quickRefresh(); }, 20);
      setTimeout(function(){ quickRefresh(); }, 180);
      return res;
    };
    window.simpanRencanaPribadi.__arikaV321Wrapped=true;
  }

  var pendingDeleteId='';
  function hideDeletedCard(id){
    if(!id) return;
    findCards().forEach(function(card){
      if(cardMatchesId(card,id)){
        card.classList.add('arika-v321-deleting-card');
        setTimeout(function(){ try { card.remove(); } catch(_) {} }, 80);
      }
    });
  }
  function wrapDelete(){
    if(typeof window.hapusRencanaPribadi !== 'function' || window.hapusRencanaPribadi.__arikaV321Wrapped) return;
    var old=window.hapusRencanaPribadi;
    window.hapusRencanaPribadi=function(id){
      pendingDeleteId=safe(id);
      return old.apply(this, arguments);
    };
    window.hapusRencanaPribadi.__arikaV321Wrapped=true;
  }
  function wrapConfirm(){
    if(typeof window.showCustomConfirm !== 'function' || window.showCustomConfirm.__arikaV321Wrapped) return;
    var old=window.showCustomConfirm;
    window.showCustomConfirm=function(message, onOk){
      var isDelete=/hapus\s+reminder|hapus\s+rencana/i.test(safe(message));
      if(isDelete && typeof onOk === 'function'){
        var idAtConfirm=pendingDeleteId;
        var wrapped=function(){
          hideDeletedCard(idAtConfirm);
          setTimeout(function(){ hideDeletedCard(idAtConfirm); quickRefresh(); }, 40);
          return onOk.apply(this, arguments);
        };
        return old.apply(this, [message, wrapped].concat(Array.prototype.slice.call(arguments,2)));
      }
      return old.apply(this, arguments);
    };
    window.showCustomConfirm.__arikaV321Wrapped=true;
  }

  function install(){ injectStyle(); wrapSave(); wrapDelete(); wrapConfirm(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', install); else install();
  window.addEventListener('load', function(){ setTimeout(install, 80); });
})();

/* =========================================================
   v322 - Rencana Saya: status realisasi langsung terbaca setelah simpan jurnal
   Basis: v321. Patch hanya untuk Rencana Saya di Beranda.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V322_RENCANA_REALISASI_INSTANT_STATUS__) return;
  window.__ARIKA_V322_RENCANA_REALISASI_INSTANT_STATUS__ = true;

  var STORE_KEY = 'arika_v322_local_realized_plans';
  function byId(id){ return document.getElementById(id); }
  function safe(v){ return String(v == null ? '' : v).trim(); }
  function esc(v){ return safe(v).replace(/[&<>"']/g, function(c){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]; }); }
  function norm(v){ return safe(v).toLowerCase().replace(/[^a-z0-9]+/g,''); }
  ...
  function todayKey(){ try { return typeof getTodayKey === 'function' ? getTodayKey() : new Date().toLocaleDateString('en-CA'); } catch(_) { return new Date().toLocaleDateString('en-CA'); } }
  function currentName(){ return safe((window.currentUser||{}).nama || (window.currentUser||{}).name); }
  function currentNip(){ return safe((window.currentUser||{}).nip || (window.currentUser||{}).NIP); }
  function currentKey(){ return norm(currentNip() || currentName()); }
  function sameCurrentUser(j){
    var ck=currentKey(); if(!ck) return true;
    var jk=norm(j.nip || j.NIP || j.name || j.nama || j.Pelaksana || j.pelaksana);
    return !jk || jk===ck || norm(j.name || j.nama || j.Pelaksana || j.pelaksana)===norm(currentName());
  }
  function planId(p){ return safe(p && (p.rencanaId || p.rencana_id || p['rencana id'] || p.id)); }
  function planTitle(p){ return safe(p && (p.judul || p.rencanaJudul || p.rencana_judul || p.title || p.catatan || 'Rencana kegiatan')); }
  function planDate(p){ return dateKey(p && (p.tanggal || p.date || p.tgl)); }
  function journalId(j){ return safe(j && (j.rencanaId || j.rencana_id || j['rencana id'] || j['Rencana ID'])); }
  function journalTitle(j){ return safe(j && (j.rencanaJudul || j.rencana_judul || j['rencana judul'] || j['Judul Rencana'])); }
  function journalDate(j){ return dateKey(j && (j.date || j.tanggal || j.Tanggal || j.tgl)); }
  function journalDesc(j){ return safe(j && (j.desc || j.deskripsi || j.Deskripsi || j.uraian || j.kegiatan)); }
  ...
  function markerTitle(desc){
    ...
    var marker='[REALISASI RENCANA:';
    var pos=upper.indexOf(marker);
    if(pos<0) return '';
    var part=s.slice(pos+marker.length).split(/\r?\n/)[0].trim();
    // Hapus hanya bracket penutup paling akhir agar judul seperti [WFH/WFA] tetap utuh.
    part=part.replace(/\]\s*$/,'').trim();
    return part;
  }
  function journalMatchesPlan(plan,journal){
    if(!sameCurrentUser(journal)) return false;
    var pid=planId(plan), jid=journalId(journal);
    if(pid && jid && pid===jid) return true;
    var pt=norm(planTitle(plan));
    var jt=norm(journalTitle(journal) || markerTitle(journalDesc(journal)));
    if(pt && jt && (pt===jt || pt.indexOf(jt)>=0 || jt.indexOf(pt)>=0)){
      var pd=planDate(plan), jd=journalDate(journal);
      return !pd || !jd || pd===jd;
    }
    var dc=norm(journalDesc(journal));
    if(pt && dc && dc.indexOf(pt)>=0){
      var pd2=planDate(plan), jd2=journalDate(journal);
      return !pd2 || !jd2 || pd2===jd2;
    }
    return false;
  }
  function loadLocalRealizations(){
    try { var arr=JSON.parse(localStorage.getItem(STORE_KEY)||'[]'); return Array.isArray(arr)?arr:[]; } catch(_) { return []; }
  }
  function saveLocalRealization(row){
    if(!row) return;
    var arr=loadLocalRealizations().filter(function(x){ return safe(x.id)!==safe(row.id) && !(safe(x.rencanaId) && safe(row.rencanaId) && safe(x.rencanaId)===safe(row.rencanaId) && safe(x.date)===safe(row.date)); });
    arr.unshift(row);
    arr=arr.slice(0,80);
  ...
  function getAllJournals(){
    var rows=[];
    try { if(Array.isArray(window.arikaData)) rows=rows.concat(window.arikaData); } catch(_) {}
    rows=rows.concat(loadLocalRealizations());
    var seen={};
    return rows.filter(function(r){ var k=safe(r.id)||[safe(r.rencanaId),safe(r.date),safe(r.desc)].join('|'); if(seen[k]) return false; seen[k]=true; return true; });
  }
  function getRealizations(plan){ return getAllJournals().filter(function(j){ return journalMatchesPlan(plan,j); }); }
  function getPlans(){
    var data=[];
    try { data=typeof getStableRencanaData==='function' ? getStableRencanaData() : (window.rencanaData||[]); } catch(_) { data=window.rencanaData||[]; }
    try { data=(data||[]).filter(function(r){ return typeof isCurrentUserRencana==='function' ? isCurrentUserRencana(r) : true; }); } catch(_) {}
  ...
  function restoreLink(){
    if(window.arikaLinkedRencana && window.arikaLinkedRencana.rencanaId) return window.arikaLinkedRencana;
    try {
      var raw=sessionStorage.getItem('arika_linked_rencana_v297') || sessionStorage.getItem('arika_linked_rencana_v301') || localStorage.getItem('arika_linked_rencana_v301');
      var obj=raw?JSON.parse(raw):null;
      if(obj && obj.rencanaId){ window.arikaLinkedRencana=obj; return obj; }
    } catch(_) {}
    return null;
  }
  function findPlanFromForm(){
    var link=restoreLink();
    var plans=[];
    try { plans=typeof getStableRencanaData==='function'?getStableRencanaData():(window.rencanaData||[]); } catch(_) { plans=window.rencanaData||[]; }
    var desc=byId('in-desc')?byId('in-desc').value:'';
    var title=link && link.rencanaJudul ? link.rencanaJudul : markerTitle(desc);
    var rid=link && link.rencanaId ? link.rencanaId : '';
    var d=dateKey(byId('in-date')?byId('in-date').value:'') || todayKey();
    var ck=currentKey();
    return (plans||[]).find(function(p){
      var owner=norm(p.ownerNip || p.nip || p.ownerName || p.nama || p.name);
      var userOk=!ck || !owner || owner===ck || norm(p.ownerName||p.nama||p.name)===norm(currentName());
      if(!userOk) return false;
      if(rid && planId(p)===rid) return true;
      return title && norm(planTitle(p))===norm(title) && (!planDate(p) || planDate(p)===d);
    }) || null;
  }
  function buildLocalJournalFromForm(plan){
    var link=restoreLink();
    var d=dateKey(byId('in-date')?byId('in-date').value:'') || todayKey();
    var desc=safe(byId('in-desc')?byId('in-desc').value:'');
    if(!desc) return null;
    var rid=(link && link.rencanaId) || planId(plan) || '';
    var title=(link && link.rencanaJudul) || planTitle(plan) || markerTitle(desc);
    if(!rid && !title) return null;
    return {
      id:'v322-local-real-'+Date.now(),
      date:d,
      name:currentName(),
      nip:currentNip(),
      cat:safe(byId('in-cat')?byId('in-cat').value:'') || 'Jurnal',
      desc:desc,
      status:safe(byId('in-status')?byId('in-status').value:'') || 'Selesai',
      rencanaId:rid,
      rencanaJudul:title,
      syncStatus:'pending-local-v322',
      createdAt:new Date().toISOString()
    };
  }
  function showModal(plan, rows){
    closeModal();
    var back=document.createElement('div'); back.id='arika-v322-real-modal'; back.className='arika-v317-modal-backdrop';
    var items=rows.map(function(r){
      var meta=[journalDate(r),journalCat(r),journalStatus(r)].filter(Boolean).join(' • ');
      return '<div class="arika-v317-real-item"><div class="meta">'+esc(meta||'Realisasi')+'</div><b>'+esc(journalTitle(r)||planTitle(plan))+'</b>'+esc(journalDesc(r)||'-').replace(/\n/g,'<br>')+'</div>';
    }).join('');
    back.innerHTML='<div class="arika-v317-modal"><div class="arika-v317-modal-head"><div><h3 class="arika-v317-modal-title">Realisasi Rencana</h3><div class="arika-v317-modal-sub">'+esc(planTitle(plan))+'</div></div><button type="button" class="arika-v317-modal-close" onclick="window.closeRencanaRealizationModalV322()">Tutup</button></div>'+items+'</div>';
    back.addEventListener('click', function(ev){ if(ev.target===back) closeModal(); });
    document.body.appendChild(back);
  }
  function closeModal(){ var old=byId('arika-v322-real-modal'); if(old) old.remove(); }
  window.closeRencanaRealizationModalV322=closeModal;

  function decorate(){
    var list=byId('rencana-pribadi-list');
    if(!list || !window.currentUser || window.isAdmin) return;
    var cards=Array.prototype.slice.call(list.querySelectorAll(':scope > .p-4.rounded-2xl.border.relative, :scope > div.p-4.rounded-2xl.border.relative'));
    var plans=getPlans(); if(!cards.length || !plans.length) return;
    cards.forEach(function(card,idx){
      var h=card.querySelector('h4'); var ht=norm(h?h.textContent:'');
      var plan=plans.find(function(p){ return norm(planTitle(p))===ht; }) || plans[idx];
      if(!plan) return;
      var rows=getRealizations(plan);
      var done=rows.length>0;
      var status=card.querySelector('.arika-rencana-real-status-v317');
      if(!status){
        ...
        status.className='arika-rencana-real-status-v317';
        var anchor=card.querySelector('p.text-\\[10px\\], p');
        if(anchor && anchor.parentNode===card) card.insertBefore(status, anchor.nextSibling); else card.appendChild(status);
      }
      status.classList.toggle('done',done); status.classList.toggle('waiting',!done);
      status.innerHTML=done?'Sudah Realisasi':'Belum Realisasi';
      var row=card.querySelector('.mt-3.flex.flex-wrap.gap-2'); if(!row) return;
      var btns=Array.prototype.slice.call(row.querySelectorAll('button'));
      var realBtn=card.querySelector('.arika-btn-isi-realisasi') || btns.find(function(b){ return /isi\s+realisasi|lihat\s+realisasi/i.test(b.textContent||''); });
      ...
      if(done){
        realBtn.textContent='Lihat Realisasi';
        realBtn.className='arika-btn-isi-realisasi arika-btn-lihat-realisasi-v317 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest';
        realBtn.onclick=function(ev){ ev.preventDefault(); ev.stopPropagation(); showModal(plan,rows); return false; };
      } else {
        realBtn.textContent='Isi Realisasi';
        realBtn.className='arika-btn-isi-realisasi px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 text-[9px] font-black uppercase tracking-widest';
        realBtn.onclick=function(ev){ ev.preventDefault(); ev.stopPropagation(); if(window.mulaiRealisasiRencana) window.mulaiRealisasiRencana(plan.id || planId(plan)); return false; };
  ...
  window.applyRencanaRealizationStatusV322=decorate;
  function schedule(){
    try { decorate(); } catch(_) {}
    clearTimeout(window.__arikaV322DecorTimer);
    window.__arikaV322DecorTimer=setTimeout(function(){ try { decorate(); } catch(_) {} },80);
  }

  // Tangkap submit jurnal realisasi dan tandai rencana sebagai terealisasi secara lokal segera.
  function installSubmitCapture(){
    var form=byId('form-arika');
    if(!form || form.__arikaV322SubmitCapture) return;
    form.__arikaV322SubmitCapture=true;
    form.addEventListener('submit', function(){
      var plan=findPlanFromForm();
      var row=buildLocalJournalFromForm(plan);
      if(row){
        if(!Array.isArray(window.arikaData)) window.arikaData=[];
        var exists=window.arikaData.some(function(x){ return safe(x.id)===safe(row.id) || (row.rencanaId && safe(x.rencanaId)===row.rencanaId && dateKey(x.date||x.tanggal)===row.date); });
        if(!exists) window.arikaData.unshift(row);
        saveLocalRealization(row);
        try { saveLocalCoreCache && saveLocalCoreCache(); } catch(_) {}
        setTimeout(schedule,40); setTimeout(schedule,250); setTimeout(schedule,900);
      }
    }, true);
  }
  var oldFetch=window.fetchLiveJurnalFromSheet;
  if(typeof oldFetch==='function' && !oldFetch.__arikaV322Wrapped){
    window.fetchLiveJurnalFromSheet=async function(){
      var res=await oldFetch.apply(this,arguments);
      setTimeout(schedule,120); setTimeout(schedule,500);
      return res;
    };
    window.fetchLiveJurnalFromSheet.__arikaV322Wrapped=true;
  }
  var oldRender=window.renderRencanaPribadi;
  if(typeof oldRender==='function' && !oldRender.__arikaV322Wrapped){
    window.renderRencanaPribadi=function(){ var res=oldRender.apply(this,arguments); schedule(); return res; };
    window.renderRencanaPribadi.__arikaV322Wrapped=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ installSubmitCapture(); setTimeout(schedule,200); });
  else { installSubmitCapture(); setTimeout(schedule,100); }
  window.addEventListener('load', function(){ installSubmitCapture(); setTimeout(schedule,180); setTimeout(schedule,900); });
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(schedule,100); }, true);
})();


/* =========================================================
   v324 - Istilah tampilan: Google Sheet/Sheet -> database
   Catatan: nama fungsi internal tetap tidak diubah agar sistem tidak rusak.
   ========================================================= */
(function(){
  if(window.__ARIKA_V324_DATABASE_TERMS__) return;
  window.__ARIKA_V324_DATABASE_TERMS__ = true;
  function cleanTextNode(node){
    if(!node || node.nodeType !== 3 || !node.nodeValue) return;
    var text = node.nodeValue;
    var next = text
      .replace(/Google Sheets/g, 'database')
      .replace(/Google Sheet/g, 'database')
      .replace(/Sheet Pegawai/g, 'database pegawai')
      .replace(/Sheet Jurnal/g, 'database jurnal')
      .replace(/Sinkron Sheet/g, 'Sinkron');
    if(next !== text) node.nodeValue = next;
  }
  function cleanElement(root){
    try{
      var walker = document.createTreeWalker(root || document.body, NodeFilter.SHOW_TEXT, null);
      var n; while((n = walker.nextNode())) cleanTextNode(n);
  ...
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ cleanElement(document.body); });
  else cleanElement(document.body);
  window.addEventListener('load', function(){ cleanElement(document.body); });
  document.addEventListener('click', function(){ setTimeout(function(){ cleanElement(document.body); }, 120); }, true);
})();

/* =========================================================
   v325 - Riwayat Jurnal: cegah duplikasi sementara setelah simpan realisasi rencana
   Basis: v324. Patch hanya membersihkan data lokal sementara; data database tidak diubah.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V325_RIWAYAT_NO_DUP_REALISASI__) return;
  window.__ARIKA_V325_RIWAYAT_NO_DUP_REALISASI__ = true;
  ...
  function markerTitle(desc){
    var s=safe(desc); var up=s.toUpperCase(); var marker='[REALISASI RENCANA:'; var pos=up.indexOf(marker);
    if(pos<0) return '';
    return s.slice(pos+marker.length).split(/\r?\n/)[0].replace(/\]\s*$/,'').trim();
  }
  function descOf(r){ return safe(r && (r.desc || r.Deskripsi || r.deskripsi || r.uraian || r.kegiatan)); }
  function titleOf(r){ return safe(r && (r.rencanaJudul || r.rencana_judul || r['rencana judul'] || r['Judul Rencana'] || markerTitle(descOf(r)))); }
  function idOf(r){ return safe(r && (r.rencanaId || r.rencana_id || r['rencana id'] || r['Rencana ID'])); }
  function dateOf(r){ return dateKey(r && (r.date || r.tanggal || r.Tanggal || r.tgl)); }
  function nameOf(r){ return safe(r && (r.nip || r.NIP || r.name || r.nama || r.Pelaksana || r.pelaksana)); }
  function isRencanaRealisasiRow(r){
    if(!r) return false;
    if(idOf(r) || titleOf(r)) return true;
    return /\[\s*REALISASI\s+RENCANA\s*:/i.test(descOf(r));
  }
  function dedupeKey(r){
    if(!isRencanaRealisasiRow(r)) return '';
    var rid=idOf(r);
    var title=titleOf(r);
    var date=dateOf(r);
    var person=norm(nameOf(r));
    // rencanaId adalah kunci utama. Jika belum ada, fallback ke judul marker + tanggal + pegawai.
    if(rid) return ['rencana', rid, date, person].join('|');
    if(title) return ['judul', norm(title), date, person].join('|');
    return '';
  }
  function quality(r){
    var q=0;
    var id=safe(r && r.id);
    var sync=safe(r && r.syncStatus).toLowerCase();
    if(id && !/^v322-local-real-/i.test(id)) q+=20;
    if(!/pending|local/.test(sync)) q+=15;
    if(idOf(r)) q+=12;
    if(titleOf(r)) q+=8;
    if(safe(r && (r.linkDataDukung || r.link_data_dukung || r['Link Data Dukung']))) q+=4;
    if(safe(r && (r.createdAt || r.Timestamp || r.timestamp))) q+=2;
    return q;
  }
  function chooseBetter(a,b){ return quality(b) > quality(a) ? b : a; }
  function dedupeRows(rows){
    ...
    rows.forEach(function(row){
      ...
      if(!seen[key]) { seen[key]=row; out.push(row); return; }
      var old=seen[key]; var chosen=chooseBetter(old,row);
      if(chosen!==old){
        seen[key]=chosen;
        var idx=out.indexOf(old);
        if(idx>=0) out[idx]=chosen;
      }
    });
    return out;
  }
  function applyDedupeAndRender(reason){
    var changed=false;
    try {
      if(Array.isArray(window.arikaData)) {
        var before=window.arikaData.length;
        window.arikaData=dedupeRows(window.arikaData);
        changed=changed || before!==window.arikaData.length;
      }
    } catch(_) {}
    try {
      if(typeof filtered !== 'undefined' && Array.isArray(filtered)) {
        var beforeF=filtered.length;
        filtered=dedupeRows(filtered);
        changed=changed || beforeF!==filtered.length;
      }
    } catch(_) {}
    if(changed){
      try { if(typeof saveLocalCoreCache === 'function') saveLocalCoreCache(); } catch(_) {}
      try { if(typeof window.runFilter === 'function') window.runFilter(); } catch(_) {}
      try { if(typeof window.renderVisualCalendar === 'function') window.renderVisualCalendar(); } catch(_) {}
      try { if(typeof window.renderRencanaPribadi === 'function') window.renderRencanaPribadi(); } catch(_) {}
    }
    return changed;
  }
  window.arikaDedupeRealisasiRencanaRowsV325=applyDedupeAndRender;
  function scheduleBurst(reason){
    [40,120,300,700,1300,2500,4200].forEach(function(ms){ setTimeout(function(){ applyDedupeAndRender(reason); }, ms); });
  }

  // Setelah submit jurnal, payload lokal dan row lokal penanda realisasi bisa masuk bersamaan.
  // Burst kecil ini membersihkan duplikasi sebelum user melihat riwayat terlalu lama.
  function installSubmitWatcher(){
    var form=document.getElementById('form-arika');
    if(!form || form.__arikaV325NoDuplicateSubmit) return;
    form.__arikaV325NoDuplicateSubmit=true;
    form.addEventListener('submit', function(){ scheduleBurst('submit-jurnal'); }, false);
  }

  var oldFetch=window.fetchLiveJurnalFromSheet;
  if(typeof oldFetch==='function' && !oldFetch.__arikaV325Wrapped){
    window.fetchLiveJurnalFromSheet=async function(){
      var res=await oldFetch.apply(this,arguments);
      scheduleBurst('fetch-live');
      return res;
    };
    window.fetchLiveJurnalFromSheet.__arikaV325Wrapped=true;
  }
  var oldRunFilter=window.runFilter;
  if(typeof oldRunFilter==='function' && !oldRunFilter.__arikaV325Wrapped){
    window.runFilter=function(){
      try {
        if(Array.isArray(window.arikaData)) window.arikaData=dedupeRows(window.arikaData);
        if(typeof filtered !== 'undefined' && Array.isArray(filtered)) filtered=dedupeRows(filtered);
      } catch(_) {}
      return oldRunFilter.apply(this,arguments);
    };
    window.runFilter.__arikaV325Wrapped=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ installSubmitWatcher(); scheduleBurst('dom'); });
  else { installSubmitWatcher(); scheduleBurst('now'); }
  window.addEventListener('load', function(){ installSubmitWatcher(); scheduleBurst('load'); });
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && (ev.target.closest('#btn-rekap') || ev.target.closest('[onclick*="rekap"]'))) {
      scheduleBurst('nav-riwayat');
    }
  }, true);
})();

/* =========================================================
   v326 - Bersihkan penanda realisasi terpilih setelah rencana terpenuhi
   Basis: v325. Patch hanya untuk kartu "Realisasi untuk rencana terpilih" di tab Isi Jurnal.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V326_CLEAR_FULFILLED_LINK__) return;
  window.__ARIKA_V326_CLEAR_FULFILLED_LINK__ = true;
  ...
  function dateKey(v){
    try { if(typeof normalizeDateKeyFromSheet === 'function') return normalizeDateKeyFromSheet(v || '') || ''; } catch(_) {}
    var s=safe(v); var m=s.match(/^(\d{4})[-\/](\d{1,2})[-\/](\d{1,2})/); if(m) return m[1]+'-'+String(m[2]).padStart(2,'0')+'-'+String(m[3]).padStart(2,'0');
    return s.split('T')[0] || '';
  }
  function descOf(j){ return safe(j && (j.desc || j.Deskripsi || j.deskripsi || j.uraian || j.kegiatan || '')); }
  function titleOf(j){ return safe(j && (j.rencanaJudul || j.rencana_judul || j['rencana judul'] || j['Judul Rencana'] || '')); }
  function ridOf(j){ return safe(j && (j.rencanaId || j.rencana_id || j['rencana id'] || j['Rencana ID'] || '')); }
  function markerTitle(desc){
    var m=safe(desc).match(/\[\s*REALISASI\s+RENCANA\s*:\s*([^\]]+)\]/i);
    return m ? safe(m[1]) : '';
  }
  function currentLink(){
    if(window.arikaLinkedRencana && window.arikaLinkedRencana.rencanaId) return window.arikaLinkedRencana;
    var raw='';
    try { raw=sessionStorage.getItem('arika_linked_rencana_v326') || sessionStorage.getItem('arika_linked_rencana_v297') || sessionStorage.getItem('arika_linked_rencana_v301') || localStorage.getItem('arika_linked_rencana_v301') || localStorage.getItem('arika_linked_rencana_v326') || ''; } catch(_) {}
    if(raw){ try { var o=JSON.parse(raw); if(o && (o.rencanaId || o.rencanaJudul)) return o; } catch(_) {} }
    return null;
  }
  function clearStores(){
    try {
      ['arika_linked_rencana_v297','arika_linked_rencana_v301','arika_linked_rencana_v326'].forEach(function(k){ sessionStorage.removeItem(k); });
      ['arika_linked_rencana_v301','arika_linked_rencana_v326'].forEach(function(k){ localStorage.removeItem(k); });
    } catch(_) {}
  }
  function hideCard(){
    var card=byId('jurnal-linked-rencana-card');
  ...
  function clearLink(reason){
    window.arikaLinkedRencana = null;
    clearStores();
    hideCard();
    try { if(typeof window.updateLinkedRencanaCard === 'function') window.updateLinkedRencanaCard(); } catch(_) { hideCard(); }
    window.__ARIKA_V326_LAST_CLEAR_REASON__ = reason || 'clear';
  }
  function isFulfilled(link){
    if(!link) return false;
    var rid=safe(link.rencanaId), title=safe(link.rencanaJudul), tanggal=dateKey(link.tanggal || '');
    var rows=Array.isArray(window.arikaData) ? window.arikaData : [];
    if(!rows.length) return false;
    return rows.some(function(j){
      var jid=ridOf(j), jt=titleOf(j) || markerTitle(descOf(j));
      var jd=dateKey(j.date || j.tanggal || j.Tanggal || '');
      if(rid && jid && rid===jid) return true;
      if(title && jt && norm(title)===norm(jt) && (!tanggal || !jd || tanggal===jd)) return true;
      return false;
    });
  }
  function clearIfFulfilled(reason){
    var link=currentLink();
    if(link && isFulfilled(link)) { clearLink(reason || 'fulfilled'); return true; }
    return false;
  }

  // Jangan tampilkan lagi kartu realisasi terpilih apabila rencana tersebut sudah punya jurnal terkait.
  var oldUpdate = window.updateLinkedRencanaCard;
  if(typeof oldUpdate === 'function' && !oldUpdate.__arikaV326Wrapped){
    window.updateLinkedRencanaCard = function(){
      if(clearIfFulfilled('before-card-render')) return;
      return oldUpdate.apply(this, arguments);
    };
    window.updateLinkedRencanaCard.__arikaV326Wrapped = true;
  }

  // Sesudah jurnal berhasil disimpan, bersihkan konteks realisasi agar form Isi Jurnal kembali menjadi jurnal biasa.
  function installSubmitCleaner(){
    var form=byId('form-arika');
    if(!form || form.__arikaV326SubmitCleaner) return;
    form.__arikaV326SubmitCleaner = true;
    form.addEventListener('submit', function(){
      var linkAtSubmit=currentLink();
      if(!linkAtSubmit) return;
      var tries=0;
      var timer=setInterval(function(){
        tries++;
        var successVisible=false;
        try {
          var msg=byId('msg-success');
          var actions=byId('jurnal-success-actions');
          successVisible=!!((msg && !msg.classList.contains('hidden')) || (actions && !actions.classList.contains('hidden')));
        } catch(_) {}
        if(successVisible || isFulfilled(linkAtSubmit) || tries>36){
          clearInterval(timer);
          if(successVisible || isFulfilled(linkAtSubmit)) clearLink('after-submit-success');
        }
      }, 250);
    }, false);
  }
  ...
  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV326Wrapped){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      setTimeout(function(){
        installSubmitCleaner();
        if(id === 'jurnal') clearIfFulfilled('nav-jurnal');
      }, 80);
      return res;
    };
    window.nav.__arikaV326Wrapped = true;
  }

  var oldFetch = window.fetchLiveJurnalFromSheet;
  if(typeof oldFetch === 'function' && !oldFetch.__arikaV326Wrapped){
    window.fetchLiveJurnalFromSheet = async function(){
      var res = await oldFetch.apply(this, arguments);
      setTimeout(function(){ clearIfFulfilled('fetch-live'); }, 150);
      return res;
    };
    window.fetchLiveJurnalFromSheet.__arikaV326Wrapped = true;
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ installSubmitCleaner(); setTimeout(function(){ clearIfFulfilled('dom'); }, 250); });
  else { installSubmitCleaner(); setTimeout(function(){ clearIfFulfilled('now'); }, 150); }
  window.addEventListener('load', function(){ installSubmitCleaner(); setTimeout(function(){ clearIfFulfilled('load'); }, 500); });
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#btn-jurnal')) setTimeout(function(){ installSubmitCleaner(); clearIfFulfilled('btn-jurnal'); }, 120);
  }, true);
})();
...
/* =========================================================
   v327 - Perjelas kata ARIKA di kartu sapaan Beranda
   Basis: v326. Patch hanya untuk keterbacaan teks sapaan.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V327_SAPAAN_BRAND_READABLE__) return;
  window.__ARIKA_V327_SAPAAN_BRAND_READABLE__ = true;
  function apply(){
    try{
      var el=document.getElementById('welcome-subtitle');
      if(!el) return;
      var txt=(el.textContent||'').trim();
      if(!txt || txt.indexOf('ARIKA')<0) return;
      if(el.querySelector && el.querySelector('.arika-home-brand-word')) return;
      el.innerHTML = txt.replace(/\bARIKA\b/g, '<span class="arika-home-brand-word">ARIKA</span>');
    }catch(_){}
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(apply,80); }); else setTimeout(apply,80);
  window.addEventListener('load', function(){ setTimeout(apply,200); setTimeout(apply,1000); });
  var oldNav=window.nav;
  if(typeof oldNav==='function' && !oldNav.__arikaV327Sapaan){
    window.nav=function(id){ var res=oldNav.apply(this,arguments); if(id==='beranda') setTimeout(apply,120); return res; };
    window.nav.__arikaV327Sapaan=true;
  }
})();


/* =========================================================
   v328 - Perjelas ARIKA pada label Beranda ARIKA • Portal Jurnal Harian
   Basis: v327. Patch hanya untuk keterbacaan label kecil pada kartu sapaan.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V328_KICKER_BRAND_READABLE__) return;
  window.__ARIKA_V328_KICKER_BRAND_READABLE__ = true;
  function apply(){
    try{
      var el=document.querySelector('#view-beranda .arika-home-line-kicker');
      if(!el) return;
      if(el.querySelector('.arika-home-kicker-brand')) return;
      var txt=(el.textContent||'').trim();
      if(!txt || txt.indexOf('ARIKA')<0) return;
      el.innerHTML = txt.replace(/\bARIKA\b/g, '<span class="arika-home-kicker-brand">ARIKA</span>');
    }catch(_){ }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(apply,80); }); else setTimeout(apply,80);
  window.addEventListener('load', function(){ setTimeout(apply,200); setTimeout(apply,1000); });
  var oldNav=window.nav;
  if(typeof oldNav==='function' && !oldNav.__arikaV328Kicker){
    window.nav=function(id){ var res=oldNav.apply(this,arguments); if(id==='beranda') setTimeout(apply,120); return res; };
    window.nav.__arikaV328Kicker=true;
  }
})();

/* =========================================================
   v331 - Konsistensi status realisasi Rencana Saya dengan Riwayat/Database
   Basis: v330. Patch hanya meluruskan penanda Sudah/Belum Realisasi di PC.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V331_REALISASI_STATUS_DB_CONSISTENT__) return;
  window.__ARIKA_V331_REALISASI_STATUS_DB_CONSISTENT__ = true;
  ...
  function sameCurrentUser(j){
    var ck=currentKey(); if(!ck) return true;
    var jk=norm(journalNip(j) || journalName(j));
    return !jk || jk===ck || norm(journalName(j))===norm(currentName());
  }
  function markerTitle(desc){
    var s=safe(desc); var upper=s.toUpperCase(); var marker='[REALISASI RENCANA:'; var pos=upper.indexOf(marker);
    if(pos<0) return ''; var part=s.slice(pos+marker.length).split(/\r?\n/)[0].trim();
    return part.replace(/\]\s*$/,'').trim();
  }
  function isFakeLocalRow(j){ return /^v322-local-real-/i.test(safe(j && j.id)) || /pending-local-v322/i.test(safe(j && j.syncStatus)); }
  function isDeletedRow(j){ return /deleted|hapus/i.test(safe(j && (j.syncStatus || j.statusData || j.deleted))); }
  function rowsFromVisibleJournalData(){
    var rows=[];
    try { if(Array.isArray(window.arikaData)) rows=rows.concat(window.arikaData); } catch(_) {}
    // Jangan pakai STORE_KEY v322 karena bisa membuat status PC "Sudah Realisasi" padahal riwayat/database kosong.
    return rows.filter(function(j){ return j && !isFakeLocalRow(j) && !isDeletedRow(j); });
  }
  function journalMatchesPlan(plan,journal){
    if(!sameCurrentUser(journal)) return false;
    var pid=planId(plan), jid=journalId(journal);
  ...
  function realRows(plan){ return rowsFromVisibleJournalData().filter(function(j){ return journalMatchesPlan(plan,j); }); }
  function getPlans(){
    var data=[];
    try { data=typeof getStableRencanaData==='function' ? getStableRencanaData() : (window.rencanaData||[]); } catch(_) { data=window.rencanaData||[]; }
    try { data=(data||[]).filter(function(r){ return typeof isCurrentUserRencana==='function' ? isCurrentUserRencana(r) : true; }); } catch(_) {}
  ...
  function closeModal(){ var old=byId('arika-v331-real-modal'); if(old) old.remove(); }
  window.closeRencanaRealizationModalV331=closeModal;
  function showModal(plan, rows){
    closeModal();
    var back=document.createElement('div'); back.id='arika-v331-real-modal'; back.className='arika-v317-modal-backdrop';
    var items=rows.map(function(r){
      var meta=[journalDate(r),journalCat(r),journalStatus(r)].filter(Boolean).join(' • ');
      return '<div class="arika-v317-real-item"><div class="meta">'+esc(meta||'Realisasi')+'</div><b>'+esc(journalTitle(r)||planTitle(plan))+'</b>'+esc(journalDesc(r)||'-').replace(/\n/g,'<br>')+'</div>';
    }).join('');
    back.innerHTML='<div class="arika-v317-modal"><div class="arika-v317-modal-head"><div><h3 class="arika-v317-modal-title">Realisasi Rencana</h3><div class="arika-v317-modal-sub">'+esc(planTitle(plan))+'</div></div><button type="button" class="arika-v317-modal-close" onclick="window.closeRencanaRealizationModalV331()">Tutup</button></div>'+items+'</div>';
    back.addEventListener('click', function(ev){ if(ev.target===back) closeModal(); });
    document.body.appendChild(back);
  }
  function purgeOldStaleStore(){
  ...
  function decorate(){
    ...
    cards.forEach(function(card,idx){
      ...
      var plan=plans.find(function(p){ return norm(planTitle(p))===ht; }) || plans[idx];
      if(!plan) return;
      var rows=realRows(plan); var done=rows.length>0;
      var status=card.querySelector('.arika-rencana-real-status-v317');
      if(!status){ status=document.createElement('div'); status.className='arika-rencana-real-status-v317'; var anchor=card.querySelector('p.text-\\[10px\\], p'); if(anchor && anchor.parentNode===card) card.insertBefore(status, anchor.nextSibling); else card.appendChild(status); }
      status.classList.toggle('done',done); status.classList.toggle('waiting',!done);
      status.innerHTML=done?'Sudah Realisasi':'Belum Realisasi';
      var row=card.querySelector('.mt-3.flex.flex-wrap.gap-2'); if(!row) return;
      var btns=Array.prototype.slice.call(row.querySelectorAll('button'));
  ...
  window.applyRencanaRealizationStatusV331=decorate;
  function schedule(){ try{ decorate(); }catch(_){} clearTimeout(window.__arikaV331DecorTimer); window.__arikaV331DecorTimer=setTimeout(function(){ try{ decorate(); }catch(_){} },120); }
  var oldRender=window.renderRencanaPribadi;
  if(typeof oldRender==='function' && !oldRender.__arikaV331Wrapped){
    window.renderRencanaPribadi=function(){ var res=oldRender.apply(this,arguments); schedule(); return res; };
    window.renderRencanaPribadi.__arikaV331Wrapped=true;
  }
  var oldFetch=window.fetchLiveJurnalFromSheet;
  if(typeof oldFetch==='function' && !oldFetch.__arikaV331Wrapped){
    window.fetchLiveJurnalFromSheet=async function(){ var res=await oldFetch.apply(this,arguments); setTimeout(schedule,150); setTimeout(schedule,650); return res; };
    window.fetchLiveJurnalFromSheet.__arikaV331Wrapped=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ purgeOldStaleStore(); setTimeout(schedule,350); }); else { purgeOldStaleStore(); setTimeout(schedule,150); }
  window.addEventListener('load', function(){ purgeOldStaleStore(); setTimeout(schedule,250); setTimeout(schedule,1000); });
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(schedule,140); }, true);
})();


/* =========================================================
   v334 - Rencana full sync consistency
   Tujuan: rencana yang sudah ada di database tidak hilang dari PC/mobile hanya karena berada di baris lama/cache/maxRows.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V334_RENCANA_FULL_SYNC__) return;
  window.__ARIKA_V334_RENCANA_FULL_SYNC__ = true;
  function norm(v){ return String(v||'').toLowerCase().replace(/[^a-z0-9]/g,''); }
  function getVal(o,keys){ if(!o)return''; for(var i=0;i<keys.length;i++){var k=keys[i]; if(Object.prototype.hasOwnProperty.call(o,k)&&o[k]!=null&&String(o[k]).trim()!=='')return o[k];} var lower={}; Object.keys(o||{}).forEach(function(k){lower[norm(k)]=o[k];}); for(var j=0;j<keys.length;j++){var v=lower[norm(keys[j])]; if(v!=null&&String(v).trim()!=='')return v;} return ''; }
  function dateKey(v){ try{ if(typeof normalizeDateKeyFromSheet==='function') return normalizeDateKeyFromSheet(v||'')||''; }catch(_){} if(v instanceof Date&&!isNaN(v)) return v.getFullYear()+'-'+String(v.getMonth()+1).padStart(2,'0')+'-'+String(v.getDate()).padStart(2,'0'); var s=String(v||'').trim(); var m=s.match(/^(\d{4})[-\/]?(\d{2})[-\/]?(\d{2})/); if(m)return m[1]+'-'+m[2]+'-'+m[3]; m=s.match(/^(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})/); if(m)return m[3]+'-'+String(m[2]).padStart(2,'0')+'-'+String(m[1]).padStart(2,'0'); return s.slice(0,10); }
  function time(v){ try{ if(typeof extractTime==='function') return extractTime(v||'')||''; }catch(_){} return String(v||'').trim(); }
  function mapRencana(rows){ return (rows||[]).map(function(r,idx){ return { id:String(getVal(r,['id','ID','id_rencana','ID Rencana','timestamp','Timestamp'])||('rencana-'+idx)), rencanaId:String(getVal(r,['rencanaId','rencana id','rencana_id','Rencana ID','ID Relasi Rencana'])||getVal(r,['id','ID'])||('rencana-'+idx)), ownerName:String(getVal(r,['ownerName','owner_name','nama','Nama','nama_pegawai','Nama Pegawai','pegawai','Pegawai','name','Name','pelaksana','Pelaksana','pemilik','Pemilik'])||''), ownerNip:String(getVal(r,['ownerNip','owner_nip','nip','NIP','nip_pegawai','NIP Pegawai','Nomor Induk'])||''), tanggal:dateKey(getVal(r,['tanggal','Tanggal','date','Date','tgl','Tgl','tanggal rencana','Tanggal Rencana'])||''), jamReminder:time(getVal(r,['jamReminder','jam_reminder','jam reminder','Jam Reminder','waktuReminder','waktu_reminder','waktu reminder','Waktu Reminder','jam','Jam','waktu','Waktu'])||''), periode:String(getVal(r,['periode','Periode','jenis','Jenis','type','Type'])||'Reminder'), judul:String(getVal(r,['judul','Judul','rencana','Rencana','kegiatan','Kegiatan','title','Title'])||''), catatan:String(getVal(r,['catatan','Catatan','detail','Detail','deskripsi','Deskripsi','note','Note'])||''), status:String(getVal(r,['status','Status'])||'Aktif'), createdAt:String(getVal(r,['createdAt','created_at','timestamp','Timestamp'])||''), syncStatus:'synced' }; }).filter(function(r){ return r.judul || r.catatan; }); }
  function key(r){ return String((r&& (r.rencanaId || r.id)) || [r.tanggal,norm(r.ownerName),norm(r.ownerNip),norm(r.judul),norm(r.catatan)].join('|')); }
  function mergeRencana(oldList,newList){ var m=new Map(); (oldList||[]).forEach(function(r){ if(r) m.set(key(r),r); }); (newList||[]).forEach(function(r){ if(r) m.set(key(r),Object.assign({},m.get(key(r))||{},r)); }); return Array.from(m.values()); }
  async function fetchRencanaLengkap(){
    if(!window.currentUser || !window.SCRIPT_URL && typeof SCRIPT_URL==='undefined') return;
    if(window.__ARIKA_V334_FETCHING_RENCANA__) return;
    window.__ARIKA_V334_FETCHING_RENCANA__ = true;
    try{
      var url = SCRIPT_URL + (SCRIPT_URL.indexOf('?')>=0 ? '&':'?') + 'mode=core&full=true&maxRows=1&rencanaOnly=1&_=' + Date.now();
      var res = await fetch(url,{cache:'no-store'});
      var data = await res.json();
      if(data && Array.isArray(data.rencana) && data.rencana.length){
        var mapped = mapRencana(data.rencana);
        window.rencanaData = mergeRencana(window.rencanaData||[], mapped);
        try{ if(typeof saveLocalRencana==='function') saveLocalRencana(window.rencanaData); }catch(_){}
        try{ if(typeof saveLocalCoreCache==='function') saveLocalCoreCache(); }catch(_){}
        try{ window.renderRencanaPribadi && window.renderRencanaPribadi(); }catch(_){}
        try{ window.renderAgendaSaya && window.renderAgendaSaya(); }catch(_){}
        try{ window.updateHomeModernStats && window.updateHomeModernStats(); }catch(_){}
      }
    }catch(e){ console.warn('Rencana lengkap belum bisa dimuat:', e); }
    finally{ window.__ARIKA_V334_FETCHING_RENCANA__ = false; }
  }
  window.fetchRencanaLengkapDariDatabase = fetchRencanaLengkap;
  var oldFetch = window.fetchCloudData;
  if(typeof oldFetch==='function' && !oldFetch.__arikaV334RencanaFull){
    window.fetchCloudData = async function(){ var res = await oldFetch.apply(this, arguments); setTimeout(fetchRencanaLengkap, 800); return res; };
    window.fetchCloudData.__arikaV334RencanaFull = true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(fetchRencanaLengkap, 1800); }); else setTimeout(fetchRencanaLengkap, 1000);
  window.addEventListener('focus', function(){ setTimeout(fetchRencanaLengkap, 300); });
})();


/* =========================================================
   v335 - PC Rencana Saya consistency fix
   Basis: v334. Perbaikan khusus PC: rencana aktif tampil semua, cache/tombstone lama tidak menutupi rencana database.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V335_PC_RENCANA_CONSISTENCY__) return;
  window.__ARIKA_V335_PC_RENCANA_CONSISTENCY__ = true;
  function safe(v){ return String(v == null ? '' : v).trim(); }
  function norm(v){ return safe(v).toLowerCase().replace(/[^a-z0-9]/g,''); }
  function byId(id){ return document.getElementById(id); }
  function isMobilePage(){ return /mobile\.html/i.test(location.pathname); }
  function purgeRencanaOnlyCache(){
    try { localStorage.removeItem('arika_deleted_rencana_tombstones_v1'); } catch(_) {}
    try { sessionStorage.removeItem('arika_deleted_rencana_tombstones_v1'); } catch(_) {}
    // Jangan hapus rencana lokal pending; hanya bersihkan core cache lama yang sering membatasi rencana PC.
    try { localStorage.removeItem('arika_core_cache_v36'); } catch(_) {}
    try { sessionStorage.removeItem('arika_core_cache_v36'); } catch(_) {}
  }
  function getApiUrl(){ try { return typeof SCRIPT_URL !== 'undefined' ? SCRIPT_URL : (window.SCRIPT_URL || ''); } catch(_) { return window.SCRIPT_URL || ''; } }
  function getVal(o, keys){
    if(!o) return '';
    for(var i=0;i<keys.length;i++){ var k=keys[i]; if(Object.prototype.hasOwnProperty.call(o,k) && o[k] != null && String(o[k]).trim() !== '') return o[k]; }
    var lower={}; Object.keys(o||{}).forEach(function(k){ lower[norm(k)] = o[k]; });
    for(var j=0;j<keys.length;j++){ var v=lower[norm(keys[j])]; if(v!=null && String(v).trim() !== '') return v; }
    return '';
  }
  function dateKey(v){
    try { if(typeof normalizeDateKeyFromSheet === 'function') return normalizeDateKeyFromSheet(v || '') || ''; } catch(_) {}
    if(v instanceof Date && !isNaN(v)) return v.getFullYear()+'-'+String(v.getMonth()+1).padStart(2,'0')+'-'+String(v.getDate()).padStart(2,'0');
    var s=safe(v); var m=s.match(/^(\d{4})[-\/]?(\d{2})[-\/]?(\d{2})/); if(m) return m[1]+'-'+m[2]+'-'+m[3];
    m=s.match(/^(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})/); if(m) return m[3]+'-'+String(m[2]).padStart(2,'0')+'-'+String(m[1]).padStart(2,'0');
    return s.slice(0,10);
  }
  function timeKey(v){ try { if(typeof extractTime === 'function') return extractTime(v || '') || ''; } catch(_) {} return safe(v); }
  function mapRows(rows){
    return (rows||[]).map(function(r,idx){
      var id=safe(getVal(r,['id','ID','id_rencana','ID Rencana'])) || ('rencana-db-'+idx);
      return {
        id:id,
        rencanaId:safe(getVal(r,['rencanaId','rencana id','rencana_id','Rencana ID'])) || id,
        ownerName:safe(getVal(r,['ownerName','owner_name','nama','Nama','nama pegawai','Nama Pegawai','pegawai','Pegawai','pelaksana','Pelaksana','name','Name'])),
        ownerNip:safe(getVal(r,['ownerNip','owner_nip','nip','NIP','nip pegawai','NIP Pegawai'])),
        tanggal:dateKey(getVal(r,['tanggal','Tanggal','date','Date','tgl','Tgl','tanggal rencana','Tanggal Rencana'])),
        jamReminder:timeKey(getVal(r,['jamReminder','jam reminder','Jam Reminder','waktuReminder','waktu reminder','jam','Jam','waktu','Waktu'])),
        periode:safe(getVal(r,['periode','Periode','jenis','Jenis'])) || 'Reminder',
        judul:safe(getVal(r,['judul','Judul','rencana','Rencana','kegiatan','Kegiatan','title','Title'])),
        catatan:safe(getVal(r,['catatan','Catatan','detail','Detail','deskripsi','Deskripsi','note','Note'])),
        status:safe(getVal(r,['status','Status'])) || 'Aktif',
        createdAt:safe(getVal(r,['createdAt','created_at','timestamp','Timestamp'])),
        syncStatus:'synced'
      };
    }).filter(function(r){ return r.judul || r.catatan; });
  }
  function key(r){ return safe((r && (r.rencanaId || r.id)) || '') || [safe(r&&r.tanggal), norm(r&&r.ownerNip), norm(r&&r.ownerName), norm(r&&r.judul), norm(r&&r.catatan)].join('|'); }
  function merge(a,b){ var m=new Map(); (a||[]).forEach(function(x){ if(x) m.set(key(x), x); }); (b||[]).forEach(function(x){ if(x) m.set(key(x), Object.assign({}, m.get(key(x))||{}, x)); }); return Array.from(m.values()); }
  async function fetchRencanaDatabaseNow(reason){
    if(isMobilePage() || !window.currentUser) return;
    var urlBase=getApiUrl(); if(!urlBase) return;
    if(window.__ARIKA_V335_FETCHING__) return;
    window.__ARIKA_V335_FETCHING__ = true;
    try{
      purgeRencanaOnlyCache();
      var url=urlBase+(urlBase.indexOf('?')>=0?'&':'?')+'mode=core&full=true&maxRows=1&rencanaOnly=1&pc=1&v=335&_='+Date.now();
      var res=await fetch(url,{cache:'no-store'});
      var data=await res.json();
      if(data && Array.isArray(data.rencana)){
        var mapped=mapRows(data.rencana);
        window.rencanaData=merge(window.rencanaData||[], mapped);
        try { if(typeof saveLocalRencana === 'function') saveLocalRencana(window.rencanaData); } catch(_) {}
        try { if(typeof renderRencanaPribadi === 'function') renderRencanaPribadi(); } catch(_) {}
        try { if(typeof window.applyRencanaRealizationStatusV331 === 'function') window.applyRencanaRealizationStatusV331(); } catch(_) {}
        try { if(typeof updateHomeModernStats === 'function') updateHomeModernStats(); } catch(_) {}
        var hint=byId('rencana-v335-sync-hint');
        if(hint) hint.remove();
      }
    }catch(e){ console.warn('v335 rencana database sync belum berhasil:', e); }
    finally{ window.__ARIKA_V335_FETCHING__ = false; }
  }
  window.fetchRencanaDatabaseNowV335 = fetchRencanaDatabaseNow;
  function schedule(){ setTimeout(function(){ fetchRencanaDatabaseNow('schedule'); }, 250); setTimeout(function(){ fetchRencanaDatabaseNow('late'); }, 1600); }
  var oldRender=window.renderRencanaPribadi;
  if(typeof oldRender==='function' && !oldRender.__arikaV335ShowAll){
    window.renderRencanaPribadi=function(){ var res=oldRender.apply(this, arguments); setTimeout(function(){
      try{
        var list=byId('rencana-pribadi-list');
        if(list && window.currentUser && !window.isAdmin){
          // Jika database masih proses, beri catatan ringkas, bukan membuat user mengira data hilang permanen.
          if(!byId('rencana-v335-sync-hint')){
            var div=document.createElement('div'); div.id='rencana-v335-sync-hint'; div.className='p-2 rounded-xl bg-cyan-50 border border-cyan-100 text-[9px] font-black uppercase tracking-widest text-cyan-700 text-center mb-2'; div.textContent='Sinkron rencana database...';
            list.parentNode && list.parentNode.insertBefore(div, list);
            setTimeout(function(){ var h=byId('rencana-v335-sync-hint'); if(h) h.remove(); }, 4200);
          }
        }
      }catch(_){}
    },60); return res; };
    window.renderRencanaPribadi.__arikaV335ShowAll=true;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', schedule); else schedule();
  window.addEventListener('load', schedule);
  window.addEventListener('focus', function(){ setTimeout(function(){ fetchRencanaDatabaseNow('focus'); }, 200); });
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#btn-beranda')) setTimeout(function(){ fetchRencanaDatabaseNow('beranda'); }, 120); }, true);
})();

/* =========================================================
   v339 - PC Rencana Live Sync After Mobile Input
   Basis: v337 mobile persist. Patch khusus PC agar rencana dari mobile cepat muncul tanpa reload.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V338_PC_RENCANA_LIVE_SYNC__) return;
  window.__ARIKA_V338_PC_RENCANA_LIVE_SYNC__ = true;

  var lastSync = 0;
  var syncing = false;
  var FAST_INTERVAL = 7000;   // saat Beranda aktif
  var SLOW_INTERVAL = 18000;  // saat tab lain aktif

  function getApi(){ try { return (typeof SCRIPT_URL !== 'undefined' && SCRIPT_URL) ? SCRIPT_URL : (window.SCRIPT_URL || ''); } catch(_) { return window.SCRIPT_URL || ''; } }
  function isMobileViewport(){ return window.matchMedia && window.matchMedia('(max-width:720px)').matches; }
  function isLogged(){ return !!window.currentUser && !!(window.currentUser.nama || window.currentUser.nip); }
  function getActiveView(){ try { return localStorage.getItem('arika_last_view_v16') || localStorage.getItem('arika_last_view') || ''; } catch(_) { return ''; } }
  function berandaVisible(){ var v=document.getElementById('view-beranda'); return !!(v && !v.classList.contains('hidden')); }
  function shouldSync(){ return !isMobileViewport() && isLogged() && !!getApi(); }
  function rencanaKey(r){
    if(!r) return '';
    var id = String(r.rencanaId || r.rencana_id || r['rencana id'] || r.id || '').trim();
    if(id) return 'id:'+id;
    return ['k', r.ownerNip||r.nip||'', r.ownerName||r.nama||'', r.tanggal||'', r.jamReminder||'', r.judul||'', r.catatan||''].join('|').toLowerCase().replace(/\s+/g,'');
  }
  function normalizeRencana(r){
    r = r || {};
    return {
      id: String(r.id || r.ID || r['ID'] || r.rencanaId || r.rencana_id || ('rencana-db-' + Date.now() + '-' + Math.random().toString(36).slice(2,6))),
      rencanaId: String(r.rencanaId || r.rencana_id || r['rencana id'] || r.id || ''),
      ownerName: String(r.ownerName || r.nama || r.name || r.Pelaksana || ''),
      ownerNip: String(r.ownerNip || r.nip || r.NIP || ''),
      tanggal: String(r.tanggal || r.date || r.Tanggal || ''),
      jamReminder: String(r.jamReminder || r.jam_reminder || r['jam reminder'] || r.waktu || r.jam || ''),
      periode: String(r.periode || 'Reminder'),
      judul: String(r.judul || r.title || ''),
      catatan: String(r.catatan || r.note || ''),
      status: String(r.status || r.Status || 'Aktif'),
      createdAt: String(r.createdAt || r.created_at || r.Timestamp || '')
    };
  }
  function mergeRencana(localList, serverList){
    var map = new Map();
    (Array.isArray(localList) ? localList : []).forEach(function(r){ var n=normalizeRencana(r); var k=rencanaKey(n); if(k) map.set(k, Object.assign({}, n, r)); });
    (Array.isArray(serverList) ? serverList : []).forEach(function(r){ var n=normalizeRencana(r); var k=rencanaKey(n); if(!k) return; map.set(k, Object.assign({}, map.get(k)||{}, n, {syncStatus:'synced'})); });
    return Array.from(map.values()).sort(function(a,b){ return String(b.createdAt || b.tanggal || '').localeCompare(String(a.createdAt || a.tanggal || '')); });
  }
  async function fetchRencanaRecent(reason){
    if(!shouldSync() || syncing) return;
    var now = Date.now();
    var minGap = berandaVisible() ? 4500 : 11000;
    if(now - lastSync < minGap) return;
    syncing = true; lastSync = now;
    try{
      var api = getApi();
      // Tidak pakai full=true agar PC tidak mengambil seluruh jurnal. Rencana terbaru dari mobile biasanya berada di baris terbaru.
      var url = api + (api.indexOf('?')>=0 ? '&' : '?') + 'mode=core&maxRows=1500&pcRencanaLive=1&_=' + Date.now();
      var res = await fetch(url, { cache:'no-store' });
      var data = await res.json();
      if(data && Array.isArray(data.rencana)){
        var before = (window.rencanaData || []).length;
        window.rencanaData = mergeRencana(window.rencanaData || [], data.rencana || []);
        try { if(typeof saveLocalRencana === 'function') saveLocalRencana(window.rencanaData); } catch(_) {}
        try { if(typeof saveStableRencanaData === 'function') saveStableRencanaData(window.rencanaData); } catch(_) {}
        var after = (window.rencanaData || []).length;
        if(after !== before || berandaVisible()){
          try { window.renderRencanaPribadi && window.renderRencanaPribadi({source:'v339-pc-rencana-live-'+(reason||'sync')}); } catch(_) {}
          try { window.decorateRencanaRealisasiCards && window.decorateRencanaRealisasiCards(); } catch(_) {}
          try { window.renderWfhWfaDay && document.getElementById('view-wfh') && !document.getElementById('view-wfh').classList.contains('hidden') && window.renderWfhWfaDay(); } catch(_) {}
        }
        var sync = document.getElementById('sync-status');
        if(sync && berandaVisible()) sync.textContent = '↻ Rencana sinkron';
      }
    }catch(e){
      // Diamkan agar tidak mengganggu penggunaan PC.
    }finally{
      syncing = false;
    }
  }
  function startPolling(){
    if(window.__arikaV338RencanaPoll) clearInterval(window.__arikaV338RencanaPoll);
    window.__arikaV338RencanaPoll = setInterval(function(){ fetchRencanaRecent('poll'); }, berandaVisible() ? FAST_INTERVAL : SLOW_INTERVAL);
  }
  function triggerSoon(reason, delay){ setTimeout(function(){ fetchRencanaRecent(reason); }, delay || 350); }

  var oldNav = window.nav;
  if(typeof oldNav === 'function' && !oldNav.__arikaV338RencanaSync){
    window.nav = function(id){
      var res = oldNav.apply(this, arguments);
      if(id === 'beranda' || id === 'wfh') triggerSoon('nav-'+id, 250);
      startPolling();
      return res;
    };
    window.nav.__arikaV338RencanaSync = true;
  }
  var oldFetch = window.fetchCloudData;
  if(typeof oldFetch === 'function' && !oldFetch.__arikaV338RencanaSync){
    window.fetchCloudData = async function(){
      var res = await oldFetch.apply(this, arguments);
      triggerSoon('after-fetchCloudData', 600);
      return res;
    };
    window.fetchCloudData.__arikaV338RencanaSync = true;
  }

  document.addEventListener('visibilitychange', function(){ if(!document.hidden) triggerSoon('visibility', 250); });
  window.addEventListener('focus', function(){ triggerSoon('focus', 250); });
  window.addEventListener('storage', function(ev){ if(ev && /rencana|cache|mobile/i.test(String(ev.key||''))) triggerSoon('storage', 250); });
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ startPolling(); triggerSoon('dom', 900); });
  else { startPolling(); triggerSoon('init', 900); }
  window.addEventListener('load', function(){ startPolling(); triggerSoon('load', 1200); });
})();

/* =========================================================
   v342 - PC Button Click Feedback
   Basis: v341. Patch hanya untuk memberi penanda visual saat tombol/aksi PC diklik.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V342_PC_BUTTON_CLICK_FEEDBACK__) return;
  window.__ARIKA_V342_PC_BUTTON_CLICK_FEEDBACK__ = true;

  function byId(id){ return document.getElementById(id); }
  function isInputTarget(el){ return el && el.closest && el.closest('input, textarea, select, option'); }
  function injectStyle(){
    if(byId('arika-v342-button-click-feedback-style')) return;
    var st=document.createElement('style');
    st.id='arika-v342-button-click-feedback-style';
    st.textContent = `
      @media (min-width: 721px){
        button, a[onclick], .nav-btn, .rekap-filter-chip, [role="button"]{ -webkit-tap-highlight-color: transparent; }
        .arika-click-feedback-target{ position:relative!important; overflow:hidden; }
        .arika-click-feedback-target.arika-click-press{ transform:translateY(1px) scale(.985)!important; filter:brightness(.97)!important; }
        .arika-click-feedback-target.arika-click-ok::after{
          content:'✓';
          position:absolute;
          top:6px;
          right:7px;
          ...
          pointer-events:none;
          animation:arikaClickOkPop .72s ease both;
        }
        .arika-click-feedback-target.arika-click-wait::before{
          content:'Diproses';
          position:absolute;
          left:50%;
          bottom:6px;
        ...
        #arika-click-toast-v342.show{opacity:1; transform:translateX(-50%) translateY(0);}
      }
    `;
    document.head.appendChild(st);
  }
  function toast(text){
    try{
      var t=byId('arika-click-toast-v342');
      if(!t){ t=document.createElement('div'); t.id='arika-click-toast-v342'; document.body.appendChild(t); }
      return; // v353: tidak menampilkan toast teks klik
      t.classList.add('show');
      clearTimeout(window.__arikaClickToastTimer342);
      window.__arikaClickToastTimer342=setTimeout(function(){ t.classList.remove('show'); }, 780);
    }catch(_){}
  }
  ...
  function isImportantAction(el){
    var txt=(el.innerText || el.textContent || '').toLowerCase();
    return /simpan|hapus|keluar|sinkron|refresh|export|download|selesai|realisasi|login|masuk|kirim|update|edit/.test(txt);
  }
  function mark(el){
    if(!el || !el.classList) return;
    if(el.disabled || el.getAttribute('aria-disabled') === 'true') return;
    el.classList.add('arika-click-feedback-target','arika-click-press','arika-click-ok');
    clearTimeout(el.__arikaClickPressTimer342);
    clearTimeout(el.__arikaClickOkTimer342);
    clearTimeout(el.__arikaClickWaitTimer342);
    el.__arikaClickPressTimer342=setTimeout(function(){ el.classList.remove('arika-click-press'); }, 160);
    el.__arikaClickWaitTimer342=setTimeout(function(){
      if(el && el.isConnected && isImportantAction(el)) el.classList.add('arika-click-wait');
    }, 240);
    el.__arikaClickOkTimer342=setTimeout(function(){
      try{ el.classList.remove('arika-click-ok','arika-click-wait'); }catch(_){}
    }, 980);
    // v353: toast teks 'Klik diterima' dinonaktifkan agar tidak mengganggu tampilan.
  }
  function handler(ev){
    try{
      if(window.matchMedia && window.matchMedia('(max-width:720px)').matches) return;
      if(isInputTarget(ev.target)) return;
      var el=ev.target && ev.target.closest && ev.target.closest('button, a[onclick], [role="button"], .nav-btn, .rekap-filter-chip, .panduan-topic-btn, .tab-btn');
      if(!el) return;
      if(el.closest && el.closest('#view-login-user')) return; // login sudah punya fokus sendiri.
      mark(el);
    }catch(_){}
  }
  function boot(){ injectStyle(); document.addEventListener('click', handler, true); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
})();


/* =========================================================
   v345 - Survei Bulan Ini Cek Database, bukan hanya cache browser
   Basis: v344. Patch khusus PC agar setelah history/cache browser dihapus,
   ARIKA tidak langsung meminta survei sebelum data survei dari database selesai dicek.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V345_SURVEI_DATABASE_CHECK__) return;
  window.__ARIKA_V345_SURVEI_DATABASE_CHECK__ = true;

  var surveyDbCheckDone = false;
  var surveyDbCheckStarted = false;

  function byId(id){ return document.getElementById(id); }
  function normalizeSafe(v){
    try { if(typeof normalize === 'function') return normalize(v); } catch(_) {}
    return String(v || '').trim().toLowerCase().replace(/[^a-z0-9]/g,'');
  }
  function monthNow(){
    try { if(typeof getCurrentSurveyMonth === 'function') return getCurrentSurveyMonth(); } catch(_) {}
    try { if(typeof getCurrentMonth === 'function') return getCurrentMonth(); } catch(_) {}
    return new Date().toISOString().slice(0,7);
  }
  function formatMonthSafe(month){
    try { if(typeof formatBulanIndonesia === 'function') return formatBulanIndonesia(month); } catch(_) {}
    return month || 'bulan ini';
  }
  function userKey(){
    var u = window.currentUser || {};
    var nip = normalizeSafe(u.nip || u.NIP || '');
    var nama = normalizeSafe(u.nama || u.name || '');
    return { nip:nip, nama:nama, any:(nip || nama) };
  }
  function surveyMatchesCurrentUser(row){
    if(!row || !window.currentUser) return false;
    var month = monthNow();
    var smonth = String(row.bulan || row.periode || row.month || '').slice(0,7);
    if(smonth !== month) return false;
    var key = userKey();
    var snip = normalizeSafe(row.nip || row.NIP || '');
    var sname = normalizeSafe(row.nama || row.name || row.pegawai || '');
    return (!!key.nip && !!snip && key.nip === snip) || (!!key.nama && !!sname && key.nama === sname);
  }
  function hasSurveyThisMonth(){
    try { return Array.isArray(window.surveiData) && window.surveiData.some(surveyMatchesCurrentUser); }
    catch(_) { return false; }
  }
  function showSurveyCheckingState(){
    var card = byId('survey-arika-card');
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
    var monthLabel = byId('survey-month-label');
    if(!card || !form || !statusBox) return;
    if(!window.currentUser || window.isAdmin) { card.classList.add('hidden'); return; }
    card.classList.remove('hidden');
    if(monthLabel) monthLabel.innerText = formatMonthSafe(monthNow());
    form.classList.add('hidden');
    statusBox.classList.remove('hidden');
    statusBox.innerHTML = 'Mengecek status survei <b>' + (formatMonthSafe(monthNow())) + '</b> dari database...';
  }
  function showSurveyAlreadySubmitted(){
    var card = byId('survey-arika-card');
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
    var monthLabel = byId('survey-month-label');
    ...
    form.classList.add('hidden');
    statusBox.classList.remove('hidden');
    statusBox.innerHTML = 'Terima kasih, survei ARIKA untuk <b>' + (formatMonthSafe(monthNow())) + '</b> sudah tercatat di database.';
  }
  function allowSurveyForm(){
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
    var card = byId('survey-arika-card');
    var monthLabel = byId('survey-month-label');
    ...
    statusBox.classList.add('hidden');
    statusBox.innerHTML = '';
    try { window.initSurveyStars && window.initSurveyStars(); } catch(_) {}
  }
  function renderSurveySafely(){
    if(!window.currentUser || window.isAdmin) {
      var card = byId('survey-arika-card');
    ...
    if(hasSurveyThisMonth()) {
      surveyDbCheckDone = true;
      showSurveyAlreadySubmitted();
      return;
    }
    if(!surveyDbCheckDone) {
      showSurveyCheckingState();
      startSurveyDbCheck();
      return;
    }
    allowSurveyForm();
  }
  function startSurveyDbCheck(){
    if(surveyDbCheckStarted) return;
    surveyDbCheckStarted = true;
    // fetchCloudData sudah mengambil survei dari database bersama data core.
    if(typeof window.fetchCloudData === 'function') {
      try {
        Promise.resolve(window.fetchCloudData({ force:true, reason:'survey-db-check' }))
          .catch(function(){})
          .finally(function(){
            surveyDbCheckDone = true;
            surveyDbCheckStarted = false;
            setTimeout(renderSurveySafely, 120);
            try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(_) {}
          });
      } catch(_) {
        surveyDbCheckDone = true;
        surveyDbCheckStarted = false;
  ...
  var oldRenderSurvey = window.renderSurveiPegawai;
  if(typeof oldRenderSurvey === 'function' && !oldRenderSurvey.__arikaV345SurveyDbCheck){
    window.renderSurveiPegawai = function(){
      // Jangan tampilkan form survei sampai database selesai dicek.
      renderSurveySafely();
    };
    window.renderSurveiPegawai.__arikaV345SurveyDbCheck = true;
  }

  var oldFetch = window.fetchCloudData;
  if(typeof oldFetch === 'function' && !oldFetch.__arikaV345SurveyDbCheck){
    window.fetchCloudData = async function(){
      var res;
      try { res = await oldFetch.apply(this, arguments); }
      finally {
        surveyDbCheckDone = true;
        surveyDbCheckStarted = false;
        setTimeout(renderSurveySafely, 100);
      }
      return res;
    };
    window.fetchCloudData.__arikaV345SurveyDbCheck = true;
  }

  // Perkuat reminder/fokus agar tidak menampilkan survei pending palsu setelah history browser dihapus.
  window.hasUserFilledSurveyThisMonthForReminder = function(){
    if(!window.currentUser || window.isAdmin) return true;
    if(hasSurveyThisMonth()) return true;
    if(!surveyDbCheckDone) return true; // status masih dicek, jangan tampilkan reminder palsu
    return false;
  };

  function boot(){
    setTimeout(renderSurveySafely, 200);
    setTimeout(renderSurveySafely, 1500);
    setTimeout(function(){ try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(_) {} }, 1800);
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('focus', function(){ if(window.currentUser && !window.isAdmin) { surveyDbCheckDone = false; renderSurveySafely(); } });
})();


/* v353 - Hilangkan tulisan/toast "Klik diterima" pada HTML PC. Visual klik tetap ada. */
(function(){
  function injectNoClickToastStyle(){
    if(document.getElementById('arika-v353-no-click-toast-style')) return;
    var st=document.createElement('style');
    st.id='arika-v353-no-click-toast-style';
    st.textContent='#arika-click-toast-v342{display:none!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important;}';
    document.head.appendChild(st);
  }
  function removeClickToast(){
    try{ var t=document.getElementById('arika-click-toast-v342'); if(t) t.remove(); }catch(_){}
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ injectNoClickToastStyle(); removeClickToast(); });
  else { injectNoClickToastStyle(); removeClickToast(); }
  window.addEventListener('load', function(){ injectNoClickToastStyle(); removeClickToast(); });
})();

/* =========================================================
   v359 - PC Manual Filter Fix
   Fokus: filter manual Riwayat Jurnal dan Laporan Lembur PC.
   Memperbaiki input 03062026 agar tidak berubah menjadi angka acak seperti 62/00/326.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V359_PC_MANUAL_FILTER_FIX__) return;
  window.__ARIKA_V359_PC_MANUAL_FILTER_FIX__ = true;

  var monthNames = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
  function byId(id){ return document.getElementById(id); }
  function pad2(v){ return String(v).padStart(2,'0'); }
  function todayMonth(){ var d=new Date(); return d.getFullYear()+'-'+pad2(d.getMonth()+1); }
  function validDate(y,m,d){ var dt=new Date(y,m-1,d); return dt.getFullYear()===y && dt.getMonth()===(m-1) && dt.getDate()===d; }
  function makeMonth(y,m){ y=Number(y); m=Number(m); if(!y || y<1900 || y>2100 || m<1 || m>12) return ''; return y+'-'+pad2(m); }
  function makeDate(y,m,d){ y=Number(y); m=Number(m); d=Number(d); if(!validDate(y,m,d)) return ''; return y+'-'+pad2(m)+'-'+pad2(d); }
  function monthDisplay(key){ var m=/^(\d{4})-(\d{2})$/.exec(String(key||'')); return m ? m[2]+'/'+m[1] : ''; }
  function dateDisplay(key){ var m=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(key||'')); return m ? m[3]+'/'+m[2]+'/'+m[1] : ''; }
  function monthLabel(key){ var m=/^(\d{4})-(\d{2})$/.exec(String(key||'')); if(!m) return 'Semua Bulan'; return (monthNames[Number(m[2])-1]||m[2])+' '+m[1]; }
  ...
  function parseDateAny(raw){
    ...
    var d=digits(raw);
    if(/^\d{8}$/.test(d)){
      // Prioritas untuk pengguna Indonesia: 03062026 = 03/06/2026.
      var dmy = makeDate(d.slice(4,8), d.slice(2,4), d.slice(0,2));
      if(dmy) return dmy;
      var ymd = makeDate(d.slice(0,4), d.slice(4,6), d.slice(6,8));
      if(ymd) return ymd;
    }
    return '';
  }
  function parseMonthAny(raw, allowDate){
    raw=String(raw||'').trim().replace(/[.]/g,'/').replace(/\s+/g,'');
    if(!raw) return '';
    if(allowDate){ var date=parseDateAny(raw); if(date) return date.slice(0,7); }
    var m;
    if((m=/^(\d{1,2})[-\/](\d{4})$/.exec(raw))) return makeMonth(m[2],m[1]);
    if((m=/^(\d{4})[-\/](\d{1,2})$/.exec(raw))) return makeMonth(m[1],m[2]);
    var d=digits(raw);
    if(/^\d{6}$/.test(d)){
      // 062026 = Juni 2026; 202606 juga didukung.
      if(/^(19|20)\d{2}/.test(d.slice(0,4))) return makeMonth(d.slice(0,4), d.slice(4,6));
      return makeMonth(d.slice(2,6), d.slice(0,2));
    }
    return '';
  }
  function partialDateOrMonth(raw){
    var d=digits(raw);
    if(!d) return '';
    if(d.length <= 6){
      d=d.slice(0,6);
  ...
  function updateRekapLabel(){
    var label=byId('rekap-month-label'); if(!label) return;
    var date=byId('filter-rekap-date')?.value || '';
    var month=byId('filter-rekap-month')?.value || '';
    label.textContent = date ? dateDisplay(date) : monthLabel(month);
    label.classList.toggle('active', !!(date || month));
    label.title = date ? 'Filter tanggal aktif' : (month ? 'Filter bulan aktif' : 'Semua bulan');
  }
  function setHidden(id,val){ var el=byId(id); if(el) el.value=val||''; }
  function scheduleRekap(){
    clearTimeout(window.__arikaV359RekapTimer);
    window.__arikaV359RekapTimer=setTimeout(function(){
      try{ if(typeof window.runFilter==='function') window.runFilter({deferHeavy:true}); }catch(e){ console.warn(e); }
      try{ if(typeof window.renderVisualCalendar==='function') window.renderVisualCalendar(); }catch(e){}
    }, 80);
  }
  function handleRekapInput(options){
    options=options||{};
    var input=byId('filter-rekap-manual'); if(!input) return true;
    input.setAttribute('type','text'); input.setAttribute('inputmode','numeric'); input.setAttribute('maxlength','10'); input.setAttribute('autocomplete','off');
    input.removeAttribute('oninput');
    var raw=String(input.value||'').trim();
    if(!raw){
      setHidden('filter-rekap-date',''); setHidden('filter-rekap-month','');
      input.classList.remove('border-rose-300','bg-rose-50');
      updateRekapLabel(); if(options.render) scheduleRekap(); return true;
    }
    var d=digits(raw);
    var date='', month='';
    if(d.length===8) date=parseDateAny(d);
    if(date){
      month=date.slice(0,7);
      input.value=dateDisplay(date);
      setHidden('filter-rekap-date',date); setHidden('filter-rekap-month',month);
      input.classList.remove('border-rose-300','bg-rose-50');
      updateRekapLabel(); if(options.render) scheduleRekap(); return true;
    }
    month=parseMonthAny(raw,false);
    if(month){
      input.value=monthDisplay(month);
      setHidden('filter-rekap-date',''); setHidden('filter-rekap-month',month);
      input.classList.remove('border-rose-300','bg-rose-50');
      updateRekapLabel(); if(options.render) scheduleRekap(); return true;
    }
    if(d && d.length<8){
      input.value=partialDateOrMonth(raw);
      input.classList.remove('border-rose-300','bg-rose-50');
      updateRekapLabel();
      return false;
    }
  ...
  function setRekapMonth(month, render){
    var input=byId('filter-rekap-manual');
    setHidden('filter-rekap-date',''); setHidden('filter-rekap-month',month||'');
    if(input) input.value = month ? monthDisplay(month) : '';
    updateRekapLabel(); if(render!==false) scheduleRekap();
  }
  window.applyRekapManualPeriod=function(value){ var input=byId('filter-rekap-manual'); if(input && typeof value==='string') input.value=value; return handleRekapInput({render:true}); };
  window.setRekapMonthToCurrent=function(){ setRekapMonth(todayMonth(), true); };
  window.clearRekapMonth=function(){ setRekapMonth('', true); };
  window.shiftRekapMonth=function(delta){
    var base=byId('filter-rekap-date')?.value?.slice(0,7) || byId('filter-rekap-month')?.value || parseMonthAny(byId('filter-rekap-manual')?.value,false) || todayMonth();
    var m=/^(\d{4})-(\d{2})$/.exec(base); var dt=m?new Date(Number(m[1]),Number(m[2])-1+Number(delta||0),1):new Date();
    setRekapMonth(dt.getFullYear()+'-'+pad2(dt.getMonth()+1), true);
  };

  function updateLemburLabel(){
    var el=byId('filt-lembur-bulan'), label=byId('lembur-month-label'); if(!el || !label) return;
    var key=el.dataset.arikaIsoValue || parseMonthAny(el.value,true);
    label.textContent=key ? monthLabel(key) : 'Semua Bulan';
    label.classList.toggle('active', !!key);
  }
  function scheduleLembur(){ clearTimeout(window.__arikaV359LemburTimer); window.__arikaV359LemburTimer=setTimeout(function(){ try{ if(typeof window.renderLemburTable==='function') window.renderLemburTable(); }catch(e){console.warn(e);} },80); }
  function handleLemburInput(options){
    options=options||{};
    var el=byId('filt-lembur-bulan'); if(!el) return '';
    el.setAttribute('type','text'); el.setAttribute('inputmode','numeric'); el.setAttribute('maxlength','10'); el.removeAttribute('oninput'); el.removeAttribute('onchange');
    var raw=String(el.value||'').trim();
    if(!raw){ el.dataset.arikaIsoValue=''; el.classList.remove('border-rose-300','bg-rose-50'); updateLemburLabel(); if(options.render) scheduleLembur(); return ''; }
    var key=parseMonthAny(raw,true);
    if(key){
      el.value=monthDisplay(key); el.dataset.arikaIsoValue=key; el.classList.remove('border-rose-300','bg-rose-50'); updateLemburLabel(); if(options.render) scheduleLembur(); return key;
    }
    var d=digits(raw);
    if(d && d.length<8){
      var mm=d.slice(0,6);
      el.value=mm.length<=2 ? mm : mm.slice(0,2)+'/'+mm.slice(2);
      el.dataset.arikaIsoValue=''; el.classList.remove('border-rose-300','bg-rose-50'); updateLemburLabel(); return '';
    }
    el.classList.add('border-rose-300','bg-rose-50'); updateLemburLabel(); return '';
  }
  function setLemburMonth(key, render){ var el=byId('filt-lembur-bulan'); if(!el) return; el.value=key?monthDisplay(key):''; el.dataset.arikaIsoValue=key||''; el.classList.remove('border-rose-300','bg-rose-50'); updateLemburLabel(); if(render!==false) scheduleLembur(); }
  window.applyLemburMonthInput=function(el){ if(el && el.id==='filt-lembur-bulan'){} return handleLemburInput({render:true}); };
  window.setLemburMonthToCurrent=function(){ setLemburMonth(todayMonth(), true); };
  window.clearLemburMonth=function(){ setLemburMonth('', true); };
  window.shiftLemburMonth=function(delta){ var base=byId('filt-lembur-bulan')?.dataset?.arikaIsoValue || parseMonthAny(byId('filt-lembur-bulan')?.value,true) || todayMonth(); var m=/^(\d{4})-(\d{2})$/.exec(base); var dt=m?new Date(Number(m[1]),Number(m[2])-1+Number(delta||0),1):new Date(); setLemburMonth(dt.getFullYear()+'-'+pad2(dt.getMonth()+1), true); };

  function intercept(ev){
    var t=ev.target;
    if(!t) return;
    if(t.id==='filter-rekap-manual'){
      try{ ev.stopImmediatePropagation(); }catch(_){}
      if(ev.type==='input') handleRekapInput({render:true});
      else if(ev.type==='change' || ev.type==='blur') handleRekapInput({render:true});
    } else if(t.id==='filt-lembur-bulan'){
      try{ ev.stopImmediatePropagation(); }catch(_){}
      if(ev.type==='input') handleLemburInput({render:true});
      else if(ev.type==='change' || ev.type==='blur') handleLemburInput({render:true});
    }
  }
  function install(){
    var r=byId('filter-rekap-manual');
    if(r){ r.removeAttribute('oninput'); r.setAttribute('autocomplete','off'); r.setAttribute('inputmode','numeric'); r.setAttribute('maxlength','10'); r.placeholder='062026 atau 03062026'; }
    var l=byId('filt-lembur-bulan');
    if(l){ l.removeAttribute('oninput'); l.removeAttribute('onchange'); l.setAttribute('autocomplete','off'); l.setAttribute('inputmode','numeric'); l.setAttribute('maxlength','10'); l.placeholder='062026'; }
    handleRekapInput({render:false});
    handleLemburInput({render:false});
  }
  ['input','change','blur'].forEach(function(type){ document.addEventListener(type, intercept, true); });
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', install); else install();
  setTimeout(install,250); setTimeout(install,1000); setTimeout(install,2200);
})();


/* v365 - Dashboard Saya: tambah KPI Jam Lembur Klaim dan hilangkan catatan teknis absensi. */
(function(){
  if(window.__ARIKA_V365_DASHBOARD_KLAIM_LEMBUR__) return;
  window.__ARIKA_V365_DASHBOARD_KLAIM_LEMBUR__ = true;
  ...
  function manualMonth(){
    try {
      var el=document.getElementById('pegawai-dashboard-month');
      if(!el) return currentMonth();
      if(el.dataset && el.dataset.periodMode === 'all') return '';
      if(typeof getManualMonthValueById === 'function') return getManualMonthValueById('pegawai-dashboard-month', currentMonth());
      var raw=(el.dataset && el.dataset.arikaIsoValue) || el.value || '';
      var m=String(raw).match(/(\d{4})[-\/](\d{2})|(\d{2})[-\/](\d{4})/);
  ...
  function sameUser(row){
    try { if(typeof personMatchesRow === 'function') return personMatchesRow(row, window.currentUser); } catch(_) {}
    var u=window.currentUser||{};
    var rn=norm(row && (row.name||row.nama||row.Pelaksana));
    var un=norm(u.nama||u.name);
    var rnip=norm(row && (row.nip||row.NIP));
    var unip=norm(u.nip||u.NIP);
    return (!!rn && !!un && rn===un) || (!!rnip && !!unip && rnip===unip);
  }
  function isOvertime(row){
    try { if(typeof isAdminOvertimeRow === 'function') return isAdminOvertimeRow(row); } catch(_) {}
    if(!row) return false;
    if(row.isLembur === true) return true;
    var v=String(row.isLembur || row.lembur || row['Lembur?'] || '').toLowerCase().trim();
    return ['true','ya','iya','yes','1','lembur'].includes(v);
  }
  function claimMinutes(row){
    if(!row) return 0;
    var raw = row.lamaLembur || row.lama_lembur || row['lama lembur'] || row['Lama Lembur'] || row.durasiLembur || row['durasi lembur'] || row.durasi || '';
    try { if(typeof parseLamaLemburMenit === 'function') return Number(parseLamaLemburMenit(raw)) || 0; } catch(_) {}
    var s=String(raw||'').toLowerCase().replace(',', '.').trim();
    if(!s || s === '0' || s === '-') return 0;
    var h=0,m=0;
    var mh=s.match(/(\d+(?:\.\d+)?)\s*jam/); if(mh) h=parseFloat(mh[1])||0;
    var mm=s.match(/(\d+)\s*menit/); if(mm) m=parseInt(mm[1],10)||0;
    if(mh || mm) return Math.round(h*60+m);
    if(/^\d+(?:\.\d+)?$/.test(s)) return Math.round(parseFloat(s)*60);
    var hm=s.match(/^(\d{1,2}):(\d{2})$/); if(hm) return (parseInt(hm[1],10)||0)*60+(parseInt(hm[2],10)||0);
    return 0;
  }
  function formatMinutes(minutes){
    try { if(typeof formatOvertimeHours === 'function') return formatOvertimeHours(minutes); } catch(_) {}
    var h=Math.round((Number(minutes||0)/60)*10)/10;
    return String(h).replace('.', ',');
  }
  function updateClaimKpi(){
    var target=document.getElementById('pegawai-kpi-jam-lembur-klaim');
    if(!target || !window.currentUser || window.isAdmin) return;
    var month=manualMonth();
    var rows=(window.arikaData||[]).filter(function(row){
      if(!sameUser(row) || !isOvertime(row)) return false;
      if(!month) return true;
      return String(row.date || row.tanggal || '').slice(0,7) === month;
    });
    var total=rows.reduce(function(sum,row){ return sum + claimMinutes(row); }, 0);
    target.textContent = formatMinutes(total);
  }

  var oldRender = window.renderDashboardPegawai;
  if(typeof oldRender === 'function' && !oldRender.__arikaV365ClaimWrapped){
    window.renderDashboardPegawai = function(){
      var res = oldRender.apply(this, arguments);
      try { updateClaimKpi(); } catch(e) { console.warn('KPI klaim lembur gagal diperbarui:', e); }
      return res;
    };
    window.renderDashboardPegawai.__arikaV365ClaimWrapped = true;
  }
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#btn-dashboard')) setTimeout(updateClaimKpi, 250);
  }, true);
  document.addEventListener('input', function(ev){ if(ev.target && ev.target.id === 'pegawai-dashboard-month') setTimeout(updateClaimKpi, 80); }, true);
  document.addEventListener('change', function(ev){ if(ev.target && ev.target.id === 'pegawai-dashboard-month') setTimeout(updateClaimKpi, 80); }, true);
  setTimeout(updateClaimKpi, 800);
})();


/* v372 - Dashboard Lembur Admin: pakai durasi klaim, bukan durasi absensi. */
(function(){
  if(window.__ARIKA_V372_ADMIN_LEMBUR_KLAIM__) return;
  window.__ARIKA_V372_ADMIN_LEMBUR_KLAIM__ = true;
  function relabel(){
    try {
      document.querySelectorAll('#admin-content-lembur .admin-overtime-label').forEach(function(el){
        if((el.textContent||'').trim().toLowerCase()==='total jam') el.textContent='Total Jam Lembur Klaim';
      });
      document.querySelectorAll('#admin-content-lembur .admin-overtime-note').forEach(function(el){
        if(/akumulasi|estimasi/i.test(el.textContent||'')) el.textContent='Berdasarkan aturan jam lembur klaim';
      });
      document.querySelectorAll('#admin-content-lembur .admin-chart-title').forEach(function(el){
        if(/Jam Lembur per Fungsi/i.test(el.textContent||'')) el.textContent='Jam Lembur Klaim per Fungsi';
      });
    } catch(_) {}
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', relabel); else relabel();
  document.addEventListener('click', function(ev){ if(ev.target && ev.target.closest && ev.target.closest('#admin-tab-lembur,[onclick*="switchAdminTab(\'lembur\')"]')) setTimeout(relabel,200); }, true);
  setInterval(function(){ if(document.getElementById('admin-content-lembur') && !document.getElementById('admin-content-lembur').classList.contains('hidden')) relabel(); }, 2500);
})();


/* v373 - Admin/Ketua Tim/Verifikator: label dan laporan dashboard lembur memakai istilah Jam Lembur Klaim. */
(function(){
  if(window.__ARIKA_V373_ADMIN_LEMBUR_KLAIM_LABEL__) return;
  window.__ARIKA_V373_ADMIN_LEMBUR_KLAIM_LABEL__ = true;
  function relabelAdminLemburKlaim(){
    try {
      document.querySelectorAll('#admin-content-lembur .admin-overtime-label').forEach(function(el){
        var t=(el.textContent||'').trim().toLowerCase();
        if(t==='total jam klaim' || t==='total jam' || t==='jam klaim') el.textContent='Total Jam Lembur Klaim';
      });
      document.querySelectorAll('#admin-content-lembur .admin-overtime-note').forEach(function(el){
        var t=el.textContent||'';
        if(/aturan klaim|akumulasi|estimasi|absensi/i.test(t)) el.textContent='Berdasarkan aturan jam lembur klaim';
      });
      document.querySelectorAll('#admin-content-lembur .admin-chart-title').forEach(function(el){
        if(/Jam.*Lembur.*Fungsi|Jam.*Klaim.*Fungsi/i.test(el.textContent||'')) el.textContent='Jam Lembur Klaim per Fungsi';
      });
      document.querySelectorAll('#admin-content-lembur th').forEach(function(th){
        var t=(th.textContent||'').trim().toLowerCase();
        if(t==='klaim' || t==='jam' || t==='durasi klaim') th.textContent='Jam Lembur Klaim';
      });
    } catch(e) {}
  }
  function wrapDownload(fnName){
    var old=window[fnName];
    if(typeof old !== 'function' || old.__arikaV373Wrapped) return;
    window[fnName]=function(){
      relabelAdminLemburKlaim();
      return old.apply(this, arguments);
    };
    window[fnName].__arikaV373Wrapped=true;
  }
  function boot(){
    relabelAdminLemburKlaim();
    wrapDownload('downloadAdminOvertimeExcel');
    wrapDownload('downloadAdminOvertimeReportWord');
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', boot); else boot();
  window.addEventListener('load', function(){ setTimeout(boot,80); setTimeout(boot,600); });
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#admin-tab-lembur,[onclick*="switchAdminTab(\'lembur\')"],[onclick*="downloadAdminOvertime"]')) setTimeout(boot,120);
  }, true);
  setInterval(function(){
    var panel=document.getElementById('admin-content-lembur');
    if(panel && !panel.classList.contains('hidden')) boot();
  }, 2500);
})();


/* v374 - Dashboard Saya: pertahankan KPI Jam Lembur Absensi sebagai durasi absensi mentah.
   Dashboard Lembur admin/verifikator tetap memakai durasi klaim. */
(function(){
  if(window.__ARIKA_V374_DASHBOARD_SAYA_ABSENSI_PRESERVE__) return;
  window.__ARIKA_V374_DASHBOARD_SAYA_ABSENSI_PRESERVE__ = true;
  ...
  function isOvertime(row){
    ...
    if(row.isLembur === true) return true;
    var v = String(row.isLembur || row.lembur || row['Lembur?'] || '').toLowerCase().trim();
    return ['true','ya','iya','yes','1','lembur'].indexOf(v) >= 0;
  }
  function parseMinutesFallback(value){
    var raw = String(value || '').trim().toLowerCase().replace(',', '.');
    if(!raw || raw === '-' || raw === '0') return 0;
    var h = 0, m = 0;
    var mh = raw.match(/(\d+(?:\.\d+)?)\s*(?:jam|j|hour|hours)/);
    if(mh) h = parseFloat(mh[1]) || 0;
    var mm = raw.match(/(\d+)\s*(?:menit|mnt|min)/);
  ...
  function timeToMinutes(t){
    ...
    if(isNaN(h) || isNaN(min) || h < 0 || h > 47 || min < 0 || min > 59) return null;
    return h * 60 + min;
  }
  function rawAbsensiMinutes(row){
    if(!row) return 0;
    var start = row.start || row.jamMulai || row.jam_mulai || row['Jam Mulai'] || row['Jam Mulai Lembur'] || '';
    var end = row.end || row.jamSelesai || row.jam_selesai || row['Jam Selesai'] || row['Jam Selesai Lembur'] || '';
    var s = timeToMinutes(start), e = timeToMinutes(end);
    if(s !== null && e !== null){
      if(e < s) e += 24 * 60;
      return Math.max(0, e - s);
    }
    return parseMinutesFallback(row.lamaLembur || row.lama_lembur || row['Lama Lembur'] || row.durasiLembur || row.durasi || '');
  }
  function formatHours(minutes){
    var h = Math.round((Number(minutes || 0) / 60) * 10) / 10;
    return String(h).replace('.', ',');
  }
  function updateAbsensiKpi(){
    try {
      var target = document.getElementById('pegawai-kpi-jam-lembur');
      if(!target || !window.currentUser || window.isAdmin) return;
      var month = manualMonth();
      var rows = (window.arikaData || []).filter(function(row){
        if(!sameUser(row) || !isOvertime(row)) return false;
        if(!month) return true;
        return String(row.date || row.tanggal || row.Tanggal || '').slice(0,7) === month;
      });
      var total = rows.reduce(function(sum,row){ return sum + rawAbsensiMinutes(row); }, 0);
      target.textContent = formatHours(total);
    } catch(e){ console.warn('KPI Jam Lembur Absensi gagal dipulihkan:', e); }
  }

  var oldRender = window.renderDashboardPegawai;
  if(typeof oldRender === 'function' && !oldRender.__arikaV374AbsensiWrapped){
    window.renderDashboardPegawai = function(){
      var res = oldRender.apply(this, arguments);
      setTimeout(updateAbsensiKpi, 0);
      setTimeout(updateAbsensiKpi, 180);
      return res;
    };
    window.renderDashboardPegawai.__arikaV374AbsensiWrapped = true;
  }
  document.addEventListener('click', function(ev){
    if(ev.target && ev.target.closest && ev.target.closest('#btn-dashboard-pegawai,#pegawai-dashboard-month-current,#pegawai-dashboard-month-all,#pegawai-dashboard-month-prev,#pegawai-dashboard-month-next,#pegawai-dashboard-sync')){
      setTimeout(updateAbsensiKpi, 250);
      setTimeout(updateAbsensiKpi, 900);
    }
  }, true);
  document.addEventListener('input', function(ev){ if(ev.target && ev.target.id === 'pegawai-dashboard-month') setTimeout(updateAbsensiKpi, 90); }, true);
  document.addEventListener('change', function(ev){ if(ev.target && ev.target.id === 'pegawai-dashboard-month') setTimeout(updateAbsensiKpi, 90); }, true);
  window.addEventListener('focus', function(){ setTimeout(updateAbsensiKpi, 250); });
  setTimeout(updateAbsensiKpi, 900);
})();


/* v375 - Dashboard Saya: kunci Jam Lembur Absensi memakai durasi absensi mentah.
   Dashboard Lembur admin/verifikator/ketua tim tetap memakai Jam Lembur Klaim. */
(function(){
  if(window.__ARIKA_V375_DASHBOARD_SAYA_ABSENSI_LOCK__) return;
  window.__ARIKA_V375_DASHBOARD_SAYA_ABSENSI_LOCK__ = true;
  ...
  function monthValue(){
    try{
      ...
      var raw=(el.dataset && el.dataset.arikaIsoValue) || el.value || '';
      var m=String(raw).match(/(\d{4})[-\/](\d{2})|(\d{2})[-\/](\d{4})/);
      return m ? (m[1] ? (m[1]+'-'+m[2]) : (m[4]+'-'+m[3])) : currentMonth();
    }catch(_){ return currentMonth(); }
  }
  function sameUser(row){
    try { if(typeof personMatchesRow === 'function') return personMatchesRow(row, window.currentUser); } catch(_) {}
  ...
  function isOvertime(row){
    try { if(typeof isAdminOvertimeRow === 'function') return isAdminOvertimeRow(row); } catch(_) {}
    if(!row) return false;
    if(row.isLembur === true || row.hasExplicitLemburMark === true) return true;
    var v=String(row.isLembur || row.lembur || row['Lembur?'] || '').toLowerCase().trim();
    return ['true','ya','iya','yes','1','lembur','checked'].indexOf(v) >= 0;
  }
  function timeToMinutes(t){
  ...
  function parseDuration(raw){
    ...
    if(!s || s==='-' || s==='0') return 0;
    var h=0,m=0, has=false;
    var mh=s.match(/(\d+(?:\.\d+)?)\s*(?:jam|j|hour|hours)/); if(mh){h=parseFloat(mh[1])||0;has=true;}
    var mm=s.match(/(\d+)\s*(?:menit|mnt|min)/); if(mm){m=parseInt(mm[1],10)||0;has=true;}
    if(has) return Math.round(h*60+m);
    if(/^\d+(?:\.\d+)?$/.test(s)) return Math.round(parseFloat(s)*60);
    var hm=s.match(/^(\d{1,2}):(\d{2})$/); if(hm) return (parseInt(hm[1],10)||0)*60+(parseInt(hm[2],10)||0);
    return 0;
  }
  function absensiMinutes(row){
    var start=row && (row.start || row.jamMulai || row.jam_mulai || row['Jam Mulai'] || row['Jam Mulai Lembur'] || '');
    var end=row && (row.end || row.jamSelesai || row.jam_selesai || row['Jam Selesai'] || row['Jam Selesai Lembur'] || '');
    var s=timeToMinutes(start), e=timeToMinutes(end);
    if(s!==null && e!==null){ if(e<s) e+=1440; return Math.max(0,e-s); }
    return parseDuration(row && (row.lamaLembur || row.lama_lembur || row['Lama Lembur'] || row.durasiLembur || row.durasi || ''));
  }
  function fmt(minutes){ var h=Math.round((Number(minutes||0)/60)*10)/10; return String(h).replace('.', ','); }
  function visibleDashboard(){ var v=document.getElementById('view-dashboard-pegawai'); return !!(v && !v.classList.contains('hidden')); }
  function lockAbsensiKpi(){
    try{
      if(!visibleDashboard() || !window.currentUser || window.isAdmin) return;
      var target=document.getElementById('pegawai-kpi-jam-lembur');
      if(!target) return;
      ...
      target.textContent=fmt(rows.reduce(function(sum,row){return sum+absensiMinutes(row);},0));
      var label=target.closest('.admin-kpi-card') || target.parentElement;
      if(label){
        Array.prototype.slice.call(label.querySelectorAll('.admin-kpi-label,.text-slate-400,.text-slate-500')).forEach(function(el){
          if(/jam lembur/i.test(el.textContent||'')) el.textContent='Jam Lembur Absensi';
          if(/durasi klaim|aturan klaim/i.test(el.textContent||'')) el.textContent='Berdasarkan durasi absensi';
        });
      }
    }catch(e){ console.warn('v375 lock absensi KPI gagal:', e); }
  }
  var old=window.renderDashboardPegawai;
  if(typeof old==='function' && !old.__arikaV375AbsensiLocked){
    window.renderDashboardPegawai=function(){
      var res=old.apply(this, arguments);
      lockAbsensiKpi();
      setTimeout(lockAbsensiKpi,30);
      setTimeout(lockAbsensiKpi,180);
      setTimeout(lockAbsensiKpi,700);
      return res;
    };
    window.renderDashboardPegawai.__arikaV375AbsensiLocked=true;
  }
  ['click','input','change'].forEach(function(type){
    document.addEventListener(type,function(ev){
      if(ev.target && ev.target.closest && (ev.target.closest('#btn-dashboard-pegawai') || ev.target.closest('#view-dashboard-pegawai') || ev.target.id==='pegawai-dashboard-month')){
        setTimeout(lockAbsensiKpi,80); setTimeout(lockAbsensiKpi,400);
      }
    },true);
  });
  window.addEventListener('focus',function(){setTimeout(lockAbsensiKpi,180);});
  setInterval(lockAbsensiKpi,1200);
  setTimeout(lockAbsensiKpi,700);
})();


/* v378 - Perbaiki gate navigasi: Administrator boleh membuka tab WFH/WFA. */
(function(){
  if(window.__ARIKA_V378_ADMIN_WFH_NAV_GATE_FIX__) return;
  window.__ARIKA_V378_ADMIN_WFH_NAV_GATE_FIX__ = true;
  function isAdminAccount(){
    try { if(window.isAdmin) return true; } catch(_) {}
    try {
      var r = String(window.userRole || (window.currentUser && (window.currentUser.role || window.currentUser.Role || window.currentUser.peran || window.currentUser.akses)) || '').toLowerCase();
      return r.indexOf('admin') >= 0;
    } catch(_) { return false; }
  }
  function openWfhViewForAdmin(){
    try{
      document.querySelectorAll('.view-section').forEach(function(s){ s.classList.add('hidden'); });
      var target=document.getElementById('view-wfh');
      if(target) target.classList.remove('hidden');
      document.querySelectorAll('#nav-menu button').forEach(function(b){ b.classList.remove('nav-active'); });
      var btn=document.getElementById('btn-wfh');
      if(btn){ btn.classList.remove('hidden'); btn.style.display='inline-flex'; btn.classList.add('nav-active'); }
      try { window.renderWfhTab && window.renderWfhTab(); } catch(_) {}
      try { window.renderWfh && window.renderWfh(); } catch(_) {}
      try { window.renderWfhMonitoring && window.renderWfhMonitoring(); } catch(_) {}
      try {
        var session = typeof readLocalSession === 'function' ? readLocalSession() : null;
        if(session && typeof persistSession === 'function'){ session.lastView='wfh'; persistSession(session); }
      } catch(_) {}
      return true;
    }catch(e){ console.warn('v378 admin WFH open gagal:', e); return false; }
  }
  function wrapNav(){
    if(typeof window.nav !== 'function' || window.nav.__arikaV378AdminWfhGate) return;
    var oldNav=window.nav;
    window.nav=function(id){
      if(id === 'wfh' && isAdminAccount()) return openWfhViewForAdmin();
      return oldNav.apply(this, arguments);
    };
    window.nav.__arikaV378AdminWfhGate=true;
  }
  function ensureButton(){
    try{
      var btn=document.getElementById('btn-wfh');
      if(btn){ btn.classList.remove('hidden'); btn.style.display='inline-flex'; btn.removeAttribute('aria-hidden'); }
    }catch(_){}
    wrapNav();
  }
...
/* =========================================================
   v383 - Survei Bulanan Strict Database Check
   Basis: v382. Perbaikan khusus setelah cache/history browser dibersihkan:
   form survei tidak boleh muncul sebelum status bulan berjalan selesai dicek dari database.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_V383_SURVEI_STRICT_DB_CHECK__) return;
  window.__ARIKA_V383_SURVEI_STRICT_DB_CHECK__ = true;

  var checkState = { key:'', checked:false, checking:false, lastStart:0 };
  ...
  function pad2(n){ n = String(n || '').replace(/\D/g,''); return n.length === 1 ? '0' + n : n.slice(0,2); }
  function monthNow(){
    try { if(typeof getCurrentSurveyMonth === 'function') return String(getCurrentSurveyMonth()).slice(0,7); } catch(_) {}
    try { if(typeof getCurrentMonth === 'function') return String(getCurrentMonth()).slice(0,7); } catch(_) {}
    var d = new Date();
    return d.getFullYear() + '-' + pad2(d.getMonth()+1);
  }
  ...
  function parseMonthKey(value, fallbackDate){
    var s = String(value || '').trim();
    if(s){
      var m;
      // yyyy-mm atau yyyy/mm, termasuk timestamp yyyy-mm-dd...
      m = s.match(/(20\d{2})[-\/](\d{1,2})/);
      if(m) return m[1] + '-' + pad2(m[2]);
      // dd/mm/yyyy atau dd-mm-yyyy
      m = s.match(/^(\d{1,2})[-\/]([01]?\d)[-\/](20\d{2})$/);
      if(m) return m[3] + '-' + pad2(m[2]);
      // mm/yyyy atau mm-yyyy
      m = s.match(/^([01]?\d)[-\/](20\d{2})$/);
      if(m) return m[2] + '-' + pad2(m[1]);
      // Date string umum
      var d = new Date(s);
      if(!isNaN(d.getTime())) return d.getFullYear() + '-' + pad2(d.getMonth()+1);
    }
    if(fallbackDate) return parseMonthKey(fallbackDate, '');
    return '';
  }
  function currentUserKey(){
    var u = window.currentUser || {};
    return {
      nip: normalizeSafe(u.nip || u.NIP || ''),
      nama: normalizeSafe(u.nama || u.name || u.pegawai || ''),
      month: monthNow()
    };
  }
  function currentCheckKey(){
    var k = currentUserKey();
    return [k.month, k.nip || k.nama || 'nouser'].join('|');
  }
  function rowMatchesUserAndMonth(row){
    if(!row || !window.currentUser) return false;
    var k = currentUserKey();
    var rMonth = parseMonthKey(row.bulan || row.periode || row.month || '', row.createdAt || row.timestamp || row.Timestamp || '');
    if(rMonth !== k.month) return false;
    var rNip = normalizeSafe(row.nip || row.NIP || row.nipPegawai || row['NIP Pegawai'] || '');
    var rNama = normalizeSafe(row.nama || row.name || row.pegawai || row['Nama Pegawai'] || '');
    return (!!k.nip && !!rNip && k.nip === rNip) || (!!k.nama && !!rNama && k.nama === rNama);
  }
  function hasSurveyInLoadedData(){
    try { return Array.isArray(window.surveiData) && window.surveiData.some(rowMatchesUserAndMonth); }
    catch(_) { return false; }
  }
  function hideSurveyForAdminOrNoUser(){
    var card = byId('survey-arika-card');
    if(!window.currentUser || window.isAdmin){ if(card) card.classList.add('hidden'); return true; }
    return false;
  }
  function setSurveyCheckingUI(){
    if(hideSurveyForAdminOrNoUser()) return;
    var card = byId('survey-arika-card');
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
    var monthLabel = byId('survey-month-label');
    if(!card || !form || !statusBox) return;
    card.classList.remove('hidden');
    if(monthLabel) monthLabel.innerText = formatMonth(monthNow());
    form.classList.add('hidden');
    statusBox.classList.remove('hidden');
    statusBox.innerHTML = 'Mengecek status survei <b>' + formatMonth(monthNow()) + '</b> dari database...';
  }
  function setSurveyDoneUI(){
    if(hideSurveyForAdminOrNoUser()) return;
    var card = byId('survey-arika-card');
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
    ...
    form.classList.add('hidden');
    statusBox.classList.remove('hidden');
    statusBox.innerHTML = 'Terima kasih, survei ARIKA untuk <b>' + formatMonth(monthNow()) + '</b> sudah tercatat di database.';
  }
  function setSurveyFormUI(){
    if(hideSurveyForAdminOrNoUser()) return;
    var card = byId('survey-arika-card');
    var form = byId('form-survei-arika');
    var statusBox = byId('survey-status-box');
  ...
  function renderSurveyStrict(){
    if(hideSurveyForAdminOrNoUser()) return;
    var key = currentCheckKey();
    if(hasSurveyInLoadedData()){
      checkState.key = key;
  ...
  function waitUntilCloudIdle(startedAt, cb){
    if(!window.__ARIKA_FETCHING_CLOUD__ || (Date.now() - startedAt) > MAX_WAIT_MS){
      cb();
      return;
    }
    setTimeout(function(){ waitUntilCloudIdle(startedAt, cb); }, 250);
  }
  function markCheckedAndRender(){
    checkState.key = currentCheckKey();
    checkState.checked = true;
    checkState.checking = false;
    setTimeout(renderSurveyStrict, 80);
    setTimeout(function(){ try { window.renderFokusHariIni && window.renderFokusHariIni(); } catch(_) {} }, 160);
  }
  function startDatabaseCheck(){
    if(!window.currentUser || window.isAdmin) return;
    if(checkState.checking) return;
    checkState.checking = true;
    checkState.lastStart = Date.now();
    setSurveyCheckingUI();

    var startedAt = Date.now();
    function finishAfterIdle(){
      waitUntilCloudIdle(startedAt, markCheckedAndRender);
    }
    try {
      if(typeof window.fetchCloudData === 'function'){
        var beforeFetching = !!window.__ARIKA_FETCHING_CLOUD__;
        if(beforeFetching){
          finishAfterIdle();
          return;
        }
        Promise.resolve(window.fetchCloudData({ force:true, reason:'survey-v383-strict' }))
          .catch(function(){})
          .finally(finishAfterIdle);
        return;
      }
    } catch(_) {}
    setTimeout(markCheckedAndRender, 1800);
  }

  // Override render survei paling akhir agar cache browser kosong tidak memunculkan form palsu.
  window.renderSurveiPegawai = renderSurveyStrict;

  // Reminder/Fokus tidak boleh menganggap survei pending selama database masih dicek.
  window.hasUserFilledSurveyThisMonthForReminder = function(){
    if(!window.currentUser || window.isAdmin) return true;
    if(hasSurveyInLoadedData()) return true;
    if(!checkState.checked || checkState.checking) return true;
    return false;
  };

  var oldFetch = window.fetchCloudData;
  if(typeof oldFetch === 'function' && !oldFetch.__arikaV383SurveyStrict){
    window.fetchCloudData = async function(){
      var res;
      try { res = await oldFetch.apply(this, arguments); }
      finally {
        if(window.currentUser && !window.isAdmin){
          waitUntilCloudIdle(Date.now(), function(){
            checkState.key = currentCheckKey();
            checkState.checked = true;
            checkState.checking = false;
            setTimeout(renderSurveyStrict, 80);
    ...
    window.fetchCloudData.__arikaV383SurveyStrict = true;
  }

  // Saat survei berhasil disimpan, tandai juga secara lokal agar tidak muncul lagi di sesi yang sama.
  var oldSaveSurvey = window.simpanSurveiArika;
  if(typeof oldSaveSurvey === 'function' && !oldSaveSurvey.__arikaV383SurveyStrict){
    window.simpanSurveiArika = async function(){
      var res = await oldSaveSurvey.apply(this, arguments);
      checkState.key = currentCheckKey();
      checkState.checked = true;
      checkState.checking = false;
      setTimeout(renderSurveyStrict, 120);
      return res;
    };
    window.simpanSurveiArika.__arikaV383SurveyStrict = true;
  }

  function boot(){
    setTimeout(renderSurveyStrict, 250);
    setTimeout(renderSurveyStrict, 1400);
    setTimeout(renderSurveyStrict, 3200);
  }
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot); else boot();
...
/* v384 - Fix Dashboard Saya: tombol/filter "Semua Bulan" harus benar-benar menampilkan semua periode.
   Penyebab lama: beberapa listener filter lama mengubah kembali mode all menjadi month saat event change/input terjadi. */
(function(){
  'use strict';
  if(window.__ARIKA_V384_DASHBOARD_SEMUA_BULAN_FIX__) return;
  window.__ARIKA_V384_DASHBOARD_SEMUA_BULAN_FIX__ = true;

  function byId(id){ return document.getElementById(id); }
  function isDashVisible(){ var v=byId('view-dashboard-pegawai'); return !!(v && !v.classList.contains('hidden')); }
  function setAllState(){
    ...
    if(el){
      ...
      el.dataset.arikaIsoValue = '';
      el.value = '';
      el.setAttribute('placeholder','Semua bulan');
    }
    var label = byId('pegawai-dashboard-filter-v290-label');
    if(label) label.textContent = 'Semua';
    var cur = byId('pegawai-dashboard-month-current');
    var all = byId('pegawai-dashboard-month-all');
    if(cur){ cur.classList.remove('active','is-active'); cur.setAttribute('aria-pressed','false'); }
    if(all){ all.classList.add('active','is-active'); all.setAttribute('aria-pressed','true'); }
  }
  function renderAll(){
    setAllState();
    setTimeout(function(){
      setAllState();
      try { if(typeof window.renderDashboardPegawai === 'function') window.renderDashboardPegawai({ fromV384SemuaBulan:true, allMonths:true }); } catch(e){ console.warn('v384 render dashboard semua bulan gagal:', e); }
      setTimeout(setAllState, 40);
      setTimeout(setAllState, 220);
    }, 20);
  }

  // Override fungsi lama agar tidak memicu event input/change yang mengembalikan mode ke bulan berjalan.
  window.clearPegawaiDashboardMonth = function(){ renderAll(); };

  // Tangkap klik tombol Semua sebelum handler lama berjalan.
  document.addEventListener('click', function(ev){
    var btn = ev.target && ev.target.closest && ev.target.closest('#pegawai-dashboard-month-all');
    if(!btn) return;
    ev.preventDefault();
    ev.stopPropagation();
  ...
  // Jika mode sudah all, jangan biarkan listener lama mengubahnya hanya karena input kosong.
  document.addEventListener('change', function(ev){
    if(ev.target && ev.target.id === 'pegawai-dashboard-month' && ev.target.dataset && ev.target.dataset.periodMode === 'all'){
      setTimeout(setAllState, 0);
      setTimeout(function(){ if(isDashVisible()) renderAll(); }, 60);
    }
  }, true);

  var oldRender = window.renderDashboardPegawai;
  if(typeof oldRender === 'function' && !oldRender.__arikaV384SemuaBulanFix){
    window.renderDashboardPegawai = function(){
      var el = byId('pegawai-dashboard-month');
      var wasAll = !!(el && el.dataset && el.dataset.periodMode === 'all');
      var res = oldRender.apply(this, arguments);
      if(wasAll){ setAllState(); setTimeout(setAllState, 60); setTimeout(setAllState, 300); }
      return res;
    };
    window.renderDashboardPegawai.__arikaV384SemuaBulanFix = true;
  }
})();

...
const S={pegawai:[],jurnal:[],rencana:[],user:null,linked:null,edit:null,loaded:false};
...
function saveCache(){try{localStorage.setItem(CACHE,JSON.stringify({pegawai:S.pegawai,jurnal:S.jurnal,rencana:S.rencana,ts:Date.now()}));}catch(_){}}
function loadCache(){try{const c=JSON.parse(localStorage.getItem(CACHE)||'{}'); if(c.pegawai)S.pegawai=c.pegawai; if(c.jurnal)S.jurnal=(c.jurnal||[]).map(normalizeMobileLemburFlag); if(c.rencana)S.rencana=c.rencana;}catch(_){}}
...
function renderAll(){if(!S.user)return; $('helloText').textContent='Halo, '+(S.user.nama||'Pegawai').split(' ')[0]; renderStats(); renderHomePlans(); renderPlans(); renderHistory(); renderWfh(); renderLinked();}
...
function renderHistory(){
  ...
  if(listEl){listEl.innerHTML=rows.length?rows.map(j=>`<div class="card"><div class="row-between"><div><div class="meta">${fmt(j.date)} • ${esc(j.cat)}</div><p class="plan-title">${esc(j.status)}</p></div><div class="row">${j.rencanaId||j.rencanaJudul?'<span class="pill pill-ok">Terkait Rencana</span>':''}${j.isLembur?'<span class="pill pill-lembur">Lembur</span>':''}</div></div><div class="journal-desc">${esc(j.desc)}</div>${j.isLembur?`<p class="small"><b>Lembur:</b> ${esc(j.start||'-')} - ${esc(j.end||'-')} ${j.lamaLembur?`• ${esc(j.lamaLembur)}`:''}${j.suratTugas?` • ST: ${esc(j.suratTugas)}`:''}</p>`:''}${j.linkDataDukung?`<p class="small"><b>Data dukung:</b><span class="mobile-link-text">${esc(j.linkDataDukung)}</span></p>`:''}<div class="row" style="margin-top:12px"><button class="btn btn-soft" onclick="ARIKA.editJournal('${esc(j.id)}')">Edit</button></div></div>`).join(''):`<div class="card list-empty">Belum ada jurnal pada rentang tanggal ini.</div>`;}
...
function hitungDurasiLembur(start,end){if(!start||!end)return ''; const a=start.split(':').map(Number), b=end.split(':').map(Number); if(a.length<2||b.length<2)return ''; let m1=a[0]*60+a[1], m2=b[0]*60+b[1]; if(m2<m1)m2+=1440; const diff=Math.max(0,m2-m1); const h=Math.floor(diff/60), m=diff%60; return h+' jam'+(m?(' '+m+' menit'):'');}
...
window.ARIKA={startReal,viewReal,editJournal,closeModal,cancelLink(){S.linked=null;renderLinked();},markDone,setHistoryPage,setHistoryRange,toggleLemburFields};

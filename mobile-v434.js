...
async function strictPostMobile(action,payload,ms=16000){const c=new AbortController(); const t=setTimeout(()=>c.abort(),ms); try{const r=await fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},cache:'no-store',body:JSON.stringify({action,payload}),signal:c.signal}); const txt=await r.text(); let d={}; try{d=txt?JSON.parse(txt):{};}catch(_){d={success:r.ok,raw:txt};} if(d&&d.success===false)throw new Error(d.message||d.error||'Permintaan ditolak server.'); return d;}finally{clearTimeout(t);}}
...
async function doLogin(e){e.preventDefault(); const name=($('loginHidden').value||$('loginName').value||'').trim(); const pass=$('loginPass').value.trim(); const p=S.pegawai.find(x=>norm(x.nama)===norm(name)); if(!p){toast('Pilih username dari daftar.'); return;} if(!pass){toast('Kata sandi wajib diisi.'); return;} try{loading(true,'Memeriksa akun...'); const res=await strictPostMobile('login_password',{nip:p.nip,password:pass,userAgent:navigator.userAgent},18000); if(res && (res.mustChangePassword===true || String(res.mustChangePassword).toLowerCase()==='true')){toast('Password masih sementara. Buka versi PC untuk mengganti password terlebih dahulu.'); return;} const profile=Object.assign({},p,res.profile||{}); S.user=profile; localStorage.setItem(LS,JSON.stringify(profile)); $('loginView').classList.add('hidden'); $('appShell').classList.remove('hidden'); setDefaults(); renderAll(); loadCore(true);}catch(err){console.warn('Login mobile gagal',err); toast(err.message||'Login gagal. Pastikan kata sandi benar.');}finally{loading(false);}}
...
/* ARIKA Mobile v391 - istilah tampilan Jurnal -> Kegiatan */
(function(){
  function rep(s){ return (typeof s==='string') ? s.replace(/JURNAL/g,'KEGIATAN').replace(/Jurnal/g,'Kegiatan').replace(/jurnal/g,'kegiatan') : s; }
  function skip(n){ var p=n&&n.parentNode; return !p || !p.tagName || /^(SCRIPT|STYLE|TEXTAREA|INPUT|SELECT|OPTION|CODE|PRE)$/i.test(p.tagName); }
  window.arikaMobileApplyKegiatanTerminology=function(root){
    try{
      root=root||document.body; if(!root) return; if(document.title) document.title=rep(document.title);
      var w=document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {acceptNode:function(n){ return (!skip(n) && /jurnal/i.test(n.nodeValue||'')) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP; }});
      var arr=[]; while(w.nextNode()) arr.push(w.currentNode); arr.forEach(function(n){ n.nodeValue=rep(n.nodeValue); });
      Array.prototype.forEach.call(root.querySelectorAll?root.querySelectorAll('[title],[placeholder],[aria-label]'):[], function(el){ ['title','placeholder','aria-label'].forEach(function(a){ var v=el.getAttribute(a); if(/jurnal/i.test(v||'')) el.setAttribute(a, rep(v)); }); });
    }catch(e){}
  };
  function install(){ window.arikaMobileApplyKegiatanTerminology(); if(!window.__arikaMobileV391Obs && window.MutationObserver){ window.__arikaMobileV391Obs=true; var t=null; new MutationObserver(function(){ clearTimeout(t); t=setTimeout(window.arikaMobileApplyKegiatanTerminology,120); }).observe(document.body,{childList:true,subtree:true,characterData:true}); } }
...
// v399: Mobile memakai login password backend v31/v32; ubah password tetap melalui PC.

/* ARIKA v434 mobile marker: admin account gate is PC-only. */
(function(){ window.__ARIKA_MOBILE_V434__ = true; })();

/* =========================================================
   ARIKA v420 - Performance Stage 4 DOM Load Reduction
   Tujuan: mengurangi beban DOM/browser dengan progressive DOM,
   bukan merender/menahan ratusan card/baris sekaligus.
   ========================================================= */
(function(){
  'use strict';
  if(window.__ARIKA_DOM_V420__) return;
  window.__ARIKA_DOM_V420__ = true;
  ...
  function log(){ try{ console.info.apply(console, ['ARIKA v420 DOM'].concat([].slice.call(arguments))); }catch(_){} }
  function warn(){ try{ console.warn.apply(console, ['ARIKA v420 DOM'].concat([].slice.call(arguments))); }catch(_){} }
  function idle(fn){
  ...
  function compactAll(reason){
    nextFrame(function(){
      ...
      document.documentElement.setAttribute('data-arika-dom-stage', 'v420');
    });
  }
  ...
  function wrapRender(name){
    ...
    window[name].__arikaV420DomWrapped = true;
    wrapped[name] = true;
    return true;
  }
  ...
  function injectCss(){
    if(document.getElementById('arika-dom-v420-style')) return;
    var style = document.createElement('style');
    style.id = 'arika-dom-v420-style';
    style.textContent = [

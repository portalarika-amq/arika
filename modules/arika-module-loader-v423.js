/* =========================================================
   ARIKA v423 - Modular Frontend Loader
...
(function(){
  'use strict';
  var VERSION = 'v423-wfh-legacy-stable';
  var BASE = './modules/';
  ...
  var manifest = {
    ...
    router:      { file: 'arika-render-router-v422.js', eager: true },
    dom:         { file: 'arika-dom-v422.js', eager: true },
    wfh:         { file: 'arika-wfh-v423.js', route: 'wfh' },
  ...
  function log(){ try{ console.info.apply(console, ['ARIKA v423'].concat([].slice.call(arguments))); }catch(_){} }
  function warn(){ try{ console.warn.apply(console, ['ARIKA v423'].concat([].slice.call(arguments))); }catch(_){} }
  function load(name){
    ...
    loading[name] = new Promise(function(resolve, reject){
      ...
      s.src = BASE + manifest[name].file + '?v=423';
      s.defer = true;
      s.onload = function(){
        loaded[name] = true;
        try{
  ...
  log('loader aktif. WFH/WFA dikembalikan ke tampilan monitoring harian stabil tanpa rekap bulanan dan tanpa kalender.');
})();

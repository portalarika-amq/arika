/* =========================================================
   ARIKA v433 - Modular Frontend Loader
...
(function(){
  'use strict';
  var VERSION = 'v433-wfh-single-controller';
  var BASE = './modules/';
  ...
  function log(){ try{ console.info.apply(console, ['ARIKA v433'].concat([].slice.call(arguments))); }catch(_){} }
  function warn(){ try{ console.warn.apply(console, ['ARIKA v433'].concat([].slice.call(arguments))); }catch(_){} }
  function load(name){
    ...
    loading[name] = new Promise(function(resolve, reject){
      ...
      s.src = BASE + manifest[name].file + '?v=433';
      s.defer = true;
      s.onload = function(){
        loaded[name] = true;
        try{
  ...
  log('loader aktif. WFH/WFA memakai controller tunggal v433 tanpa loop state.');
})();

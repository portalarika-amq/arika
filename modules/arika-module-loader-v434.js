/* =========================================================
   ARIKA v434 - Modular Frontend Loader
...
(function(){
  'use strict';
  var VERSION = 'v434-admin-security-gate';
  var BASE = './modules/';
  ...
  var manifest = {
    ...
    router:      { file: 'arika-render-router-v433.js', eager: true },
    dom:         { file: 'arika-dom-v422.js', eager: true },
    wfh:         { file: 'arika-wfh-v433.js', route: 'wfh' },
  ...
  function log(){ try{ console.info.apply(console, ['ARIKA v434'].concat([].slice.call(arguments))); }catch(_){} }
  function warn(){ try{ console.warn.apply(console, ['ARIKA v434'].concat([].slice.call(arguments))); }catch(_){} }
  function load(name){
    ...
    loading[name] = new Promise(function(resolve, reject){
      ...
      s.src = BASE + manifest[name].file + '?v=434';
      s.defer = true;
      s.onload = function(){
        loaded[name] = true;
        try{
  ...
  log('loader aktif. Admin account security gated khusus Administrator; WFH/WFA memakai controller tunggal stabil.');
})();

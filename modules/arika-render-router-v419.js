/* ARIKA v419 - Active tab render router with v37 light endpoint bridge */
(function(){
  'use strict';
  var timers = Object.create(null);
  function safe(fn){ try{ return fn(); }catch(e){ console.warn('ARIKA v419 router:', e); } }
  ...
  async function syncEndpoint(id){
    ...
    if(window.arikaSyncEndpointForTab) {
      try{ return await window.arikaSyncEndpointForTab(id, { source:'router-v419' }); }
      catch(e){ console.warn('ARIKA v419 endpoint ringan gagal:', id, e); }
    }
    return null;
  }
  function renderLegacy(id){
    if(id==='beranda'){
      ...
      else if(window.renderRencanaPribadi) window.renderRencanaPribadi({ berandaOnly:true, fromV419:true });
      ...
      if(window.renderRencanaPribadi) window.renderRencanaPribadi({ fromV419:true });
    } else if(id==='rekap'){
      if(window.runFilter) window.runFilter({ keepPage:true, fromV419:true });
      ...
      if(window.refreshWfhWfaData) window.refreshWfhWfaData({ fromV419:true });
      else if(window.renderWfhWfaDay) window.renderWfhWfaDay({ fromV419:true });
    } else if(id==='admin'){
      ...
      if(window.renderLemburTable) window.renderLemburTable({ fromV419:true });
    } else if(id==='dashboard-pegawai'){
      if(window.renderDashboardPegawai) window.renderDashboardPegawai({ fromV419:true });
    }
  }
  ...
  window.arikaInitModule_router = function(){ if(window.arikaRenderActiveTabOnly) window.arikaRenderActiveTabOnly('router-init-v419'); };
})();

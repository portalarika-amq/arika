/* ARIKA v422 - Active tab render router with v37 light endpoint bridge */
(function(){
  'use strict';
  var timers = Object.create(null);
  function safe(fn){ try{ return fn(); }catch(e){ console.warn('ARIKA v422 router:', e); } }
  ...
  async function syncEndpoint(id){
    ...
    if(window.arikaSyncEndpointForTab) {
      try{ return await window.arikaSyncEndpointForTab(id, { source:'router-v422' }); }
      catch(e){ console.warn('ARIKA v422 endpoint ringan gagal:', id, e); }
    }
    return null;
  }
  function renderLegacy(id){
    if(id==='beranda'){
      ...
      if(window.refreshWfhWfaData) window.refreshWfhWfaData({ fromV422:true });
      else if(window.renderWfhWfaDay) window.renderWfhWfaDay({ fromV422:true });
    } else if(id==='admin'){
  ...
  window.arikaInitModule_router = function(){ if(window.arikaRenderActiveTabOnly) window.arikaRenderActiveTabOnly('router-init-v422'); };
})();
